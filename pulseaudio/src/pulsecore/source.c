...
#define VCONFKEY_SOUND_SOURCE_STATE "memory/private/sound/sourcestate" 	//hsguy.son (add)
...
#ifdef __TIZEN__
	//hsguy.son (add)
	if(s->state != PA_SOURCE_RUNNING)
	{
		vconf_set_int(VCONFKEY_SOUND_SOURCE_STATE, 0);
		pa_log_debug("source state not running.");
	}
	else
	{
		vconf_set_int(VCONFKEY_SOUND_SOURCE_STATE, 1);
		pa_log_debug("source state running.");
	}
	//hsguy.son (add_end)
