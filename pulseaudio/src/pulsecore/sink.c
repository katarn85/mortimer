...
#define VCONFKEY_SOUND_SINK_STATE "memory/private/sound/sinkstate" 	//hsguy.son (add)
...
static int sink_set_state(pa_sink *s, pa_sink_state_t state) {
	...
	{
		vconf_set_int(VCONFKEY_SOUND_SINK_STATE, 0);
		pa_log_debug("sink state not running.");
	}
	else
	{
		vconf_set_int(VCONFKEY_SOUND_SINK_STATE, 1);
		pa_log_debug("sink state running.");
	}
	//hsguy.son (add_end)
