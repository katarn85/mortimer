...
int pa_alsa_set_sw_params(snd_pcm_t *pcm, snd_pcm_uframes_t avail_min, pa_bool_t period_event, int start_threshold, int stop_threshold, int sampling_rate) {
#else
...
#ifdef __TIZEN__
    /* prevent */
    if ((err = snd_pcm_sw_params_set_stop_threshold(pcm, swparams, (snd_pcm_uframes_t)stop_threshold)) < 0) {
    /* end of prevent */
        pa_log_warn("Unable to set stop threshold: %s\n", pa_alsa_strerror(err));
        return err;
    }

    if ((err = snd_pcm_sw_params_set_start_threshold(pcm, swparams, (snd_pcm_uframes_t)start_threshold)) < 0) {
