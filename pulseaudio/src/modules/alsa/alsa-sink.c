...
#ifdef __TIZEN__
                if (u->first) {
                    //Hawk-P platform requires silence buffer amount of 32ms * 2 at first.
                    u->memchunk.memblock = pa_memblock_ref(u->sink->silence.memblock);
                    u->memchunk.index = u->sink->silence.index;
                    u->memchunk.length = PA_MIN(u->sink->silence.length, n_bytes);
                } else {
...
#ifdef __TIZEN__
                /* To avoid assert reset for Hawk-P platform because it able to return EAGAIN code when after_avail is TRUE */
...
#ifdef __TIZEN__
    /* Hawk-P platform requires that the sizes of start/stop threshold should same as hw buffer size. */
    if ((err = pa_alsa_set_sw_params(u->pcm_handle, avail_min, !u->use_tsched, (u->hwbuf_size / u->frame_size) / 2, u->hwbuf_size / u->frame_size, u->sink->sample_spec.rate)) < 0) {
...
#ifdef __TIZEN__
    /* adjust buffer size of Hawk-P */
    pa_sink_set_max_request_within_thread(u->sink, (u->hwbuf_size / 2) - u->hwbuf_unused);
#else
...
#ifdef __TIZEN__
#if 0 /* This code is not used in TV */
...
static int process_rewind(struct userdata *u) {
    ...
    if (PA_UNLIKELY((unused = pa_alsa_safe_avail(u->pcm_handle, u->hwbuf_size, &u->sink->sample_spec)) < 0)) {
		...
		if ((r = try_recover(u, "snd_pcm_avail", (int) unused)) == 0) {
			pa_log("try recover in process_rewind() succeed (r = %d)", r);
		} else {
			pa_log("try recover in process_rewind() failed! (r = %d)", r);
			return -1;
		}
    }
...
#endif
#if __TIZEN__
        /* Hawk-P platform doens't support rewind for pcm. */
        out_frames = 0;
#else
...
#ifdef __TIZEN__
        /* adjust buffer size of Hawk-P */
        pa_sink_set_fixed_latency(u->sink, pa_bytes_to_usec((u->hwbuf_size * 3) / 10, &ss));
#else
