...
#endif
#define SINK_BLUETOOTH           "bluez" 	//hsguy.son (add)
...
//hsguy.son (test)
	if(pa_streq(s->sink->name, SINK_BLUETOOTH))
	{
		u->timeout = 0;
		pa_log_error(">>>hsguy.son Sink %s : %d", s->sink->name, u->timeout);
	}
//hsguy.son (test_end)
...
static pa_hook_result_t device_new_hook_cb(pa_core *c, pa_object *o, struct userdata *u) {
	...
	//hsguy.son (test)
	if((sink && (!strncmp (sink->name, SINK_BLUETOOTH, 5))))
	{
		u->timeout = 5;
		pa_log_error(">>>hsguy.son Sink %s : %d", sink->name, u->timeout);
	}
	//hsguy.son (test_end)
