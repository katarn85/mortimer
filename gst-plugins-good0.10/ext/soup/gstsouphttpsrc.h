...
#define MAX_AUTH_INFO_SIZE 2048
#endif

#ifdef GST_SUPPORT_STOP_AT
typedef void MhegStopAtCallBackFn(void* user_data);
#endif
...
#ifdef GST_SUPPORT_STORAGE_SERVICE
   gchar *sp_name;
   gchar *app_id;			   /*file ID  to request body*/
   gchar *user_id_for_auth; 	/*user_id to login on samsung account server*/
   gchar *access_token_for_auth; /*token to login on samsung account server*/
#endif
...
#endif
#ifdef GST_SUPPORT_STORAGE_SERVICE
  gboolean update_oauth_info; /*new autho info got from dropbox/skydirver*/
  gchar new_auth_info[MAX_AUTH_INFO_SIZE];
#endif
#ifdef GST_SUPPORT_STOP_AT
gboolean disconnect_flag;
MhegStopAtCallBackFn *stop_at_cb;
gpointer stop_at_cb_param;
guint64 stop_at_position;
#endif

  gchar* ssl_cert_path; /* SSL certification path */
};
