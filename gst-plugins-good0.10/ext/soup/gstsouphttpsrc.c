...
enum
{
  ...
  PROP_AHS_STREAMING,
#endif
#ifdef GST_SUPPORT_STORAGE_SERVICE
  PROP_SP_NAME,
  PROP_APP_ID,
  PROP_USER_ID_FOR_AUTH,
  PROP_ACCESS_TOKEN_FOR_AUTH,
#endif
#ifdef SEEK_CHANGES
  PROP_EXTRA_HEADERS,
  PROP_RANGESIZE,
...
#endif
  PROP_SOUPCOOKIES,
#ifdef GST_SUPPORT_STOP_AT
  PROP_DISCONNECT,
  PROP_MHEG_STOPAT_CB,
  PROP_MHEG_STOPAT_CB_PARAM,
  PROP_STOP_AT_SIZE
#endif
};
...
#ifdef GST_SUPPORT_STORAGE_SERVICE
#define AUTH_TIMESTAMP_SEED	 ""
#endif
...
#endif
#ifdef GST_SUPPORT_STORAGE_SERVICE
static void gst_soup_http_src_get_info_by_key(gchar *psrc, gchar *ptarget, gchar* pdest);
static gchar *gst_soup_auth_gen_nonce();
static gchar *gst_soup_auth_gen_timestamp();
static void gst_soup_update_auth_info(GstSoupHTTPSrc * src);
#endif
...
static void gst_soup_http_src_authenticate_cb (SoupSession * session,
    ...
    GstSoupHTTPSrc * src);
static gboolean gst_soup_http_src_certificate_cb(SoupMessage* msg, GTlsCertificate* certificate, GTlsCertificateFlags errors, gpointer data);
...
gst_soup_http_src_class_init (GstSoupHTTPSrcClass * klass)
{
  ...
  g_object_class_install_property (gobject_class,
      PROP_SSL_CERT_PATH,
      g_param_spec_string ("ssl-cert-path", "SSL-Certification-Path",
          "Value of the SSL Certification Path",
          DEFAULT_USER_AGENT, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  g_object_class_install_property (gobject_class,
      PROP_AUTOMATIC_REDIRECT,
...
#endif
#ifdef GST_SUPPORT_STORAGE_SERVICE
	g_object_class_install_property (gobject_class, PROP_SP_NAME,
		g_param_spec_string ("sp-name", "sp-name",
			"service provider name for storage service", "",
			G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

	g_object_class_install_property (gobject_class, PROP_APP_ID,
		g_param_spec_string ("app-id", "app-id",
			"content ID from storage service", "",
			G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

	g_object_class_install_property (gobject_class, PROP_USER_ID_FOR_AUTH,
		g_param_spec_string ("user-id-for-auth", "user-id-for-auth",
			"user id to login in samsung account server", "",
			G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

	g_object_class_install_property (gobject_class, PROP_ACCESS_TOKEN_FOR_AUTH,
		g_param_spec_string ("access-token-for-auth", "access-token-for-auth",
			"access token to login in samsung account server", "",
			G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property (gobject_class, PROP_CERTIFICATE_PATH,
		g_param_spec_string ("certificate-path", "set-certificate-path",
			"set certificate path for https stream", "",
			G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  ...
  g_object_class_install_property (gobject_class,
      ...
      g_param_spec_boolean ("disconnect", "disconnect",
        "disconnect url and send eos event",
        FALSE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  g_object_class_install_property (gobject_class, PROP_STOP_AT_SIZE,
    g_param_spec_uint64 ("stop-position", "stop_at position",
        "Total size of read by src",
        0, G_MAXUINT64, 0, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  g_object_class_install_property (gobject_class, PROP_MHEG_STOPAT_CB,
      g_param_spec_pointer("stop-at-cb", "stop_at cb",
        "stop_at callback func",
        G_PARAM_READWRITE ));
  g_object_class_install_property (gobject_class, PROP_MHEG_STOPAT_CB_PARAM,
      g_param_spec_pointer("stop-at-cb-param", "stop_at cb param",
        "stop_at callback func param",
        G_PARAM_READWRITE ));
#endif
...
gst_soup_http_src_init (GstSoupHTTPSrc * src, GstSoupHTTPSrcClass * g_class)
{
  ...
  src->ssl_cert_path = NULL;
  src->user_id = NULL;
  src->user_pw = NULL;
  src->proxy_id = NULL;
  src->proxy_pw = NULL;
  src->cookies = NULL;
  src->iradio_mode = TRUE;
  src->loop = NULL;
  src->context = NULL;
  src->session = NULL;
  src->msg = NULL;
  src->certificate_path = NULL;
#ifdef SEEK_CHANGES
  src->file_size = 0;
  src->range_size = 0;
#endif
  ...
  src->sp_name = NULL;
  src->app_id = NULL;
  src->user_id_for_auth = NULL;
  src->access_token_for_auth = NULL;
#endif
#ifdef GST_SUPPORT_STOP_AT
  src->disconnect_flag = FALSE;
  src->stop_at_cb = NULL;
  src->stop_at_cb_param = NULL;
  src->stop_at_position = 0;
#endif
...
gst_soup_http_src_finalize (GObject * gobject)
{
  ...
  if (src->ssl_cert_path) {
    g_free (src->ssl_cert_path);
    src->ssl_cert_path;
  }
  if (src->proxy != NULL) {
  ...
  g_free (src->proxy_id);
  g_free (src->proxy_pw);
  if (src->certificate_path != NULL) {
    g_free(src->certificate_path);
  }
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_USER_AGENT:
      ...
      break;
    case PROP_SSL_CERT_PATH:
        if (src->ssl_cert_path)
            g_free (src->ssl_cert_path);
        src->ssl_cert_path = g_value_dup_string (value);
      break;
    ...
    case PROP_SP_NAME:
      if (src->sp_name)
        g_free (src->sp_name);
      src->sp_name = g_value_dup_string (value);
      break;
    case PROP_APP_ID:
      if (src->app_id)
        g_free (src->app_id);
      src->app_id = g_value_dup_string (value);
      break;
    case PROP_USER_ID_FOR_AUTH:
      if (src->user_id_for_auth)
        g_free (src->user_id_for_auth);
      src->user_id_for_auth = g_value_dup_string (value);
      break;
    case PROP_ACCESS_TOKEN_FOR_AUTH:
      if (src->access_token_for_auth)
        g_free (src->access_token_for_auth);
      src->access_token_for_auth = g_value_dup_string (value);
      break;
#endif
    case PROP_IS_LIVE:
    ...
    case PROP_DISCONNECT:{
      src->disconnect_flag = g_value_get_boolean(value);
      GST_INFO_OBJECT (src, "PROP_STOP_POSITION,src->disconnect_flag : %d",src->disconnect_flag);
      if( src->disconnect_flag ){
        gst_soup_http_src_session_close(src);
        if(src->stop_at_cb && src->stop_at_cb_param){
          src->stop_at_cb(src->stop_at_cb_param);
        }
      }
      break;
    }
    case PROP_MHEG_STOPAT_CB:{
      src->stop_at_cb = (MhegStopAtCallBackFn *)g_value_get_pointer(value);
    }
    break;
    case PROP_MHEG_STOPAT_CB_PARAM:{
      src->stop_at_cb_param = (void*)g_value_get_pointer(value);
    }
    break;
    case PROP_STOP_AT_SIZE:{
      src->stop_at_position = g_value_get_uint64(value);
    }
    break;
#endif
    case PROP_CERTIFICATE_PATH:
      if (src->certificate_path){
        g_free (src->certificate_path);
		src->certificate_path = NULL;
      	}
	  src->certificate_path = g_value_dup_string (value);
      break;
    default:
    ...
    GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_USER_AGENT:
      ...
      break;
    case PROP_SSL_CERT_PATH:
      g_value_set_string (value, src->ssl_cert_path);
      break;
    case PROP_AUTOMATIC_REDIRECT:
...
#ifdef GST_SUPPORT_STORAGE_SERVICE
    case PROP_SP_NAME:
      g_value_set_string (value, src->sp_name);
      break;
    case PROP_APP_ID:
      g_value_set_string (value, src->app_id);
      break;
    case PROP_USER_ID_FOR_AUTH:
      g_value_set_string (value, src->user_id_for_auth);
      break;
    case PROP_ACCESS_TOKEN_FOR_AUTH:
      g_value_set_string (value, src->access_token_for_auth);
      break;
#endif
#ifdef GST_SUPPORT_STOP_AT
    case PROP_DISCONNECT:{
      g_value_set_boolean (value, src->disconnect_flag);
      break;
    }
    case PROP_MHEG_STOPAT_CB:{
      g_value_set_pointer(value,src->stop_at_cb);
      break;
    }
    case PROP_MHEG_STOPAT_CB_PARAM:{
      g_value_set_pointer(value,src->stop_at_cb_param);
      break;
    }
    case PROP_STOP_AT_SIZE:{
      g_value_set_uint64 (value, src->stop_at_position);
      break;
    }
#endif
    case PROP_CERTIFICATE_PATH:
      g_value_set_string (value, src->certificate_path);
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
      break;
...
_append_extra_header (GQuark field_id, const GValue * value, gpointer user_data)
{
  ...
  if (!g_ascii_strcasecmp(field_name, "Cookie")) {
    ...
    if (strlen(field_content) > 0) {
      ...
      tmp_field = strtok_r (field_content, ";", &usr_ptr);
      ...
      while (tmp_field != NULL) {
        ...
        tmp_field = strtok_r (NULL, ";", &usr_ptr);
      }
      soup_uri_free (uri);
    }
  }
...
gst_soup_http_src_got_headers_cb (SoupMessage * msg, GstSoupHTTPSrc * src)
{
...
#ifdef GST_SUPPORT_STOP_AT
      if(src->stop_at_position > 0 && src->stop_at_position < total_length){
        src->file_size = src->stop_at_position;
      }
      else
#endif
      {
  ...
  if (src->ret == GST_FLOW_CUSTOM_ERROR &&
    ...
    src->seekable = FALSE;
    GST_ELEMENT_WARNING (src, RESOURCE, SEEK,
        (_("Server does not support seeking.")),
        ("Server does not accept Range HTTP header, URL: %s", src->location));
#ifndef GST_EXT_SOUP_MODIFICATION
    /*if souphttpsec not supports seeking, demux works in push-mode, no need stop working*/
...
gst_soup_http_src_build_message (GstSoupHTTPSrc * src)
{
  ...
  g_signal_connect (src->msg, "accept-certificate",
      G_CALLBACK (gst_soup_http_src_certificate_cb), src);
...
#ifdef GST_SUPPORT_STORAGE_SERVICE
  if (src->update_oauth_info) {
      gst_soup_update_auth_info(src);
      if (src->update_oauth_info)
        GST_INFO_OBJECT(src, "cannot connect samsung account!");
  }
#endif
...
/* For videohub DRM dash uri which has custom data */
  ...
  if ( has_custom_data !=NULL && has_mpd !=NULL ) {
    ...
    gchar *usr_ptr = NULL;
    videohub_dash_normal_uri = strtok_r(src->location,"[]<>", &usr_ptr);
    src->location = NULL;
  ...
  if (!src->session) {
    ...
    return FALSE;
      ...
      g_object_set(src->session,
        SOUP_SESSION_SSL_STRICT, FALSE, SOUP_SESSION_CERTIFICATE_PATH, src->ssl_cert_path, NULL);
  }
  ...
  if (src->certificate_path != NULL)
      g_object_set(src->session,
                   ...
                   SOUP_SESSION_CERTIFICATE_PATH, src->certificate_path, NULL);
...
gst_soup_http_src_do_seek (GstBaseSrc * bsrc, GstSegment * segment)
{
  ...
  src->request_position = segment->start;
#ifdef GST_SUPPORT_STORAGE_SERVICE
  if (src->sp_name != NULL && !strcmp(src->sp_name, "dropbox")) {
    src->update_oauth_info = TRUE;
    GST_INFO_OBJECT(src, "Need to update auth info before doing seek");
  }
#endif
  return TRUE;
}
...
static gboolean 
gst_soup_http_src_certificate_cb(SoupMessage* msg, GTlsCertificate* certificate, GTlsCertificateFlags errors, gpointer data)
{
  GstSoupHTTPSrc * src = (GstSoupHTTPSrc *)data;

  GST_DEBUG_OBJECT (src, "website has no certificate file, ingore it");
  return TRUE;
}

#ifdef GST_SUPPORT_STORAGE_SERVICE
/*
<Value>OAuth oauth_token=&quot;ccj6f7nzwfxebk6j&quot;, oauth_consumer_key=&quot;8o3iym4gvq9xzd5&quot;,
oauth_version=&quot;1.0&quot;, oauth_signature_method=&quot;HMAC-SHA1&quot;, oauth_timestamp=&quot;
1397440931&quot;, oauth_nonce=&quot;2696232712728373668&quot;, oauth_signature=&quot;
pJDnkPn2f3yLO1NaAe14JQbS2RQ%3D&quot;</Value>
*/
static void gst_soup_http_src_get_info_by_key(gchar *psrc, gchar *ptarget, gchar* pdest)

{
  gchar *str_start = NULL, *str_end = NULL;
  int copy_str = 0;

  if (psrc == NULL || ptarget == NULL || pdest == NULL )
    return;

  str_start = strstr(psrc,ptarget);
  str_start += strlen(ptarget) + strlen("&quot;");
  str_end = strstr(str_start, "&quot;");

  if (str_end != NULL) {
    memcpy (pdest, str_start, str_end-str_start);
  }
}
...
static gchar *gst_soup_auth_gen_nonce() {
  ...
  const gchar *chars = "abcdefghijklmnopqrstuvwxyz"
  ...
  seed = (unsigned int)time(NULL);
   ...
   if(rndinit) {srand(time(NULL) * getpid()
  ); rndinit=0;} // seed random number generator - FIXME: we can do better ;)

  len=15+floor(rand_r(&seed)*16.0/(double)RAND_MAX);
  nc = (gchar*) g_malloc((len+1)*sizeof(gchar));
  for(i=0;i<len; i++) {
    nc[i] = chars[ rand_r(&seed) % max ];
  }
  nc[i]='\0';
...
{
  gchar *nc = NULL;
  const gchar *pc = AUTH_TIMESTAMP_SEED;
  int i = 0;
  int  timestamp_len  = strlen(pc);
  unsigned int seedp = 0;
  seedp = (unsigned int)time(NULL);

  //srand(time(NULL));
  int nRandom = rand_r(&seedp)/*rand()*/%(timestamp_len - 10);// Required Timestamp's length is 10

  nc = (gchar*) g_malloc(16*sizeof(gchar));
  for (i = 0; i < 10; i++) {
    nc[i] = pc[nRandom+i];
  }
  nc[i]='\0';
...
static void gst_soup_update_auth_info(GstSoupHTTPSrc * src)
{
  gchar auth_field[512] = {0};
  gchar post_body[512] = {0};
  gchar body_len_to_str[16] = {0};
  gchar post_url[512] = {0};
  gchar *oauth_once = NULL;
  gchar *oauth_timestamp = NULL;
  int body_len = 0;
  ...
  if (src->proxy == NULL) {
    session = soup_session_sync_new_with_options (
                        ...
                        SOUP_SESSION_USER_AGENT, "get auth info",
                        NULL);
  }
  else {
    session = soup_session_sync_new_with_options (
                        SOUP_SESSION_PROXY_URI, src->proxy,
                        SOUP_SESSION_TIMEOUT, 10,
                        SOUP_SESSION_USER_AGENT, "get auth info",
                        NULL);
  }
  snprintf(post_url, "https://fwk.allshareplay.com/ssg/websvc/%s/files/download", src->sp_name);
  GST_DEBUG_OBJECT (src, "post url is : %s\n", post_url);
  SoupMessage *msg = soup_message_new ("POST", post_url);
  if (msg == NULL) {
  	return;
  }
  oauth_once = gst_soup_auth_gen_nonce();
  oauth_timestamp = gst_soup_auth_gen_timestamp();

  snprintf(auth_field, "OAuth oauth_consumer_key=%s,oauth_nonce=%s,oauth_signature=_NONE_,oauth_signature_method=_NONE_,oauth_timestamp=%s,oauth_token=%s,oauth_version=1.0",
          "z61d999w3j",oauth_once,oauth_timestamp,src->access_token_for_auth);
  ...
  if (oauth_timestamp != NULL) {
    g_free(oauth_timestamp);
    oauth_timestamp = NULL;
  }

  snprintf(post_body, "<Request><UserID>%s</UserID><File><ID>%s</ID><Type>files</Type></File></Request>",
                      src->user_id_for_auth, src->app_id);
  body_len = strlen(post_body);

  GST_DEBUG_OBJECT (src, "autho info is: %s\n", auth_field);
  GST_DEBUG_OBJECT (src, "post body is: %s, length is: %d\n", post_body, body_len);
  snprintf(body_len_to_str, "%d", body_len);
  /*append authorization info*/
  soup_message_headers_append (msg->request_headers, "Host","fwk.allshareplay.com");
  soup_message_headers_append (msg->request_headers, "Accept","*/*");
  soup_message_headers_append (msg->request_headers, "Authorization",auth_field);

  soup_message_set_request (msg, "text/xml",SOUP_MEMORY_COPY, post_body, body_len);

  soup_message_headers_append (msg->request_headers, "x-pcw-appId","z61d999w3j");
  soup_message_headers_append (msg->request_headers, "x-pcw-authType","osp2");
  soup_message_headers_append (msg->request_headers, "x-pcw-loginId","vd.tv@samsung.com");
  soup_message_headers_append (msg->request_headers, "x-pcw-userId",src->user_id_for_auth);
  soup_message_headers_append (msg->request_headers, "x-pcw-version","v1");
  soup_message_headers_append (msg->request_headers, "Content-Length",body_len_to_str);

  soup_session_send_message (session, msg);

  /*If body contains auth info*/
  if (msg->response_body->length && strstr((char*)msg->response_body->data,"Authorization")) {
    gchar auth_token[128] = {0};
    gchar oauth_consumer_key[128] = {0};
    gchar oauth_version[128] = {0};
    gchar oauth_signature_method[128] = {0};
    gchar oauth_timestamp[128] = {0};
    gchar oauth_nonce[128] = {0};
    gchar oauth_signature[128] = {0};
    gchar *src_data = (gchar*)(msg->response_body->data);
    GstStructure *s = NULL;
    src->update_oauth_info = FALSE;
    // if (msg->response_body->length)
    GST_DEBUG_OBJECT (src, "response is :%d,  %s\n", msg->status_code,(char*)msg->response_body->data);
    memset(src->new_auth_info, 0, MAX_AUTH_INFO_SIZE);
    gst_soup_http_src_get_info_by_key(src_data,"auth_token=",auth_token);
    gst_soup_http_src_get_info_by_key(src_data,"oauth_consumer_key=",oauth_consumer_key);
    gst_soup_http_src_get_info_by_key(src_data,"oauth_version=",oauth_version);
    gst_soup_http_src_get_info_by_key(src_data,"oauth_signature_method=",oauth_signature_method);
    gst_soup_http_src_get_info_by_key(src_data,"oauth_timestamp=",oauth_timestamp);
    gst_soup_http_src_get_info_by_key(src_data,"oauth_nonce=",oauth_nonce);
    gst_soup_http_src_get_info_by_key(src_data,"oauth_signature=",oauth_signature);

    snprintf(src->new_auth_info, "OAuth oauth_token=\"%s\", oauth_consumer_key=\"%s\", oauth_version=\"%s\", oauth_signature_method=\"%s\", oauth_timestamp=\"%s\", oauth_nonce=\"%s\", oauth_signature=\"%s\"",
            auth_token,
            oauth_consumer_key,
            oauth_version,
            ...
            oauth_nonce,
            oauth_signature);
    GST_DEBUG_OBJECT(src, "new auth info  is: %s\n", src->new_auth_info);
    if (src->extra_headers != NULL) {
      gst_structure_free(src->extra_headers);
      src->extra_headers = NULL;
    }
    s = gst_structure_new("extra_header", "Authorization", G_TYPE_STRING, src->new_auth_info, NULL);
    src->extra_headers = s ? gst_structure_copy(s) : NULL;
  ...
  msg = NULL;
//  g_object_unref(msg);
//  gst_soup_http_src_session_close(session);
}
