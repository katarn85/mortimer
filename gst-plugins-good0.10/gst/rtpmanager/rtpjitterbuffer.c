 ...
 */
  /**
	* @fn		rtp_jitter_buffer_set_clock_rate
	* @brief
	*
	* @param[in]/[out]	: RTPJitterBuffer *jbuf, guint32 clock_rate
	* @return			: void
	* @warning			: None
	* @exception		: None
	* @see
...
rtp_jitter_buffer_set_clock_rate (RTPJitterBuffer *jbuf, guint32 clock_rate)
{
	...
	{
		if (jbuf->clock_rate == INVALID_OR_RESET_WITH_MINUS_ONE)
		{
 ...
 * Returns: the current clock-rate
  */
	...
	* @brief
	*
	* @param[in]/[out]	: RTPJitterBuffer *jbuf
	* @return			: guint32
	* @warning			: None
	* @exception		: None
	* @see
...
get_buffer_level (RTPJitterBuffer * jbuf)
{
  ...
  while (high_buf)
  {
	if (high_buf->dts != INVALID_OR_RESET_WITH_MINUS_ONE)
	{
		break;
	}
	else
  ...
  while (low_buf)
  {
	if (low_buf->dts != INVALID_OR_RESET_WITH_MINUS_ONE)
	{
		break;
	}
	else
...
calculate_skew (RTPJitterBuffer * jbuf, guint32 rtptime, GstClockTime time)
{
  ...
  if (jbuf->last_rtptime != INVALID_OR_RESET_WITH_MINUS_ONE && ext_rtptime == jbuf->last_rtptime)
  {
...
* @brief
*
* @param[in]/[out]	: RTPJitterBuffer *jbuf, GList *list, GList *item
* @return		: void
* @warning		: None
* @exception		: None
* @see
    ...
    gboolean * tail, gint * percent)
{
  ...
  if (item->seqnum == INVALID_OR_RESET_WITH_MINUS_ONE)
  {
	goto append;
  }
  else
  ...
  for (list = jbuf->packets->head; list; list = g_list_next (list)) {
    ...
    if (qitem->seqnum == INVALID_OR_RESET_WITH_MINUS_ONE)
	{
		continue;
	}
	else
  ...
  dts = item->dts;
  if (item->rtptime == INVALID_OR_RESET_WITH_MINUS_ONE)
  {
	goto append;
  }
  else
...
append:
  ...
  else if (percent) //for #RET change
  {
	*percent = -1;
  }
...
// ++ FEC Changes Book keeping.
/**
 * rtp_fecjitter_buffer_insert:
 * @jbuf: an #RTPJitterBuffer
 * @buf: a buffer
 * @tail: TRUE when the tail element changed.
 *
 ...
 */
 ...
 /**
* @fn		rtp_fecjitter_buffer_insert
* @brief
*
* @param[in]/[out]	: RTPJitterBuffer *jbuf, GstBuffer *buf, gint *percent
* @return		: gboolean
* @warning		: None
* @exception		: None
* @see
*/
gboolean
rtp_fecjitter_buffer_insert(RTPJitterBuffer *jbuf, RTPJitterBufferItem * item, gint *percent)
{
    GList *list=NULL;
    guint16 seqnum=ZERO;
    g_return_val_if_fail (jbuf != NULL, FALSE);
    ...
    seqnum = item->seqnum;
    //GST_DEBUG_OBJECT(jbuf, "Seq num of new fec buffer in list: %d", seqnum);
    /* loop the list to skip strictly smaller seqnum buffers */
    for (list = jbuf->packets->head; list; list = g_list_next (list))
    {
        guint16 qseq=INVALID_OR_RESET_WITH_MINUS_ONE;
        ...
        {
            GST_DEBUG_OBJECT(jbuf, "Position of new fec buffer in list: %d", gap);
            break;
        }
...
append:
    ...
    {
        *percent = INVALID_OR_RESET_WITH_MINUS_ONE;
    }
    return TRUE;
...
/**
 * fec_jitter_buffer_pop:
 * @jbuf: an #RTPJitterBuffer
 * @percent: the buffering percent
 *
 * Pops the oldest buffer(tail) from the packet queue of @jbuf. The popped buffer will
 * have its timestamp adjusted with the incomming running_time and the detected
 * clock skew.
 *
 * Returns: a #RTPJitterBufferItem or %NULL when there was no packet in the queue.
 */
RTPJitterBufferItem *
fec_jitter_buffer_pop (RTPJitterBuffer * jbuf, gint * percent)
{
  GList *item = NULL;
  GQueue *queue;
