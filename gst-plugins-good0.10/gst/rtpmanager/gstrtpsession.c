...
#include <string.h> //for #RET change
...
# define ZERO 0
# define ONE 1
# define DEFAULT_DEADLINE_VALUE 100
# define MAX_RTT_DELAY_ALLOWED 40
...
struct _GstRtpSessionPrivate
{
  ...
  gboolean wait_send;			//for #RET change

  /* caps mapping */
  GHashTable *ptmap;
  GstClockTime send_latency;	//for #RET change
...
/**
	* @fn		gst_rtp_session_sync_rtcp
	* @brief
	*
	* @param[in]/[out]	:RTPSession * sess,GstBuffer * buffer, gpointer user_data
...
static gint gst_rtp_session_clock_rate (RTPSession * sess, guint8 payload,
    gpointer user_data);
/**
	* @fn			gst_rtp_session_rtx_src
	* @brief
	*
	* @param[in]/[out]	:RTPSession * sess, guint8 payload, gpointer user_data
	* @return			: gint
	* @warning			: None
	* @exception		: None
	* @see
	*/
static gint gst_rtp_session_rtx_src (RTPSession * sess, guint8 payload,
    gpointer user_data);
...
static GstClockTime gst_rtp_session_request_time (RTPSession * session,
    gpointer user_data);
/**
	* @fn		gst_rtp_session_notify_nack
	* @brief
	*
	* @param[in]/[out]	: RTPSession * sess, guint16 seqnum, guint16 blp, gpointer user_data
	* @return			: void
	* @warning			: None
	* @exception		: None
	* @see
	*/
static void gst_rtp_session_notify_nack (RTPSession * sess, guint16 seqnum, guint16 blp, gpointer user_data);
...
/**
	* @fn		gst_rtp_session_sink_setcaps
	* @brief
	*
	* @param[in]/[out]	: GstPad * pad, GstCaps * caps
	* @return			: gboolean
	* @warning			: None
	* @exception		: None
	* @see
...
/**
	* @fn		gst_rtp_session_setcaps_send_rtp
	* @brief
	*
	* @param[in]/[out]	: GstPad * pad, GstCaps * caps
	* @return			: gboolean
...
gst_rtp_session_init (GstRtpSession * rtpsession, GstRtpSessionClass * klass)
{
  ...
  rtpsession->priv->cond = g_cond_new ();					//for #RET change
...
gst_rtp_session_finalize (GObject * object)
{
  ...
  g_cond_free (rtpsession->priv->cond);   //for #RET change
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id)
  {
	...
	case PROP_DO_RETRANSMISSION:  //for #RET change
		g_object_set_property (G_OBJECT (priv->session), "do-retransmission", value);
		break;
    ...
    GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id)
  {
	...
	case PROP_BANDWIDTH:
		...
		break;
	case PROP_DO_RETRANSMISSION:     //for #RET change
		g_object_get_property (G_OBJECT (priv->session), "do-retransmission", value);
		break;
	case PROP_RTCP_FRACTION:
...
/* get current clock time and convert to running time */
    rt = clock_time - base_time; //for #RET change
    if (rtpsession->priv->use_pipeline_clock)
	{
      ntpns = rt;  //for #RET change
    }
	else
	{
      GTimeVal current;
...
stop_rtcp_thread (GstRtpSession * rtpsession)
{
  ...
  rtpsession->priv->wait_send = FALSE;		//for #RET change
  GST_RTP_SESSION_SIGNAL (rtpsession); 		//for #RET change
...
gst_rtp_session_change_state (GstElement * element, GstStateChange transition)
{
  ...
  switch (transition)
  {
	...
	case GST_STATE_CHANGE_READY_TO_PAUSED:
		GST_RTP_SESSION_LOCK (rtpsession);  //for #RET change
...
//for #RET change
/* called when the session manager needs the rtx-src */
/**
	* @fn		gst_rtp_session_rtx_src
	* @brief
	*
...
static gint
gst_rtp_session_rtx_src (RTPSession * sess, guint8 payload,
    gpointer user_data)
{
  gint result = INVALID_OR_RESET_WITH_MINUS_ONE;
  GstRtpSession *rtpsession=NULL;
  GstCaps *caps=NULL;
  const GstStructure *s=NULL;
  const gchar* rtx_scheme=NULL;
  ...
  if (gst_structure_get_int (s, "apt", &result))
  {
	rtx_scheme = gst_structure_get_string(s, "a-rtxscheme");
  ...
  GST_DEBUG_OBJECT (rtpsession, "GAURAV parsed rtx-src %d", result);
done:
...
gst_rtp_session_event_recv_rtp_src (GstPad * pad, GstEvent * event)
{
  ...
  switch (GST_EVENT_TYPE (event))
  {
	...
	{
		GstClockTime running_time=INVALID_OR_RESET_WITH_MINUS_ONE;
		guint seqnum=INVALID_OR_RESET_WITH_MINUS_ONE, delay=ZERO, deadline=DEFAULT_DEADLINE_VALUE, max_delay=ZERO;
		if (!gst_structure_get_clock_time (s, "running-time", &running_time))
		{
			running_time = INVALID_OR_RESET_WITH_MINUS_ONE;
		}
		else
		{
			GST_DEBUG("Warning");
		...
		{
			ssrc = INVALID_OR_RESET_WITH_MINUS_ONE;
		}
		else
		{
			GST_DEBUG("Warning");
		...
		{
			seqnum = INVALID_OR_RESET_WITH_MINUS_ONE;
		}
		else
		{
			GST_DEBUG("Warning");
		...
		{
			deadline = DEFAULT_DEADLINE_VALUE;
		}
		else
		{
			GST_DEBUG("Warning");
		...
		/* estimated RTT */
		if (max_delay > MAX_RTT_DELAY_ALLOWED)
		{
			max_delay -= MAX_RTT_DELAY_ALLOWED;
		}
		else
		{
			max_delay = ZERO;
...
gst_rtp_session_event_send_rtp_sink (GstPad * pad, GstEvent * event)
{
  ...
  switch (GST_EVENT_TYPE (event)) {
    ...
    case GST_EVENT_EOS:{
      ...
      rtp_session_schedule_bye (rtpsession->priv->session,
          current_time); //for #RET change
      break;
    }
    default:{
...
//for #RET change
/**
* @fn		gst_rtp_session_event_send_rtp_src
* @brief
*
...
*/
static gboolean
gst_rtp_session_event_send_rtp_src (GstPad * pad,
    GstEvent * event)
{
  GstRtpSession *rtpsession=NULL;
  gboolean ret = FALSE;
...
gst_rtp_session_getcaps_send_rtp (GstPad * pad)
{
  ...
  ssrc = rtp_session_suggest_ssrc (priv->session); //for #RET change
...
//for #RET change
/**
	* @fn		gst_rtp_session_query_send_rtp
	* @brief
	*
	* @param[in]/[out]	:GstPad * pad, GstQuery * query
	* @return			: gboolean
	* @warning			: None
	* @exception		: None
	* @see
    ...
    gboolean is_list)
{
  ...
  if (GST_CLOCK_TIME_IS_VALID (timestamp)) {
    ...
    running_time =
        ...
        timestamp);
    running_time += priv->send_latency; //for #RET change
  }
  else
  {
    /* no timestamp. */
...
create_send_rtp_sink (GstRtpSession * rtpsession)
{
  ...
  gst_pad_set_query_function (rtpsession->send_rtp_sink,  //for #RET change
