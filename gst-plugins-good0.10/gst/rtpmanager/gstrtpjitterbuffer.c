...
#define MAX_FEC_PACKET_LIST_SIZE 2
#define INVALID_OR_RESET_WITH_MINUS_ONE -1
#define ZERO 0
#define ZERO_ZERO 0.0
#define PLUS_ONE 1
#define TWO 2
#define SEVEN 7
#define EIGHT 8
#define THOUSAND 1000
#define MAX_SEQNUM_VAL 0xffff
...
#define DEFAULT_DO_FEC  FALSE

// ++ FEC Changes Bookkeeping
#define DEFAULT_RTP_PKTS_FOR_FEC 15
#define DEFAULT_FEC_SPAN 2
// -- FEC Changes Bookkeeping
...
struct _GstRtpJitterBufferPrivate
{
  ...
  GstPad *rtcprtxsinkpad, *rtxsinkpad; //for #RET change
  GstPad *rtcpfecsinkpad, *fecsinkpad; //FEC changes
  guint32 fec_ssrc; //ONLY USED IN CASE OF SSSRC multiplexing FEC

  RTPJitterBuffer *jbuf;
  // ++ FEC Changes Bookkeeping
  RTPJitterBuffer *jbuffec;
  RTPJitterBuffer *fecrtpjbuf;
  gint fecapt;
  // -- FEC Changes Bookkeeping
  GMutex *jbuf_lock;
  ...
  gboolean do_fec;  //FEC changes
  gint last_fecpt;
  // ++ FEC Changes Bookkeeping
  guint32 next_seqnum_fec;
  guint32 next_in_seqnum_fec;
  guint32 last_in_seqnum_fec;
  GstClockTime last_in_dts_fec;
  gint32 clock_rate_fec;
  // -- FEC Changes Bookkeeping
  ...
  GstClockID eos_id;
// gboolean reached_npt_stop; 	//for #RET change

  /* state */
  gboolean eos;

  /* clock rate and rtp timestamp offset */
  gint apt;  					//for #RET change
  gint last_rtxpt;  			//for #RET change
  gint last_pt;
  gint32 clock_rate;
  gint64 clock_base;
  ...
  /* for sync */
  GstSegment segment;
  GstClockID clock_id;			//for #RET change
  GstClockTime timer_timeout;	//for #RET change
  ...
  guint64 ext_rtptime;			//for #RET change
  GstBuffer *last_sr;			//for #RET change
...
static GstStaticPadTemplate gst_rtp_jitter_buffer_sink_rtx_template =
GST_STATIC_PAD_TEMPLATE ("sink_rtx",
    GST_PAD_SINK,
    GST_PAD_REQUEST,
    GST_STATIC_CAPS ("application/x-rtp, "
        "clock-rate = (int) [ 1, 2147483647 ]"
         ...
         * "encoding-name = (string) "
         */ )
    );/**< pad template for rtx sink */


static GstStaticPadTemplate gst_rtp_jitter_buffer_sink_rtcp_rtx_template =
GST_STATIC_PAD_TEMPLATE ("sink_rtcp_rtx",
    GST_PAD_SINK,
    GST_PAD_REQUEST,
    GST_STATIC_CAPS ("application/x-rtcp")
    );/**< pad template for rtcp rtx sink */
...
//FEC changes
static GstStaticPadTemplate gst_rtp_jitter_buffer_sink_fec_template =
GST_STATIC_PAD_TEMPLATE ("sink_fec",
    GST_PAD_SINK,
    GST_PAD_REQUEST,
    GST_STATIC_CAPS ("application/x-rtp, "
        "clock-rate = (int) [ 1, 2147483647 ]"
...
static GstStaticPadTemplate gst_rtp_jitter_buffer_sink_rtcp_fec_template =
GST_STATIC_PAD_TEMPLATE ("sink_rtcp_fec",
    GST_PAD_SINK,
    GST_PAD_REQUEST,
    GST_STATIC_CAPS ("application/x-rtcp")
    );
...
static GstFlowReturn gst_rtp_jitter_buffer_chain_rtcp (GstPad * pad,
    GstBuffer * buffer);
/**
	* @fn		gst_rtp_jitter_buffer_sink_rtcp_rtx_event
	* @brief
	*
	* @param[in]/[out]	: GstPad * pad, GstEvent * event
	* @return			: gboolean
	* @warning			: None
	* @exception		: None
	* @see
	*/
static gboolean gst_rtp_jitter_buffer_sink_rtcp_rtx_event (GstPad *pad, GstEvent *event);
/**
	* @fn		gst_rtp_jitter_buffer_chain_rtcp_rtx
	* @brief
	*
	* @param[in]/[out]	: GstPad * pad, GstBuffer * buffer
	* @return			: GstFlowReturn
	* @warning			: None
	* @exception		: None
	* @see
	*/
static GstFlowReturn gst_rtp_jitter_buffer_chain_rtcp_rtx (GstPad *pad, GstBuffer *buffer);
/**
	* @fn		gst_rtp_jitter_buffer_sink_rtx_event
	* @brief
	*
	* @param[in]/[out]	: GstPad * pad, GstEvent * event
	* @return			: gboolean
	...
	* @see
	*/
static gboolean gst_rtp_jitter_buffer_sink_rtx_event (GstPad *pad, GstEvent *event);
/**
	* @fn		gst_rtp_jitter_buffer_chain_rtx
	* @brief
	*
	* @param[in]/[out]	: GstPad * pad, GstBuffer * buffer
	* @return			:GstFlowReturn
	...
	* @see
	*/
static GstFlowReturn gst_rtp_jitter_buffer_chain_rtx (GstPad *pad, GstBuffer *buffer);


//FEC changes
static gboolean gst_rtp_jitter_buffer_sink_rtcp_fec_event (GstPad * pad, GstEvent * event);
static GstFlowReturn gst_rtp_jitter_buffer_chain_rtcp_fec (GstPad * pad, GstBuffer * buffer);
static gboolean gst_rtp_jitter_buffer_sink_fec_event (GstPad * pad, GstEvent * event);
static GstFlowReturn gst_rtp_jitter_buffer_chain_fec (GstPad * pad, GstBuffer * buffer);
...
/**
	...
	* @brief
	*
	* @param[in]/[out]	: GstRtpJitterBuffer * jitterbuffer
	* @return			: void
	* @warning			: None
	* @exception		: None
	* @see
...
//for #RET change
  gst_element_class_add_static_pad_template (element_class,
      &gst_rtp_jitter_buffer_sink_rtcp_rtx_template);
  gst_element_class_add_static_pad_template (element_class,
      &gst_rtp_jitter_buffer_sink_rtx_template);

 //FEC changes
  gst_element_class_add_static_pad_template (element_class,
	  &gst_rtp_jitter_buffer_sink_rtcp_fec_template);
  gst_element_class_add_static_pad_template (element_class,
	  &gst_rtp_jitter_buffer_sink_fec_template);
...
//for #RET change
   ...
   *
   * When INVALID_OR_RESET_WITH_MINUS_ONE is used, the max jitter will be used as extra delay.
   *
   * Since: 1.2
   */
  g_object_class_install_property (gobject_class, PROP_RTX_DELAY,
      g_param_spec_int ("rtx-delay", "RTX Delay",
          ...
          "event (INVALID_OR_RESET_WITH_MINUS_ONE automatic)", INVALID_OR_RESET_WITH_MINUS_ONE, G_MAXINT, DEFAULT_RTX_DELAY,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
//for #RET change
  /**
   ...
   *
   * When INVALID_OR_RESET_WITH_MINUS_ONE is used, the value will be estimated based on observed packet
   * reordering.
   *
   * Since: 1.2
   */
  g_object_class_install_property (gobject_class, PROP_RTX_DELAY_REORDER,
      g_param_spec_int ("rtx-delay-reorder", "RTX Delay Reorder",
          "Sending retransmission event when this much reordering (INVALID_OR_RESET_WITH_MINUS_ONE automatic)",
          INVALID_OR_RESET_WITH_MINUS_ONE, G_MAXINT, DEFAULT_RTX_DELAY_REORDER,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
//for #RET change
  /**
   ...
   *
   * When INVALID_OR_RESET_WITH_MINUS_ONE is used, the value will be estimated based on observed round
   * trip time.
   *
   * Since: 1.2
   */
  g_object_class_install_property (gobject_class, PROP_RTX_RETRY_TIMEOUT,
      g_param_spec_int ("rtx-retry-timeout", "RTX Retry Timeout",
          ...
          "ms (INVALID_OR_RESET_WITH_MINUS_ONE automatic)", INVALID_OR_RESET_WITH_MINUS_ONE, G_MAXINT, DEFAULT_RTX_RETRY_TIMEOUT,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
//for #RET change
  /**
   ...
   *
   * When INVALID_OR_RESET_WITH_MINUS_ONE is used, the value will be estimated based on the jitterbuffer
  ...
  g_object_class_install_property (gobject_class, PROP_RTX_RETRY_PERIOD,
      g_param_spec_int ("rtx-retry-period", "RTX Retry Period",
          ...
          "(INVALID_OR_RESET_WITH_MINUS_ONE automatic)", INVALID_OR_RESET_WITH_MINUS_ONE, G_MAXINT, DEFAULT_RTX_RETRY_PERIOD,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

 //FEC changes
  /**
   * GstRtpJitterBuffer::do-fec:
   *
   * Correct the Error Packets when the recieved packet is having any error or if Packet is lost.
   *
   * Since: 1.2
   */
  g_object_class_install_property (gobject_class, PROP_DO_FEC,
      g_param_spec_boolean ("do-fec", "Do FEC",
          "Do Error Recovery when there is a error in packet or on lost packet",
          DEFAULT_DO_FEC, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    ...
    GstRtpJitterBufferClass * klass)
{
  ...
  priv->do_retransmission = DEFAULT_DO_RETRANSMISSION;  		//for #RET change
  priv->rtx_delay = DEFAULT_RTX_DELAY;							//for #RET change
  priv->rtx_delay_reorder = DEFAULT_RTX_DELAY_REORDER;			//for #RET change
  priv->rtx_retry_timeout = DEFAULT_RTX_RETRY_TIMEOUT;			//for #RET change
  priv->rtx_retry_period = DEFAULT_RTX_RETRY_PERIOD;			//for #RET change

  priv->do_fec = DEFAULT_DO_FEC; //FEC changes
  priv->chain_rtx=FALSE;
  priv->timers = g_array_new(FALSE,TRUE,sizeof(TimerData));	//for #RET change

  priv->jbuf = rtp_jitter_buffer_new ();
  priv->jbuf_lock = g_mutex_new ();
  priv->jbuf_timer = g_cond_new ();
  priv->jbuf_event = g_cond_new ();
  // ++ FEC Changes Bookkeeping
  priv->jbuffec = rtp_jitter_buffer_new ();
  priv->fecrtpjbuf = rtp_jitter_buffer_new ();
  // -- FEC Changes Bookkeeping
...
/**
	...
	* @brief
	*
	* @param[in]/[out]	: gpointer data, guint type, GstClockTime dts, GstClockTime pts, guint seqnum, guint count, guint rtptime
	* @return			: RTPJitterBufferItem*
	* @warning			: None
	* @exception		: None
	* @see
	*/
...
/**
	...
	* @brief
	*
	* @param[in]/[out]	: RTPJitterBufferItem * item
	* @return			: void
	* @warning			: None
	* @exception		: None
	* @see
...
gst_rtp_jitter_buffer_finalize (GObject * object)
{
  ...
  GstRtpJitterBufferPrivate *priv=NULL;
  ...
  // ++ FEC Changes Bookkeeping
  rtp_jitter_buffer_flush (priv->jbuffec, (GFunc) free_item, NULL);
  g_object_unref (priv->jbuffec);
  rtp_jitter_buffer_flush (priv->fecrtpjbuf, (GFunc) free_item, NULL);
  g_object_unref (priv->fecrtpjbuf);
  // -- FEC Changes Bookkeeping
...
gst_rtp_jitter_buffer_iterate_internal_links (GstPad * pad)
{
  ...
  else if (pad == jitterbuffer->priv->rtcprtxsinkpad)
  {
		//for #RET change
		otherpad = NULL;
  }
  else if (pad == jitterbuffer->priv->rtxsinkpad)
  {
		//for #RET change
		otherpad = NULL;
  }
  else if (pad == jitterbuffer->priv->rtcpfecsinkpad)
  {
		otherpad = NULL;
  }
  else if (pad == jitterbuffer->priv->fecsinkpad)
  {
		otherpad = NULL;
  }
  else
...
/**
	...
	* @see
	*/
static GstPad *create_rtcp_rtx_sink (GstRtpJitterBuffer *jitterbuffer)
{
  GstRtpJitterBufferPrivate *priv=NULL;

  priv = jitterbuffer->priv;

  GST_DEBUG_OBJECT (jitterbuffer, "creating RTCP RTX sink pad");

  priv->rtcprtxsinkpad =
      gst_pad_new_from_static_template
      (&gst_rtp_jitter_buffer_sink_rtcp_rtx_template, "sink_rtcp_rtx");
  gst_pad_set_chain_function (priv->rtcprtxsinkpad,
      gst_rtp_jitter_buffer_chain_rtcp_rtx);
  gst_pad_set_event_function (priv->rtcprtxsinkpad,(GstPadEventFunction)gst_rtp_jitter_buffer_sink_rtcp_rtx_event);

  gst_pad_set_iterate_internal_links_function (priv->rtcprtxsinkpad,
      gst_rtp_jitter_buffer_iterate_internal_links);
  gst_pad_set_active (priv->rtcprtxsinkpad, TRUE);
  gst_element_add_pad (GST_ELEMENT_CAST (jitterbuffer), priv->rtcprtxsinkpad);

  return priv->rtcprtxsinkpad;
}
/**
	* @fn		create_rtx_sink
	* @brief
	...
	* @see
	*/
static GstPad *create_rtx_sink (GstRtpJitterBuffer *jitterbuffer)
{
  GstRtpJitterBufferPrivate *priv=NULL;

  priv = jitterbuffer->priv;

  GST_DEBUG_OBJECT (jitterbuffer, "creating RTP RTX sink pad");

  priv->rtxsinkpad =
      gst_pad_new_from_static_template
      (&gst_rtp_jitter_buffer_sink_rtx_template, "sink_rtx");
  gst_pad_set_chain_function (priv->rtxsinkpad,
      gst_rtp_jitter_buffer_chain_rtx);
  gst_pad_set_event_function (priv->rtxsinkpad,(GstPadEventFunction)gst_rtp_jitter_buffer_sink_rtx_event);

  gst_pad_set_iterate_internal_links_function (priv->rtxsinkpad,
      gst_rtp_jitter_buffer_iterate_internal_links);
  gst_pad_set_active (priv->rtxsinkpad, TRUE);
  gst_element_add_pad (GST_ELEMENT_CAST (jitterbuffer), priv->rtxsinkpad);

  return priv->rtxsinkpad;
}

//FEC changes
static GstPad *
create_rtcp_fec_sink (GstRtpJitterBuffer * jitterbuffer)
{
  GstRtpJitterBufferPrivate *priv;

  priv = jitterbuffer->priv;

  GST_DEBUG_OBJECT (jitterbuffer, "creating RTCP FEC sink pad");

  priv->rtcpfecsinkpad = gst_pad_new_from_static_template (&gst_rtp_jitter_buffer_sink_rtcp_fec_template, "sink_rtcp_fec");
  gst_pad_set_chain_function (priv->rtcpfecsinkpad, gst_rtp_jitter_buffer_chain_rtcp_fec);
  gst_pad_set_event_function (priv->rtcpfecsinkpad, (GstPadEventFunction) gst_rtp_jitter_buffer_sink_rtcp_fec_event);
  gst_pad_set_iterate_internal_links_function (priv->rtcpfecsinkpad, gst_rtp_jitter_buffer_iterate_internal_links);
  gst_pad_set_active (priv->rtcpfecsinkpad, TRUE);
  gst_element_add_pad (GST_ELEMENT_CAST (jitterbuffer), priv->rtcpfecsinkpad);

  return priv->rtcpfecsinkpad;
}
static GstPad *
create_fec_sink (GstRtpJitterBuffer * jitterbuffer)
{
  GstRtpJitterBufferPrivate *priv;

  priv = jitterbuffer->priv;

  GST_DEBUG_OBJECT (jitterbuffer, "creating RTP FEC sink pad");

  priv->fecsinkpad = gst_pad_new_from_static_template (&gst_rtp_jitter_buffer_sink_fec_template, "sink_fec");
  gst_pad_set_chain_function (priv->fecsinkpad, gst_rtp_jitter_buffer_chain_fec);
  gst_pad_set_event_function (priv->fecsinkpad, (GstPadEventFunction) gst_rtp_jitter_buffer_sink_fec_event);
  gst_pad_set_iterate_internal_links_function (priv->fecsinkpad, gst_rtp_jitter_buffer_iterate_internal_links);
  gst_pad_set_active (priv->fecsinkpad, TRUE);
  gst_element_add_pad (GST_ELEMENT_CAST (jitterbuffer), priv->fecsinkpad);
  return priv->fecsinkpad;
}
...
/**
	...
	*/
static void
remove_rtcp_rtx_sink (GstRtpJitterBuffer *jitterbuffer)
{
  GstRtpJitterBufferPrivate *priv=NULL;

  priv = jitterbuffer->priv;

  GST_DEBUG_OBJECT (jitterbuffer, "removing RTCP RTX sink pad");

  gst_pad_set_active (priv->rtcprtxsinkpad, FALSE);

  gst_element_remove_pad (GST_ELEMENT_CAST (jitterbuffer), priv->rtcprtxsinkpad);
  priv->rtcprtxsinkpad = NULL;
}
/**
	* @fn		remove_rtx_sink
	* @brief
	...
	*/
static void
remove_rtx_sink (GstRtpJitterBuffer *jitterbuffer)
{
  GstRtpJitterBufferPrivate *priv=NULL;

  priv = jitterbuffer->priv;

  GST_DEBUG_OBJECT (jitterbuffer, "removing RTP RTX sink pad");

  gst_pad_set_active (priv->rtxsinkpad, FALSE);

  gst_element_remove_pad (GST_ELEMENT_CAST (jitterbuffer), priv->rtxsinkpad);
  priv->rtxsinkpad = NULL;
}

//FEC changes
static void
remove_rtcp_fec_sink (GstRtpJitterBuffer * jitterbuffer)
{
  GstRtpJitterBufferPrivate *priv;

  priv = jitterbuffer->priv;

  GST_DEBUG_OBJECT (jitterbuffer, "removing RTCP FEC sink pad");

  gst_pad_set_active (priv->rtcpfecsinkpad, FALSE);

  gst_element_remove_pad (GST_ELEMENT_CAST (jitterbuffer), priv->rtcpfecsinkpad);
  priv->rtcpfecsinkpad = NULL;
}

static void
remove_fec_sink (GstRtpJitterBuffer * jitterbuffer)
{
  GstRtpJitterBufferPrivate *priv;

  priv = jitterbuffer->priv;

  GST_DEBUG_OBJECT (jitterbuffer, "removing RTP FEC sink pad");

  gst_pad_set_active (priv->fecsinkpad, FALSE);

  gst_element_remove_pad (GST_ELEMENT_CAST (jitterbuffer), priv->fecsinkpad);
  priv->fecsinkpad = NULL;
}
    ...
    GstPadTemplate * templ, const gchar * name)
{
  ...
  else if (templ == gst_element_class_get_pad_template (klass, "sink_rtcp_rtx"))
  {
	//for #RET change
	if (priv->rtcprtxsinkpad != NULL)
	{
		goto exists;
	}
	if((result = create_rtcp_rtx_sink (jitterbuffer))==NULL)
	{
		; //NULL Bypassing CRC
	}
	else
  ...
  else if (templ == gst_element_class_get_pad_template (klass, "sink_rtx"))
  {
	//for #RET change
	if (priv->rtxsinkpad != NULL)
	{
		goto exists;
	}
	if((result = create_rtx_sink (jitterbuffer))==NULL)
	{
		; //NULL Bypassing CRC
	}
	else
  ...
  else if (templ == gst_element_class_get_pad_template (klass, "sink_rtcp_fec"))
  {
	if (priv->rtcpfecsinkpad != NULL)
	{
		goto exists;
	}
	result = create_rtcp_fec_sink (jitterbuffer);
  }
  else if (templ == gst_element_class_get_pad_template (klass, "sink_fec"))
  {
	if (priv->fecsinkpad != NULL)
	{
		goto exists;
	}
	else
	...
	result = create_fec_sink (jitterbuffer);
  }
  else
  {
    goto wrong_template;
...
gst_rtp_jitter_buffer_release_pad (GstElement * element, GstPad * pad)
{
  ...
  else if (priv->rtcprtxsinkpad == pad)
  {
	//for #RET change
	remove_rtcp_rtx_sink (jitterbuffer);
  }
  else if (priv->rtxsinkpad == pad)
  {
	//for #RET change
	remove_rtx_sink (jitterbuffer);
  }
  else if (priv->rtcpfecsinkpad == pad)
  {
  	remove_rtcp_fec_sink (jitterbuffer);
  }
  else if (priv->fecsinkpad == pad)
  {
  	remove_fec_sink (jitterbuffer);
  }
  else
  {
  	goto wrong_pad;
...
gst_rtp_jitter_buffer_clear_pt_map (GstRtpJitterBuffer * jitterbuffer)
{
  ...
  JBUF_LOCK (priv);
  priv->clock_rate = INVALID_OR_RESET_WITH_MINUS_ONE;
  /* do not clear current content, but refresh state for new arrival */
  GST_DEBUG_OBJECT (jitterbuffer, "reset jitterbuffer");
  rtp_jitter_buffer_reset_skew (priv->jbuf);
  //priv->last_popped_seqnum = INVALID_OR_RESET_WITH_MINUS_ONE;
  //priv->next_seqnum = INVALID_OR_RESET_WITH_MINUS_ONE;
  JBUF_UNLOCK (priv);
}
    ...
    GstCaps * caps)
{
  ...
  guint val;
  GstClockTime tval;
  gint apt=ZERO;  //for #RET change

  priv = jitterbuffer->priv;

  /* first parse the caps */
  caps_struct = gst_caps_get_structure (caps, ZERO);

  GST_DEBUG_OBJECT (jitterbuffer, "got caps");
/* //for #RET change
  if(!priv->do_retransmission)
  {
     rtx_scheme = gst_structure_get_string(caps_struct, "a-rtxscheme");
     if(rtx_scheme && ((!strcmp(rtx_scheme,"session")) || (!strcmp(rtx_scheme,"ssrc"))))
	{
	priv->do_retransmission = TRUE;
	GST_DEBUG("GENERIC NACK IMPLEMENTATION ENABLED");
        }
     else
	GST_DEBUG ("RTX scheme Either not found or Not matched with session and ssrc");
  }
*/
//for #RET change
  if (gst_structure_get_int (caps_struct, "apt", &apt))
  {
	priv->apt = apt;
  }
  else
  {
	priv->apt = INVALID_OR_RESET_WITH_MINUS_ONE;
  }

  GST_DEBUG_OBJECT (jitterbuffer, "got apt %d", priv->apt);
  /* we need a clock-rate to convert the rtp timestamps to GStreamer time and to
   * measure the amount of data in the buffer */
  if (!gst_structure_get_int (caps_struct, "clock-rate", &priv->clock_rate))
    goto error;

  if (priv->clock_rate <= ZERO)
  {
  	goto wrong_rate;
  }
  ...
  if (gst_structure_get_uint (caps_struct, "clock-base", &val))
	...
	{
		priv->clock_base = INVALID_OR_RESET_WITH_MINUS_ONE;
	}
  ...
  if (gst_structure_get_uint (caps_struct, "seqnum-base", &val))
  {
	...
	if ((priv->next_in_seqnum == INVALID_OR_RESET_WITH_MINUS_ONE) && (val != ZERO)) //to be removed fix for commonrtsp
	{
		priv->next_in_seqnum = val;
	}
	if ((priv->next_seqnum == INVALID_OR_RESET_WITH_MINUS_ONE) && (val != ZERO))  //to be removed fix for commonrtsp
	{
  ...
  if (gst_structure_get_clock_time (caps_struct, "npt-stop", &tval))
	...
	{
		priv->npt_stop = INVALID_OR_RESET_WITH_MINUS_ONE;
	}
...
// ++ FEC Changes Bookkeeping
/*
 * Must be called with JBUFFEC_LOCK held
 */

static gboolean
gst_fecjitter_buffer_sink_parse_caps (GstRtpJitterBuffer * jitterbuffer,
    GstCaps * caps)
{
  GstRtpJitterBufferPrivate *priv;
  GstStructure *caps_struct;
  ...
  if (gst_structure_get_int (caps_struct, "apt", &apt))
	...
	{
		priv->fecapt = INVALID_OR_RESET_WITH_MINUS_ONE;
	}

  GST_DEBUG_OBJECT (jitterbuffer, "got fec apt %d", priv->fecapt);

  /* we need a clock-rate to convert the rtp timestamps to GStreamer time and to
   * measure the amount of data in the buffer */
  if (!gst_structure_get_int (caps_struct, "clock-rate", &priv->clock_rate_fec))
  {
	goto error;
  }
  else
  ...
  GST_DEBUG_OBJECT (jitterbuffer, "got clock-rate %d", priv->clock_rate_fec);
...
gst_rtp_jitter_buffer_flush_stop (GstRtpJitterBuffer * jitterbuffer)
{
  ...
  priv->last_popped_seqnum = INVALID_OR_RESET_WITH_MINUS_ONE;
  priv->last_out_time = INVALID_OR_RESET_WITH_MINUS_ONE;
  priv->next_seqnum = INVALID_OR_RESET_WITH_MINUS_ONE;
//for #RET change
  priv->ips_rtptime = INVALID_OR_RESET_WITH_MINUS_ONE;
  priv->ips_dts = GST_CLOCK_TIME_NONE;
  priv->packet_spacing = ZERO;
  priv->next_in_seqnum = INVALID_OR_RESET_WITH_MINUS_ONE;
  priv->clock_rate = INVALID_OR_RESET_WITH_MINUS_ONE;
  // ++ FEC Changes Bookkeeping
  priv->clock_rate_fec = INVALID_OR_RESET_WITH_MINUS_ONE;
  priv->next_in_seqnum_fec = INVALID_OR_RESET_WITH_MINUS_ONE;
  priv->next_seqnum_fec = INVALID_OR_RESET_WITH_MINUS_ONE;
  // ++ FEC Changes Bookkeeping
  priv->eos = FALSE;
  priv->estimated_eos = INVALID_OR_RESET_WITH_MINUS_ONE;
  priv->last_elapsed = ZERO;
//priv->reached_npt_stop = FALSE; //for #RET change
  priv->ext_timestamp = INVALID_OR_RESET_WITH_MINUS_ONE;
  GST_DEBUG_OBJECT (jitterbuffer, "flush and reset jitterbuffer");
  rtp_jitter_buffer_flush (priv->jbuf, (GFunc) free_item, NULL);
  rtp_jitter_buffer_reset_skew (priv->jbuf);
  remove_all_timers (jitterbuffer); //for #RET change
  // ++ FEC Changes Bookkeeping
  rtp_jitter_buffer_flush (priv->jbuffec, (GFunc) free_item, NULL);
  rtp_jitter_buffer_flush (priv->fecrtpjbuf, (GFunc) free_item, NULL);
  // -- FEC Changes Bookkeeping
  ...
  priv = jitterbuffer->priv;
	...
	switch (transition)
	{
		...
		case GST_STATE_CHANGE_READY_TO_PAUSED:
			...
			// ++ FEC Changes Bookkeeping
			priv->clock_base = INVALID_OR_RESET_WITH_MINUS_ONE;
			priv->peer_latency = ZERO;
			priv->last_pt = INVALID_OR_RESET_WITH_MINUS_ONE;
			priv->last_rtxpt = INVALID_OR_RESET_WITH_MINUS_ONE;    //for #RET change
			priv->apt = INVALID_OR_RESET_WITH_MINUS_ONE;			//for #RET change
			//FEC changes
			priv->last_fecpt = INVALID_OR_RESET_WITH_MINUS_ONE;
			priv->fec_ssrc = ZERO;
  ...
  switch (GST_EVENT_TYPE (event))
  {
	case GST_EVENT_LATENCY:
	{
		...
		* large latency or queues, we would start rebuffering again. */
		if (rtp_jitter_buffer_get_mode (priv->jbuf) ==RTP_JITTER_BUFFER_MODE_BUFFER)
		{
 ...
 * LOCK */
 /**
	...
	* @brief
	*
	* @param[in]/[out]	: GstRtpJitterBuffer *jitterbuffer, GstEvent *event
	* @return			: gboolean
	* @warning			: None
	* @exception		: None
	* @see
...
queue_event (GstRtpJitterBuffer *jitterbuffer, GstEvent *event)
{
  ...
  switch (GST_EVENT_TYPE (event))
  {
	...
	{
		GstFormat format;
		gdouble rate=INVALID_OR_RESET_WITH_MINUS_ONE, arate=INVALID_OR_RESET_WITH_MINUS_ONE;
		gint64 start=INVALID_OR_RESET_WITH_MINUS_ONE, stop=INVALID_OR_RESET_WITH_MINUS_ONE, time=INVALID_OR_RESET_WITH_MINUS_ONE;
		gboolean update=ZERO;
...
gst_rtp_jitter_buffer_sink_event (GstPad * pad, GstEvent * event)
{
  ...
  switch (GST_EVENT_TYPE (event))
  {
	case GST_EVENT_FLUSH_START:
		ret = gst_pad_push_event (priv->srcpad, event); //for #RET change
		gst_rtp_jitter_buffer_flush_start (jitterbuffer);
...
/**
	...
	*/
static gboolean
gst_rtp_jitter_buffer_sink_rtcp_rtx_event (GstPad *pad, GstEvent *event)
{
  gboolean ret=TRUE;
  GstRtpJitterBuffer *jitterbuffer=NULL;
  ...
  GST_DEBUG_OBJECT (jitterbuffer, "RTCP RTX EVENT received %s", GST_EVENT_TYPE_NAME (event));
...
/**
	...
	*/
static gboolean
gst_rtp_jitter_buffer_sink_rtx_event (GstPad *pad, GstEvent *event)
{
	gboolean ret=TRUE;
	GstRtpJitterBuffer *jitterbuffer=NULL;

	jitterbuffer = GST_RTP_JITTER_BUFFER (gst_pad_get_parent (pad));
	GST_DEBUG_OBJECT (jitterbuffer, "RTP RTX EVENT received %s", GST_EVENT_TYPE_NAME (event));
...
//FEC changes
static gboolean
gst_rtp_jitter_buffer_sink_rtcp_fec_event (GstPad * pad, GstEvent * event)
{
	gboolean ret=TRUE;
	GstRtpJitterBuffer *jitterbuffer=NULL;

	jitterbuffer = GST_RTP_JITTER_BUFFER (gst_pad_get_parent (pad));
	GST_DEBUG_OBJECT (jitterbuffer, "RTP FEC EVENT received %s", GST_EVENT_TYPE_NAME (event));
...
static gboolean
gst_rtp_jitter_buffer_sink_fec_event (GstPad * pad, GstEvent * event)
{
	gboolean ret=TRUE;
	GstRtpJitterBuffer *jitterbuffer=NULL;
...
// ++ FEC Changes Bookkeeping
/*
 * Must be called with JBUFFEC_LOCK held, will release the LOCK when emiting the
 * signal. The function returns GST_FLOW_ERROR when a parsing error happened and
 * GST_FLOW_WRONG_STATE when the element is shutting down. On success
 * GST_FLOW_OK is returned.
 */
static GstFlowReturn
gst_fec_jitter_buffer_get_clock_rate (GstRtpJitterBuffer * jitterbuffer,
    guint8 pt)
{
  GValue ret = { 0 };
  GValue args[2] = { {0}, {0} };
...
//  ???????????????????????????????? Need to check if it requried or not.
//  JBUF_LOCK (jitterbuffer->priv);
//  JBUFFEC_UNLOCK (jitterbuffer->priv);
//  g_signal_emitv (args, gst_rtp_jitter_buffer_signals[SIGNAL_REQUEST_PT_MAP], 0,
//      &ret);
//  JBUFFEC_LOCK_CHECK (jitterbuffer->priv, out_flushing);
//  JBUF_LOCK_CHECK (jitterbuffer->priv, out_flushing);
  ...
  res = gst_fecjitter_buffer_sink_parse_caps (jitterbuffer, caps);
  gst_caps_unref (caps);
...
check_buffering_percent (GstRtpJitterBuffer * jitterbuffer, gint * percent)
{
  ...
  if (*percent != INVALID_OR_RESET_WITH_MINUS_ONE && priv->npt_stop != INVALID_OR_RESET_WITH_MINUS_ONE &&
...
// ++ FEC Changes Bookkeeeping
/* call with jbuf lock held */
static void
check_buffering_percent_fec (GstRtpJitterBuffer * jitterbuffer, gint * percent)
{
  GstRtpJitterBufferPrivate *priv = jitterbuffer->priv;
  ...
  if (*percent != INVALID_OR_RESET_WITH_MINUS_ONE && priv->npt_stop != INVALID_OR_RESET_WITH_MINUS_ONE &&
      priv->npt_stop - priv->npt_start <=
      rtp_jitter_buffer_get_delay (priv->jbuffec))
  {
    GST_DEBUG_OBJECT (jitterbuffer, "short stream; faking full buffer");
    rtp_jitter_buffer_set_buffering (priv->jbuffec, FALSE);
    *percent = 100;
  }
  else
  {
...
/* call with jbuf lock held */
static void
check_buffering_percent_fec_rtp (GstRtpJitterBuffer * jitterbuffer, gint * percent)
{
  GstRtpJitterBufferPrivate *priv = jitterbuffer->priv;
  ...
  if (*percent != INVALID_OR_RESET_WITH_MINUS_ONE && priv->npt_stop != INVALID_OR_RESET_WITH_MINUS_ONE &&
      priv->npt_stop - priv->npt_start <=
      rtp_jitter_buffer_get_delay (priv->fecrtpjbuf)) {
    GST_DEBUG_OBJECT (jitterbuffer, "short stream; faking full buffer");
    rtp_jitter_buffer_set_buffering (priv->fecrtpjbuf, FALSE);
    *percent = 100;
  }
  else
  {
...
post_buffering_percent (GstRtpJitterBuffer *jitterbuffer, gint percent)
{
  ...
  gst_message_set_buffering_stats (message, GST_BUFFERING_LIVE, INVALID_OR_RESET_WITH_MINUS_ONE, INVALID_OR_RESET_WITH_MINUS_ONE, INVALID_OR_RESET_WITH_MINUS_ONE);
...
/**
	...
	* @brief
	*
	* @param[in]/[out]	: GstRtpJitterBuffer *jitterbuffer, GstClockTime timestamp
	* @return			: GstClockTime
	* @warning			: None
	* @exception		: None
	* @see
...
apply_offset (GstRtpJitterBuffer *jitterbuffer, GstClockTime timestamp)
{
  GstRtpJitterBufferPrivate *priv=NULL;
  priv = jitterbuffer->priv;
	if (timestamp == INVALID_OR_RESET_WITH_MINUS_ONE)
	{
		return INVALID_OR_RESET_WITH_MINUS_ONE;
	}
...
/**
	...
	* @brief
	*
	* @param[in]/[out]	: GstRtpJitterBuffer *jitterbuffer, TimerType type, guint16 seqnum
	* @return			: TimerData*
	* @warning			: None
	* @exception		: None
	* @see
...
find_timer (GstRtpJitterBuffer *jitterbuffer, TimerType type, guint16 seqnum)
{
  ...
  gint i=INVALID_OR_RESET_WITH_MINUS_ONE, len=INVALID_OR_RESET_WITH_MINUS_ONE;
...
/**
	...
	* @brief
	*
	* @param[in]/[out]	: GstRtpJitterBuffer *jitterbuffer, TimerData *timer
	* @return			: GstClockTime
	* @warning			: None
	* @exception		: None
	* @see
...
{
  GstRtpJitterBufferPrivate *priv = jitterbuffer->priv;
  GstClockTime test_timeout=INVALID_OR_RESET_WITH_MINUS_ONE;

  if ((test_timeout = timer->timeout) == INVALID_OR_RESET_WITH_MINUS_ONE)
  {
	return INVALID_OR_RESET_WITH_MINUS_ONE;
  }
...
recalculate_timer (GstRtpJitterBuffer *jitterbuffer, TimerData *timer)
{
  ...
  if (priv->clock_id)
  {
	...
	GST_DEBUG ("%" GST_TIME_FORMAT " <> %" GST_TIME_FORMAT,GST_TIME_ARGS (timeout), GST_TIME_ARGS (priv->timer_timeout));
	if (timeout == INVALID_OR_RESET_WITH_MINUS_ONE || timeout < priv->timer_timeout)
	{
		unschedule_current_timer (jitterbuffer);
	}
  }
...
/**
	...
	* @brief
	*
	* @param[in]/[out]	: GstRtpJitterBuffer * jitterbuffer, TimerType type,
    ...
    GstClockTime duration)
{
  GstRtpJitterBufferPrivate *priv = jitterbuffer->priv;
  TimerData *timer=NULL;
  gint len=INVALID_OR_RESET_WITH_MINUS_ONE;
  ...
  len = priv->timers->len;
  g_array_set_size (priv->timers, len + PLUS_ONE);
  ...
  if (type == TIMER_TYPE_EXPECTED)
  {
    ...
    timer->rtx_retry = ZERO;
  }
  timer->num_rtx_retry = ZERO;
...
{
  GstRtpJitterBufferPrivate *priv = jitterbuffer->priv;
  gboolean seqchange=ZERO, timechange=ZERO;
  guint16 oldseq=INVALID_OR_RESET_WITH_MINUS_ONE;
...
/**
	...
	* @brief
	*
	* @param[in]/[out]	: GstRtpJitterBuffer *jitterbuffer, TimerType type, guint16 seqnum, GstClockTime timeout
	* @return			: void
	* @warning			: None
	* @exception		: None
	* @see
...
{
  GstRtpJitterBufferPrivate *priv = jitterbuffer->priv;
  guint idx=INVALID_OR_RESET_WITH_MINUS_ONE;
...
remove_all_timers (GstRtpJitterBuffer *jitterbuffer)
{
  ...
  g_array_set_size (priv->timers, ZERO);
  unschedule_current_timer (jitterbuffer);
}
//for #RET change
...
/**
	...
	* @brief
	*
	* @param[in]/[out]	: GstRtpJitterBuffer *jitterbuffer, guint16 seqnum,GstClockTime dts, gboolean do_next_seqnum
	* @return			: void
	* @warning			: None
	* @exception		: None
	* @see
    ...
    GstClockTime dts, gboolean do_next_seqnum)
{
   ...
   * the timer for the seqnum */
	...
	{
		TimerData *test = &g_array_index (priv->timers, TimerData, i);
		gint gap=INVALID_OR_RESET_WITH_MINUS_ONE;
		gap = gst_rtp_buffer_compare_seqnum (test->seqnum, seqnum);
		GST_DEBUG_OBJECT (jitterbuffer, "%d, #%d<->#%d gap %d", i,test->seqnum, seqnum, gap);
		if (gap == ZERO)
		{
		...
		else if (gap > priv->rtx_delay_reorder)
		{
			...
			* missing packet to be this reordered */
			if (test->num_rtx_retry == ZERO && test->type == TIMER_TYPE_EXPECTED)
			{
				reschedule_timer (jitterbuffer, test, test->seqnum, INVALID_OR_RESET_WITH_MINUS_ONE, ZERO, FALSE);
			}
			else
			{
				GST_DEBUG("Warning");
	...
	if (priv->packet_spacing > ZERO && do_next_seqnum && priv->do_retransmission)
	{
		GstClockTime expected=INVALID_OR_RESET_WITH_MINUS_ONE, delay=INVALID_OR_RESET_WITH_MINUS_ONE;
		...
		{
      		add_timer (jitterbuffer, TIMER_TYPE_EXPECTED, priv->next_in_seqnum,ZERO,
	...
	else if (timer && timer->type != TIMER_TYPE_DEADLINE)
	{
		...
		if (timer->num_rtx_retry > ZERO)
		{
			GstClockTime rtx_last=INVALID_OR_RESET_WITH_MINUS_ONE;

			/* we scheduled a retry for this packet and now we have it */
			priv->num_rtx_success++;
			/* all the previous retry attempts failed */
			priv->num_rtx_failed += timer->num_rtx_retry - INVALID_OR_RESET_WITH_MINUS_ONE;
			/* number of retries before receiving the packet */
			if (priv->avg_rtx_num == ZERO_ZERO)
			{
				priv->avg_rtx_num = timer->num_rtx_retry;
			}
			else
			{
				priv->avg_rtx_num = (timer->num_rtx_retry + SEVEN * priv->avg_rtx_num) / EIGHT;
			}
			...
			* packet, start with when we scheduled this timeout last */
			...
			if (dts > rtx_last)
			{
				GstClockTime delay=INVALID_OR_RESET_WITH_MINUS_ONE;
				/* we have a valid delay if this packet arrived after we scheduled the
				* request */
				delay = dts - rtx_last;
				if (priv->avg_rtx_rtt == ZERO)
				{
					priv->avg_rtx_rtt = delay;
				}
				else
				{
					priv->avg_rtx_rtt = (delay + SEVEN * priv->avg_rtx_rtt) / EIGHT;
				}
			}
...
/**
	...
	* @brief
	*
	* @param[in]/[out]	: GstRtpJitterBuffer *jitterbuffer, guint32 rtptime,GstClockTime dts
	* @return			: void
	* @warning			: None
	* @exception		: None
	* @see
    ...
    GstClockTime dts)
{
   ...
   * rtptime to estimate the packet spacing. */
	if (priv->ips_rtptime != rtptime)
	{
		...
		if (priv->ips_dts != INVALID_OR_RESET_WITH_MINUS_ONE && dts != INVALID_OR_RESET_WITH_MINUS_ONE && dts > priv->ips_dts)
		{
			priv->packet_spacing = dts - priv->ips_dts;
			GST_DEBUG_OBJECT (jitterbuffer,"new packet spacing %" GST_TIME_FORMAT,GST_TIME_ARGS (priv->packet_spacing));
		}
...
/**
	...
	* @brief
	*
	* @param[in]/[out]	: GstRtpJitterBuffer *jitterbuffer, guint32 expected,guint16 seqnum, GstClockTime dts, gint gap
	* @return			: void
	* @warning			: None
	* @exception		: None
	* @see
...
{
  GstRtpJitterBufferPrivate *priv = jitterbuffer->priv;
  GstClockTime total_duration=INVALID_OR_RESET_WITH_MINUS_ONE, duration=INVALID_OR_RESET_WITH_MINUS_ONE, expected_dts=INVALID_OR_RESET_WITH_MINUS_ONE;
  TimerType type;
  ...
  duration = total_duration / (gap + PLUS_ONE);
  ...
  GST_DEBUG_OBJECT (jitterbuffer, "duration %" GST_TIME_FORMAT,
	...
	{
		GstClockTime gap_time=INVALID_OR_RESET_WITH_MINUS_ONE;
		guint lost_packets=INVALID_OR_RESET_WITH_MINUS_ONE;
		...
		* the timer thread */
		...
		priv->last_in_dts + duration, ZERO, gap_time);
  ...
  expected_dts = priv->last_in_dts + duration;
	...
	{
		add_timer (jitterbuffer, type, expected, ZERO, expected_dts, ZERO, duration);
		expected_dts += duration;
		expected++;
	}
}
/**
	* @fn		Make_OriginalPacket
	* @brief
	*
	...
	* @see
	*/
static GstBuffer *Make_OriginalPacket(GstBuffer *buffer)
{
  GstBuffer *outbuf=NULL;
  guint size=INVALID_OR_RESET_WITH_MINUS_ONE;
  guint header_len=INVALID_OR_RESET_WITH_MINUS_ONE,payload_len=INVALID_OR_RESET_WITH_MINUS_ONE;
  guint8 *data=NULL, *outdata=NULL;
  guint8 *payload=NULL;
  guint16 new_seq_num=INVALID_OR_RESET_WITH_MINUS_ONE;
  ...
  gst_buffer_unref(buffer); //deallocating buffer

  if (G_UNLIKELY (!gst_rtp_buffer_validate (outbuf)))
  {
	GST_DEBUG("--->>>not valid buffer");
  }
  else
  {
	GST_DEBUG("-->> buffer valid");
  }
  return outbuf;
}
static GstFlowReturn
gst_rtp_jitter_buffer_chain (GstPad * pad, GstBuffer * buffer)
{
  ...
  guint64 latency_ts;
  gboolean tail;
  gint percent = INVALID_OR_RESET_WITH_MINUS_ONE;
  gint fecpercent = INVALID_OR_RESET_WITH_MINUS_ONE;
  guint8 pt;
//for #RET change
  guint32 expected=ZERO, rtptime=ZERO;
  gboolean do_next_seqnum = FALSE;
  RTPJitterBufferItem *item=NULL;
  //FEC CHANGES
  RTPJitterBufferItem *item2=NULL;
  ...
  rtptime = gst_rtp_buffer_get_timestamp (buffer); //for #RET change

  GST_DEBUG_OBJECT (jitterbuffer,
      "Received packet #%d at time %" GST_TIME_FORMAT ", discont %d", seqnum,
      GST_TIME_ARGS (timestamp), GST_BUFFER_IS_DISCONT (buffer));
  ...
  if (G_UNLIKELY (priv->clock_rate == INVALID_OR_RESET_WITH_MINUS_ONE))
  {
...
/*************editted for #RET change ******************/
   expected = priv->next_in_seqnum; //for #RET change
  /* now check against our expected seqnum */
  if (G_LIKELY (expected != INVALID_OR_RESET_WITH_MINUS_ONE))
  {
	gint gap;
	gap = gst_rtp_buffer_compare_seqnum (expected, seqnum);
	GST_DEBUG_OBJECT (jitterbuffer, "expected #%d, got #%d, gap of %d", expected, seqnum, gap);
	if (G_LIKELY (gap == ZERO))
	{
		/* packet is expected */
		calculate_packet_spacing (jitterbuffer, rtptime, timestamp);
		do_next_seqnum = TRUE;
	}
	else
	{
		...
		else
		{
			...
			else
			{
				...
				calculate_expected (jitterbuffer, expected, seqnum, timestamp, gap);
				do_next_seqnum = TRUE;
			}
		}
		if (G_UNLIKELY (reset))
		{
			...
			// ++ FEC Changes Bookkeeping
			rtp_jitter_buffer_flush (priv->fecrtpjbuf, (GFunc) free_item, NULL);
			rtp_jitter_buffer_reset_skew (priv->fecrtpjbuf);
			// -- FEC Changes Bookkeeping
  ...
  else
  {
	...
	* while we wait */
	set_timer (jitterbuffer, TIMER_TYPE_DEADLINE, seqnum, timestamp);
	do_next_seqnum = TRUE;
	/* take rtptime and dts to calculate packet spacing */
	priv->ips_rtptime = rtptime;
	priv->ips_dts = timestamp;
  }
  if (do_next_seqnum)
  {
	priv->last_in_seqnum = seqnum;
	priv->last_in_dts = timestamp;
	priv->next_in_seqnum = (seqnum + PLUS_ONE) & MAX_SEQNUM_VAL;
  }

/* let's check if this buffer is too late, we can only accept packets with
* bigger seqnum than the one we last pushed. */
  if (G_LIKELY (priv->last_popped_seqnum != INVALID_OR_RESET_WITH_MINUS_ONE))
  {
	gint gap;
	gap = gst_rtp_buffer_compare_seqnum (priv->last_popped_seqnum, seqnum);
	/* priv->last_popped_seqnum >= seqnum, we're too late. */
	if (G_UNLIKELY (gap <= ZERO))
	{
		goto too_late;
	}
  }
  ...
  // checking if fec enabled then we need to check fec buffer queue and if it is full then
  // we need to delete old buffer and then insert new rtp packet in it.

  if(priv->do_fec)
  {
		item2 = alloc_item(buffer, ITEM_TYPE_BUFFER, timestamp, timestamp, seqnum, 1, rtptime);

		/* If Required RTP Packets list for fec packets is full then remove drop oldest packet here. */
		if( G_LIKELY(rtp_jitter_buffer_num_packets(priv->fecrtpjbuf) >= (DEFAULT_RTP_PKTS_FOR_FEC * DEFAULT_FEC_SPAN)))
		{
			RTPJitterBufferItem *old_item;

			old_item = fec_jitter_buffer_pop (priv->fecrtpjbuf, &fecpercent);
			if (old_item) {
				GST_DEBUG_OBJECT (jitterbuffer, "Queue full, dropping old fec packet #%d",
				old_item->seqnum);
		...
		* have a duplicate. */
		if (G_UNLIKELY (!rtp_fecjitter_buffer_insert (priv->fecrtpjbuf, item2, &fecpercent)))
		{
			//goto duplicate;
		}
		else
		...
		GST_DEBUG_OBJECT (jitterbuffer, "Pushed RTP packet for fec #%d, now %d packets",
		seqnum, rtp_jitter_buffer_num_packets (priv->fecrtpjbuf));

		check_buffering_percent_fec_rtp (jitterbuffer, &fecpercent);
  }
  else
  {
	; /*NULL*/
  ...
  item = alloc_item (buffer, ITEM_TYPE_BUFFER, timestamp, timestamp, seqnum, PLUS_ONE, rtptime);
  ...
  /* update timers */
  update_timers (jitterbuffer, seqnum, timestamp, do_next_seqnum);
...
finished:
  ...
  if (percent != INVALID_OR_RESET_WITH_MINUS_ONE)
	{
...
/**
* @fn		update_estimated_eos
* @brief
*
* @param[in]/[out]	: GstRtpJitterBuffer *jitterbuffer, GstBuffer *outbuf
* @return		: void
* @warning		:
* @exception		: None
* @see
    ...
    RTPJitterBufferItem *item)
{
	...
	if (priv->npt_stop != INVALID_OR_RESET_WITH_MINUS_ONE && priv->ext_timestamp != INVALID_OR_RESET_WITH_MINUS_ONE
	 && priv->clock_base != INVALID_OR_RESET_WITH_MINUS_ONE && priv->clock_rate > ZERO)
	{
		guint64 elapsed=INVALID_OR_RESET_WITH_MINUS_ONE, estimated=INVALID_OR_RESET_WITH_MINUS_ONE;
		elapsed = compute_elapsed (jitterbuffer, item);
		if (elapsed > priv->last_elapsed || !priv->last_elapsed)
		{
			guint64 left=INVALID_OR_RESET_WITH_MINUS_ONE;
			GstClockTime out_time=INVALID_OR_RESET_WITH_MINUS_ONE;
			...
			else
			{
				...
				* we may never advance enough to end up in the above case */
				...
				{
					estimated = INVALID_OR_RESET_WITH_MINUS_ONE;
				}
			}
			...
			GST_TIME_FORMAT, GST_TIME_ARGS (elapsed), GST_TIME_ARGS (estimated));
      			...
      			if (estimated != INVALID_OR_RESET_WITH_MINUS_ONE && priv->estimated_eos != estimated)
	  		{
...
/**
	* @fn		pop_and_push_next
	* @brief
	*
	* @param[in]/[out]	: GstRtpJitterBuffer *jitterbuffer, guint16 seqnum
	* @return			: GstFlowReturn
	* @warning			:
	* @exception		: None
	* @see
...
{
  GstRtpJitterBufferPrivate *priv = jitterbuffer->priv;
  GstFlowReturn result=INVALID_OR_RESET_WITH_MINUS_ONE;
  RTPJitterBufferItem *item=NULL;
  GstBuffer *outbuf=NULL;
  GstEvent *outevent=NULL;
  GstClockTime dts=INVALID_OR_RESET_WITH_MINUS_ONE;
  gint percent = INVALID_OR_RESET_WITH_MINUS_ONE;
  ...
  * so the other end can push stuff in the queue again. */
  if (seqnum != INVALID_OR_RESET_WITH_MINUS_ONE)
  {
	priv->last_popped_seqnum = seqnum;
	priv->next_seqnum = (seqnum + item->count) & MAX_SEQNUM_VAL;
  }
  JBUF_UNLOCK (priv);
  ...
  if (is_buffer)
  {
	...
	GST_DEBUG_OBJECT (jitterbuffer,
	"Pushing buffer %d, time%" GST_TIME_FORMAT,
	seqnum, GST_TIME_ARGS (GST_BUFFER_TIMESTAMP (outbuf)));
...
handle_next_buffer (GstRtpJitterBuffer *jitterbuffer)
{
  ...
  RTPJitterBufferItem *item=NULL;
  guint seqnum=INVALID_OR_RESET_WITH_MINUS_ONE;
  guint32 next_seqnum=INVALID_OR_RESET_WITH_MINUS_ONE;
  gint gap=INVALID_OR_RESET_WITH_MINUS_ONE;


  /* only push buffers when PLAYING and active and not buffering */
  if (priv->blocked || !priv->active || rtp_jitter_buffer_is_buffering (priv->jbuf))
  {
...
again:
   ...
   * previous packet seqnum assume no gap. */
	if (G_UNLIKELY (next_seqnum == INVALID_OR_RESET_WITH_MINUS_ONE))
	{
		GST_DEBUG_OBJECT (jitterbuffer, "First buffer #%d", seqnum);
    ...
    GstClockTime now)
{
  GstRtpJitterBufferPrivate *priv = jitterbuffer->priv;
  GstEvent *event=NULL;
  guint delay=INVALID_OR_RESET_WITH_MINUS_ONE;
...
{
  GstRtpJitterBufferPrivate *priv = jitterbuffer->priv;
  GstClockTime duration, timestamp=INVALID_OR_RESET_WITH_MINUS_ONE;
  guint seqnum=INVALID_OR_RESET_WITH_MINUS_ONE, lost_packets=INVALID_OR_RESET_WITH_MINUS_ONE, num_rtx_retry=INVALID_OR_RESET_WITH_MINUS_ONE;
  gboolean late=FALSE;
  GstEvent *event=NULL;
  RTPJitterBufferItem *item=NULL;
  ...
  item = alloc_item (event, ITEM_TYPE_LOST, INVALID_OR_RESET_WITH_MINUS_ONE, INVALID_OR_RESET_WITH_MINUS_ONE, seqnum, lost_packets, INVALID_OR_RESET_WITH_MINUS_ONE);
  rtp_jitter_buffer_insert (priv->jbuf, item, NULL, NULL);
...
/**
	...
	* @brief
	*
	* @param[in]/[out]	: GstRtpJitterBuffer *jitterbuffer, TimerData *timer, GstClockTime now
	* @return			: gboolean
	* @warning			:
	* @exception		: None
	* @see
...
wait_next_timeout (GstRtpJitterBuffer *jitterbuffer)
{
  ...
  {
	TimerData *timer = NULL;
	GstClockTime timer_timeout = INVALID_OR_RESET_WITH_MINUS_ONE;
	gint i=ZERO, len=ZERO;
	...
	for (i = ZERO; i < len; i++)
	{
		...
		else if (timer_timeout == INVALID_OR_RESET_WITH_MINUS_ONE)
		{
			/* we already have an immediate timeout, the new timer must be an
			* immediate timer with smaller seqnum to become the best */
			if (test_timeout == INVALID_OR_RESET_WITH_MINUS_ONE && test->seqnum < timer->seqnum)
			{
				save_best = TRUE;
			}
			else
		...
		else if (test_timeout == INVALID_OR_RESET_WITH_MINUS_ONE)
		{
	...
	if (timer && !priv->blocked)
	{
		...
		if (timer_timeout == INVALID_OR_RESET_WITH_MINUS_ONE || timer_timeout <= now)
		{
		...
		{
			now = timer_timeout + MAX (clock_jitter,ZERO);
...
{
  GstRtpJitterBufferPrivate *priv=NULL;
  guint64 base_rtptime=INVALID_OR_RESET_WITH_MINUS_ONE, base_time=INVALID_OR_RESET_WITH_MINUS_ONE;
  guint32 clock_rate=INVALID_OR_RESET_WITH_MINUS_ONE;
  guint64 last_rtptime=INVALID_OR_RESET_WITH_MINUS_ONE;
  guint64 clock_base=INVALID_OR_RESET_WITH_MINUS_ONE;
  guint64 ext_rtptime=INVALID_OR_RESET_WITH_MINUS_ONE, diff=INVALID_OR_RESET_WITH_MINUS_ONE;
  gboolean drop = FALSE;
  priv = jitterbuffer->priv;


  /* get the last values from the jitterbuffer */
  rtp_jitter_buffer_get_sync (priv->jbuf, &base_rtptime, &base_time,&clock_rate, &last_rtptime);
  ...
  if (base_rtptime ==INVALID_OR_RESET_WITH_MINUS_ONE || clock_rate == INVALID_OR_RESET_WITH_MINUS_ONE || base_time == INVALID_OR_RESET_WITH_MINUS_ONE)
  {
		...
		if (ext_rtptime > last_rtptime)
		{
			...
			if (diff > clock_rate)
			{
				...
				* (sync might use other methods) */
				ext_rtptime = INVALID_OR_RESET_WITH_MINUS_ONE;
			}
			else
			{
				GST_DEBUG("Warning");
  ...
  if (!drop)
  {
	...
	gst_rtp_jitter_buffer_signals[SIGNAL_HANDLE_SYNC], ZERO, s);
	JBUF_LOCK (priv);
...
ignore_buffer:
 ...
 /**
	...
	*/
static GstFlowReturn
gst_rtp_jitter_buffer_chain_rtcp_rtx (GstPad *pad, GstBuffer *buffer)
{
  GstRtpJitterBuffer *jitterbuffer=NULL;
  //GstRtpJitterBufferPrivate *priv;
  GstFlowReturn ret = GST_FLOW_OK;
  ...
  GST_DEBUG_OBJECT (jitterbuffer, "RECEIVED RTCP RTX packet");
done:
	gst_buffer_unref (buffer);
	gst_object_unref (jitterbuffer);
	return ret;
...
/**
	...
	*/
static GstFlowReturn
gst_rtp_jitter_buffer_chain_rtx (GstPad *pad, GstBuffer *buffer)
{
  GstRtpJitterBuffer *jitterbuffer=NULL;
  GstRtpJitterBufferPrivate *priv=NULL;
  GstFlowReturn ret = GST_FLOW_OK;
  guint16 seqnum=INVALID_OR_RESET_WITH_MINUS_ONE;
  GstClockTime timestamp=INVALID_OR_RESET_WITH_MINUS_ONE;
  guint64 latency_ts=INVALID_OR_RESET_WITH_MINUS_ONE;
  gboolean tail=INVALID_OR_RESET_WITH_MINUS_ONE;
  gint percent = INVALID_OR_RESET_WITH_MINUS_ONE;
  guint8 pt=INVALID_OR_RESET_WITH_MINUS_ONE;
  guint32 expected=INVALID_OR_RESET_WITH_MINUS_ONE, rtptime=INVALID_OR_RESET_WITH_MINUS_ONE;
  gboolean do_next_seqnum = FALSE;
  gint32 clock_rate=INVALID_OR_RESET_WITH_MINUS_ONE; //FIX ME TO BE REMOVED
  RTPJitterBufferItem *item=NULL;
  ...
  priv = jitterbuffer->priv;
  GST_DEBUG_OBJECT (jitterbuffer, "RECEIVED RTP RTX packet");
  if(priv->chain_rtx==FALSE)
  {
	goto invalid_buffer;
  }
  pt = gst_rtp_buffer_get_payload_type (buffer);
  ...
  * jitterbuffer. */
  timestamp = GST_BUFFER_TIMESTAMP (buffer);
  /* bring to running time */
  timestamp = gst_segment_to_running_time (&priv->segment, GST_FORMAT_TIME,timestamp);
  seqnum = gst_rtp_buffer_get_seq (buffer);
  ...
  GST_TIME_ARGS (timestamp), GST_BUFFER_IS_DISCONT (buffer));
  JBUF_LOCK_CHECK (priv, out_flushing);
  clock_rate = priv->clock_rate; //FIX ME TO BE REMOVED
  if (G_UNLIKELY (priv->last_rtxpt != pt))
  {
	GstCaps *caps=NULL;
	GST_DEBUG_OBJECT (jitterbuffer, "pt changed from %u to %u", priv->last_rtxpt,pt);
	priv->last_rtxpt = pt;
	/* reset clock-rate so that we get a new one */
	priv->clock_rate = INVALID_OR_RESET_WITH_MINUS_ONE;  //removing this so that it will take the clock rate of the main stream
	/* Try to get the clock-rate from the caps first if we can. If there are no
	* caps we must fire the signal to get the clock-rate. */
	if ((caps = GST_BUFFER_CAPS (buffer)))
	{
  ...
  {
	/* no clock rate given on the caps, try to get one with the signal */
	if (gst_rtp_jitter_buffer_get_clock_rate (jitterbuffer,pt) == GST_FLOW_WRONG_STATE)
	{
		goto out_flushing;
	}
	if (G_UNLIKELY (priv->clock_rate == INVALID_OR_RESET_WITH_MINUS_ONE))
  ...
  if(clock_rate != priv->clock_rate)
  {
	priv->clock_rate = clock_rate;
	rtp_jitter_buffer_set_clock_rate (priv->jbuf, priv->clock_rate);
	GST_DEBUG ("RESETTING CLOCK RATE TO PREVIOUS ONE");
  }
  if (G_UNLIKELY (priv->apt == INVALID_OR_RESET_WITH_MINUS_ONE))
  {
	GST_DEBUG ("NO APT FOUND");
	goto no_apt;
  }
  else
  {
	gst_rtp_buffer_set_payload_type (buffer, priv->apt);
  }
  buffer = Make_OriginalPacket(buffer);
  seqnum = gst_rtp_buffer_get_seq (buffer); // this is the OSN generated after changing we need this for timers
  /* don't accept more data on EOS */
  if (G_UNLIKELY (priv->eos))
  {
	goto have_eos;
  ...
  if (G_LIKELY (expected != INVALID_OR_RESET_WITH_MINUS_ONE))
  {
	...
	else
	{
		...
		if (gap < ZERO)
		{
			...
			{
				/* too old packet, reset */
				GST_DEBUG_OBJECT (jitterbuffer, "reset: buffer too old %d < %d", gap,-RTP_MAX_MISORDER);
				reset = TRUE;
			}
			else
			{
				GST_DEBUG_OBJECT (jitterbuffer, "old packet received");
				GST_DEBUG_OBJECT (jitterbuffer, "i gues code is working fine for RET");
			}
		}
		else
		{
  ...
  * bigger seqnum than the one we last pushed. */
  if (G_LIKELY (priv->last_popped_seqnum != INVALID_OR_RESET_WITH_MINUS_ONE))
  {
	...
	if (G_UNLIKELY (gap <= ZERO))
	{
		GST_DEBUG_OBJECT (jitterbuffer,"Sorry, i guess ret packet is too late to be inserted");
		goto too_late;
	}
  }
  /* let's drop oldest packet if the queue is already full and drop-on-latency
  ...
  * out as fast as possible. */
  ...
  {
    latency_ts =
	gst_util_uint64_scale_int (priv->latency_ms, priv->clock_rate, THOUSAND);
  ...
  item = alloc_item (buffer, ITEM_TYPE_BUFFER, timestamp, timestamp, seqnum, 1, rtptime);
  /* we need to make the metadata writable before pushing it in the jitterbuffer
  * because the jitterbuffer will update the timestamp */
  buffer = gst_buffer_make_metadata_writable (buffer);
  /* now insert the packet into the queue in sorted order. This function returns
  ...
  * when the tail buffer changed */
  ...
  GST_DEBUG_OBJECT (jitterbuffer, "Pushed packet #%d, now %d packets, tail: %d",seqnum, rtp_jitter_buffer_num_packets (priv->jbuf), tail);
  check_buffering_percent (jitterbuffer, &percent);
...
static GstFlowReturn
gst_rtp_jitter_buffer_chain_rtcp_fec (GstPad * pad, GstBuffer * buffer)
{
  GstRtpJitterBuffer *jitterbuffer;
  //GstRtpJitterBufferPrivate *priv;
  GstFlowReturn ret = GST_FLOW_OK;
  ...
  GST_DEBUG_OBJECT (jitterbuffer, "RECEIVED RTCP FEC packet");
done:
  gst_buffer_unref (buffer);
  gst_object_unref (jitterbuffer);
...
static GstFlowReturn
gst_rtp_jitter_buffer_chain_fec (GstPad * pad, GstBuffer * buffer)
{
    GstRtpJitterBuffer *jitterbuffer;
    GstRtpJitterBufferPrivate *priv;
    GstFlowReturn ret = GST_FLOW_OK;
    ...
    RTPJitterBufferItem *item, *reconstructeditem;
    ...
    pt = gst_rtp_buffer_get_payload_type (buffer);
    GST_DEBUG_OBJECT (jitterbuffer, "FEC RECEIVED RTP FEC packet payload type :%x ", pt);
   ...
   * jitterbuffer. */
    ...
    GST_DEBUG_OBJECT (jitterbuffer,
                      "Received fec packet #%d at time %" GST_TIME_FORMAT, seqnum,
                      GST_TIME_ARGS (timestamp));

    JBUF_LOCK_CHECK(priv, out_flushing);

    if (G_UNLIKELY (priv->last_fecpt != pt))
	{
        GstCaps *caps;

        GST_DEBUG_OBJECT (jitterbuffer, "pt changed from %u to %u", priv->last_fecpt,
                          pt);
        priv->last_fecpt = pt;
        /* reset clock-rate so that we get a new one */
        priv->clock_rate_fec = INVALID_OR_RESET_WITH_MINUS_ONE;  //removing this so that it will take the clock rate of the main stream
        /* Try to get the clock-rate from the caps first if we can. If there are no
     * caps we must fire the signal to get the clock-rate. */
        if ((caps = GST_BUFFER_CAPS (buffer)))
		{
            gst_fecjitter_buffer_sink_parse_caps (jitterbuffer, caps);
        }
		else
		{
			/*NULL*/
    ...
    if (G_UNLIKELY (priv->clock_rate_fec == INVALID_OR_RESET_WITH_MINUS_ONE))
	{
        /* no clock rate given on the caps, try to get one with the signal */
        if (gst_fec_jitter_buffer_get_clock_rate (jitterbuffer,
                                                  pt) == GST_FLOW_WRONG_STATE)
		{
            goto out_flushing;
		}
   ...
   * have a duplicate. */
    ...
    if (G_UNLIKELY (!rtp_fecjitter_buffer_insert (priv->jbuffec, item, &percent))){
        goto duplicate;
    }
	else
	{
    ...
    GST_DEBUG_OBJECT (jitterbuffer, "Pushed FEC packet #%d,now %d packets", seqnum,rtp_jitter_buffer_num_packets (priv->jbuffec));

    check_buffering_percent_fec (jitterbuffer, &percent);
    // -- FEC Changes Bookkeeping

    if(rtp_jitter_buffer_num_packets(priv->jbuffec) >= MAX_FEC_PACKET_LIST_SIZE && (rtp_jitter_buffer_num_packets(priv->fecrtpjbuf))>=DEFAULT_RTP_PKTS_FOR_FEC)
		{
			GstBuffer* reconstructedRTPPkt = NULL;
			RTPJitterBufferItem * old_item;
			old_item = fec_jitter_buffer_pop (priv->jbuffec, &percent);
			if(old_item != NULL) {
				GST_DEBUG_OBJECT (jitterbuffer, " Calling gst_xor_rtp_reconstruct_packet() in chain fec for packet: %d", old_item->seqnum);
				reconstructedRTPPkt = gst_xor_rtp_reconstruct_packet((GList*)priv->fecrtpjbuf->packets->head, old_item);
				if(reconstructedRTPPkt)
				{
					GST_DEBUG_OBJECT (jitterbuffer, " RTP Packet Reconstruction successful : ");
					timestamp = GST_BUFFER_TIMESTAMP (reconstructedRTPPkt);
					timestamp = gst_segment_to_running_time (&priv->segment, GST_FORMAT_TIME,
					             timestamp);
					seqnum = gst_rtp_buffer_get_seq (reconstructedRTPPkt);
					rtptime = gst_rtp_buffer_get_timestamp (reconstructedRTPPkt);
					reconstructedRTPPkt = gst_buffer_make_metadata_writable (reconstructedRTPPkt);
					reconstructeditem = alloc_item (reconstructedRTPPkt, ITEM_TYPE_BUFFER, timestamp, timestamp, seqnum, 1, rtptime);
					if (G_LIKELY (priv->last_popped_seqnum != INVALID_OR_RESET_WITH_MINUS_ONE))
					{
						gint gap;
						gap = gst_rtp_buffer_compare_seqnum (priv->last_popped_seqnum, seqnum);
						if (G_UNLIKELY (gap <= ZERO))
						{
							GST_WARNING_OBJECT(jitterbuffer, "Reconstructed Packet is too late");
						}
					}
					if (G_UNLIKELY (!rtp_jitter_buffer_insert (priv->jbuf, reconstructeditem, &tail_rtp, &percent_rtp)))
					{
						GST_WARNING_OBJECT (priv->jbuf, "Duplicate packet #%d detected, dropping", seqnum);
						free_item (reconstructeditem);
					}
					else
					{
						; /*NULL*/
				...
				{
					GST_DEBUG_OBJECT (jitterbuffer, "RTP Packet Reconstruction Failed: ");
				}

				GST_DEBUG_OBJECT (jitterbuffer, " FEC Queue full, dropping old fec packet #%d",	old_item->seqnum);
				free_item (old_item);
			}
	...
	{
		//only one FEC packet count
		GST_DEBUG_OBJECT (jitterbuffer, "Not sufficient FEC packets : %d or fecrtp packets: %d ",rtp_jitter_buffer_num_packets(priv->jbuffec),
		rtp_jitter_buffer_num_packets(priv->fecrtpjbuf));
	}
  ...
  switch (GST_QUERY_TYPE (query)) {
    case GST_QUERY_LATENCY:
    {
      ...
      if ((res = gst_pad_peer_query (priv->sinkpad, query))) {
        ...
        max_latency = INVALID_OR_RESET_WITH_MINUS_ONE;
    ...
    guint prop_id, const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id)
  {
	...
	case PROP_DO_FEC:
		JBUF_LOCK (priv);
		priv->do_fec = g_value_get_boolean (value);
		JBUF_UNLOCK (priv);
		break;
    ...
    guint prop_id, GValue * value, GParamSpec * pspec)
{
 ...
 switch (prop_id)
 {
	...
	case PROP_DO_FEC:
		JBUF_LOCK (priv);
		g_value_set_boolean (value, priv->do_fec);
		JBUF_UNLOCK (priv);
		break;
    	default:
		G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
