...
gst_rtp_pt_demux_chain (GstPad * pad, GstBuffer * buf)
{
  ...
  GstCaps *caps;
  guint16 seq_num=INVALID_OR_RESET_WITH_MINUS_ONE;
  ...
  seq_num = gst_rtp_buffer_get_seq(buf);
  if(seq_num != (rtpdemux->last_seqnum+1))
  {
	GST_DEBUG_OBJECT (rtpdemux, "seqnum %d not received for pt %d",rtpdemux->last_seqnum+1, pt);
  }
  rtpdemux->last_seqnum=seq_num;
...
gst_rtp_pt_demux_setup (GstRtpPtDemux * ptdemux)
{
  ...
  ptdemux->last_seqnum= INVALID_OR_RESET_WITH_MINUS_ONE;
  return TRUE;
}
