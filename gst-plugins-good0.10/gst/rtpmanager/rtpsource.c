...
#define PLUS_ONE 1
#define TWO_BYTE_SHIFT_FOR_SEQNUM 16
...
rtp_source_reset (RTPSource * src)
{
  ...
  g_array_set_size (src->nacks, ZERO);
...
/**
	* @fn		rtp_conflicting_address_free
	* @brief
	*
	* @param[in]/[out]	:RTPConflictingAddress *addr
	* @return		: 	void
	* @warning		: 	None
	* @exception	: None
	* @see
...
rtp_source_finalize (GObject * object)
{
  ...
  g_list_foreach (src->conflicting_addresses,(GFunc)rtp_conflicting_address_free, NULL);
    ...
    gpointer user_data)
{
  ...
  if (str)  {
    return strcmp (str, gst_structure_get_string (old,
            field)) == 0;
  } else {
    GST_ERROR("g_value_get_string return null");
    return FALSE;
  }
}
    ...
    gpointer user_data)
{
  ...
  src->callbacks.rtx_src = cb->rtx_src;
 ...
 */
 /**
	* @fn		rtp_source_is_marked_bye
	* @brief
	*
	* @param[in]/[out]	:RTPSource *src
	* @return			: gboolean
	* @warning			: None
	* @exception		: None
	* @see
    ...
    RTPArrivalStats * arrival)
{
  ...
  if (stats->cycles == INVALID_OR_RESET_WITH_MINUS_ONE)
  {
	GST_DEBUG ("received first buffer");
	/* first time we heard of this source */
	init_seq (src, seqnr);
	src->stats.max_seq = seqnr - PLUS_ONE;
	src->curr_probation = src->probation;
  }
  ...
  {
	expected = src->stats.max_seq + PLUS_ONE;
 ...
 */
/**
	* @fn		rtp_source_mark_bye
	* @brief
	*
	* @param[in]/[out]	:RTPSource *src, const gchar *reason
	* @return		: 	void
	* @warning		: 	None
	* @exception		: None
	* @see
 ...
 */
 /**
* @fn		rtp_source_register_nack
* @brief
*
* @param[in]/[out]	: RTPSource *src, guint16 seqnum
* @return		: void
* @warning		: None
* @exception		: None
* @see
...
{
  guint i=ZERO, len=ZERO;
  guint32 dword = seqnum << TWO_BYTE_SHIFT_FOR_SEQNUM;
  gint diff = TWO_BYTE_SHIFT_FOR_SEQNUM;
  ...
  len = src->nacks->len;
	for (i = ZERO; i < len; i++)
	{
		...
		tseq = tdword >> TWO_BYTE_SHIFT_FOR_SEQNUM;
		diff = gst_rtp_buffer_compare_seqnum (tseq, seqnum);
		if (diff < TWO_BYTE_SHIFT_FOR_SEQNUM)
		{
			break;
		}
	}
   ...
   * if not to far away */
	...
	else if(diff < TWO_BYTE_SHIFT_FOR_SEQNUM)
	{
		/* we can merge it */
		dword = g_array_index (src->nacks, guint32, i);
		dword |= PLUS_ONE<<(diff - PLUS_ONE);
		GST_DEBUG ("merge NACK #%u at %u with NACK #%u -> 0x%08x", seqnum, i,
		dword >> TWO_BYTE_SHIFT_FOR_SEQNUM, dword);
 ...
 */
 /**
* @fn		rtp_source_get_nacks
* @brief
*
* @param[in]/[out]	: RTPSource *src, guint *n_nacks
* @return		: guint32*
* @warning		: None
* @exception		: None
* @see
