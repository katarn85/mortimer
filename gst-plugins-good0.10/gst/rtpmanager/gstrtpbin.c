...
#define MINUS_HUNDRED -100
#define WAIT_TIMEOUT_FOR_CHECKING_SESSION 100
#define TWO 2
/* sink pads */
...
enum
{
  ...
  SIGNAL_REQUEST_RTX_MAP,   //for #RET change
  SIGNAL_REQUEST_FID_MAP,   //---------------
  SIGNAL_REQUEST_FEC_MAP,   //FEC changes
...
#define DEFAULT_DO_SSRC_PAD_FOUND_MAX_TRIES 6
...
{
  static GType rtcp_sync_type = ZERO;
...
static guint gst_rtp_bin_signals[LAST_SIGNAL] = { ZERO }; /*initializing rtp bin signals array to zero */
...
static void free_stream (GstRtpBinStream * stream);
 /**
	* @fn		gst_rtp_bin_remove_internal_session
	* @brief
	*
	* @param[in]/[out]	: GstRtpBin *bin, gint session_id
	* @return			: void
	* @warning			: None
	* @exception		: None
	* @see
	*/
static void gst_rtp_bin_remove_internal_session (GstRtpBin *bin, gint session_id);
...
create_session (GstRtpBin * rtpbin, gint id)
{
  ...
  g_object_set (session, "do-retransmission", rtpbin->do_retransmission, NULL); //for #RET change
...
free_session (GstRtpBinSession * sess, GstRtpBin * bin)
{
 ...
 /**
	...
	* @brief
	*
	* @param[in]/[out]	: GstRtpBinSession *session
	* @return			: guint
	* @warning			: None
	* @exception		: None
	* @see
	*/
static guint get_rtx_map (GstRtpBinSession *session)
{
	GstRtpBin *bin=NULL;
	guint rt=ZERO;
	GValue ret = { ZERO };
	GValue args[TWO] = { {ZERO}, {ZERO} };
	GST_DEBUG ("searching RTX val ");
	GST_RTP_SESSION_LOCK (session);
	bin = session->bin;
	GST_DEBUG ("emiting signal for RTX in session %d", session->id);
	/* not in cache, send signal to request caps */
	g_value_init (&args[ZERO], GST_TYPE_ELEMENT);
	g_value_set_object (&args[ZERO], bin);
	g_value_init (&args[ONE], G_TYPE_UINT);
	g_value_set_uint (&args[ONE], session->id);
	g_value_init (&ret, G_TYPE_UINT);
	GST_RTP_SESSION_UNLOCK (session);
	g_signal_emitv (args, gst_rtp_bin_signals[SIGNAL_REQUEST_RTX_MAP], ZERO, &ret);
	GST_RTP_SESSION_LOCK (session);
	g_value_unset (&args[ZERO]);
	g_value_unset (&args[ONE]);
	rt = g_value_get_uint (&ret);
	g_value_unset (&ret);
	GST_DEBUG ("RTX VALUE RETRIVED FROM SIGNAL %d",rt);
	GST_RTP_SESSION_UNLOCK (session);
	return rt;
}
/**
...
* @see
*/
static guint get_fid_map (GstRtpBinSession *session)
{
	GstRtpBin *bin=NULL;
	guint rt=ZERO;
	guint zero=ZERO;
	GValue ret = { ZERO };
	GValue args[TWO] = { {ZERO}, {ZERO} };
	GST_DEBUG ("searching FID val ");

	GST_RTP_SESSION_LOCK (session);
	bin = session->bin;
	GST_DEBUG ("emiting signal for FID in session %d", session->id);
	...
	GST_RTP_SESSION_UNLOCK (session);
	g_signal_emitv (args, gst_rtp_bin_signals[SIGNAL_REQUEST_FID_MAP], zero, &ret);
	GST_RTP_SESSION_LOCK (session);
	...
	rt = g_value_get_uint (&ret);
	g_value_unset (&ret);
	GST_DEBUG ("FID VALUE RETRIVED FROM SIGNAL %d",rt);
	GST_RTP_SESSION_UNLOCK (session);
	return rt;
}
/**
...
* @see
*/
static guint get_fec_map (GstRtpBinSession *session)
{
	GstRtpBin *bin;
	guint rt;
	GValue ret = { ZERO };
	GValue args[2] = { {ZERO}, {ZERO} };
	GST_DEBUG ("searching FEC val ");
	GST_RTP_SESSION_LOCK (session);
	bin = session->bin;
	GST_DEBUG ("emiting signal for FEC in session %d", session->id);
	/* not in cache, send signal to request caps */
	g_value_init (&args[ZERO], GST_TYPE_ELEMENT);
	g_value_set_object (&args[ZERO], bin);
	...
	g_value_init (&ret, G_TYPE_UINT);
	GST_RTP_SESSION_UNLOCK (session);
	g_signal_emitv (args, gst_rtp_bin_signals[SIGNAL_REQUEST_FEC_MAP], ZERO, &ret);
	GST_RTP_SESSION_LOCK (session);
	g_value_unset (&args[ZERO]);
	g_value_unset (&args[1]);
	rt = g_value_get_uint (&ret);
	g_value_unset (&ret);
	GST_DEBUG ("FEC VALUE RETRIVED FROM SIGNAL %d",rt);
	GST_RTP_SESSION_UNLOCK (session);
	return rt;
}
...
get_pt_map (GstRtpBinSession * session, guint pt)
{
  ...
  g_signal_emitv (args, gst_rtp_bin_signals[SIGNAL_REQUEST_PT_MAP], ZERO, &ret);
...
gst_rtp_bin_reset_sync (GstRtpBin * rtpbin)
{
  ...
  for (clients = rtpbin->clients; clients; clients = g_slist_next (clients)) {
    ...
    for (streams = client->streams; streams; streams = g_slist_next (streams)) {
      ...
      stream->rt_delta = ZERO;
      stream->rtp_delta = ZERO;
      stream->clock_base = MINUS_HUNDRED * GST_SECOND;
    }
  }
...
/**
	* @fn		gst_rtp_bin_propagate_property_to_session
	* @brief
	*
	* @param[in]/[out]	: GstRtpBin *bin,const gchar *name, const GValue *value
	* @return			: void
	* @warning			: None
	* @exception		: None
	* @see
    ...
    const gchar *name, const GValue *value)
{
	...
	for (sessions = bin->sessions; sessions; sessions = g_slist_next(sessions))
	{
		GstRtpBinSession *session = (GstRtpBinSession *)sessions->data;
		GST_RTP_SESSION_LOCK (session);
		g_object_set_property (G_OBJECT (session->session), name, value);
		GST_RTP_SESSION_UNLOCK (session);
	}
	GST_RTP_BIN_UNLOCK (bin);
}
    ...
    guint64 * ntpnstime)
{
  ...
  {
    GST_OBJECT_UNLOCK (bin);
    rt = INVALID_OR_RESET_WITH_MINUS_ONE;
    ntpns = INVALID_OR_RESET_WITH_MINUS_ONE;
  }
  if (running_time)
  {
    *running_time = rt;
    ...
    gint64 rtp_clock_base)
{
  ...
  if (bin->ntp_sync) {
	...
	if (stream != ostream && stream->clock_base >= ZERO && (stream->clock_base != clock_base))
	{
		GST_DEBUG_OBJECT (bin, "reset upon clock base change");
		ostream->clock_base = -100 * GST_SECOND;
		ostream->rtp_delta = ZERO;
      }
...
create_stream (GstRtpBinSession * session, guint32 ssrc)
{
  ...
  g_object_set (buffer, "do-retransmission", rtpbin->do_retransmission, NULL); //for #RET change
  g_object_set (buffer, "do-fec", rtpbin->do_fec, NULL);  //FEC changes
  if (!rtpbin->ignore_pt)
	{
		gst_bin_add (GST_BIN_CAST (rtpbin), demux);
	}
...
//for #RET change
 gst_rtp_bin_signals[SIGNAL_REQUEST_RTX_MAP] =
      g_signal_new ("request-rtx-map", G_TYPE_FROM_CLASS (klass),
      G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET (GstRtpBinClass, request_rtx_map),
      NULL, NULL, gst_rtp_bin_marshal_UINT__UINT, G_TYPE_UINT, ONE, G_TYPE_UINT);
//for #RET change
 gst_rtp_bin_signals[SIGNAL_REQUEST_FID_MAP] =
      g_signal_new ("request-fid-map", G_TYPE_FROM_CLASS (klass),
      G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET (GstRtpBinClass, request_fid_map),
      NULL, NULL, gst_rtp_bin_marshal_UINT__UINT, G_TYPE_UINT, ONE, G_TYPE_UINT);
//FEC changes
 gst_rtp_bin_signals[SIGNAL_REQUEST_FEC_MAP] =
      g_signal_new ("request-fec-map", G_TYPE_FROM_CLASS (klass),
      G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET (GstRtpBinClass, request_fec_map),
      NULL, NULL, gst_rtp_bin_marshal_UINT__UINT, G_TYPE_UINT, 1, G_TYPE_UINT);
...
//for #RET change
  /**
   * GstRtpBin::remove-internal-session:
   * @rtpbin: the object which received the signal
   * @id: the session id
   *
   * Request to remove the internal RTPSession session @id.
   */
  gst_rtp_bin_signals[SIGNAL_GET_INTERNAL_SESSION] =
      g_signal_new ("remove-internal-session", G_TYPE_FROM_CLASS (klass),
      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION, G_STRUCT_OFFSET (GstRtpBinClass,
          remove_internal_session), NULL, NULL,g_cclosure_marshal_VOID__INT,
      G_TYPE_NONE, ONE, G_TYPE_INT);
...
//FEC changes
  g_object_class_install_property (gobject_class, PROP_DO_FEC,
	 g_param_spec_boolean ("do-fec", "Do FEC",
		  "Send an event downstream to request forward error correction",
  ...
  klass->remove_internal_session =
      GST_DEBUG_FUNCPTR (gst_rtp_bin_remove_internal_session); //for #RET change
  GST_DEBUG_CATEGORY_INIT (gst_rtp_bin_debug, "rtpbin", ZERO, "RTP bin");
}
...
gst_rtp_bin_init (GstRtpBin * rtpbin, GstRtpBinClass * klass)
{
  ...
  rtpbin->do_retransmission = DEFAULT_DO_RETRANSMISSION; //for #RET change
  rtpbin->do_fec = DEFAULT_DO_FEC; //FEC changes
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  rtpbin = GST_RTP_BIN (object);
	...
	case PROP_USE_PIPELINE_CLOCK:
		{
			...
			for (sessions = rtpbin->sessions; sessions;sessions = g_slist_next(sessions))
			{
				GstRtpBinSession *session = (GstRtpBinSession *) sessions->data;
				g_object_set (G_OBJECT(session->session),"use-pipeline-clock", rtpbin->use_pipeline_clock, NULL);
			}
			GST_RTP_BIN_UNLOCK (rtpbin);
		}
		break;
	...
	case PROP_DO_RETRANSMISSION:
		...
		GST_RTP_BIN_UNLOCK (rtpbin);
		gst_rtp_bin_propagate_property_to_session (rtpbin,"do-retransmission", value);
		gst_rtp_bin_propagate_property_to_jitterbuffer (rtpbin,"do-retransmission", value);
		break;
	case PROP_DO_FEC:		   //FEC changes
		GST_RTP_BIN_LOCK (rtpbin);
		rtpbin->do_fec = g_value_get_boolean (value);
		GST_RTP_BIN_UNLOCK (rtpbin);
		gst_rtp_bin_propagate_property_to_jitterbuffer (rtpbin, "do-fec", value);
		break;
    ...
    GValue * value, GParamSpec * pspec)
{
  ...
  rtpbin = GST_RTP_BIN (object);
	...
	case PROP_DO_FEC: 	//FEC changes
		GST_RTP_BIN_LOCK (rtpbin);
		g_value_set_boolean (value, rtpbin->do_fec);
		GST_RTP_BIN_UNLOCK (rtpbin);
		break;
	default:
		G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
    ...
    case GST_MESSAGE_BUFFERING:
    {
      ...
      if (G_LIKELY (stream)) {
        ...
        if (G_UNLIKELY (change)) {
          ...
          /* pause all streams */
          min_out_time = INVALID_OR_RESET_WITH_MINUS_ONE;
          for (sessions = rtpbin->sessions; sessions;
              sessions = g_slist_next (sessions)) {
            GstRtpBinSession *session = (GstRtpBinSession *) sessions->data;
		...
		if (!active)
		{
			...
			if(last_out == INVALID_OR_RESET_WITH_MINUS_ONE)
			{
				last_out = ZERO;
			}
			if (min_out_time == INVALID_OR_RESET_WITH_MINUS_ONE || last_out < min_out_time)
			{
...
//for #RET change
  ...
  guint fid = ZERO;
  GSList *walk=NULL;
  static guint count=ZERO;


  rtpbin = session->bin;
  if(rtpbin->do_retransmission)
  {
	//for #RET change
	rtx = get_rtx_map(session);
  }
      fec = get_fec_map(session);  //FEC changes

  GST_DEBUG_OBJECT (rtpbin, "new SSRC pad %08x, %s:%s", ssrc,
      GST_DEBUG_PAD_NAME (pad));

 GST_DEBUG("do_retrans %d, rtx %d, de_fec %d, fec %d",rtpbin->do_retransmission,rtx,rtpbin->do_fec,fec);
/***************************for #RET change *********************************/
  if((rtpbin->do_retransmission && (rtx == ONE))|| (rtpbin->do_fec==TRUE && (fec==ONE)) )
  {
	fid = get_fid_map(session);
	GST_DEBUG ("FID VALUE %d",fid);
	GST_RTP_BIN_SHUTDOWN_LOCK (rtpbin, shutdown);
	GST_RTP_SESSION_LOCK (session);
	while(count < DEFAULT_DO_SSRC_PAD_FOUND_MAX_TRIES)
	{
		for (walk = rtpbin->sessions; walk; walk = g_slist_next (walk))
		{
			GstRtpBinSession *sess = (GstRtpBinSession *)walk->data;
			...
			GST_RTP_SESSION_UNLOCK (session);
			GST_RTP_BIN_SHUTDOWN_UNLOCK (rtpbin);
			sess_fid= get_fid_map(sess);
			GST_RTP_BIN_SHUTDOWN_LOCK (rtpbin, shutdown);
			GST_RTP_SESSION_LOCK (session);

			if((sess != session) && (fid == sess_fid))
			{
				if ((stream = find_stream_by_ssrc (sess, ssrc)))
				{
					if(rtx == ONE)
					{
						/* get pad and link */
						GST_DEBUG_OBJECT (rtpbin, "linking jitterbuffer RTP RTXXXXXX");
						padname = g_strdup_printf ("src_%d", ssrc);
						srcpad = gst_element_get_static_pad (element, padname);
						g_free (padname);
						sinkpad = gst_element_get_request_pad (stream->buffer, "sink_rtx");
						gst_pad_link (srcpad, sinkpad);
						gst_object_unref (sinkpad);
						gst_object_unref (srcpad);

						GST_DEBUG_OBJECT (rtpbin, "linking jitterbuffer RTCP RTXXXXXX");
						padname = g_strdup_printf ("rtcp_src_%d", ssrc);
						srcpad = gst_element_get_static_pad (element, padname);
						g_free (padname);
						sinkpad = gst_element_get_request_pad (stream->buffer, "sink_rtcp_rtx");
						gst_pad_link (srcpad, sinkpad);
						gst_object_unref (sinkpad);
						gst_object_unref (srcpad);
					}
					...
					{
						/* get pad and link */
						GST_DEBUG_OBJECT (rtpbin, "linking jitterbuffer RTP FECXXXXX SESSION MULTIPLEXING");
						padname = g_strdup_printf ("src_%d", ssrc);
						srcpad = gst_element_get_static_pad (element, padname);
						g_free (padname);
						sinkpad = gst_element_get_request_pad (stream->buffer, "sink_fec");
						gst_pad_link (srcpad, sinkpad);
						gst_object_unref (sinkpad);
						gst_object_unref (srcpad);
						GST_DEBUG_OBJECT (rtpbin, "linking jitterbuffer RTCP FECXXXXX SESSION MULTIPLEXING");
						padname = g_strdup_printf ("rtcp_src_%d", ssrc);
						srcpad = gst_element_get_static_pad (element, padname);
						g_free (padname);
						sinkpad = gst_element_get_request_pad (stream->buffer, "sink_rtcp_fec");
						gst_pad_link (srcpad, sinkpad);
						gst_object_unref (sinkpad);
						gst_object_unref (srcpad);
					}
		...
		else if(stream == NULL)
		{
			GST_RTP_SESSION_UNLOCK (session);
			GST_RTP_BIN_SHUTDOWN_UNLOCK (rtpbin);
			g_usleep(WAIT_TIMEOUT_FOR_CHECKING_SESSION);
			GST_RTP_BIN_SHUTDOWN_LOCK (rtpbin, shutdown);
			GST_RTP_SESSION_LOCK (session);
		}
		count++;
  ...
  else
  {
	...
	if(rtpbin->do_fec || fec == 2)
	{
		for (walk = session->streams; walk; walk = g_slist_next (walk))
		{
			...
			{
				/* get pad and link */
				GST_DEBUG_OBJECT (rtpbin, "linking jitterbuffer RTP FECXXXXX SSRC MULTIPLEXING");
				padname = g_strdup_printf ("src_%d", ssrc);
				srcpad = gst_element_get_static_pad (element, padname);
				g_free (padname);
				sinkpad = gst_element_get_request_pad (stream->buffer, "sink_fec");
				...
				GST_DEBUG_OBJECT (rtpbin, "linking jitterbuffer RTCP FECXXXXX SSRC MULTIPLEXING");
				padname = g_strdup_printf ("rtcp_src_%d", ssrc);
				srcpad = gst_element_get_static_pad (element, padname);
				g_free (padname);
				sinkpad = gst_element_get_request_pad (stream->buffer, "sink_rtcp_fec");
...
static void
gst_rtp_bin_remove_internal_session (GstRtpBin *bin, gint session_id)
{
	GstRtpBinSession *session=NULL;

	GST_RTP_BIN_LOCK (bin);
	GST_DEBUG_OBJECT (bin, "removing internal RTPSession object, index: %d",session_id);
	session = find_session_by_id (bin, session_id);
	if (session)
	{
		if (session->recv_rtp_sink_ghost)
		{
			remove_recv_rtp (bin, session);
		}
		if (session->recv_rtcp_sink_ghost)
		{
			remove_recv_rtcp (bin, session);
		}
		if (session->send_rtp_sink_ghost)
		{
			remove_send_rtp (bin, session);
		}
		if (session->send_rtcp_src_ghost)
		{
			remove_rtcp (bin, session);
		}
		bin->sessions = g_slist_remove (bin->sessions, session);
		free_session(session,bin);
	}
	GST_RTP_BIN_UNLOCK (bin);
	return;
}
