...
static GstStaticPadTemplate gst_rtp_ac3_pay_sink_template =
    ...
    GST_PAD_SINK,
    GST_PAD_ALWAYS,
    GST_STATIC_CAPS ("audio/ac3; " "audio/x-ac3; " "audio/x-eac3; ")
    );
...
/* Edited for EAC3 support */

  if ((str = gst_structure_get_name (structure))) {
    if ((strcmp (str, "audio/x-ac3") == 0) || (strcmp (str, "audio/ac3") == 0) )
      gst_basertppayload_set_options (payload, "audio", TRUE, "AC3", rate);
    else if (strcmp (str, "audio/x-eac3") == 0)
      gst_basertppayload_set_options (payload, "audio", TRUE, "EAC3", rate);
    else
      goto wrong_type;
  } else
    goto wrong_type;
...
wrong_type:
  {
    GST_ERROR_OBJECT (rtpac3pay, "unsupported media type '%s'",
        GST_STR_NULL (str));
    return FALSE;
  }
...
gst_rtp_ac3_pay_flush (GstRtpAC3Pay * rtpac3pay)
{
  ...
  GstFlowReturn ret;
  const gchar *str;
  const struct GstCaps * caps;
  GstStructure * structure;

  caps = (const struct GstCaps *) GST_PAD_CAPS(rtpac3pay->payload.sinkpad);

  structure = gst_caps_get_structure ((GstCaps *)caps, 0);
  ...
  while (avail > 0) {
	...
	FT = 1;
        ...
        if (maxlen == 0)
        {
          GST_ERROR_OBJECT (rtpac3pay, "maxlen is 0, So, it occur divied by zero");
          return FALSE;
        }
    ...
    GstBuffer * buffer)
{
  ...
  while (TRUE) {
    ...
    {
    	frmsizecod=((((p[2]&0x07)<<8)|p[3])+1);

    	GST_DEBUG_OBJECT (rtpac3pay, "frmsizecod %u", frmsizecod);

	if(frmsizecod > 2048)
	break;

	frame_size=frmsizecod*2;
    }
