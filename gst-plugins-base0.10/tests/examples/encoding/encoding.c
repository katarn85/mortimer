    ...
    const GstCaps * acodec)
{
  ...
  if (!a)
  {
        gchar *capsstr_container = gst_caps_to_string (container);
	a = g_strdup_printf ("%.10s",
    g_uri_escape_string (capsstr_container, NULL, FALSE));
	g_free(capsstr_container);
  }
  if (!b)
  {
    gchar *capsstr_vcodec = gst_caps_to_string (vcodec);
    b = g_strdup_printf ("%.10s",
        g_uri_escape_string (capsstr_vcodec, NULL, FALSE));
    g_free(capsstr_vcodec);
  }
  if (!c)
  {
    gchar *capsstr_acodec = gst_caps_to_string (acodec);
    c = g_strdup_printf ("%.10s",
        g_uri_escape_string (capsstr_acodec, NULL, FALSE));
    g_free(capsstr_acodec);
  }
