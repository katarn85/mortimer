...
static GstStaticCaps mid_caps = GST_STATIC_CAPS ("audio/midi,  parsed = (bool) false");
...
static GstStaticCaps mp3_caps = GST_STATIC_CAPS ("audio/mpeg, "
    "mpegversion = (int) 1, layer = (int) [ 1, 3 ], parsed = false");
...
/*in push-mode, typefind stores at most 128KB data*/
#define GST_MP3_TYPEFIND_TRY_SYNC (128*1024) /* 128kB */
...
suggest:
  {
    ...
    gst_type_find_suggest_simple (tf, prob, "audio/mpeg",
        "mpegversion", G_TYPE_INT, 1, "layer", G_TYPE_INT, layer, "parsed", G_TYPE_BOOLEAN, FALSE, NULL);
  }
}
...
static GstStaticCaps ac3_caps = GST_STATIC_CAPS ("audio/x-ac3, framed = (bool) false");

#define AC3_CAPS (gst_static_caps_get(&ac3_caps))

static GstStaticCaps eac3_caps = GST_STATIC_CAPS ("audio/x-eac3, framed = (bool) false");
...
/*** audio/x-dts ***/
static GstStaticCaps dts_caps = GST_STATIC_CAPS ("audio/x-dts, framed = (bool) false");
...
plugin_init (GstPlugin * plugin)
{
  ...
  TYPE_FIND_REGISTER (plugin, "audio/x-dts", GST_RANK_MARGINAL, dts_type_find,
