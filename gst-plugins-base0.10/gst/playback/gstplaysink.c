...
#ifdef GST_EXT_USE_SPLITTER
#define GST_PLAYSINK_GET_BLOCK_COND(playsink) (((GstPlaySink *)playsink)->block_cond)
#define GST_PLAYSINK_GET_BLOCK_LOCK(playsink) (((GstPlaySink *)playsink)->block_lock)
#define GST_PLAYSINK_BLOCK_WAIT(playsink)     (g_cond_wait (GST_PLAYSINK_GET_BLOCK_COND (playsink), GST_PLAYSINK_GET_BLOCK_LOCK (playsink)))
#define GST_PLAYSINK_BLOCK_SIGNAL(playsink)   (g_cond_signal (GST_PLAYSINK_GET_BLOCK_COND (playsink)))
#define GST_PLAYSINK_BLOCK_BROADCAST(playsink)(g_cond_broadcast (GST_PLAYSINK_GET_BLOCK_COND (playsink)))
#define GST_PLAYSINK_BLOCK_LOCK(playsink)     (g_mutex_lock (GST_PLAYSINK_GET_BLOCK_LOCK(playsink)))
#define GST_PLAYSINK_BLOCK_UNLOCK(playsink)   (g_mutex_unlock (GST_PLAYSINK_GET_BLOCK_LOCK(playsink)))

#define GST_PLAYSINK_GET_PUSH_EVENT_COND(playsink) (((GstPlaySink *)playsink)->event_cond)
#define GST_PLAYSINK_GET_PUSH_EVENT_LOCK(playsink) (((GstPlaySink *)playsink)->event_lock)
#define GST_PLAYSINK_PUSH_EVENT_WAIT(playsink)     (g_cond_wait (GST_PLAYSINK_GET_PUSH_EVENT_COND (playsink), GST_PLAYSINK_GET_PUSH_EVENT_LOCK (playsink)))
#define GST_PLAYSINK_PUSH_EVENT_SIGNAL(playsink)   (g_cond_signal (GST_PLAYSINK_GET_PUSH_EVENT_COND (playsink)))
#define GST_PLAYSINK_PUSH_EVENT_BROADCAST(playsink)(g_cond_broadcast (GST_PLAYSINK_GET_PUSH_EVENT_COND (playsink)))
#define GST_PLAYSINK_PUSH_EVENT_LOCK(playsink)     (g_mutex_lock (GST_PLAYSINK_GET_PUSH_EVENT_LOCK(playsink)))
#define GST_PLAYSINK_PUSH_EVENT_UNLOCK(playsink)   (g_mutex_unlock (GST_PLAYSINK_GET_PUSH_EVENT_LOCK(playsink)))
...
#ifdef GST_EXT_USE_SPLITTER
  GstElement *audio_splitter;  // audiodatasplitter
  GstElement *subpcm_queue;
  GstElement *subpcm_sink;

  GstElement *spdif_queue;
  GstElement *spdif_sink;
  GstElement *fake_sink;
  gint audio_output_enable;
#ifdef GST_EXT_USE_SUBPCM_VOLUME
  GstElement *subpcm_volume;
#endif
  GMutex    *block_lock;
  GCond     *block_cond;
  gboolean  ss_pad_blocked;

  GThread   *event_thread;
  GMutex    *event_lock;
  GCond     *event_cond;
  gboolean  event_thread_exit;
#endif
...
#ifdef GST_EXT_USE_SPLITTER
static gpointer gst_play_sink_push_event_thread (gpointer data);
static gboolean gst_play_sink_deactivate_audio_src (GstPlaySink * playsink, gboolean activate);
static gboolean gst_play_sink_activate_audio_src (GstPlaySink * playsink, gboolean activate);
...
#endif
#ifdef GST_EXT_USE_SPLITTER
  g_object_class_install_property (gobject_klass, PROP_AUDIO_OUTPUT_MODE,
      g_param_spec_boolean ("audio-output-mode", "Set Audio Output",
          "set audio output - enable/disable", TRUE,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
#endif
...
{
  GST_DEBUG_OBJECT(playsink, "init");
...
#ifdef GST_EXT_USE_SPLITTER
  playsink->audio_splitter = NULL;
  playsink->subpcm_queue = NULL;
  playsink->subpcm_sink = NULL;

  playsink->spdif_queue = NULL;
  playsink->spdif_sink = NULL;
  playsink->fake_sink = NULL;
  g_atomic_int_set (&playsink->audio_output_enable, TRUE);
#ifdef GST_EXT_USE_SUBPCM_VOLUME
  playsink->subpcm_volume = NULL;
#endif

  playsink->block_lock = g_mutex_new ();
  playsink->block_cond = g_cond_new ();
  playsink->ss_pad_blocked = FALSE;

  playsink->event_lock = g_mutex_new ();
  playsink->event_cond = g_cond_new ();
  playsink->event_thread_exit = FALSE;
  playsink->event_thread = NULL;
#endif
...
#ifdef GST_EXT_USE_SPLITTER
  if (playsink->event_thread) {
    playsink->event_thread_exit = TRUE;
		GST_PLAYSINK_PUSH_EVENT_SIGNAL(playsink);
    GST_DEBUG_OBJECT (playsink, "joining thread");
    g_thread_join (playsink->event_thread);
    GST_DEBUG_OBJECT (playsink, "joined thread");
    playsink->event_thread = NULL;
  }

  if (playsink->event_lock) {
    g_mutex_free (playsink->event_lock);
    playsink->event_lock = NULL;
  }

  if (playsink->event_cond) {
    g_cond_free (playsink->event_cond);
    playsink->event_cond = NULL;
  }

  if (playsink->block_lock) {
    g_mutex_free (playsink->block_lock);
    playsink->block_lock = NULL;
  }

  if (playsink->block_cond) {
    g_cond_free (playsink->block_cond);
    playsink->block_cond = NULL;
  }
#endif
...
#ifdef GST_EXT_USE_SPLITTER
#ifdef GST_EXT_USE_SUBPCM_VOLUME
  if(playsink->subpcm_volume) {
    GST_LOG_OBJECT (playsink, "new volume=%.03f, mute=%d", volume, playsink->mute);
    if (!playsink->mute) {
      g_object_set (playsink->subpcm_volume, "volume", volume, NULL);
    }
  }
#endif
#endif
...
gst_play_sink_set_mute (GstPlaySink * playsink, gboolean mute)
{
  ...
  if(playsink->subpcm_volume) {
    if (mute)
      g_object_set (playsink->subpcm_volume, "volume", (gdouble) 0.0, NULL);
    else
      g_object_set (playsink->subpcm_volume, "volume", (gdouble) playsink->volume, NULL);
  }
#endif
#endif
  GST_PLAY_SINK_UNLOCK (playsink);
...
#ifdef GST_EXT_USE_SPLITTER
static gpointer
gst_play_sink_push_event_thread (gpointer data)
{
  GstPlaySink *playsink = (GstPlaySink *) data;

  while (!playsink->event_thread_exit) {

    GST_DEBUG_OBJECT (playsink, "event thread is started. waiting for signal.\n");

    GST_PLAYSINK_PUSH_EVENT_LOCK (playsink);

    GST_PLAYSINK_PUSH_EVENT_WAIT (playsink);


    if ( playsink->event_thread_exit ) {
      GST_DEBUG_OBJECT (playsink, "exiting event thread\n");
      GST_PLAYSINK_PUSH_EVENT_UNLOCK (playsink);
      break;
    }

    GST_DEBUG_OBJECT (playsink, "push custom event to synchronizer src pad\n");
    gst_pad_push_event (playsink->audio_srcpad_stream_synchronizer,
...
static void
gst_play_sink_src_pad_block_cb (GstPad *pad, gboolean blocked, gpointer user_data)
{
  GstPlaySink *playsink = (GstPlaySink *) user_data;

  if ( blocked ) {

    GST_PLAYSINK_BLOCK_LOCK (playsink);

    GST_DEBUG_OBJECT(playsink, "src pad is blocked");
    playsink->ss_pad_blocked = TRUE;

    GST_PLAYSINK_BLOCK_SIGNAL (playsink);

    GST_PLAYSINK_BLOCK_UNLOCK (playsink);

  } else {
    GST_DEBUG_OBJECT(playsink, "src pad is unblocked");
  }
}

static gboolean
gst_play_sink_deactivate_audio_src (GstPlaySink * playsink, gboolean activate)
{
  GstPad *srcpad = NULL;
  GstPad *sinkpad = NULL;
  ...
  if (activate) {
    GST_WARNING_OBJECT (playsink, "input is %d", activate);
    goto DONE;
  }

  srcpad = playsink->audio_srcpad_stream_synchronizer;

  GST_LOG_OBJECT (playsink, "start blockig");
  
  if (!playsink->audiochain) {
    GST_WARNING_OBJECT (playsink, "audiochain didn't create yet");
    return FALSE;
  }

  gst_element_get_state (playsink->audiochain->sink, &state, &pending, time);

  if (state <= GST_STATE_PAUSED) {
    GST_DEBUG_OBJECT (playsink, "sink state is PAUSED, async pad block\n");
    playsink->ss_pad_blocked = FALSE;

    if (!gst_pad_set_blocked_async (srcpad, TRUE, gst_play_sink_src_pad_block_cb, playsink)) {
      GST_ERROR_OBJECT (playsink, "audio synchronizer src pad block fail");
      goto DONE;
    }

    GST_LOG_OBJECT(playsink, "change SINK state\n");
    gst_element_set_state (playsink->audiochain->sink, GST_STATE_PLAYING);
    gst_element_set_state (playsink->spdif_sink, GST_STATE_PLAYING);
    gst_element_set_state (playsink->subpcm_sink, GST_STATE_PLAYING);

    GST_PLAYSINK_BLOCK_LOCK (playsink);
    GST_LOG_OBJECT(playsink, "pad blocked (%s)", (playsink->ss_pad_blocked)?"OO":"XX");

    if (!playsink->ss_pad_blocked) {
      GST_PLAYSINK_PUSH_EVENT_LOCK (playsink);
      GST_PLAYSINK_PUSH_EVENT_SIGNAL (playsink);
      GST_PLAYSINK_PUSH_EVENT_UNLOCK (playsink);
      GST_PLAYSINK_BLOCK_WAIT (playsink);
    }

    GST_PLAYSINK_BLOCK_UNLOCK (playsink);
  } else {

    if (!gst_pad_set_blocked_async (srcpad, TRUE, NULL, playsink)) {
      GST_ERROR_OBJECT (playsink, "audio synchronizer src pad block fail");
      goto DONE;
    }
  }

  GST_LOG_OBJECT (playsink, "end blockig");
  sinkpad = gst_element_get_static_pad (playsink->audio_splitter, "sink");

  if (!sinkpad || !srcpad) {
    GST_ERROR_OBJECT (playsink, "Failed get audio_splitter pad");
    goto DONE;
  }

  if (!gst_pad_is_blocking(srcpad)) {
    GST_ERROR_OBJECT (playsink, "Stream Synchronized is not blocked yet. Can't unlink now.");
  }

  /* unlink syncronizer element, audio splitter element */
  if (!gst_pad_unlink (srcpad, sinkpad)) {
    GST_ERROR_OBJECT (playsink, "Failed to unlink");
    goto DONE;
  }
  ...
  GST_LOG_OBJECT(playsink, "sink state change to NULL");

  if (playsink->audio_sink)
      GST_OBJECT_FLAG_UNSET (G_OBJECT (playsink->audio_sink), GST_ELEMENT_IS_SINK);
  if (playsink->subpcm_sink)
      GST_OBJECT_FLAG_UNSET (G_OBJECT (playsink->subpcm_sink), GST_ELEMENT_IS_SINK);
  if (playsink->spdif_sink)
      GST_OBJECT_FLAG_UNSET (G_OBJECT (playsink->spdif_sink), GST_ELEMENT_IS_SINK);
  if (playsink->fake_sink)
      GST_OBJECT_FLAG_SET (G_OBJECT (playsink->fake_sink), GST_ELEMENT_IS_SINK);

  /* NOTE : check if we need to set 'async' property to FALSE every sink */

  gst_element_set_state(playsink->audiochain->chain.bin, GST_STATE_NULL);
  gst_element_set_state(playsink->subpcm_sink, GST_STATE_NULL);
  gst_element_set_state(playsink->spdif_sink, GST_STATE_NULL);
  gst_element_set_state(playsink->subpcm_volume, GST_STATE_NULL);
  gst_element_set_state(playsink->subpcm_queue, GST_STATE_NULL);
  gst_element_set_state(playsink->spdif_queue, GST_STATE_NULL);
  gst_element_set_state(playsink->audio_splitter, GST_STATE_NULL);

  gst_element_set_locked_state(playsink->audiochain->chain.bin, TRUE);
  gst_element_set_locked_state(playsink->subpcm_sink, TRUE);
  gst_element_set_locked_state(playsink->spdif_sink, TRUE);
  gst_element_set_locked_state(playsink->subpcm_volume, TRUE);
  gst_element_set_locked_state(playsink->subpcm_queue, TRUE);
  gst_element_set_locked_state(playsink->spdif_queue, TRUE);
  gst_element_set_locked_state(playsink->audio_splitter, TRUE);

  if (!playsink->fake_sink) {
    GST_DEBUG_OBJECT(playsink, "make fake sink and add bin");

    playsink->fake_sink = gst_element_factory_make ("fakesink", "fake_sink");
    g_object_set(G_OBJECT (playsink->fake_sink), "sync", TRUE, "async", FALSE, NULL);
    gst_bin_add (GST_BIN_CAST (playsink), playsink->fake_sink);
    GST_OBJECT_FLAG_SET (G_OBJECT (playsink->fake_sink), GST_ELEMENT_IS_SINK);
  }

  sinkpad = gst_element_get_static_pad (playsink->fake_sink, "sink");

  if (GST_PAD_LINK_OK != gst_pad_link(srcpad, sinkpad)) {
    GST_ERROR_OBJECT (playsink, "Failed to link fakesink\n");
    goto DONE;
  }

  GstClock *clock = gst_element_get_clock(playsink);
  gst_element_set_clock(playsink->fake_sink, clock);
  gst_object_unref(clock);

  gst_element_set_locked_state(playsink->fake_sink, FALSE);
  gst_element_set_state(playsink->fake_sink, GST_STATE_PAUSED);

  if(playsink->video_sink) {
    GstBaseSink *video_stream_sink = NULL;
    video_stream_sink = (GstBaseSink*)(playsink->video_sink);
    GST_OBJECT_LOCK (video_stream_sink);
    if (playsink->fake_sink) {
      GstBaseSink *fake_sink = NULL;
      fake_sink = (GstBaseSink*)(playsink->fake_sink);
      GST_OBJECT_LOCK (fake_sink);
      fake_sink->segment = video_stream_sink->segment;
      fake_sink->have_newsegment = TRUE;
      GST_OBJECT_UNLOCK (fake_sink);
    }
    GST_OBJECT_UNLOCK (video_stream_sink);
  }

  GST_LOG_OBJECT(playsink, "fake sink state change to parent");
  gst_element_sync_state_with_parent(playsink->fake_sink);

  /* unblocked audio synchronizer src pad */
  if (!gst_pad_set_blocked_async (srcpad, FALSE, NULL, playsink)) {
  	GST_ERROR_OBJECT (playsink, "audio synchronizer src pad block fail TRUE");
    goto DONE;
  }
...
static gboolean
gst_play_sink_activate_audio_src (GstPlaySink * playsink, gboolean activate)
{
  GstPad *srcpad = NULL;
  GstPad *sinkpad = NULL;
  GstState state = GST_STATE_VOID_PENDING;
  ...
  /* blocked audio synchronizer src pad */
  GST_LOG_OBJECT (playsink, "start blockig");

  gst_element_get_state (playsink->fake_sink, &state, &pending, time);

  if(state <= GST_STATE_PAUSED) {
    GST_LOG_OBJECT(playsink, "sink state is PAUSED\n");
    playsink->ss_pad_blocked = FALSE;
    ...
    GST_LOG_OBJECT(playsink, "change SINK state to PLAYING for pad blocking\n");
    gst_element_set_state (playsink->fake_sink, GST_STATE_PLAYING);
    ...
    if (!gst_pad_set_blocked_async (srcpad, TRUE, NULL, playsink)) {
      GST_WARNING_OBJECT (playsink, "audio synchronizer src pad block fail");
      goto DONE;
    }
  }
  ...
  if (!sinkpad) {
    GST_ERROR_OBJECT (playsink, "Failed get audio fakesink pad");
  	goto DONE;
  }

  /* unlink syncronizer element, audio fakesink element */
  if (!gst_pad_unlink (srcpad, sinkpad)) {
    GST_ERROR_OBJECT (playsink, "Failed to unlink");
  	goto DONE;
  }
  ...
  GST_LOG_OBJECT(playsink, "audio fake_sink state change to NULL");

  if (playsink->audio_sink)
      GST_OBJECT_FLAG_SET (G_OBJECT (playsink->audio_sink), GST_ELEMENT_IS_SINK);
  if (playsink->subpcm_sink)
      GST_OBJECT_FLAG_SET (G_OBJECT (playsink->subpcm_sink), GST_ELEMENT_IS_SINK);
  if (playsink->spdif_sink)
      GST_OBJECT_FLAG_SET (G_OBJECT (playsink->spdif_sink), GST_ELEMENT_IS_SINK);
  if (playsink->fake_sink)
      GST_OBJECT_FLAG_UNSET (G_OBJECT (playsink->fake_sink), GST_ELEMENT_IS_SINK);

  gst_element_set_state(playsink->fake_sink, GST_STATE_NULL);
  gst_element_set_locked_state(playsink->fake_sink, TRUE);

  sinkpad = gst_element_get_static_pad (playsink->audio_splitter, "sink");

  if (GST_PAD_LINK_OK != gst_pad_link(srcpad, sinkpad)) {
    GST_ERROR_OBJECT (playsink, "Failed to link audio_splitter\n");
  	goto DONE;
  }

  gst_element_set_locked_state(playsink->audiochain->chain.bin, FALSE);
  gst_element_set_locked_state(playsink->subpcm_sink, FALSE);
  gst_element_set_locked_state(playsink->spdif_sink, FALSE);
  gst_element_set_locked_state(playsink->subpcm_volume, FALSE);
  gst_element_set_locked_state(playsink->subpcm_queue, FALSE);
  gst_element_set_locked_state(playsink->spdif_queue, FALSE);
  gst_element_set_locked_state(playsink->audio_splitter, FALSE);

  /* get/set clock */
  GstClock *clock = gst_element_get_clock(playsink);
  gst_element_set_clock(playsink->audio_sink, clock);
  gst_element_set_clock(playsink->subpcm_sink, clock);
  gst_element_set_clock(playsink->spdif_sink, clock);
  gst_object_unref(clock);

  gst_element_set_state(playsink->audiochain->chain.bin, GST_STATE_PAUSED);
  gst_element_set_state(playsink->subpcm_sink, GST_STATE_PAUSED);
  gst_element_set_state(playsink->spdif_sink, GST_STATE_PAUSED);
  gst_element_set_state(playsink->subpcm_volume, GST_STATE_PAUSED);
  gst_element_set_state(playsink->subpcm_queue, GST_STATE_PAUSED);
  gst_element_set_state(playsink->spdif_queue, GST_STATE_PAUSED);
  gst_element_set_state(playsink->audio_splitter, GST_STATE_PAUSED);


  if(playsink->video_sink) {
    GstBaseSink *video_stream_sink = NULL;
    GstBaseSink *audio_stream_sink1 = NULL;
    GstBaseSink *audio_stream_sink2 = NULL;
    GstBaseSink *audio_stream_sink3 = NULL;

    video_stream_sink = (GstBaseSink*)(playsink->video_sink);
    GST_OBJECT_LOCK (video_stream_sink);
    if (playsink->audio_sink) {
      audio_stream_sink1 = (GstBaseSink*)(playsink->audio_sink);
      GST_OBJECT_LOCK (audio_stream_sink1);
      audio_stream_sink1->segment = video_stream_sink->segment;
      audio_stream_sink1->have_newsegment = TRUE;
      GST_OBJECT_UNLOCK (audio_stream_sink1);
    }

    if (playsink->subpcm_sink) {
      audio_stream_sink2 = (GstBaseSink*)(playsink->subpcm_sink);
      GST_OBJECT_LOCK (audio_stream_sink2);
      audio_stream_sink2->segment = video_stream_sink->segment;
      audio_stream_sink2->have_newsegment = TRUE;
      GST_OBJECT_UNLOCK (audio_stream_sink2);
    }

    if (playsink->spdif_sink) {
      audio_stream_sink3 = (GstBaseSink*)(playsink->spdif_sink);
      GST_OBJECT_LOCK (audio_stream_sink3);
      audio_stream_sink3->segment = video_stream_sink->segment;
      audio_stream_sink3->have_newsegment = TRUE;
      GST_OBJECT_UNLOCK (audio_stream_sink3);
    }
    GST_OBJECT_UNLOCK (video_stream_sink);
  }

  GST_LOG_OBJECT(playsink, "audio sink state change to parent");
  gst_element_sync_state_with_parent(playsink->audio_splitter);
  gst_element_sync_state_with_parent(playsink->audiochain->chain.bin);
  gst_element_sync_state_with_parent(playsink->subpcm_queue);
  gst_element_sync_state_with_parent(playsink->spdif_queue);
  gst_element_sync_state_with_parent(playsink->subpcm_volume);
  gst_element_sync_state_with_parent(playsink->subpcm_sink);
  gst_element_sync_state_with_parent(playsink->spdif_sink);


  /* unblocked audio synchronizer src pad */
  if (!gst_pad_set_blocked_async (srcpad, FALSE, NULL, playsink)) {
  	GST_ERROR_OBJECT (playsink, "audio synchronizer src pad unblock fail");
  	goto DONE;
  }
...
void
gst_play_sink_set_audio_output_mode (GstPlaySink * playsink, gboolean enable)
{
  gboolean ret = TRUE;
  ...
  g_atomic_int_set (&playsink->audio_output_enable, enable);

  GST_DEBUG_OBJECT (playsink, "set audio output : %d", enable);

  if (!playsink->event_thread) {
    GST_DEBUG_OBJECT (playsink, "create event thread to send custom event");

#if !GLIB_CHECK_VERSION (2, 31, 0)
    playsink->event_thread =
          g_thread_create ((GThreadFunc) gst_play_sink_push_event_thread, playsink,
                            TRUE, NULL);
#else
    playsink->event_thread = g_thread_new ("event_thread",
          (GThreadFunc) gst_play_sink_push_event_thread, playsink);
#endif
    if (!playsink->event_thread) {
      GST_ERROR_OBJECT(playsink, "failed to create event thread");
    }
  }

  if (enable) {
    ret = gst_play_sink_activate_audio_src(playsink, TRUE);
  } else {
    ret = gst_play_sink_deactivate_audio_src(playsink, FALSE);
  }

  if (!ret) {
    g_atomic_int_set (&playsink->audio_output_enable, !enable);
    GST_WARNING_OBJECT (playsink, "audio activate/deactivate failed");
    GST_DEBUG_OBJECT (playsink, "return set audio output : %d", !enable);
    return;
  }
...
gboolean
gst_play_sink_get_audio_output_mode (GstPlaySink * playsink)
{
  ...
  result = g_atomic_int_get (&playsink->audio_output_enable);
  GST_DEBUG_OBJECT (playsink, "get audio output : %d", result);
  return result;
}
...
static void
splitter_pad_added_cb (GstElement * element, GstPad * pad, GstPlaySink * playsink)
{
  GstCaps *caps = NULL;
  GstStructure *str = NULL;
  const char *caps_name = NULL;
  GstPad* srcpad = NULL;
  GstPad* sinkpad = NULL;
  ...
  if (!caps) {
    GST_ERROR_OBJECT (playsink, "caps is null");
    return;
  }

  str = gst_caps_get_structure(caps, 0);
  if (!str) {
    GST_ERROR_OBJECT (playsink, "str is null");
    goto ERROR;
  }

  caps_name = gst_structure_get_name(str);
  if (!caps_name) {
    GST_ERROR_OBJECT (playsink, "caps name is null");
    goto ERROR;
  }

  GST_DEBUG_OBJECT (playsink, "new pad : %s:%s, caps : %s\n", GST_DEBUG_PAD_NAME(pad), caps_name);

  if (g_str_equal(caps_name, "audio/x-raw-int")) {
    playsink->subpcm_queue = gst_element_factory_make ("queue", "subpcm_queue");
#ifdef GST_EXT_USE_SUBPCM_VOLUME
    playsink->subpcm_volume = gst_element_factory_make ("volume","subpcm_volume");
    if (!playsink->subpcm_volume) {
      GST_ERROR_OBJECT (playsink, "fail to make subpcm_volume element");
      goto ERROR;
    }
#endif
    playsink->subpcm_sink = gst_element_factory_make ("alsasink", "subpcm_sink");

    g_object_set (G_OBJECT (playsink->subpcm_sink), "provide-clock", FALSE,  NULL);
    g_object_set (G_OBJECT (playsink->subpcm_sink), "async", TRUE,  NULL);
    g_object_set (G_OBJECT (playsink->subpcm_sink), "query-position-support", FALSE,  NULL);
    g_object_set (G_OBJECT (playsink->subpcm_sink), "device", "hw:0,3", NULL);

    g_object_set(G_OBJECT(playsink->subpcm_queue), "max-size-buffers", 0, NULL);
    g_object_set(G_OBJECT(playsink->subpcm_queue), "max-size-time", 3*GST_SECOND, NULL);

    gst_bin_add (GST_BIN_CAST (playsink), playsink->subpcm_queue);
#ifdef GST_EXT_USE_SUBPCM_VOLUME
    gst_bin_add (GST_BIN_CAST (playsink), playsink->subpcm_volume);
#endif
    gst_bin_add (GST_BIN_CAST (playsink), playsink->subpcm_sink);

    srcpad = gst_element_get_pad(playsink->audio_splitter, "subpcm");
    sinkpad = gst_element_get_pad(playsink->subpcm_queue, "sink");

    if (!srcpad || !sinkpad) {
      GST_ERROR_OBJECT (playsink, "fail to get pad for subpcm");
      goto ERROR;
    }

    ret = gst_pad_link(srcpad, sinkpad);
    if (ret != GST_PAD_LINK_OK) {
      GST_ERROR_OBJECT (playsink, "fail to link (splitter : subpcm_queue)");
      goto ERROR;
    }
#ifdef GST_EXT_USE_SUBPCM_VOLUME
    if (gst_element_link_many(playsink->subpcm_queue, playsink->subpcm_volume, playsink->subpcm_sink, NULL) == FALSE) {
      GST_ERROR_OBJECT (playsink, "fail to link (subpcm_queue : subpcm_volume : subpcm_sink)");
      goto ERROR;
    }
#else
    if (gst_element_link(playsink->subpcm_queue, playsink->subpcm_sink) == FALSE) {
      GST_ERROR_OBJECT (playsink, "fail to link (subpcm_queue : subpcm_sink)");
      goto ERROR;
    }
#endif
    if (gst_element_sync_state_with_parent(playsink->subpcm_queue) == FALSE) {
      GST_ERROR_OBJECT (playsink, "fail to set state of subpcm_queue");
      goto ERROR;
    }
#ifdef GST_EXT_USE_SUBPCM_VOLUME
    if (gst_element_sync_state_with_parent(playsink->subpcm_volume) == FALSE) {
      GST_ERROR_OBJECT (playsink, "fail to set state of subpcm_volume");
      goto ERROR;
    }
#endif
    if (gst_element_sync_state_with_parent(playsink->subpcm_sink) == FALSE) {
      GST_ERROR_OBJECT (playsink, "fail to set state of subpcm_sink");
      goto ERROR;
    }
  } else if (g_str_equal(caps_name, "audio/x-spdif-es"))	{

    playsink->spdif_queue = gst_element_factory_make ("queue", "spdif_queue");
    playsink->spdif_sink = gst_element_factory_make ("spdifsink", "spdif_sink");

    g_object_set (G_OBJECT (playsink->spdif_sink), "async", FALSE,  NULL);
    g_object_set (G_OBJECT (playsink->spdif_queue), "max-size-buffers", 0, NULL);
    g_object_set (G_OBJECT (playsink->spdif_queue), "max-size-time", 3*GST_SECOND, NULL);

    gst_bin_add (GST_BIN_CAST (playsink), playsink->spdif_queue);
    gst_bin_add (GST_BIN_CAST (playsink), playsink->spdif_sink);

    srcpad = gst_element_get_pad(playsink->audio_splitter, "spdif_es");
    sinkpad = gst_element_get_pad(playsink->spdif_queue, "sink");

    if (!srcpad || !sinkpad) {
      GST_ERROR_OBJECT (playsink, "fail to get pad for spdif");
      goto ERROR;
    }

    ret = gst_pad_link(srcpad, sinkpad);
    if (ret != GST_PAD_LINK_OK) {
      GST_ERROR_OBJECT (playsink, "fail to link (splitter : spdif_queue)");
      goto ERROR;
    }

    if (gst_element_link(playsink->spdif_queue, playsink->spdif_sink) == FALSE) {
      GST_ERROR_OBJECT (playsink, "fail to link (spdif_queue : spdif_sink)");
      goto ERROR;
    }

    if (gst_element_sync_state_with_parent(playsink->spdif_queue) == FALSE) {
      GST_ERROR_OBJECT (playsink, "fail to set state of spdif_queue");
      goto ERROR;
    }

    if (gst_element_sync_state_with_parent(playsink->spdif_sink) == FALSE) {
      GST_ERROR_OBJECT (playsink, "fail to set state of spdif_sink");
      goto ERROR;
    }
  } else {
    GST_ERROR_OBJECT (playsink, "unexpected caps name[ %s ]\n", caps_name);
    goto ERROR;
  }
...
static gboolean
gst_play_sink_check_audiosubdata(GstPlaySink * playsink)
{
  GstCaps *audio_caps = NULL;
  GstStructure* str = NULL;
  char *caps_str = NULL;
  gboolean has_sub_data = FALSE;

  GST_DEBUG_OBJECT (playsink, "check whether the audio subdata exist or not.");

  if (playsink->audio_srcpad_stream_synchronizer)
    audio_caps = gst_pad_get_caps(playsink->audio_srcpad_stream_synchronizer);

  if (audio_caps) {
    str = gst_caps_get_structure (audio_caps, 0);
    caps_str = gst_caps_to_string (audio_caps);

    GST_DEBUG_OBJECT (playsink, "audio caps %s", caps_str);

    if ((str) && (gst_structure_has_field(str, "hassubdata"))) {
      gst_structure_get_boolean(str, "hassubdata", &has_sub_data);
    }
  ...
  GST_DEBUG_OBJECT (playsink, "has_sub_data %s", (has_sub_data)?"OO":"XX");
...
gst_play_sink_reconfigure (GstPlaySink * playsink)
{
  ...
  if (need_audio) {
...
#ifdef GST_EXT_USE_SPLITTER
        if (playsink->audio_splitter) {
          g_signal_handlers_disconnect_by_func (playsink->audio_splitter, splitter_pad_added_cb,
              playsink);
          gst_bin_remove(GST_BIN_CAST (playsink), playsink->audio_splitter);
          gst_element_set_state (playsink->audio_splitter, GST_STATE_NULL);
          playsink->audio_splitter = NULL;
        }

        if (playsink->subpcm_queue) {
          gst_bin_remove(GST_BIN_CAST (playsink), playsink->subpcm_queue);
          gst_element_set_state (playsink->subpcm_queue, GST_STATE_NULL);
          playsink->subpcm_queue = NULL;
        }
#ifdef GST_EXT_USE_SUBPCM_VOLUME
        if (playsink->subpcm_volume) {
          gst_bin_remove(GST_BIN_CAST (playsink), playsink->subpcm_volume);
          gst_element_set_state (playsink->subpcm_volume, GST_STATE_NULL);
          playsink->subpcm_volume = NULL;
        }
#endif
        if (playsink->subpcm_sink) {
          gst_bin_remove(GST_BIN_CAST (playsink), playsink->subpcm_sink);
          gst_element_set_state (playsink->subpcm_sink, GST_STATE_NULL);
          playsink->subpcm_sink = NULL;
        }

        if (playsink->spdif_queue) {
          gst_bin_remove(GST_BIN_CAST (playsink), playsink->spdif_queue);
          gst_element_set_state (playsink->spdif_queue, GST_STATE_NULL);
          playsink->spdif_queue = NULL;
        }

        if (playsink->spdif_sink) {
          gst_bin_remove(GST_BIN_CAST (playsink), playsink->spdif_sink);
          gst_element_set_state (playsink->spdif_sink, GST_STATE_NULL);
          playsink->spdif_sink = NULL;
        }

        if (playsink->fake_sink) {
          gst_bin_remove(GST_BIN_CAST (playsink), playsink->fake_sink);
          gst_element_set_state (playsink->fake_sink, GST_STATE_NULL);
          playsink->fake_sink = NULL;
        }
#endif
    ...
    if (playsink->audiochain) {
...
#ifdef GST_EXT_USE_SPLITTER
      if ((playsink->audio_sink) &&
          (g_strrstr(GST_ELEMENT_NAME(playsink->audio_sink), "alsasink")) &&
          (gst_play_sink_check_audiosubdata(playsink))) {
        GST_DEBUG_OBJECT (playsink, "create audio splitter");

        if (!playsink->audio_splitter)
            playsink->audio_splitter = gst_element_factory_make ("audiodatasplitter", "audiodata_splitter");
        ...
        if (playsink->audio_splitter) {
          ...
          g_signal_connect (G_OBJECT (playsink->audio_splitter), "pad-added",
              G_CALLBACK (splitter_pad_added_cb), playsink);

          gst_bin_add (GST_BIN_CAST (playsink), playsink->audio_splitter);

          sinkpad =
              gst_element_get_static_pad (playsink->audio_splitter, "sink");

          srcpad =
              gst_element_get_static_pad (playsink->audio_splitter, "mainpcm");

          if (sinkpad && srcpad) {
            gst_pad_link_full (playsink->audio_srcpad_stream_synchronizer,
                sinkpad, GST_PAD_LINK_CHECK_NOTHING);

            gst_element_sync_state_with_parent(playsink->audio_splitter);

            gst_pad_link_full (srcpad,
            ...
            GST_ERROR_OBJECT (playsink, "fail to link audio_splitter. remove it");
      ...
      if (!playsink->audio_splitter) {
        GST_DEBUG_OBJECT (playsink, "link streamsynchronizer:audiobin");
        gst_pad_link_full (playsink->audio_srcpad_stream_synchronizer,
            playsink->audiochain->sinkpad, GST_PAD_LINK_CHECK_NOTHING);
      }
#else
...
gst_play_sink_send_event_to_sink (GstPlaySink * playsink, GstEvent * event)
{
  ...
  if (playsink->send_event_mode == MODE_FIRST) {
    ...
    GST_DEBUG_OBJECT (playsink, "Send Event to all sink element in playsink.");
    return
...
#ifdef GST_EXT_USE_SPLITTER
      if (playsink->audio_srcpad_stream_synchronizer){
        GstPad *srcpad = playsink->audio_srcpad_stream_synchronizer;

        GST_PLAYSINK_PUSH_EVENT_BROADCAST(playsink);

        if (gst_pad_is_blocked(srcpad)) {
          gst_pad_set_blocked_async_full (srcpad, FALSE, gst_play_sink_src_pad_block_cb,
              gst_object_ref (playsink), (GDestroyNotify) gst_object_unref);
        }
      }
#endif
...
#endif
#ifdef GST_EXT_USE_SPLITTER
    case PROP_AUDIO_OUTPUT_MODE:
      gst_play_sink_set_audio_output_mode(playsink, g_value_get_boolean(value));
      break;
#endif
...
#ifdef GST_EXT_USE_SPLITTER
    case PROP_AUDIO_OUTPUT_MODE:
      g_value_set_boolean(value, gst_play_sink_get_audio_output_mode(playsink));
      break;
#endif
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, spec);
