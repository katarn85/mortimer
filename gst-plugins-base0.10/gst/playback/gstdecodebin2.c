...
#ifdef GST_EXT_DECODEBIN
  gboolean need_decrypt;        /* if data need is encrypted, drm plugin is added before decoding */
  gboolean force_push_mode;     /* to set the force-push-mode on typefind */
#endif
#ifdef GST_EXT_DECODEBIN_PRO
...
#ifdef GST_EXT_DECODEBIN
#define DEFAULT_NEED_DECRYPT      FALSE
#define DEFAULT_FORCE_PUSH_MODE   FALSE
#endif
...
#ifdef GST_EXT_DECODEBIN
  GstElement *drm_element;      /* Optional drm element for Codec/Decoder */
#endif
};
...
#ifdef GST_EXT_DECODEBIN
  /**
   * GstDecodeBin2::need-decrypt
   *
   * Add drm plugin to decrypt the data
   *
   * If set to %TRUE, then drm plugin will be added in front of decoder element to get clear stream.
   *
   */
  g_object_class_install_property (gobject_klass, PROP_NEED_DECRYPT,
      g_param_spec_boolean ("need-decrypt", "Add drm plugin to decrypt",
          "Add drm plugin to decrypt in front of decoder element",
          DEFAULT_NEED_DECRYPT,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

  g_object_class_install_property (gobject_klass, PROP_FORCE_PUSH_MODE,
      g_param_spec_boolean ("force-push-mode", "Set force-push-mode on typefind",
          "Typefind will run as push mode when typefinding.",
          DEFAULT_FORCE_PUSH_MODE,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
#endif
	...
	g_object_class_install_property (gobject_klass, PROP_USE_TRUSTZONE,
      g_param_spec_boolean ("use-trustzone", "Add trustzone flag to decodebin2",
          ...
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
#endif
  g_object_class_install_property (gobject_klass, PROP_CHANGE_TRACK,
        g_param_spec_boolean ("change-track", "when change track, not returns wrong_state to upstream",
...
#ifdef GST_EXT_DECODEBIN
  decode_bin->need_decrypt = DEFAULT_NEED_DECRYPT;
  decode_bin->force_push_mode = DEFAULT_FORCE_PUSH_MODE;
#endif
  decode_bin->change_track = FALSE;
}
...
#ifdef GST_EXT_DECODEBIN
static void
gst_decode_bin_set_force_push_mode (GstDecodeBin * dbin, gboolean force_push_mode)
{
  GST_DEBUG_OBJECT (dbin, "Set force-push-mode on typefind");

  dbin->force_push_mode = force_push_mode;
  g_object_set (dbin->typefind, "force-push-mode", force_push_mode, NULL);
}
#endif
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_EXPOSE_ALL_STREAMS:
      ...
      break;
#ifdef GST_EXT_DECODEBIN
    case PROP_NEED_DECRYPT:
      dbin->need_decrypt = g_value_get_boolean (value);
      break;
    case PROP_FORCE_PUSH_MODE:
      gst_decode_bin_set_force_push_mode (dbin, g_value_get_boolean (value));
      break;
#endif
#ifdef GST_EXT_DECODEBIN_PRO
    ...
    case PROP_CHANGE_TRACK:
      GST_DEBUG_OBJECT (dbin, "set change-track to %d", g_value_get_boolean(value));
	  dbin->change_track = g_value_get_boolean (value);
	  if(dbin->typefind)
	  	g_object_set(dbin->typefind, "change-track", dbin->change_track, NULL);
	  else
	    GST_ERROR_OBJECT (dbin, "Decodebin2 has no typefind!!!!!!!!!!!!!!");
	  break;
	default:
...
#ifdef GST_EXT_DECODEBIN
    case PROP_NEED_DECRYPT:
      g_value_set_boolean (value, dbin->need_decrypt);
      break;
    case PROP_FORCE_PUSH_MODE:
      g_value_set_boolean (value, dbin->force_push_mode);
      break;
#endif
#ifdef GST_EXT_DECODEBIN_PRO
	case PROP_USE_TRUSTZONE:
...
#endif
    case PROP_CHANGE_TRACK:
	  g_value_set_boolean (value, dbin->change_track);
	  break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
      break;
    ...
    GstDecodeChain * chain)
{
  ...
  GstPad *mqpad = NULL;
#ifdef GST_EXT_DECODEBIN
  GstPad *drm_src_pad = NULL;
  static GstElementFactory *dec_factory = NULL;
#endif
...
#ifdef GST_EXT_DECODEBIN
  while ((factories->n_values > 0) || (dec_factory)) {
    GstElementFactory *drm_factory = NULL;
#else
  while (factories->n_values > 0) {
#endif
    GstAutoplugSelectResult ret;
    GstElementFactory *factory = NULL;
    GstDecodeElement *delem = NULL;
    GstElement *element = NULL;
    GstPad *sinkpad = NULL;
    gboolean subtitle = NULL;
...
#endif
    {
      ...
      switch (ret) {
        ...
        case GST_AUTOPLUG_SELECT_SKIP:
          ...
          continue;
        case GST_AUTOPLUG_SELECT_SKIP_ALL:
          GST_DEBUG_OBJECT (dbin, "autoplug select requested skip_all");
          res = FALSE;
          goto beach;
          continue;
        default:
      ...
      factory = dec_factory;
      GST_DEBUG_OBJECT (dbin,
          "try with selected dec factory '%s' after drm",
          gst_plugin_feature_get_name (GST_PLUGIN_FEATURE_CAST (factory)));
    }

    /* FIXME : Currently, DRMBridge class is Codec/Decoder. If the class is changed, below code will be chnaged. */
    if ((dbin->need_decrypt) && (!dec_factory) &&
        (strstr (gst_element_factory_get_klass (factory), "Codec/Decoder"))) {

      GST_DEBUG_OBJECT (dbin, "Create DRM element");
      drm_factory = gst_element_factory_find ("DRMBridge");

      if (drm_factory != NULL) {
        gboolean skip = FALSE;
        GList *l;
        GST_DEBUG_OBJECT (dbin, "Check the chain before adding DRM element");
        ...
        for (l = chain->elements; l; l = l->next) {
          ...
          if ((delem->drm_element) ||
              (gst_element_get_factory (otherelement) == drm_factory)) {
            GST_DEBUG_OBJECT (dbin,
                "Skipping drm factory because it was already used in this chain");
            skip = TRUE;
            break;
          }
        ...
        if ((skip) || (strstr (GST_PLUGIN_FEATURE_NAME(factory), "DRMBridge"))) {
          GST_DEBUG_OBJECT (dbin, "adding original factory from list.");
          gst_object_unref (drm_factory);
          drm_factory = NULL;
        }
      }
    }
    ...
    if (drm_factory != NULL) {
      /* add drm element manually. */
      /* keep the decoder element, this will be added after drm element. */
      GST_DEBUG_OBJECT (dbin,
          "keep %s to add after drm element.",
          gst_plugin_feature_get_name (GST_PLUGIN_FEATURE (factory)));

      dec_factory = factory;

      if ((element = gst_element_factory_create (drm_factory, NULL)) == NULL) {
        GST_WARNING_OBJECT (dbin, "Could not create an drm element from %s",
            gst_plugin_feature_get_name (GST_PLUGIN_FEATURE (drm_factory)));
        continue;
      }
    } else {
      dec_factory = NULL;
...
#ifdef GST_EXT_DECODEBIN
    if (drm_factory) {
      GST_DEBUG_OBJECT (dbin, "link drm element without checking any condition.");
      if ((gst_pad_link_full (pad, sinkpad, GST_PAD_LINK_CHECK_NOTHING)) != GST_PAD_LINK_OK) {
    ...
    if (!delem) {
      ...
      delem->drm_element = NULL;
    }

    if (drm_factory && dec_factory) {
      delem->drm_element = gst_object_ref (element);
      CHAIN_MUTEX_UNLOCK (chain);
    } else {
...
#ifdef GST_EXT_DECODEBIN
        if (dtmp->drm_element) {
          gst_bin_remove (GST_BIN (dbin), dtmp->drm_element);
          gst_element_set_state (dtmp->drm_element, GST_STATE_NULL);
          gst_object_unref (dtmp->drm_element);
        }
#endif
...
#ifdef GST_EXT_DECODEBIN
    if (drm_factory && dec_factory) {
      drm_src_pad = gst_element_get_static_pad (element, "src");
      pad = drm_src_pad; /* FIXME : need to keep to restore for converter case */
      GST_DEBUG_OBJECT (dbin,
        "add holded elem after drm pad %s:%s", GST_DEBUG_PAD_NAME(pad));
      continue;
    }
#endif
...
#ifdef GST_EXT_DECODEBIN
    if (delem->drm_element) {
      if (GST_OBJECT_PARENT (delem->drm_element) ==
          GST_OBJECT_CAST (chain->dbin))
        gst_bin_remove (GST_BIN_CAST (chain->dbin), delem->drm_element);
      if (!hide) {
        gst_element_set_state (delem->drm_element, GST_STATE_NULL);
      }
    }
#endif
...
#ifdef GST_EXT_DECODEBIN
      if (delem->drm_element) {
        gst_object_unref (delem->drm_element);
        delem->drm_element = NULL;
      }
#endif
