...
#ifdef GST_EXT_URIDECODEBIN
  /* for id3demux */
  guint id3_np_sig_id;         /* id3demux new-pad signal id */
#endif
...
#ifdef GST_EXT_URIDECODEBIN
  /* signal fired to check hw resource from the proposed list of factories */
  GstAutoplugSelectResult (*autoplug_check_resource) (GstElement * element,
    GstPad * pad, GstCaps * caps, GstElementFactory * factory);
#endif
};
...
enum
{
  ...
  SIGNAL_SOURCE_SETUP,
#ifdef GST_EXT_URIDECODEBIN
  SIGNAL_AUTOPLUG_CHECK_RESOURCE,
#endif
  LAST_SIGNAL
};
...
#ifdef GST_EXT_URIDECODEBIN
static void id3_new_pad (GstElement * element, GstPad * pad,
    GstURIDecodeBin * bin);
#endif
...
#ifdef GST_EXT_URIDECODEBIN
static GstAutoplugSelectResult
gst_uri_decode_bin_autoplug_check_resource (GstElement * element, GstPad * pad,
    GstCaps * caps, GstElementFactory * factory)
{
  GST_DEBUG_OBJECT (element, "default autoplug-check-resource returns TRY");
...
gst_uri_decode_bin_class_init (GstURIDecodeBinClass * klass)
{
...
#ifdef GST_EXT_URIDECODEBIN
	/**
	 * GstURIDecodeBin::autoplug-check-resource:
	 * @bin: The uridecodebin.
	 * @pad: The #GstPad.
	 * @caps: The #GstCaps.
	 * @factory: A #GstElementFactory to use.
  ...
  gst_uri_decode_bin_signals[SIGNAL_AUTOPLUG_CHECK_RESOURCE] =
      g_signal_new ("autoplug-check-resource", G_TYPE_FROM_CLASS (klass),
      G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET (GstURIDecodeBinClass,
        autoplug_check_resource), _gst_select_accumulator, NULL,
      gst_play_marshal_ENUM__OBJECT_BOXED_OBJECT,
      GST_TYPE_AUTOPLUG_SELECT_RESULT, 3, GST_TYPE_PAD, GST_TYPE_CAPS,
      GST_TYPE_ELEMENT_FACTORY);
#endif
...
#ifdef GST_EXT_URIDECODEBIN
  klass->autoplug_check_resource =
      GST_DEBUG_FUNCPTR (gst_uri_decode_bin_autoplug_check_resource);
#endif
...
static const gchar *download_media[] = {
  ...
  "video/x-flv", "video/x-msvideo", "video/webm", "audio/mpeg", "audio/x-wav", NULL
};
#else
static const gchar *download_media[] = {
  "video/quicktime", "video/mj2", "audio/x-m4a", "application/x-3gp",
    ...
    GstCaps * caps, GstElementFactory * factory, GstURIDecodeBin * dec)
{
  ...
  GST_DEBUG_OBJECT(element, "factory_name [%s]\n", factory_name);

  if (g_strrstr(factory_name, "DRMBridge") && !(g_strrstr(factory_name, "drm"))) {
    GST_DEBUG_OBJECT(element, "skipping [%s] by not required\n", factory_name);
    return GST_AUTOPLUG_SELECT_SKIP;
  }

  g_signal_emit (dec,
      gst_uri_decode_bin_signals[SIGNAL_AUTOPLUG_CHECK_RESOURCE], 0, pad, caps, factory,
      &result);
    ...
    GstCaps * caps, GstURIDecodeBin * decoder)
{
...
#ifdef GST_EXT_URIDECODEBIN
  if (decoder->download) 
    decoder->is_download = TRUE;
  else
    decoder->is_download = FALSE;
#else
  ...
  GstElementFactory *id3demux_factory = NULL;
  GstElement *id3demux = NULL;
  gboolean is_icydemux = g_strrstr(caps_str, "application/x-icy") ? TRUE : FALSE;

  id3demux_factory = gst_element_factory_find("id3demux");

  GST_DEBUG_OBJECT(decoder, "download %s", (decoder->download)?"OO":"XX");
  GST_DEBUG_OBJECT(decoder, "is_download %s", (decoder->is_download)?"OO":"XX");

  if ((id3demux_factory) &&
      (!decoder->decodebins) &&
      ((decoder->download || decoder->ring_buffer_max_size) && (g_strrstr(caps_str, "application/x-id3"))))
  {
    GST_DEBUG_OBJECT(decoder, "create id3demux in front of queue2 to support pull mode streaming");

    id3demux = gst_element_factory_create(GST_ELEMENT_FACTORY(id3demux_factory), NULL);
    if (!id3demux)
    {
      GST_ERROR_OBJECT (decoder, "failed to create id3demux element\n");

      gst_object_unref (id3demux_factory);
      goto no_id3demux;
    }

    gst_object_unref (id3demux_factory);

    decoder->id3_np_sig_id =
      g_signal_connect( G_OBJECT(id3demux), "pad-added",
      G_CALLBACK (id3_new_pad), decoder);

    gst_bin_add (GST_BIN_CAST (decoder), id3demux);

    if (!gst_element_link_pads (typefind, "src", id3demux, "sink")) {
      gst_object_unref (id3demux_factory);
      goto could_not_link;
    }

    gst_element_set_state (id3demux, GST_STATE_PLAYING);
    decoder->id3demux = id3demux;
  ...
  GST_DEBUG_OBJECT (decoder, "create queue2 for streaming");
#endif
  ...
  gst_pad_query_peer_duration (decoder->source, &format, &duration);  
  
  if( !is_icydemux && duration)
    g_object_set (queue, "use-rate-estimate", FALSE, NULL);
  else
    GST_DEBUG_OBJECT (decoder, "NOT set use-rate-estimate to FALSE icydemux [%d]  duration [%d]", is_icydemux,duration);    
#endif
...
#ifdef GST_EXT_URIDECODEBIN
  if (decoder->id3demux) {
    if (!gst_element_link_pads (decoder->id3demux, "src", queue, "sink")) {
      GST_ERROR_OBJECT (decoder, "id3demux - queue link fail");
      goto could_not_link;
    }
  }
  else
  ...
  if (!gst_element_link_pads (typefind, "src", queue, "sink")) {
    GST_ERROR_OBJECT (decoder, "typefind - queue link fail");
    goto could_not_link;
  }
...
no_id3demux:
  {
    post_missing_plugin_error (GST_ELEMENT_CAST (decoder), "id3demux");
    return;
  }
#endif
no_decodebin:
...
#ifdef GST_EXT_URIDECODEBIN
/* is called when a id3demux element created a new pad. */
static void
id3_new_pad (GstElement * element, GstPad * pad, GstURIDecodeBin * bin)
{
  GstCaps *caps = NULL;
  guint probability = 0; // test

  GST_DEBUG_OBJECT (bin, "Found new pad %s.%s in id3demux element %s",
          GST_DEBUG_PAD_NAME (pad), GST_ELEMENT_NAME (element));

  caps = gst_pad_get_caps(pad);

  type_found (bin->typefind, probability,  caps, bin);
...
remove_source (GstURIDecodeBin * bin)
{
  ...
  if (bin->id3demux) {
    ...
    if (bin->id3_np_sig_id) {
      g_signal_handler_disconnect (bin->id3demux, bin->id3_np_sig_id);
      bin->id3_np_sig_id = 0;
    }

    GST_DEBUG_OBJECT (bin, "removing old id3demux element");
    gst_element_set_state (bin->id3demux, GST_STATE_NULL);
    gst_bin_remove (GST_BIN_CAST (bin), bin->id3demux);
    bin->id3demux = NULL;
  }
#endif
