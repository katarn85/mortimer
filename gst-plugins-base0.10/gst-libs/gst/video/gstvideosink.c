 ...
 *  Copyright (C) <2009> Tim-Philipp Muller <tim centricular net>
 *
...
gst_video_sink_show_preroll_frame (GstBaseSink * bsink, GstBuffer * buf)
{
  ...
  GST_INFO_OBJECT (bsink, "rendering frame, ts=%" GST_TIME_FORMAT,
      GST_TIME_ARGS (GST_BUFFER_TIMESTAMP (buf)));
...
gst_video_sink_show_frame (GstBaseSink * bsink, GstBuffer * buf)
{
  ...
  guint64 T1 = get_time();
  GstFlowReturn ret = klass->show_frame (GST_VIDEO_SINK_CAST (bsink), buf);
  guint64 T2 = get_time();
  if (T2-T1 > MIN_LATENCY)
    GST_WARNING_OBJECT(bsink,"xvimage show_frame[ %lld ms ]", (T2-T1)/1000);
  return ret;
}
