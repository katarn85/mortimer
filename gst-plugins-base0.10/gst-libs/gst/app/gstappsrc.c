...
#define DEFAULT_PROP_NOQUEUE       FALSE
...
#define FREERUN_BUFFER_THRESHOLD   100
enum
{
  PROP_0,
  PROP_CAPS,
...
gst_app_src_class_init (GstAppSrcClass * klass)
{
   ...
   * GstAppSrc::noqueue
   *
   * Instruct the source to behave like a no queue source. This includes that it
   * will not queue any data.If not acceptable drop them
   */
  g_object_class_install_property (gobject_class, PROP_NOQUEUE,
      g_param_spec_boolean ("noqueue", "No Queue",
          "Whether to queu or not",
          DEFAULT_PROP_NOQUEUE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));  
  /**
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_IS_LIVE:
      ...
      break;
    case PROP_NOQUEUE:
	priv->a_noq = g_value_get_boolean (value);
	GST_ERROR_OBJECT (appsrc, "SETTING NO QUEUE OPTION %d", priv->a_noq);
      break;	  
    ...
    GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_IS_LIVE:
      ...
      break;
    case PROP_NOQUEUE:
      g_value_set_boolean (value, priv->a_noq);
      break;	  
    case PROP_MIN_LATENCY:
    {
      guint64 min;
    ...
    gboolean steal_ref)
{
  ...
  GST_DEBUG_OBJECT (appsrc, "queueing buffer %p, timestamp =%"GST_TIME_FORMAT,
    buffer, GST_TIME_ARGS( GST_BUFFER_TIMESTAMP(buffer)));
	
  if (!steal_ref)
    gst_buffer_ref (buffer);
  
  if (	(!priv->a_noq)  || (priv->queued_bytes < FREERUN_BUFFER_THRESHOLD)) //This is needed only for audio appsouce
  {
  ...
  {
    GST_ERROR_OBJECT (appsrc, "[AUDIO NO BUFFER CASE...]SKIP THE BUFFER");
