...
#ifdef GST_EXT_CODE_ENHANCEMENT
      if (buf) {
        GST_BUFFER_TIMESTAMP (buf) = priv->out_ts;
        GST_BUFFER_DURATION (buf) = priv->out_dur;
      } else {
        GST_ERROR_OBJECT(dec, "NULL buf");
      }
#else /* GST_EXT_CODE_ENHANCEMENT */
      GST_BUFFER_TIMESTAMP (buf) = priv->out_ts;
      GST_BUFFER_DURATION (buf) = priv->out_dur;
#endif /* GST_EXT_CODE_ENHANCEMENT */
  ...
  while (ret == GST_FLOW_OK) {
    ...
    if (G_LIKELY (av)) {
      ...
      if (buffer) {
        ...
        GST_ERROR_OBJECT(dec, "NULL buffer");
      }
#else /* GST_EXT_CODE_ENHANCEMENT */
      buffer = gst_buffer_make_metadata_writable (buffer);
      GST_BUFFER_TIMESTAMP (buffer) = ts;
