    ...
    case GST_QUERY_LATENCY:
    {
      ...
      if ((res =
        ...
        if (live && us_live) {
          ...
          GST_DEBUG_OBJECT (basesink,
              "peer or we are not live, don't care about latency, live[ %d ], us_live[ %d ], min_l[ %lld ], max_l[ %lld ]", live, us_live, min_l, max_l);
          min_latency = min_l;
          max_latency = max_l;
        }
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_PROVIDE_CLOCK:
      GST_INFO_OBJECT(sink, "[ old : %d => new : %d ]", sink->provide_clock, g_value_get_boolean (value));
    ...
    GstClockTime sample_offset)
{
  ...
  if (G_LIKELY (!discont && allow_align)) {
    GST_DEBUG_OBJECT (sink,
        ...
        G_GINT64_FORMAT ", sample_diff[ %lld ] next_sample[ %lld ] sample_offset[ %lld ], discont[ %d ], allow_align[ %d ]", align, max_sample_diff, sample_diff, sink->next_sample, sample_offset, discont, allow_align);
  } else {
...
gst_base_audio_sink_render (GstBaseSink * bsink, GstBuffer * buf)
{
  ...
  GST_DEBUG_OBJECT (sink,
      "sync-offset[ %" GST_TIME_FORMAT " ] = ts_offset[ %" GST_TIME_FORMAT
      "] - render-delay[ %" GST_TIME_FORMAT "] + latency[ %" GST_TIME_FORMAT" ]",
      GST_TIME_ARGS(sync_offset), GST_TIME_ARGS(ts_offset),
      GST_TIME_ARGS (render_delay), GST_TIME_ARGS(latency));
...
#ifdef AV_SYNC_DEBUG
 {
	GstClockTime ptr = GST_BUFFER_TIMESTAMP(buf);
	GstClockTime deta;
	memcpy(data,(guint8*)(&ptr),8);
	deta = *((GstClockTime*)data);
	GST_DEBUG_OBJECT (sink, "baseaudiosink written segment timestamp is: %" GST_TIME_FORMAT
      ", end: %" GST_TIME_FORMAT, GST_TIME_ARGS (ptr), GST_TIME_ARGS (deta));
  }
#endif
  ringbuf->timestamp = ctime;  // For Debugging (clipped time)
  do {
    written =
...
too_late:
  {
    GST_WARNING_OBJECT (sink, "dropping late sample");
    ret = GST_FLOW_OK;
    goto done;
  }
  /* ERRORS */
