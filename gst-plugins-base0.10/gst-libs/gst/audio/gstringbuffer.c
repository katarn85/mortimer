...
gst_ring_buffer_init (GstRingBuffer * ringbuffer)
{
  ...
  ringbuffer->timestamp_per_segment = NULL;
  ringbuffer->num_of_timestamp = 0;
  ringbuffer->debug_segwrite = 0;
}
...
gst_ring_buffer_finalize (GObject * object)
{
  ...
  if (ringbuffer->timestamp_per_segment) {
    g_free(ringbuffer->timestamp_per_segment);
  }
...
gst_ring_buffer_acquire (GstRingBuffer * buf, GstRingBufferSpec * spec)
{
  ...
  if (buf->timestamp_per_segment == NULL && buf->spec.segtotal > 0)
  {
    buf->timestamp_per_segment = g_malloc0_n(buf->spec.segtotal, sizeof(GstClockTime));
    buf->num_of_timestamp = buf->spec.segtotal;
  }
  else
  {
    GST_WARNING_OBJECT(buf, "segtotal[ %d ], buf->num_of_timestamp[ %d ]", buf->spec.segtotal, buf->num_of_timestamp);
  }
done:
  GST_OBJECT_UNLOCK (buf);
    ...
    guchar * data, gint in_samples, gint out_samples, gint * accum)
{
  ...
  gboolean reverse;
#ifdef AV_SYNC_DEBUG
  guint8 *first_data = data;
#endif
  GstClockTime one_seg_duration  = gst_util_uint64_scale_int((buf->spec.segsize/buf->spec.bytes_per_sample), GST_SECOND, buf->spec.rate);
  ...
  while (*toprocess > 0) {
    ...
    while (TRUE) {
      ...
      GST_INFO_OBJECT(buf, 
          "ts=%"GST_TIME_FORMAT", pointer at %d, write to %d-%d, diff %d, segtotal %d, segsize %d, base %d",
          GST_TIME_ARGS(buf->timestamp), segdone, writeseg, sampleoff, diff, segtotal, segsize, buf->segbase);
    ...
    buf->debug_segwrite = writeseg; // for debugging
    ...
    *sample += avail / bps;
    ...
    if(!skip) {
	  *d = 0x47;
	  memcpy(d+1, first_data, 8);
	  data_start = *((GstClockTime*)(d+1));
	  GST_DEBUG_OBJECT (buf, "ringbuffer written segment timestamp is: %" GST_TIME_FORMAT
      ", end: %" GST_TIME_FORMAT, GST_TIME_ARGS (data_start), GST_TIME_ARGS (data_start));
    }
#endif
      if (buf->timestamp_per_segment && buf->num_of_timestamp > ws && buf->timestamp) // For debugging
      {
        buf->timestamp_per_segment[ws] = buf->timestamp;
        buf->timestamp += one_seg_duration; // for next segment
      }
    } else if (!reverse) {
      if (inr >= outr)
        /* forward speed up */
