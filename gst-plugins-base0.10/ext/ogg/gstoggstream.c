...
is_granulepos_keyframe_theora (GstOggStream * pad, gint64 granulepos)
{
  ...
  frame_mask = ((gint64)1 << pad->granuleshift) - 1;
...
packet_duration_flac (GstOggStream * pad, ogg_packet * packet)
{
  ...
  if (block_size_index >= 8) {
    return (gint64)(256) << (block_size_index - 8);
  }
...
/*granuleshift is unsigned int. but actual value can be only less than 8 bit. so anything
  beyond 8 bit can be considered as garbage. update only in that condition. */
  if (pad->granuleshift > 0xFF) { 
