...
static const char *get_percent(int val, int min, int max)
{
	...
	snprintf(str, sizeof(str), "%i [%i%%]", val, p);
	return str;
}
...
{
	char *argv[] = {"amixer", "-c", "0", "cset",  "iface=MIXER,name='main out'", "MM"};
	amixer(sizeof(argv)/sizeof(char*), argv);
}
#endif
		...
		case 'c':
			{
				...
				if (i >= 0 && i < 32)
					snprintf(card, sizeof(card), "hw:%i", i);
				else {
