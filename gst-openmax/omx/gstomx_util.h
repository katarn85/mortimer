...
typedef void (*GOmxDataCb) (GOmxCore * core, gconstpointer data);
typedef void (*GOmxDataCbEx) (GOmxCore * core, OMX_EVENTTYPE event, OMX_U32 data_1, OMX_U32 data_2, OMX_PTR event_data);
...
struct GOmxCore
{
  ...
  GOmxDataCb userdata_cb;
  GOmxDataCbEx userdata_etc_cb;
  GOmxDataCb colour_description_cb;
  GOmxImp *imp;
  ...
  gint media_type;	/// 0: not setting 1: video, 2: audio
  gboolean is_eos;
};
...
struct GOmxPort
{
  ...
  gboolean got_eos; // now only for inport
  gboolean flush_started; // now only for inport
  guint num_of_frames;
};
...
#define GSTOMX_SIMPLE_BOILERPLATE(type, type_as_function, parent_type, parent_type_macro) \
GType type_as_function ## _get_type (void)                                    \
{                                                                             \
    /* The typedef for GType may be gulong or gsize, depending on the         \
     * system and whether the compiler is c++ or not. The g_once_init_*       \
