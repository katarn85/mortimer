...
request_imp (const gchar * name)
{
  ...
  if (imp->client_count == 0) {
    ...
    GST_DEBUG ("omx_init called  omx_error[ %d ]", omx_error);
    if (omx_error) {
...
g_omx_core_init (GOmxCore * core)
{
  ...
  if (!core->omx_handle) {
    GST_ERROR_OBJECT (core->object, "Fail to OMX_GetHandle(&%p) -> %d, core->component_name[ %s ]",
        core->omx_handle, core->omx_error, (char*)core->component_name);
  }
  else {
    GST_DEBUG_OBJECT (core->object, "OMX_GetHandle(&%p) -> %d, core->component_name[ %s ]",
        core->omx_handle, core->omx_error, (char*)core->component_name);
  }
  ...
  if (!core->omx_error) {
    ...
    if (0)//(strncmp(core->component_name+4, "SEC", 3) == 0)   // temporary remove
    {
...
g_omx_port_release_buffer (GOmxPort * port, OMX_BUFFERHEADERTYPE * omx_buffer)
{
  switch (port->type) {
    case GOMX_PORT_INPUT:
      GST_CAT_DEBUG_OBJECT (gstomx_util_debug, port->core->object, "EmptyThisBuffer[ %p ], size[ %d ], pts[ %lld ] buf[ %p ], AllocLen[ %d ] FilledLen[ %d ], flags[ %d ]",
        omx_buffer, omx_buffer->nSize, omx_buffer->nTimeStamp, omx_buffer->pBuffer, omx_buffer->nAllocLen, omx_buffer->nFilledLen, omx_buffer->nFlags);
      OMX_EmptyThisBuffer (port->core->omx_handle, omx_buffer);
      break;
    case GOMX_PORT_OUTPUT:
      GST_CAT_DEBUG_OBJECT (gstomx_util_debug, port->core->object, "FillThisBuffer[ %p ], size[ %d ], buf[ %p ], AllocLen[ %d ] FilledLen[ %d ], flags[ %d ]",
        omx_buffer, omx_buffer->nSize, omx_buffer->pBuffer, omx_buffer->nAllocLen, omx_buffer->nFilledLen, omx_buffer->nFlags);
...
wait_for_state (GOmxCore * core, OMX_STATETYPE state)
{
  ...
  /* for unsupport case, we need to wait for OMX_CommandStateSet to change omx state*/
  if ( (core->omx_error != OMX_ErrorNone) && (core->omx_error !=OMX_ErrorUnsupportedFormat) &&
  	(core->omx_error !=OMX_ErrorUnsupportedResolution) &&
  	(core->omx_error !=OMX_ErrorUnsupportedFramerate) &&
	(core->omx_error !=OMX_ErrorUnsupportedProfile) &&
	(core->omx_error !=OMX_ErrorUnsupportedDualDecoding))
    goto leave;
    ...
    OMX_EVENTTYPE event, OMX_U32 data_1, OMX_U32 data_2, OMX_PTR event_data)
{
  ...
  switch (event) {
    ...
    case OMX_EventError:
    {
      gboolean bNotSupport = FALSE;
	  switch(data_1)
	  {
		case OMX_ErrorUnsupportedFormat:
		case OMX_ErrorUnsupportedResolution:
		case OMX_ErrorUnsupportedFramerate:
		case OMX_ErrorUnsupportedProfile:
		case OMX_ErrorUnsupportedDualDecoding:
	        bNotSupport = TRUE;
	        break;
		case OMX_ErrorHardware:
		{
			bNotSupport = (++(core->hw_error_count) > 70) ? TRUE : FALSE;
	        break;
		}
		default:
	        break;
	  }
      GST_ERROR_OBJECT (core->object, "unrecoverable error: %s (0x%lx),  [ %s ]",
          omx_error_to_str (data_1), data_1, bNotSupport?"cannot decode anymore":"But, ignore this error by driver's request");
...
#if 0 // NOT USE ANYMORE  (replaced to OMX_EventPortSettingsChanged)
    case OMX_EventSdpVideoInfo: // If we can, we should replace this event to OMX_EventPortSettingsChanged.
    {
      OMX_VIDEO_PORTDEFINITIONTYPE* tmp = (OMX_VIDEO_PORTDEFINITIONTYPE*)event_data;
      GST_DEBUG_OBJECT (core->object, "OMX_EventSdpVideoInfo, data_1[ %d ], data_2[ %d ], event_data[ %p ]", data_1, data_2, event_data);
      GST_DEBUG_OBJECT (core->object, "w[ %d ] h[ %d ] fps[ %d ]", tmp->nFrameWidth, tmp->nFrameHeight, tmp->xFramerate);
    ...
    {
      GST_DEBUG_OBJECT (core->object, "OMX_EventSdpVbiData, data_1[ %d ], data_2[ %d ], event_data[ %p ]", data_1, data_2, event_data);
      if (core->userdata_cb) {
        core->userdata_cb(core, event_data);
      }
      break;
    }
		...
		{
			GST_DEBUG_OBJECT (core->object, "event[ 0x%x ], data_1[ %d ], data_2[ %d ], event_data[ %p ]", event, data_1, data_2, event_data);
			if (core->userdata_etc_cb) {
				core->userdata_etc_cb(core, event, data_1, data_2, event_data);
			}
			break;
		}
	case OMX_EventSdpColourDescription:
	{
		GST_DEBUG_OBJECT (core->object, "OMX_EventSdpColourDescription, data_1[ %d ], data_2[ %d ], event_data[ %p ]", data_1, data_2, event_data);
		if (core->colour_description_cb) {
			core->colour_description_cb(core, event_data);
		}
		break;
	}
    default:
    ...
    OMX_PTR app_data, OMX_BUFFERHEADERTYPE * omx_buffer)
{
  ...
  GST_CAT_DEBUG_OBJECT (gstomx_util_debug, core->object, "omx_buffer=%p",
      omx_buffer);
    ...
    OMX_PTR app_data, OMX_BUFFERHEADERTYPE * omx_buffer)
{
  GOmxCore *core;
  GOmxPort *port;
  if (!app_data || !omx_buffer)
  {
    GST_CAT_ERROR(gstomx_util_debug, "app_data[ %p ] omx_buffer[ %p ]", app_data, omx_buffer);
  ...
  GST_CAT_DEBUG_OBJECT (gstomx_util_debug, core->object, "omxbuf[ %p ] S[ %d ], V[ %d ], B[ %p ] LEN[ %d  %d ] offset[ %d ] - %d pts[ %lld ] %d %d %d",
    omx_buffer, omx_buffer->nSize, omx_buffer->nVersion, omx_buffer->pBuffer, omx_buffer->nAllocLen, omx_buffer->nFilledLen, omx_buffer->nOffset,
    omx_buffer->nTickCount, omx_buffer->nTimeStamp, omx_buffer->nFlags, omx_buffer->nOutputPortIndex, omx_buffer->nInputPortIndex);
