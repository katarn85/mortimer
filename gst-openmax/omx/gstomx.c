...
#include "gstomx_avsdec.h"
#include "gstomx_vp8dec.h"
#include "gstomx_rvdec.h"
#include "gstomx_mjpegdec.h"
#include "gstomx_ac3dec.h"
#include "gstomx_adpcmdec.h"
#include "gstomx_wmadec.h"
#include "gstomx_dtsdec.h"
#include "gstomx_radec.h"
...
static const GstStructure *tz_element_table;
static GQuark element_name_quark;
static GQuark tz_element_name_quark;
...
static GType (*get_type[]) (void) = {
  gst_omx_dummy_get_type,
      ...
      gst_omx_mpeg4dec1_get_type,
      gst_omx_h264dec_get_type,
      gst_omx_h265dec_get_type,
      ...
      gst_omx_vp8dec_get_type,
      gst_omx_avsdec_get_type,
      gst_omx_rvdec_get_type,
      gst_omx_mjpegdec_get_type,
      gst_omx_ac3dec_get_type,
      gst_omx_adpcmdec_get_type,
      gst_omx_wmadec_get_type,
      gst_omx_dtsdec_get_type,
      gst_omx_radec_get_type,
      gst_omx_g711dec_get_type,
      gst_omx_videodec_get_type,
      gst_omx_videodec1_get_type,
      gst_omx_videodec2_get_type,
      gst_omx_videodec3_get_type,
      gst_omx_videodec4_get_type,
      gst_omx_uhd_videodec_get_type,
      gst_omx_uhd_mjpegdec_get_type,
#ifdef EXPERIMENTAL
...
static GstStructure *
get_tz_element_entry(const gchar * element_name)
{
  GstStructure *element;
  if(tz_element_table)
  {
    if(!gst_structure_get ((GstStructure *) tz_element_table, element_name,GST_TYPE_STRUCTURE, &element, NULL)) 
    {
      element = NULL;
    }
  }else{
...
get_element_entry (const gchar * element_name)
{
  ...
  if (!element_table || !element_name || !gst_structure_get ((GstStructure *) element_table, element_name,
          GST_TYPE_STRUCTURE, &element, NULL)) {
    element = get_tz_element_entry(element_name);
  }
...
gstomx_get_component_info (void *core, GType type)
{
  ...
  if(!element_name)
    element_name = g_type_get_qdata(type, tz_element_name_quark);
...
void *
gstomx_set_tz_element(GstStructure* element_table,GQuark* name_quark)
{
  if(element_table != NULL)
    tz_element_table = element_table;
  if(name_quark!= NULL)
    tz_element_name_quark = *name_quark;
