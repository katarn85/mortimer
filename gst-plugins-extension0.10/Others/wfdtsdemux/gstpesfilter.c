...
#include <drmspecificinfo/drm_specific_info.h>


static const char HDCP2X_COUNTER_VALUES_LENGTH = 16;
GST_DEBUG_CATEGORY (gstwfdpesfilter_debug);
#define GST_CAT_DEFAULT (gstwfdpesfilter_debug)
...
gst_pes_filter_parse (GstPESFilter * filter)
{
  ...
  const guint8 *data;
  unsigned char *hdcp_counters = NULL;
  gint avail, datalen;
  gboolean have_size = FALSE;
  gboolean LPCM_flag = FALSE;
  ...
  /* PTS and DTS, never for mpeg2 */
    ...
    if ((flags & 0x01)) {
      ...
      if ((flags & 0x80)) {
        hdcp_counters = g_malloc(16);
        memcpy(hdcp_counters, data, HDCP2X_COUNTER_VALUES_LENGTH);
        data += 16;
    ...
    if (*(data) == 0xa0 && *(data+1) == 0x06) {
          LPCM_flag = TRUE;
          filter->lpcm_data.endianness = G_BIG_ENDIAN;
          filter->lpcm_data.sub_stream_ID = *data;
          filter->lpcm_data.emphasis = *(data+2);

          /* data+3 => bit-rate (2) samp-freq (3) num-of-channels (3)*/
          filter->lpcm_data.width = ((*(data+3) & 0xC0) >> 4) | 0x10;
          filter->lpcm_data.channels = (*(data+3) & 0x07)+1; //num-of-channels starts from one
          switch ( (*(data+3) & 0x38)>>3 )
            {
            case 1: filter->lpcm_data.rate = 44100;
                break;
            case 2: filter->lpcm_data.rate = 48000;
                break;
            case 4: filter->lpcm_data.rate = 96000;
                break;
            default: filter->lpcm_data.rate = 48000;
                break;
            }
...
push_out:
  {
    GstBuffer *out;
    HDCP_BOX_Info info = {0};
    GBytes *G_data = NULL;
    guint16 consumed;

    if(hdcp_counters != NULL)
    {
        info.is_secure = 1;
        info.iv_len = HDCP2X_COUNTER_VALUES_LENGTH;
        info.iv_aes128 = hdcp_counters;
    }else
    {
        info.is_secure = 0;
        info.iv_len = 0;
    ...
    G_data = drm_hdcp_box_set(&info);

    GstStructure *drm_info_struct = gst_structure_new("drm_info","drm_specific_info", G_TYPE_BYTES, G_data, NULL);
    ...
    if (datalen > 0) {
      ...
      if(drm_info_struct)
      {
          GST_DEBUG("attaching GBytes structure to output buffer");
          gst_buffer_set_qdata(out, g_quark_from_static_string("drm_info"), drm_info_struct);
      }
      if(G_data)
      {
          GST_DEBUG("unrefing GBytes in ffmpegdemux");
