...
GST_BOILERPLATE (WFDTsPmtStreamInfo, wfd_ts_pmt_stream_info, GObject,
    G_TYPE_OBJECT);

static void wfd_ts_pmt_stream_info_set_property (GObject * object,
    guint prop_id, const GValue * value, GParamSpec * spec);
static void wfd_ts_pmt_stream_info_get_property (GObject * object,
    guint prop_id, GValue * value, GParamSpec * spec);
static void wfd_ts_pmt_stream_info_finalize (GObject * object);

static void
wfd_ts_pmt_stream_info_base_init (gpointer klass)
{
}

static void
wfd_ts_pmt_stream_info_class_init (WFDTsPmtStreamInfoClass * klass)
{
  GObjectClass *gobject_klass = (GObjectClass *) klass;

  gobject_klass->set_property = wfd_ts_pmt_stream_info_set_property;
  gobject_klass->get_property = wfd_ts_pmt_stream_info_get_property;
  gobject_klass->finalize = wfd_ts_pmt_stream_info_finalize;
...
static void
wfd_ts_pmt_stream_info_init (WFDTsPmtStreamInfo * pmt_stream_info,
    WFDTsPmtStreamInfoClass * klass)
{
...
static void
wfd_ts_pmt_stream_info_finalize (GObject * object)
{
  WFDTsPmtStreamInfo *info = WFD_TS_PMT_STREAM_INFO (object);
...
WFDTsPmtStreamInfo *
wfd_ts_pmt_stream_info_new (guint16 pid, guint8 type)
{
  WFDTsPmtStreamInfo *info;
  info = g_object_new (WFD_TS_TYPE_PMT_STREAM_INFO, NULL);
...
void
wfd_ts_pmt_stream_info_add_language (WFDTsPmtStreamInfo * pmt_info,
    gchar * language)
{
  GValue v = { 0, };

  g_return_if_fail (WFD_TS_IS_PMT_STREAM_INFO (pmt_info));
...
void
wfd_ts_pmt_stream_info_add_descriptor (WFDTsPmtStreamInfo * pmt_info,
...
static void
wfd_ts_pmt_stream_info_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * spec)
{
  g_return_if_fail (WFD_TS_IS_PMT_STREAM_INFO (object));
...
static void
wfd_ts_pmt_stream_info_get_property (GObject * object, guint prop_id,
    GValue * value, GParamSpec * spec)
{
  WFDTsPmtStreamInfo *si;

  g_return_if_fail (WFD_TS_IS_PMT_STREAM_INFO (object));

  si = WFD_TS_PMT_STREAM_INFO (object);
