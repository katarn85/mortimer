...
#include "gstwfdtsdemux.h"
#include "wfdtspatinfo.h"
#include "wfdtspmtinfo.h"

GST_DEBUG_CATEGORY_STATIC (gstwfdtsdemux_debug);
#define GST_CAT_DEFAULT (gstwfdtsdemux_debug)
...
/* Adapter logic to maintain AV sync for the clips having continuous data of same stream.
     If submit count of same stream is more than MAX just store them in stream adapter.
     If data of a other stream is available then reset the submit count to 0 and push all the buffer stored in stream adapter */

#ifdef ADAPTER_LOGIC
#ifdef WFD_SPEC
#define MAX_CONTINUOUS_SUBMIT_COUNT 11
#else
#define MAX_CONTINUOUS_SUBMIT_COUNT 8
#endif
#endif

/* latency in mseconds */
#define TS_LATENCY 800
...
static void gst_wfd_ts_demux_base_init (GstWFDTSDemuxClass * klass);
static void gst_wfd_ts_demux_class_init (GstWFDTSDemuxClass * klass);
static void gst_wfd_ts_demux_init (GstWFDTSDemux * demux);
static void gst_wfd_ts_demux_finalize (GstWFDTSDemux * demux);
static void gst_wfd_ts_demux_reset (GstWFDTSDemux * demux);

//static void gst_wfd_ts_demux_remove_pads (GstWFDTSDemux * demux);
...
static gboolean gst_wfd_ts_demux_is_PMT (GstWFDTSDemux * demux, guint16 PID);

static gboolean gst_wfd_ts_demux_sink_event (GstPad * pad, GstEvent * event);
static gboolean gst_wfd_ts_demux_src_event (GstPad * pad, GstEvent * event);
static GstFlowReturn gst_wfd_ts_demux_chain (GstPad * pad, GstBuffer * buffer);
static gboolean gst_wfd_ts_demux_sink_setcaps (GstPad * pad, GstCaps * caps);

static gboolean gst_wfd_ts_demux_is_live (GstWFDTSDemux * demux);
static GstClock *gst_wfd_ts_demux_provide_clock (GstElement * element);
static gboolean gst_wfd_ts_demux_src_pad_query (GstPad * pad, GstQuery * query);
static const GstQueryType *gst_wfd_ts_demux_src_pad_query_type (GstPad * pad);

static GstStateChangeReturn gst_wfd_ts_demux_change_state (GstElement * element,
    GstStateChange transition);

static WFDTsPmtInfo *wfd_ts_demux_build_pmt_info (GstWFDTSDemux * demux,
    guint16 pmt_pid);
static gboolean gst_wfd_ts_demux_send_event (GstWFDTSDemux * demux, GstEvent * event);
static void gst_wfd_ts_demux_flush (GstWFDTSDemux * demux, gboolean discard);

static GstElementClass *parent_class = NULL;

/*static guint gst_wfd_ts_demux_signals[LAST_SIGNAL] = { 0 };*/

GType
gst_wfd_ts_demux_get_type (void)
{
  static GType wfd_ts_demux_type = 0;

  if (G_UNLIKELY (!wfd_ts_demux_type)) {
    static const GTypeInfo wfd_ts_demux_info = {
      sizeof (GstWFDTSDemuxClass),
      (GBaseInitFunc) gst_wfd_ts_demux_base_init,
      NULL,
      (GClassInitFunc) gst_wfd_ts_demux_class_init,
      NULL,
      NULL,
      sizeof (GstWFDTSDemux),
      0,
      (GInstanceInitFunc) gst_wfd_ts_demux_init,
    };

    wfd_ts_demux_type =
        g_type_register_static (GST_TYPE_ELEMENT, "GstWFDTSDemux",
        &wfd_ts_demux_info, 0);

    GST_DEBUG_CATEGORY_INIT (gstwfdtsdemux_debug, "wfdtsdemux", 0,
        "Wi-fi Display transport stream demultiplexer element");
  }

  return wfd_ts_demux_type;
}

static void
gst_wfd_ts_demux_base_init (GstWFDTSDemuxClass * klass)
{
  ...
  gst_element_class_set_details_simple (element_class,
      "The Wi-Fi Display Transport stream demuxer", "Codec/Demuxer",
...
gst_wfd_ts_demux_class_init (GstWFDTSDemuxClass * klass)
{
  ...
  gobject_class->finalize = (GObjectFinalizeFunc) gst_wfd_ts_demux_finalize;
  ...
  g_object_class_install_property (gobject_class, PROP_PAT_INFO,
      g_param_spec_value_array ("pat-info",
          ...
          g_param_spec_object ("flu-pat-streaminfo", "FluPATStreamInfo",
              ...
              WFD_TS_TYPE_PAT_INFO, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS),
  ...
  g_object_class_install_property (gobject_class, PROP_PMT_INFO,
      g_param_spec_object ("pmt-info",
          ...
          WFD_TS_TYPE_PMT_INFO, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));

#ifdef WFD_SPEC
  g_object_class_install_property (G_OBJECT_CLASS (klass), PROP_CURRENT_PTS,
      g_param_spec_uint ("current-PTS",
          "current Pts",
          "to capture current PTS",
          0, G_MAXUINT32, 0, G_PARAM_READWRITE));
#endif

  gstelement_class->change_state = gst_wfd_ts_demux_change_state;
  gstelement_class->provide_clock = gst_wfd_ts_demux_provide_clock;
}

static void
gst_wfd_ts_demux_init (GstWFDTSDemux * demux)
{
  GstWFDTSDemuxClass *klass = GST_WFD_TS_DEMUX_GET_CLASS (demux);

  demux->streams =
      g_malloc0 (sizeof (GstWFDTSStream *) * (MPEGTS_MAX_PID + 1));
  demux->sinkpad = gst_pad_new_from_template (klass->sink_template, "sink");
  gst_pad_set_chain_function (demux->sinkpad, gst_wfd_ts_demux_chain);
  gst_pad_set_event_function (demux->sinkpad, gst_wfd_ts_demux_sink_event);
  gst_pad_set_setcaps_function (demux->sinkpad, gst_wfd_ts_demux_sink_setcaps);
...
#ifdef WFD_SPEC
  demux->current_ts = GST_CLOCK_TIME_NONE;
#endif
  demux->submit_count = 0;
  demux->stream_count = 0;
#ifdef USE_LAST_PCR
  demux->decide_last_PCR = FALSE;
#endif


#ifdef WFD_SPEC
  demux->need_new_segment = TRUE;
#endif
}

static void
gst_wfd_ts_demux_finalize (GstWFDTSDemux * demux)
{
  gst_wfd_ts_demux_reset (demux);
...
static void
gst_wfd_ts_demux_reset (GstWFDTSDemux * demux)
{
  /* Clean up the streams and pads we allocated */
  gint i;

  for (i = 0; i < MPEGTS_MAX_PID + 1; i++) {
    GstWFDTSStream *stream = demux->streams[i];
...
static void
gst_wfd_ts_demux_no_more_pads (GstElement * demux)
{
...
#if 0
static void
gst_wfd_ts_demux_remove_pads (GstWFDTSDemux * demux)
{
...
static gboolean
gst_wfd_ts_demux_sink_setcaps (GstPad * pad, GstCaps * caps)
{
  GstWFDTSDemux *demux = GST_WFD_TS_DEMUX (gst_pad_get_parent (pad));
...
gst_wfd_ts_demux_calc_crc32 (guint8 * data, guint datalen)
{
  gint i;
...
gst_mpegts_is_dirac_stream (GstWFDTSStream * stream)
{
...
static FORCE_INLINE gboolean
gst_mpegts_stream_is_video (GstWFDTSStream * stream)
{
...
static FORCE_INLINE gboolean
gst_mpegts_stream_is_audio (GstWFDTSStream * stream)
{
  switch (stream->stream_type) {
    case ST_AUDIO_MPEG1:
    case ST_AUDIO_MPEG2:
...
static gboolean
gst_wfd_ts_demux_is_reserved_PID (GstWFDTSDemux * demux, guint16 PID)
{
 ...
 */
static FORCE_INLINE GstWFDTSStream *
gst_wfd_ts_demux_get_stream_for_PID (GstWFDTSDemux * demux, guint16 PID)
{
  GstWFDTSStream *stream = NULL;

  stream = demux->streams[PID];

  if (G_UNLIKELY (stream == NULL)) {
    stream = g_new0 (GstWFDTSStream, 1);
    ...
    else
    {
        ...
        if (!stream->adapter)
        {
            GST_ERROR("Fail to create apdater memory\n");
            return NULL;
        }
    }
    ...
    switch (PID) {
      ...
      default:
        /* mark reserved PIDs */
        if (gst_wfd_ts_demux_is_reserved_PID (demux, PID)) {
          stream->PID_type = PID_TYPE_RESERVED;
        } else {
          /* check if PMT found in PAT */
          if (gst_wfd_ts_demux_is_PMT (demux, PID)) {
...
static gboolean
gst_wfd_ts_demux_fill_stream (GstWFDTSStream * stream, guint8 id,
    guint8 stream_type, gchar **codec_name)
#else
static gboolean
gst_wfd_ts_demux_fill_stream (GstWFDTSStream * stream, guint8 id,
    guint8 stream_type)
#endif
{
  ...
  demux = stream->demux;
  klass = GST_WFD_TS_DEMUX_GET_CLASS (demux);
  ...
  switch (stream_type) {
    ...
    case ST_AUDIO_AAC_ADTS:
      template = klass->audio_template;
      name = g_strdup_printf ("audio_%04x", stream->PID);
      /* Currently, only not encrypted audio is supported. */
      caps = gst_caps_new_simple ("audio/mpeg",
    ...
    case ST_VIDEO_H264:
      ...
      caps = gst_caps_new_simple ("video/x-h264",
        ...
        "alignment", G_TYPE_STRING, "nal","stream-type",G_TYPE_STRING,"video/x-h264", NULL);
#ifdef   ADD_CODEC_TAG
      _codec ("H.264 / AVC");
#endif
      break;
    ...
    {
      GstWFDTSStream *PMT_stream =
          gst_wfd_ts_demux_get_stream_for_PID (stream->demux, stream->PMT_pid);
    ...
    case ST_BD_AUDIO_LPCM:
      ...
      break;
#ifdef WFD_SPEC
    case 0x83:                                        //Miracast LPCM
      template = klass->audio_template;
      name = g_strdup_printf ("audio_%04x", stream->PID);
      if(stream->filter.lpcm_data.sub_stream_ID == 0xA0)       // Sub Stream ID for LPCM
      {
          /* Currently, only not encrypted audio is supported. */
          caps = gst_caps_new_simple ("audio/x-raw-int", 
                     "endianness", G_TYPE_INT, stream->filter.lpcm_data.endianness,
                     "signed", G_TYPE_BOOLEAN, TRUE,
                     "depth", G_TYPE_INT, stream->filter.lpcm_data.width,
                     "width", G_TYPE_INT, stream->filter.lpcm_data.width,
                     "rate", G_TYPE_INT, stream->filter.lpcm_data.rate, 
                     "channels", G_TYPE_INT, stream->filter.lpcm_data.channels, NULL);
      } else
          caps = gst_caps_new_simple ("audio/x-lpcm", NULL);
      break;
#endif
  ...
  gst_pad_set_query_function (stream->pad,
      GST_DEBUG_FUNCPTR (gst_wfd_ts_demux_src_pad_query));
  gst_pad_set_query_type_function (stream->pad,
      GST_DEBUG_FUNCPTR (gst_wfd_ts_demux_src_pad_query_type));
  gst_pad_set_event_function (stream->pad,
      GST_DEBUG_FUNCPTR (gst_wfd_ts_demux_src_event));
  g_free (name);
...
static FORCE_INLINE gboolean
mpegts_is_elem_pid (GstWFDTSDemux * demux, guint16 PID)
{
  int i;
...
static gboolean
gst_wfd_ts_demux_setup_base_pts (GstWFDTSDemux * demux, gint64 pts)
{
  GstWFDTSStream *PCR_stream;
  GstWFDTSStream *PMT_stream;
  guint64 base_PCR;
...
//  base_PCR = PCR_stream->base_PCR;
  ...
  if (demux->base_pts == GST_CLOCK_TIME_NONE) {
	...
	GST_DEBUG_OBJECT (demux, "set base PTS %" G_GUINT64_FORMAT, pts);
  }
...
static gboolean
gst_wfd_ts_demux_send_new_segment (GstWFDTSDemux * demux,
    GstWFDTSStream * stream, gint64 pts)
{
  GstClockTime time;

  /* base_pts needs to have been set up by a call to
   * gst_wfd_ts_demux_setup_base_pts() before calling this function */
  if (demux->base_pts == GST_CLOCK_TIME_NONE)
    goto no_base_time;
...
#ifdef ADD_CODEC_TAG
static void
gst_wfd_ts_demux_send_tags_for_stream (GstWFDTSDemux * demux,
    GstWFDTSStream * stream, gchar** codec_name)
#else
static void
gst_wfd_ts_demux_send_tags_for_stream (GstWFDTSDemux * demux,
    GstWFDTSStream * stream)
#endif
{
...
#ifdef ADD_CODEC_TAG
  if((stream->flags &MPEGTS_STREAM_FLAG_IS_VIDEO) || (stream->flags &MPEGTS_STREAM_FLAG_IS_AUDIO))
  {
      list = gst_tag_list_new ();
      if(*codec_name&&(stream->flags& MPEGTS_STREAM_FLAG_IS_VIDEO)){
      ...
      if(*codec_name&&(stream->flags& MPEGTS_STREAM_FLAG_IS_AUDIO)){
        gst_tag_list_add (list, GST_TAG_MERGE_REPLACE,
...
gst_wfd_ts_demux_combine_flows (GstWFDTSDemux * demux,
    GstWFDTSStream * stream, GstFlowReturn ret)
{
  gint i;
...
static void
gst_wfd_ts_demux_sync_streams (GstWFDTSDemux * demux, GstClockTime time)
{
  gint i;
  ...
  for (i = 0; i < MPEGTS_MAX_PID + 1; i++) {
    ...
    if (GST_CLOCK_DIFF (stream->last_time, time) > (2 * GST_SECOND)) {
      ...
      if (!stream->pad && demux->pending_pads > 0) {
        ...
        gst_wfd_ts_demux_no_more_pads (GST_ELEMENT (demux));
      }
 ...
 */
static gboolean
gst_wfd_ts_demux_add_all_streams (GstWFDTSDemux * demux, GstClockTime pts)
{
  guint i;
  GstPad *srcpad;
...
/* #if 0 is added, because the below code is unreachable. As we are returning from above.
 * Because of this prevent tool is throwing warning */
#if 0
  /* When adding a stream, require either a valid base PCR, or a valid PTS */
  if (!gst_wfd_ts_demux_setup_base_pts (demux, pts)) {
  ...
  for (i = 0; i < MPEGTS_MAX_PID + 1; i++) {
    ...
    if (!gst_wfd_ts_demux_fill_stream (stream, stream->filter.id,
            stream->stream_type)) {
    ...
    /* send new_segment */
    gst_wfd_ts_demux_send_new_segment (demux, stream, pts);

    /* send tags */
    gst_wfd_ts_demux_send_tags_for_stream (demux, stream);
...
#ifdef WFD_SPEC
static GstEvent *
mpegts_demux_create_segment_event (GstPESFilter * filter, gboolean update,
    GstClockTime position)
{
  GstEvent *event;

  event = gst_event_new_new_segment_full (update, 1.0,
      1.0, GST_FORMAT_TIME, position, GST_CLOCK_TIME_NONE,
      position);
...
static GstFlowReturn
gst_wfd_ts_demux_data_cb (GstPESFilter * filter, gboolean first,
    GstBuffer * buffer, GstWFDTSStream * stream)
{
  GstWFDTSDemux *demux;
...
#else
    gint64 discont_threshold = DISCONT_THRESHOLD_OTHER;
#endif

    pts = filter->pts;
    time = MPEGTIME_TO_GSTTIME (pts) + stream->base_time;
    if (stream->last_time > time && stream->last_time - time > discont_threshold) {
  ...
  if (G_UNLIKELY (!GST_CLOCK_TIME_IS_VALID (demux->in_gap))) {
    if (GST_CLOCK_TIME_IS_VALID (demux->first_buf_ts)
        ...
        && gst_wfd_ts_demux_is_live (demux)) {
      int i;
  ...
  if (!demux->tried_adding_pads) {
    ...
    if (gst_wfd_ts_demux_add_all_streams (demux, pts)) {
  ...
  if (srcpad == NULL) {
...
#ifdef ADD_CODEC_TAG
    if (!gst_wfd_ts_demux_fill_stream (stream, filter->id, stream->stream_type,&codec_name))
#else
    if (!gst_wfd_ts_demux_fill_stream (stream, filter->id, stream->stream_type))
#endif
      goto unknown_type;
    ...
    /* send new_segment */
    // temporary solution for Galaxy4,5
//    gst_wfd_ts_demux_send_new_segment (demux, stream, pts);

    /* send tags */
#ifdef ADD_CODEC_TAG
    gst_wfd_ts_demux_send_tags_for_stream (demux, stream,&codec_name);
#else
    gst_wfd_ts_demux_send_tags_for_stream (demux, stream);
#endif
  }
...
#ifdef WFD_SPEC
  if(GST_BUFFER_TIMESTAMP(buffer) != GST_CLOCK_TIME_NONE && stream->flags == MPEGTS_STREAM_FLAG_IS_AUDIO) {
    demux->current_ts = GST_BUFFER_TIMESTAMP (buffer);
  }
#endif

#ifdef WFD_SPEC
  if (demux->need_new_segment && GST_CLOCK_TIME_IS_VALID (time)) {
    GstEvent *event;
    event = mpegts_demux_create_segment_event (filter, FALSE, time);
    gst_wfd_ts_demux_send_event (demux, event);
    demux->need_new_segment = FALSE;
  }
#endif

  ret = gst_pad_push (srcpad, buffer);

  ret = gst_wfd_ts_demux_combine_flows (demux, stream, ret);

  if (GST_CLOCK_TIME_IS_VALID (time))
    gst_wfd_ts_demux_sync_streams (demux, time);
...
unknown_type:
  {
    ...
    return gst_wfd_ts_demux_combine_flows (demux, stream, GST_FLOW_NOT_LINKED);
  }
bad_timestamp:
  {
    GST_DEBUG_OBJECT (demux, "got bad timestamp buffers, drop it");
    gst_buffer_unref (buffer);
    return gst_wfd_ts_demux_combine_flows (demux, stream, GST_FLOW_OK);
  }

}

static void
gst_wfd_ts_demux_resync_cb (GstPESFilter * filter, GstWFDTSStream * stream)
{
 ...
 */
static FORCE_INLINE gboolean
gst_mpegts_stream_parse_cat (GstWFDTSStream * stream,
...
static void
gst_mpegts_activate_pmt (GstWFDTSDemux * demux, GstWFDTSStream * stream)
{
  GST_DEBUG_OBJECT (demux, "activating PMT 0x%08x", stream->PID);

  /* gst_wfd_ts_demux_remove_pads (demux); */
 ...
 */
static FORCE_INLINE gboolean
gst_mpegts_stream_parse_pmt (GstWFDTSStream * stream,
    guint8 * data, guint datalen)
{
  GstWFDTSDemux *demux;
  gint entries;
  ...
  if (demux->check_crc)
    if (G_UNLIKELY (gst_wfd_ts_demux_calc_crc32 (data - 3, datalen) != 0))
      goto wrong_crc;
   ...
   * base_PCR. */
  gst_wfd_ts_demux_get_stream_for_PID (demux, PMT->PCR_PID);
  ...
  while (entries > 0) {
    ...
    ES_stream = gst_wfd_ts_demux_get_stream_for_PID (demux, entry.PID);
    ...
    if (ES_stream->PID_type == PID_TYPE_UNKNOWN) {
      ...
      if (stream_type == ST_PRIVATE_SECTIONS) {
        ...
        gst_pes_filter_set_callbacks (&ES_stream->filter,
            (GstPESFilterData) gst_wfd_ts_demux_data_cb,
            (GstPESFilterResync) gst_wfd_ts_demux_resync_cb, ES_stream);
 ...
 */
static FORCE_INLINE gboolean
gst_mpegts_stream_parse_private_section (GstWFDTSStream * stream,
    guint8 * data, guint datalen)
{
  GstWFDTSDemux *demux;
  GstBuffer *buffer;
  ...
  if (demux->check_crc)
    if (gst_wfd_ts_demux_calc_crc32 (data, datalen) != 0)
      goto wrong_crc;
 ...
 */
static FORCE_INLINE gboolean
gst_wfd_ts_demux_parse_adaptation_field (GstWFDTSStream * stream,
    const guint8 * data, guint data_len, guint * consumed)
{
  ...
  if (length > 0) {
    ...
    if (flags & 0x10) {
      ...
      if (G_UNLIKELY (demux->pcr[0] != -1 && demux->pcr[1] == -1
                  && ((pcr - demux->pcr[0]) >= 90000))) {
            GST_DEBUG ("new RECORDING pcr[1]:%" G_GUINT64_FORMAT, pcr);
            demux->pcr[1] = pcr;
      }

      if (demux->bitrate == -1) {
        query = gst_query_new_uri ();
        /* try getting the url with a uri query */
        if (!gst_pad_peer_query (demux->sinkpad, query)) {
          GST_ERROR_OBJECT (demux, "Failed to query peer uri");
          gst_query_unref (query);
        }

        gst_query_parse_uri (query, &uri);
        if (uri == NULL) {
          GST_WARNING_OBJECT (demux, "Could not parse the peer uri");
          gst_query_unref (query);
        }
      }

/* Use the least PCR in each first stream of Video and Audio as a last_PCR */
#ifdef USE_LAST_PCR
      if( (stream->last_PCR != -1) && (demux->decide_last_PCR == FALSE) ){ /* check store last_PCR*/
        if(pcr < stream->last_PCR) {
          stream->last_PCR = pcr ;
          GST_DEBUG_OBJECT (demux, "Last PCR was changed to: %" G_GUINT64_FORMAT, stream->last_PCR);
        }

        demux->decide_last_PCR =TRUE;
        GST_DEBUG_OBJECT (demux, "Last PCR was fixed to: %" G_GUINT64_FORMAT, stream->last_PCR);
      }
#endif

      if (stream->last_PCR != -1 && pcr < stream->last_PCR) {
        ...
        if (pcr - stream->last_PCR > 90000) {
          GST_WARNING_OBJECT (demux, "The difference between current PCR and latest PCR is larger than 90000");
        }
      ...
      if (valid_pcr) {
        GstWFDTSStream *PMT_stream = demux->streams[demux->current_PMT];
 ...
 */
static FORCE_INLINE gboolean
gst_mpegts_stream_parse_pat (GstWFDTSStream * stream,
    guint8 * data, guint datalen)
{
  GstWFDTSDemux *demux;
  gint entries;
  ...
  if (demux->check_crc)
    if (gst_wfd_ts_demux_calc_crc32 (data - 3, datalen) != 0)
      goto wrong_crc;
  ...
  while (entries--) {
    ...
    PMT_stream = gst_wfd_ts_demux_get_stream_for_PID (demux, entry.PID);
...
static gboolean
gst_wfd_ts_demux_is_PMT (GstWFDTSDemux * demux, guint16 PID)
{
  GstWFDTSStream *stream;
  GstMpegTSPAT *PAT;
  gint i;
...
gst_mpegts_stream_pes_buffer_flush (GstWFDTSStream * stream, gboolean discard)
{
  GstFlowReturn ret = GST_FLOW_OK;
...
static FORCE_INLINE GstFlowReturn
gst_mpegts_stream_pes_buffer_push (GstWFDTSStream * stream,
...
static FORCE_INLINE GstFlowReturn
gst_wfd_ts_demux_pes_buffer_flush (GstWFDTSDemux * demux, gboolean discard)
{
  gint i;
  GstFlowReturn ret = GST_FLOW_OK;
...
static FORCE_INLINE GstFlowReturn
gst_wfd_ts_demux_push_fragment (GstWFDTSStream * stream,
    const guint8 * in_data, guint in_size)
{
  GstFlowReturn ret;
 ...
 */
static FORCE_INLINE GstFlowReturn
gst_wfd_ts_demux_parse_stream (GstWFDTSDemux * demux, GstWFDTSStream * stream,
    const guint8 * in_data, guint in_size)
{
  GstFlowReturn ret;
  ...
  if (adaptation_field_control & 0x2) {
    ...
    if (!gst_wfd_ts_demux_parse_adaptation_field (stream, data,
            datalen, &consumed))
      goto done;
    ...
    if (G_UNLIKELY (stream->PID_type == PID_TYPE_UNKNOWN)) {
      if (mpegts_is_elem_pid (demux, PID)) {
        ...
        gst_pes_filter_set_callbacks (&stream->filter,
            (GstPESFilterData) gst_wfd_ts_demux_data_cb,
            (GstPESFilterResync) gst_wfd_ts_demux_resync_cb, stream);
      }
    }
...
#ifdef ADAPTER_LOGIC
          demux->submit_count++;
          if(demux->last_PID != stream->PID)
             demux->submit_count = 0;
#ifdef WFD_SPEC
          if (stream->PID == 0x1100 && stream->stream_type == 0x83)
             demux->submit_count = 0;
#endif
          demux->last_PID = stream->PID;
#endif
...
#endif
#ifdef WFD_SPEC
            //LPCM decoder is needed a complete package at the beginning (1920 bytes).
            if (stream->PID == 0x1100 && stream->stream_type == 0x83)
            {
              ret = gst_mpegts_stream_pes_buffer_push (stream, data, datalen);
            }
            /* HDCP requires continous data for decrypting, which means from
             * demuxer's point of view, that all data from PES packet should
             * be included in one GStreamer's buffer.
             *
             * Please note that "WiFi Display Specification v1.1", table B-1
             * defines video based on stream type from MPEG-TS PMT, so
             * direct usage of stream's pid shouldn't be used for video
             * detection. Flag MPEGTS_STREAM_FLAG_IS_VIDEO is set directly
             * on stream type.
             */
            else if (stream->flags & MPEGTS_STREAM_FLAG_IS_VIDEO)
            {
              ret = gst_mpegts_stream_pes_buffer_push (stream, data, datalen);
            }
            else
#endif
            {
              ret = gst_wfd_ts_demux_push_fragment (stream, data, datalen);
            }
          } else {
...
static FORCE_INLINE GstFlowReturn
gst_wfd_ts_demux_parse_transport_packet (GstWFDTSDemux * demux,
    const guint8 * data)
{
  GstFlowReturn ret = GST_FLOW_OK;
  guint16 PID;
  GstWFDTSStream *stream;
  guint src_len=0;
  guint consumed_len=0;
  guint8 *outbuf = NULL;
  GstBuffer *stream_buf = gst_buffer_new_and_alloc (demux->packetsize);
  memcpy (GST_BUFFER_DATA (stream_buf), data, demux->packetsize);
  ...
  if (G_UNLIKELY (PID == 0x1fff)) {
    gst_buffer_unref(stream_buf);
    return ret;
  }

  /* get the stream. */
  stream = gst_wfd_ts_demux_get_stream_for_PID (demux, PID);

  /* store buffer to adapter */
  gst_adapter_push(stream->adapter, stream_buf);
  GST_DEBUG_OBJECT (demux, "gst_wfd_ts_demux_parse_stream submitcount %d %d for pid 0x%04x",demux->submit_count,demux->stream_count, PID);
#ifdef ADAPTER_LOGIC
    if((demux->last_PID == stream->PID) && (demux->submit_count > MAX_CONTINUOUS_SUBMIT_COUNT))
    return ret;
  if(demux->stream_count < 2)
    demux->submit_count = 0;
#endif
  src_len = gst_adapter_available(stream->adapter);
  outbuf = (guint8 *)gst_adapter_peek(stream->adapter, src_len);
  if(!outbuf) {
    GST_ERROR("fail to peek buffer from adapter\n");
    return GST_FLOW_UNEXPECTED;
  }
  while(consumed_len < src_len) {
      /* parse the stream */
      ret = gst_wfd_ts_demux_parse_stream (demux, stream, outbuf+consumed_len+1,
          MPEGTS_NORMAL_TS_PACKETSIZE - 1);
      if(ret == GST_FLOW_RESEND)  {
        GST_WARNING_OBJECT (demux, "gst_wfd_ts_demux_parse_stream pat PACKET NOT YET RECEIVED need to parse this packet again");
        ret = GST_FLOW_OK;
        break;
      }
      demux->num_packets++;
      consumed_len += demux->packetsize;
      GST_DEBUG_OBJECT (demux, "gst_wfd_ts_demux_parse_stream consumed length %d actual length %d",consumed_len, src_len);
...
#ifdef ADAPTER_LOGIC
      if((demux->last_PID == stream->PID) && (demux->submit_count > MAX_CONTINUOUS_SUBMIT_COUNT)) {
        GST_WARNING_OBJECT (demux, "gst_wfd_ts_demux_parse_stream packets of same stream max count reached");
        goto beach;
      }
#endif
  }
...
beach:
  gst_adapter_flush(stream->adapter, consumed_len);
  return ret;
...
static gboolean
gst_wfd_ts_demux_handle_seek_push (GstWFDTSDemux * demux, GstEvent * event)
{
  gboolean res = FALSE;
  gdouble rate;
  GstFormat format;
...
static gboolean
gst_wfd_ts_demux_src_event (GstPad * pad, GstEvent * event)
{
  GstWFDTSDemux *demux = GST_WFD_TS_DEMUX (gst_pad_get_parent (pad));
  gboolean res = FALSE;
  ...
  switch (GST_EVENT_TYPE (event)) {
    case GST_EVENT_SEEK:
      res = gst_wfd_ts_demux_handle_seek_push (demux, event);
      break;
    default:
...
static FORCE_INLINE GstFlowReturn
gst_wfd_ts_demux_flush_transport_packet (GstWFDTSDemux * demux, GstWFDTSStream * stream)
{
  GstFlowReturn ret = GST_FLOW_OK;
  guint src_len=0;
  guint consumed_len=0;
  ...
  gst_adapter_flush(stream->adapter, consumed_len);
  GST_DEBUG_OBJECT(demux,"gst_wfd_ts_demux_flush_transport_packet flushed %d %d \n", consumed_len, src_len);
  return ret;
...
static void
gst_wfd_ts_demux_flush (GstWFDTSDemux * demux, gboolean discard)
{
  gint i;
  GstWFDTSStream *PCR_stream;
  GstWFDTSStream *PMT_stream;
  ...
  if(!discard)
  {
    for (i = 0; i < MPEGTS_MAX_PID + 1; i++) {
      GstWFDTSStream *stream = demux->streams[i];
      if (stream) {
        gst_wfd_ts_demux_flush_transport_packet(demux, stream);
        gst_adapter_clear(stream->adapter);
        stream->last_time = 0;
        stream->discont = TRUE;
      }
...
/* Reset last time of all streams */
  ...
  gst_wfd_ts_demux_pes_buffer_flush (demux, discard);
...
static gboolean
gst_wfd_ts_demux_send_event (GstWFDTSDemux * demux, GstEvent * event)
{
  gint i;
...
static gboolean
gst_wfd_ts_demux_sink_event (GstPad * pad, GstEvent * event)
{
  GstWFDTSDemux *demux = GST_WFD_TS_DEMUX (gst_pad_get_parent (pad));
  gboolean res = FALSE;
  ...
  switch (GST_EVENT_TYPE (event)) {
    case GST_EVENT_FLUSH_START:
      res = gst_wfd_ts_demux_send_event (demux, event);
      break;
    case GST_EVENT_FLUSH_STOP:
      gst_adapter_clear (demux->adapter);
      gst_wfd_ts_demux_flush (demux, TRUE);
      res = gst_wfd_ts_demux_send_event (demux, event);
      demux->in_gap = GST_CLOCK_TIME_NONE;
      demux->first_buf_ts = GST_CLOCK_TIME_NONE;
      demux->last_buf_ts = GST_CLOCK_TIME_NONE;
    ...
    case GST_EVENT_CUSTOM_DOWNSTREAM:
      if (gst_event_has_name (event, "GstHLSFragSwitch")) {
        GST_INFO_OBJECT (demux, "Got Fragswitch event, flushing data");
        gst_wfd_ts_demux_flush (demux, FALSE);
      }
#ifdef WFD_SPEC
        if (gst_event_has_name (event, "GstWFDRequest")) {
          GST_DEBUG_OBJECT (demux, "Got GstWFDRequest event %p", event);
          const GstStructure *structure = gst_event_get_structure (event);
          const gchar *pts = gst_structure_get_string (structure,"pts");
	   gboolean need_segment = FALSE;
          gst_structure_get_boolean (structure, "need_segment", &need_segment);

          GST_DEBUG_OBJECT (demux, "wfd request flush tsdemux");
          gst_wfd_ts_demux_flush (demux, FALSE);
	      ...
	      start_pts = g_ascii_strtoll(pts, NULL, 10);
              GST_DEBUG_OBJECT (demux, "send new segment event with pts %" GST_TIME_FORMAT,
		  		GST_TIME_ARGS(MPEGTIME_TO_GSTTIME (start_pts)));

              new_segment_event = gst_event_new_new_segment_full (FALSE, 1.0,
                        1.0, GST_FORMAT_TIME, MPEGTIME_TO_GSTTIME (start_pts),
		    	GST_CLOCK_TIME_NONE, MPEGTIME_TO_GSTTIME (start_pts));
              gst_wfd_ts_demux_send_event (demux, new_segment_event);
	    } else {
              GST_DEBUG_OBJECT (demux, "send new segment event using first incomming buffer pts");
	      demux->need_new_segment = TRUE;
	    }
          }
          gst_event_unref(event);
    ...
    case GST_EVENT_EOS:
      ...
      if (!(res = gst_wfd_ts_demux_send_event (demux, event))) {
        /* we have no streams */
    ...
    case GST_EVENT_NEWSEGMENT:
    {
      ...
      if (format == GST_FORMAT_BYTES && demux->bitrate != -1) {
        ...
        res = gst_wfd_ts_demux_send_event (demux, event);
      } else if (format == GST_FORMAT_TIME && demux->bitrate != -1){
        GST_DEBUG_OBJECT (demux, "unref new segment event...");
        gst_event_unref(event);
        res = TRUE;
      }
      break;
...
static gboolean
gst_wfd_ts_demux_is_live (GstWFDTSDemux * demux)
{
  GstQuery *query;
...
static gboolean
gst_wfd_ts_demux_provides_clock (GstElement * element)
{
  return gst_wfd_ts_demux_is_live (GST_WFD_TS_DEMUX (element));
}

static GstClock *
gst_wfd_ts_demux_provide_clock (GstElement * element)
{
  GstWFDTSDemux *demux = GST_WFD_TS_DEMUX (element);

  if (gst_wfd_ts_demux_provides_clock (element)) {
    if (demux->clock == NULL) {
...
gst_wfd_ts_demux_src_pad_query_type (GstPad * pad)
{
...
static gboolean
gst_wfd_ts_demux_src_pad_query (GstPad * pad, GstQuery * query)
{
  GstWFDTSDemux *demux = GST_WFD_TS_DEMUX (gst_pad_get_parent (pad));
  gboolean res = FALSE;
  GstPad *peer;
...
static inline void
gst_wfd_ts_demux_detect_packet_size (GstWFDTSDemux * demux, guint len)
{
...
gst_wfd_ts_demux_sync_scan (GstWFDTSDemux * demux, const guint8 * in_data,
    guint size, guint * flush)
{
  ...
  if (G_UNLIKELY (!demux->packetsize))
    gst_wfd_ts_demux_detect_packet_size (demux, sync_count);
...
static GstFlowReturn
gst_wfd_ts_demux_chain (GstPad * pad, GstBuffer * buffer)
{
  GstWFDTSDemux *demux = GST_WFD_TS_DEMUX (gst_pad_get_parent (pad));
  GstFlowReturn ret = GST_FLOW_OK;
  const guint8 *data;
  ...
  /* scan for sync codes */
  sync_count = gst_wfd_ts_demux_sync_scan (demux, data, avail, &flush);

  /* process all packets */
  for (i = 0; i < sync_count; i++) {
    ret = gst_wfd_ts_demux_parse_transport_packet (demux, demux->sync_lut[i]);
...
gst_wfd_ts_demux_change_state (GstElement * element, GstStateChange transition)
{
  GstWFDTSDemux *demux = GST_WFD_TS_DEMUX (element);
...
static GValueArray *
wfd_ts_demux_build_pat_info (GstWFDTSDemux * demux)
{
  ...
  for (i = 0; i < PAT->entries->len; i++) {
    ...
    GValue v = { 0, };
    WFDTsPatInfo *info_obj;

    info_obj = wfd_ts_pat_info_new (cur_entry->program_number, cur_entry->PID);
...
static WFDTsPmtInfo *
wfd_ts_demux_build_pmt_info (GstWFDTSDemux * demux, guint16 pmt_pid)
{
  WFDTsPmtInfo *info_obj;
  GstMpegTSPMT *PMT;
  gint i;
  ...
  info_obj = wfd_ts_pmt_info_new (PMT->program_number, PMT->PCR_PID,
      PMT->version_number);

  for (i = 0; i < PMT->entries->len; i++) {
    GstWFDTSStream *stream;
    WFDTsPmtStreamInfo *stream_info;
    GstMpegTSPMTEntry *cur_entry =
        &g_array_index (PMT->entries, GstMpegTSPMTEntry, i);

    stream = demux->streams[cur_entry->PID];
    stream_info =
        wfd_ts_pmt_stream_info_new (cur_entry->PID, stream->stream_type);
    ...
    if (stream->ES_info) {
      ...
      if (iso639_languages) {
        for (i = 0; i < DESC_ISO_639_LANGUAGE_codes_n (iso639_languages); i++) {
          ...
          wfd_ts_pmt_stream_info_add_language (stream_info,
      ...
      for (i = 0; i < gst_mpeg_descriptor_n_desc (stream->ES_info); ++i) {
        ...
        wfd_ts_pmt_stream_info_add_descriptor (stream_info,
            (gchar *) desc, 2 + DESC_LENGTH (desc));
      }
    }
    wfd_ts_pmt_info_add_stream (info_obj, stream_info);
  }
  return info_obj;
}
  ...
  switch (prop_id) {
    ...
    case PROP_PAT_INFO:
    {
      if (demux->streams[0] != NULL) {
        g_value_take_boxed (value, wfd_ts_demux_build_pat_info (demux));
      }
      break;
    }
    case PROP_PMT_INFO:
    {
      if (demux->current_PMT != 0 && demux->streams[demux->current_PMT] != NULL) {
        g_value_take_object (value, wfd_ts_demux_build_pmt_info (demux,
                demux->current_PMT));
      }
      break;
    }
#ifdef WFD_SPEC
    case PROP_CURRENT_PTS:
      g_value_set_uint (value, GSTTIME_TO_MPEGTIME (demux->current_ts));
      break;
#endif
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
...
GST_DEBUG_CATEGORY_EXTERN (gstwfdpesfilter_debug);
GST_DEBUG_CATEGORY_EXTERN (gstwfdsectionfilter_debug);

gboolean
gst_wfd_ts_demux_plugin_init (GstPlugin * plugin)
{
  GST_DEBUG_CATEGORY_INIT (gstwfdpesfilter_debug, "wfdpesfilter", 0,
      "MPEG-TS/PS PES filter output");
  GST_DEBUG_CATEGORY_INIT (gstwfdsectionfilter_debug, "gstwfdsectionfilter", 0,
      "MPEG-TS Section filter output");

  if (!gst_element_register (plugin, "wfdtsdemux",
          GST_RANK_PRIMARY, GST_TYPE_WFD_TS_DEMUX))
    return FALSE;
