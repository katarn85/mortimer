...
//#define ADAPTER_LOGIC
#define USE_LAST_PCR   /* Use the least PCR in each first stream of Video and Audio as a last_PCR */
#define ADD_CODEC_TAG

/* HDCP requires continous data for decrypting, which means from demuxer's
 * point of view, that all data from PES packet should be included in one
 * GStreamer's buffer.
 *
 * Current implementation of the demuxer will push GstBuffer if pes_buffer
 * is not big enough (and resize it). To avoid this situation limits of pes
 * buffer size are increased. The open question is if current minimum limit
 * is bigger than maximum PES packet with video which can be send by
 * transmitter.
 */
#define MPEGTS_MIN_PES_BUFFER_SIZE   512 * 1024
#define MPEGTS_MAX_PES_BUFFER_SIZE   1024 * 1024
...
#define GST_TYPE_WFD_TS_DEMUX              (gst_wfd_ts_demux_get_type())
#define GST_WFD_TS_DEMUX(obj)              (G_TYPE_CHECK_INSTANCE_CAST((obj),\
                                            GST_TYPE_WFD_TS_DEMUX,GstWFDTSDemux))
#define GST_WFD_TS_DEMUX_CLASS(klass)      (G_TYPE_CHECK_CLASS_CAST((klass),\
                                            GST_TYPE_WFD_TS_DEMUX,GstWFDTSDemuxClass))
#define GST_WFD_TS_DEMUX_GET_CLASS(klass)  (G_TYPE_INSTANCE_GET_CLASS((klass),\
                                            GST_TYPE_WFD_TS_DEMUX,GstWFDTSDemuxClass))
#define GST_IS_WFD_TS_DEMUX(obj)           (G_TYPE_CHECK_INSTANCE_TYPE((obj),\
                                            GST_TYPE_WFD_TS_DEMUX))
#define GST_IS_WFD_TS_DEMUX_CLASS(obj)     (G_TYPE_CHECK_CLASS_TYPE((klass),\
                                            GST_TYPE_WFD_TS_DEMUX))
typedef struct _GstWFDTSStream GstWFDTSStream;
...
struct _GstWFDTSDemux {
  ...
  GstWFDTSStream    **  streams;
  /* Array to perform pmts checks at gst_wfd_ts_demux_parse_adaptation_field */
  ...
  guint16           stream_count;
  guint16           last_PID;
  guint16           submit_count;
#ifdef USE_LAST_PCR
  gboolean          decide_last_PCR;
#endif
...
GType     gst_wfd_ts_demux_get_type (void);

gboolean  gst_wfd_ts_demux_plugin_init (GstPlugin *plugin);
