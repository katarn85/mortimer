...
GST_BOILERPLATE (WFDTsPmtInfo, wfd_ts_pmt_info, GObject, G_TYPE_OBJECT);

static void wfd_ts_pmt_info_finalize (GObject * object);
static void wfd_ts_pmt_info_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * spec);
static void wfd_ts_pmt_info_get_property (GObject * object, guint prop_id,
    GValue * value, GParamSpec * spec);

static void
wfd_ts_pmt_info_base_init (gpointer klass)
{
}

static void
wfd_ts_pmt_info_class_init (WFDTsPmtInfoClass * klass)
{
  GObjectClass *gobject_klass = (GObjectClass *) klass;

  gobject_klass->finalize = wfd_ts_pmt_info_finalize;
  gobject_klass->set_property = wfd_ts_pmt_info_set_property;
  gobject_klass->get_property = wfd_ts_pmt_info_get_property;
...
static void
wfd_ts_pmt_info_init (WFDTsPmtInfo * pmt_info, WFDTsPmtInfoClass * klass)
{
...
WFDTsPmtInfo *
wfd_ts_pmt_info_new (guint16 program_no, guint16 pcr_pid, guint8 version_no)
{
  WFDTsPmtInfo *info;

  info = g_object_new (WFD_TS_TYPE_PMT_INFO, NULL);
...
static void
wfd_ts_pmt_info_finalize (GObject * object)
{
  WFDTsPmtInfo *info = WFD_TS_PMT_INFO (object);
...
static void
wfd_ts_pmt_info_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * spec)
{
  g_return_if_fail (WFD_TS_IS_PMT_INFO (object));
...
static void
wfd_ts_pmt_info_get_property (GObject * object, guint prop_id,
    GValue * value, GParamSpec * spec)
{
  WFDTsPmtInfo *pmt_info;

  g_return_if_fail (WFD_TS_IS_PMT_INFO (object));

  pmt_info = WFD_TS_PMT_INFO (object);
...
void
wfd_ts_pmt_info_add_descriptor (WFDTsPmtInfo * pmt_info,
    const gchar * descriptor, guint length)
{
  GValue value = { 0 };
  GString *string;

  g_return_if_fail (WFD_TS_IS_PMT_INFO (pmt_info));
...
void
wfd_ts_pmt_info_add_stream (WFDTsPmtInfo * pmt_info,
    WFDTsPmtStreamInfo * stream)
{
  GValue v = { 0, };

  g_return_if_fail (WFD_TS_IS_PMT_INFO (pmt_info));
  g_return_if_fail (WFD_TS_IS_PMT_STREAM_INFO (stream));
