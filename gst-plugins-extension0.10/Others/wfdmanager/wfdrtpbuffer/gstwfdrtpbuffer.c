...
/**
 * SECTION:element-gstwfdrtpbuffer
 *
 ...
 * configurable time limit using the #GstWfdRTPBuffer:latency property.
 ...
 * |[
 * gst-launch rtspsrc location=rtsp://192.168.1.133:8554/mpeg1or2AudioVideoTest ! wfdrtpbuffer ! rtpmpvdepay ! mpeg2dec ! xvimagesink
 * ]| Connect to a streaming server and decode the MPEG video. The wfdrtpbuffer is
...
/*  This value is a fuse on gstreamer pipeline, when gst_wfd_rtp_buffer_chain()
 *  puts a RTP packet into the queue and gst_wfd_rtp_buffer_loop() has no chance
 *  to process it because machine is too busy. In such a case queue starts growing
 *  and lead to playback collapse. This value means upper limit above it queue will
 *  be immediately flushed.
*/
#define DEFAULT_FLUSH_ON_QUEUE_LEN 512
...
GST_BOILERPLATE (GstWfdRTPBuffer, gst_wfd_rtp_buffer, GstElement,
    GST_TYPE_ELEMENT);
...
/* pad overrides */
static GstCaps *gst_wfd_rtp_buffer_getcaps (GstPad * pad);
static GstIterator *gst_wfd_rtp_buffer_iterate_internal_links (GstPad * pad);

/* sinkpad overrides */
static gboolean gst_wfd_rtp_buffer_sink_setcaps (GstPad * pad, GstCaps * caps);
...
static gboolean gst_wfd_rtp_buffer_src_event (GstPad * pad,
    GstEvent * event);
static gboolean
gst_wfd_rtp_buffer_src_activate_push (GstPad * pad, gboolean active);
static void gst_wfd_rtp_buffer_loop (GstWfdRTPBuffer * wfdrtpbuffer);
static gboolean gst_wfd_rtp_buffer_query (GstPad * pad, GstQuery * query);

static void
gst_wfd_rtp_buffer_clear_pt_map (GstWfdRTPBuffer * wfdrtpbuffer);
static GstClockTime
gst_wfd_rtp_buffer_set_active (GstWfdRTPBuffer * wfdrtpbuffer,
    gboolean active, guint64 base_time);

void gst_rtp_bin_marshal_BOXED__UINT (GClosure     *closure,
                                             GValue       *return_value,
...
void gst_rtp_bin_marshal_UINT64__BOOLEAN_UINT64 (GClosure     *closure,
                                                        GValue       *return_value,
                                                        guint         n_param_values,
                                                        const GValue *param_values,
                                                        gpointer      invocation_hint,
...
static void
gst_wfd_rtp_buffer_base_init (gpointer klass)
{
  GstElementClass *element_class = GST_ELEMENT_CLASS (klass);

  gst_element_class_add_static_pad_template (element_class,
      &gst_wfd_rtp_buffer_src_template);
  gst_element_class_add_static_pad_template (element_class,
      &gst_wfd_rtp_buffer_sink_template);
  gst_element_class_add_static_pad_template (element_class,
      &gst_wfd_rtp_buffer_sink_rtcp_template);

  gst_element_class_set_details_simple (element_class,
      "The Wi-Fi Display RTP packet buffer", "Filter/Network/RTP",
...
gst_wfd_rtp_buffer_class_init (GstWfdRTPBufferClass * klass)
{
   ...
   *
   * The maximum latency of the wfdrtpbuffer. Packets will be kept in the buffer
  ...
  g_object_class_install_property (gobject_class, PROP_DROP_ON_LATENCY,
      g_param_spec_boolean ("drop-on-latency",
          ...
          "Tells the wfdrtpbuffer to never exceed the given latency in size",
          DEFAULT_DROP_ON_LATENCY, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  /**
   * GstWfdRTPBuffer::ts-offset:
   *
   * Adjust GStreamer output buffer timestamps in the wfdrtpbuffer with offset.
   ...
   *
   * Control the buffering and timestamping mode used by the wfdrtpbuffer.
   */
   ...
   *
   * The percent of the wfdrtpbuffer that is filled.
   *
   * Since: 0.10.19
   */
  ...
  GST_DEBUG_CATEGORY_INIT
      (wfdrtpbuffer_debug, "gstwfdrtpbuffer", 0, "WFD RTP Buffer");
}

static void
gst_wfd_rtp_buffer_init (GstWfdRTPBuffer * wfdrtpbuffer,
    GstWfdRTPBufferClass * klass)
{
  GstWfdRTPBufferPrivate *priv;

  priv = GST_WFD_RTP_BUFFER_GET_PRIVATE (wfdrtpbuffer);
  wfdrtpbuffer->priv = priv;
  ...
  gst_pad_set_activatepush_function (priv->srcpad,
      GST_DEBUG_FUNCPTR (gst_wfd_rtp_buffer_src_activate_push));
  gst_pad_set_query_function (priv->srcpad,
      GST_DEBUG_FUNCPTR (gst_wfd_rtp_buffer_query));
  gst_pad_set_getcaps_function (priv->srcpad,
      GST_DEBUG_FUNCPTR (gst_wfd_rtp_buffer_getcaps));
  ...
  gst_pad_set_setcaps_function (priv->sinkpad,
      GST_DEBUG_FUNCPTR (gst_wfd_rtp_buffer_sink_setcaps));
  gst_pad_set_getcaps_function (priv->sinkpad,
      GST_DEBUG_FUNCPTR (gst_wfd_rtp_buffer_getcaps));

  gst_element_add_pad (GST_ELEMENT (wfdrtpbuffer), priv->srcpad);
  gst_element_add_pad (GST_ELEMENT (wfdrtpbuffer), priv->sinkpad);
}

static void
gst_wfd_rtp_buffer_finalize (GObject * object)
{
  GstWfdRTPBuffer *wfdrtpbuffer;

  wfdrtpbuffer = GST_WFD_RTP_BUFFER (object);

  g_mutex_free (wfdrtpbuffer->priv->jbuf_lock);
  g_cond_free (wfdrtpbuffer->priv->jbuf_cond);

  g_object_unref (wfdrtpbuffer->priv->jbuf);

  G_OBJECT_CLASS (parent_class)->finalize (object);
}

static GstIterator *
gst_wfd_rtp_buffer_iterate_internal_links (GstPad * pad)
{
  GstWfdRTPBuffer *wfdrtpbuffer;
  GstPad *otherpad = NULL;
  GstIterator *it;

  wfdrtpbuffer = GST_WFD_RTP_BUFFER (gst_pad_get_parent (pad));

  if (pad == wfdrtpbuffer->priv->sinkpad) {
    otherpad = wfdrtpbuffer->priv->srcpad;
  } else if (pad == wfdrtpbuffer->priv->srcpad) {
    otherpad = wfdrtpbuffer->priv->sinkpad;
  } else if (pad == wfdrtpbuffer->priv->rtcpsinkpad) {
    otherpad = NULL;
  }
  ...
  gst_object_unref (wfdrtpbuffer);

  return it;
}

static GstPad *
create_rtcp_sink (GstWfdRTPBuffer * wfdrtpbuffer)
{
  GstWfdRTPBufferPrivate *priv;

  priv = wfdrtpbuffer->priv;

  GST_DEBUG_OBJECT (wfdrtpbuffer, "creating RTCP sink pad");
  ...
  gst_element_add_pad (GST_ELEMENT_CAST (wfdrtpbuffer), priv->rtcpsinkpad);

  return priv->rtcpsinkpad;
}

static void
remove_rtcp_sink (GstWfdRTPBuffer * wfdrtpbuffer)
{
  GstWfdRTPBufferPrivate *priv;

  priv = wfdrtpbuffer->priv;

  GST_DEBUG_OBJECT (wfdrtpbuffer, "removing RTCP sink pad");

  gst_pad_set_active (priv->rtcpsinkpad, FALSE);

  gst_element_remove_pad (GST_ELEMENT_CAST (wfdrtpbuffer), priv->rtcpsinkpad);
    ...
    GstPadTemplate * templ, const gchar * name)
{
  ...
  wfdrtpbuffer = GST_WFD_RTP_BUFFER (element);
  priv = wfdrtpbuffer->priv;
  ...
  if (templ == gst_element_class_get_pad_template (klass, "sink_rtcp")) {
    ...
    result = create_rtcp_sink (wfdrtpbuffer);
  } else
    goto wrong_template;
...
wrong_template:
  {
    g_warning ("gstwfdrtpbuffer: this is not our template");
    return NULL;
  }
exists:
  {
    g_warning ("gstwfdrtpbuffer: pad already requested");
    return NULL;
  }
}
...
wrong_pad:
  {
    g_warning ("gstwfdrtpbuffer: asked to release an unknown pad");
    return;
  }
}
...
gst_wfd_rtp_buffer_clear_pt_map (GstWfdRTPBuffer * wfdrtpbuffer)
{
  ...
  GST_DEBUG_OBJECT (wfdrtpbuffer, "reset wfdrtpbuffer");
  wfd_rtp_buffer_reset_skew (priv->jbuf);
    ...
    guint64 offset)
{
  ...
  if ((head = wfd_rtp_buffer_peek (priv->jbuf))) {
...
static GstCaps *
gst_wfd_rtp_buffer_getcaps (GstPad * pad)
{
  GstWfdRTPBuffer *wfdrtpbuffer;
  GstWfdRTPBufferPrivate *priv;
  GstPad *other;
  ...
  if (caps == NULL) {
    GST_DEBUG_OBJECT (wfdrtpbuffer, "copy template");
    caps = gst_caps_copy (templ);
  } else {
    GstCaps *intersect;

    GST_DEBUG_OBJECT (wfdrtpbuffer, "intersect with template");
...
static gboolean
gst_wfd_rtp_buffer_sink_parse_caps (GstWfdRTPBuffer * wfdrtpbuffer,
    GstCaps * caps)
{
  GstWfdRTPBufferPrivate *priv;
  ...
  GST_DEBUG_OBJECT (wfdrtpbuffer, "got caps");
  ...
  GST_DEBUG_OBJECT (wfdrtpbuffer, "got clock-rate %d", priv->clock_rate);
  ...
  GST_DEBUG_OBJECT (wfdrtpbuffer, "got clock-base %" G_GINT64_FORMAT,
      priv->clock_base);
  ...
  GST_DEBUG_OBJECT (wfdrtpbuffer, "got seqnum-base %d", priv->next_in_seqnum);
...
error:
  {
    GST_DEBUG_OBJECT (wfdrtpbuffer, "No clock-rate in caps!");
    return FALSE;
  }
wrong_rate:
  {
    GST_DEBUG_OBJECT (wfdrtpbuffer, "Invalid clock-rate %d", priv->clock_rate);
    return FALSE;
  }
}

static gboolean
gst_wfd_rtp_buffer_sink_setcaps (GstPad * pad, GstCaps * caps)
{
  GstWfdRTPBuffer *wfdrtpbuffer;
  GstWfdRTPBufferPrivate *priv;
  gboolean res;
  ...
  JBUF_LOCK (priv);
  res = gst_wfd_rtp_buffer_sink_parse_caps (wfdrtpbuffer, caps);
  JBUF_UNLOCK (priv);
...
static void
gst_wfd_rtp_buffer_flush_start (GstWfdRTPBuffer * wfdrtpbuffer)
{
  GstWfdRTPBufferPrivate *priv;
  ...
  GST_DEBUG_OBJECT (wfdrtpbuffer, "Disabling pop on queue");
...
static void
gst_wfd_rtp_buffer_flush_stop (GstWfdRTPBuffer * wfdrtpbuffer)
{
  GstWfdRTPBufferPrivate *priv;
  ...
  JBUF_LOCK (priv);
  GST_DEBUG_OBJECT (wfdrtpbuffer, "Enabling pop on queue");
  ...
  GST_DEBUG_OBJECT (wfdrtpbuffer, "flush and reset wfdrtpbuffer");
  wfd_rtp_buffer_flush (priv->jbuf);
  wfd_rtp_buffer_reset_skew (priv->jbuf);
  JBUF_UNLOCK (priv);
}
...
gst_wfd_rtp_buffer_src_activate_push (GstPad * pad, gboolean active)
{
  ...
  GstWfdRTPBuffer *wfdrtpbuffer = NULL;

  wfdrtpbuffer = GST_WFD_RTP_BUFFER (gst_pad_get_parent (pad));

  if (active) {
    /* allow data processing */
    gst_wfd_rtp_buffer_flush_stop (wfdrtpbuffer);

    /* start pushing out buffers */
    GST_DEBUG_OBJECT (wfdrtpbuffer, "Starting task on srcpad");
    gst_pad_start_task (wfdrtpbuffer->priv->srcpad,
        (GstTaskFunction) gst_wfd_rtp_buffer_loop, wfdrtpbuffer);
  } else {
    /* make sure all data processing stops ASAP */
    gst_wfd_rtp_buffer_flush_start (wfdrtpbuffer);

    /* NOTE this will hardlock if the state change is called from the src pad
     * task thread because we will _join() the thread. */
    GST_DEBUG_OBJECT (wfdrtpbuffer, "Stopping task on srcpad");
...
static gboolean
gst_wfd_rtp_buffer_src_event (GstPad * pad, GstEvent * event)
{
  gboolean ret = TRUE;
  GstWfdRTPBuffer *wfdrtpbuffer;
  GstWfdRTPBufferPrivate *priv;
  ...
  wfdrtpbuffer = GST_WFD_RTP_BUFFER (gst_pad_get_parent (pad));
  if (G_UNLIKELY (wfdrtpbuffer == NULL)) {
    gst_event_unref (event);
    return FALSE;
  }
  priv = wfdrtpbuffer->priv;

  GST_DEBUG_OBJECT (wfdrtpbuffer, "received %s", GST_EVENT_TYPE_NAME (event));
...
static gboolean
gst_wfd_rtp_buffer_sink_event (GstPad * pad, GstEvent * event)
{
  gboolean ret = TRUE;
  GstWfdRTPBuffer *wfdrtpbuffer;
  GstWfdRTPBufferPrivate *priv;
  ...
  switch (GST_EVENT_TYPE (event)) {
    ...
    case GST_EVENT_FLUSH_STOP:
      ret = gst_pad_push_event (priv->srcpad, event);
      ret = gst_wfd_rtp_buffer_src_activate_push (priv->srcpad, TRUE);
      break;
    case GST_EVENT_EOS:
    {
      ...
      if (ret && !priv->eos) {
        GST_INFO_OBJECT (wfdrtpbuffer, "queuing EOS");
        priv->eos = TRUE;
        JBUF_SIGNAL (priv);
      } else if (priv->eos) {
        GST_DEBUG_OBJECT (wfdrtpbuffer, "dropping EOS, we are already EOS");
      } else {
        GST_DEBUG_OBJECT (wfdrtpbuffer, "dropping EOS, reason %s",
...
newseg_wrong_format:
  {
    GST_DEBUG_OBJECT (wfdrtpbuffer, "received non TIME newsegment");
    ret = FALSE;
    gst_event_unref (event);
    goto done;
  }
...
static gboolean
gst_wfd_rtp_buffer_sink_rtcp_event (GstPad * pad, GstEvent * event)
{
  GstWfdRTPBuffer *wfdrtpbuffer;
...
gst_wfd_rtp_buffer_get_clock_rate (GstWfdRTPBuffer * wfdrtpbuffer,
    guint8 pt)
{
  GValue ret = { 0 };
  GValue args[2] = { {0}, {0} };
  ...
  g_value_set_object (&args[0], wfdrtpbuffer);
  ...
  JBUF_UNLOCK (wfdrtpbuffer->priv);
  g_signal_emitv (args, gst_wfd_rtp_buffer_signals[SIGNAL_REQUEST_PT_MAP], 0,
      &ret);
  JBUF_LOCK_CHECK (wfdrtpbuffer->priv, out_flushing);
...
no_caps:
  {
    GST_DEBUG_OBJECT (wfdrtpbuffer, "could not get caps");
    return GST_FLOW_ERROR;
  }
out_flushing:
  {
    GST_DEBUG_OBJECT (wfdrtpbuffer, "we are flushing");
    return GST_FLOW_WRONG_STATE;
  }
parse_failed:
  {
    GST_DEBUG_OBJECT (wfdrtpbuffer, "parse failed");
    return GST_FLOW_ERROR;
  }
}
...
static void
check_buffering_percent (GstWfdRTPBuffer * wfdrtpbuffer, gint * percent)
{
  GstWfdRTPBufferPrivate *priv = wfdrtpbuffer->priv;
  ...
  if (*percent != -1 && priv->npt_stop != -1 &&
      ...
      wfd_rtp_buffer_get_delay (priv->jbuf)) {
    GST_DEBUG_OBJECT (wfdrtpbuffer, "short stream; faking full buffer");
    wfd_rtp_buffer_set_buffering (priv->jbuf, FALSE);
    *percent = 100;
  }
}
...
static void
post_buffering_percent (GstWfdRTPBuffer * wfdrtpbuffer, gint percent)
{
  GstMessage *message;

  /* Post a buffering message */
  message = gst_message_new_buffering (GST_OBJECT_CAST (wfdrtpbuffer), percent);
  gst_message_set_buffering_stats (message, GST_BUFFERING_LIVE, -1, -1, -1);

  gst_element_post_message (GST_ELEMENT_CAST (wfdrtpbuffer), message);
}

static GstFlowReturn
gst_wfd_rtp_buffer_chain (GstPad * pad, GstBuffer * buffer)
{
  GstWfdRTPBuffer *wfdrtpbuffer;
  GstWfdRTPBufferPrivate *priv;
  guint16 seqnum;
  ...
  if (G_UNLIKELY (priv->last_pt != pt)) {
    ...
    GST_DEBUG_OBJECT (wfdrtpbuffer, "pt changed from %u to %u", priv->last_pt,
        pt);
    ...
    if ((caps = GST_BUFFER_CAPS (buffer))) {
      gst_wfd_rtp_buffer_sink_parse_caps (wfdrtpbuffer, caps);
    }
  }

  if (G_UNLIKELY (priv->clock_rate == -1)) {
    /* no clock rate given on the caps, try to get one with the signal */
    if (gst_wfd_rtp_buffer_get_clock_rate (wfdrtpbuffer,
  ...
  if (G_LIKELY (priv->next_in_seqnum != -1)) {
    ...
    if (G_UNLIKELY (gap != 0)) {
      GST_DEBUG_OBJECT (wfdrtpbuffer, "expected #%d, got #%d, gap of %d",
      ...
      if (gap < -RTP_MAX_MISORDER) {
        GST_DEBUG_OBJECT (wfdrtpbuffer, "reset: buffer too old %d", gap);
        reset = TRUE;
      }
      /* priv->next_in_seqnum < seqnum, this is a new packet */
      else if (G_UNLIKELY (gap > RTP_MAX_DROPOUT)) {
        GST_DEBUG_OBJECT (wfdrtpbuffer, "reset: too many dropped packets %d",
            gap);
        reset = TRUE;
      } else {
        GST_DEBUG_OBJECT (wfdrtpbuffer, "tolerable gap");
      }
    }
    if (G_UNLIKELY (reset)) {
      GST_DEBUG_OBJECT (wfdrtpbuffer, "flush and reset wfdrtpbuffer");
  ...
  if (priv->latency_ms && priv->drop_on_latency) {
    ...
    if (G_UNLIKELY (wfd_rtp_buffer_get_ts_diff (priv->jbuf) >= latency_ts)) {
      ...
      old_buf = wfd_rtp_buffer_pop (priv->jbuf, &percent);

      GST_DEBUG_OBJECT (wfdrtpbuffer, "Queue full, dropping old packet #%d",
  ...
  /* we need to make the metadata writable before pushing it in the wfdrtpbuffer
   * because the wfdrtpbuffer will update the timestamp */
  buffer = gst_buffer_make_metadata_writable (buffer);

  /* now insert the packet into the queue in sorted order. This function returns
   * FALSE if a packet with the same seqnum was already in the queue, meaning we
   * have a duplicate. */
  if (G_UNLIKELY (!wfd_rtp_buffer_insert (priv->jbuf, buffer, timestamp,
  ...
  GST_DEBUG_OBJECT (wfdrtpbuffer, "Pushed packet #%d, now %d packets, tail: %d",
      seqnum, wfd_rtp_buffer_num_packets (priv->jbuf), tail);

  check_buffering_percent (wfdrtpbuffer, &percent);

finished:
  JBUF_UNLOCK (priv);

  if (percent != -1)
    post_buffering_percent (wfdrtpbuffer, percent);
...
invalid_buffer:
  {
    ...
    GST_ELEMENT_WARNING (wfdrtpbuffer, STREAM, DECODE, (NULL),
...
no_clock_rate:
  {
    GST_WARNING_OBJECT (wfdrtpbuffer,
...
out_flushing:
  {
    ret = priv->srcresult;
    GST_DEBUG_OBJECT (wfdrtpbuffer, "flushing %s", gst_flow_get_name (ret));
    gst_buffer_unref (buffer);
    goto finished;
  }
have_eos:
  {
    ...
    GST_WARNING_OBJECT (wfdrtpbuffer, "we are EOS, refusing buffer");
    gst_buffer_unref (buffer);
    goto finished;
  }
too_late:
  {
    GST_WARNING_OBJECT (wfdrtpbuffer, "Packet #%d too late as #%d was already"
...
duplicate:
  {
    GST_WARNING_OBJECT (wfdrtpbuffer, "Duplicate packet #%d detected, dropping",
        seqnum);
    priv->num_duplicates++;
    gst_buffer_unref (buffer);
    goto finished;
...
static GstClockTime
apply_offset (GstWfdRTPBuffer * wfdrtpbuffer, GstClockTime timestamp)
{
  GstWfdRTPBufferPrivate *priv;
...
static GstClockTime
get_sync_time (GstWfdRTPBuffer * wfdrtpbuffer, GstClockTime timestamp)
{
  GstClockTime result;
  GstWfdRTPBufferPrivate *priv;
  ...
  result = timestamp + GST_ELEMENT_CAST (wfdrtpbuffer)->base_time;
    ...
    GstWfdRTPBuffer * wfdrtpbuffer)
{
  ...
  if (priv->waiting) {
    GST_INFO_OBJECT (wfdrtpbuffer, "got the NPT timeout");
...
static GstClockTime
compute_elapsed (GstWfdRTPBuffer * wfdrtpbuffer, GstBuffer * outbuf)
{
  ...
  GST_LOG_OBJECT (wfdrtpbuffer, "rtp %" G_GUINT32_FORMAT ", ext %"
 ...
 */
static void
gst_wfd_rtp_buffer_loop (GstWfdRTPBuffer * wfdrtpbuffer)
{
  GstWfdRTPBufferPrivate *priv;
  GstBuffer *outbuf;
  ...
  JBUF_LOCK_CHECK (priv, flushing);
again:
  GST_DEBUG_OBJECT (wfdrtpbuffer, "Peeking item, iter = %d", iter++);
  while (TRUE) {
    id = NULL;
    ...
    if (G_LIKELY (!priv->blocked)) {
      ...
      if (!priv->eos && (!priv->active
        ...
        outbuf = wfd_rtp_buffer_peek (priv->jbuf);
        if (outbuf != NULL) {
          elapsed = compute_elapsed (wfdrtpbuffer, outbuf);
          if (GST_BUFFER_DURATION_IS_VALID (outbuf))
            elapsed += GST_BUFFER_DURATION (outbuf);
        } else {
          GST_INFO_OBJECT (wfdrtpbuffer, "no buffer, using last_elapsed");
          elapsed = priv->last_elapsed;
        }

        delay = wfd_rtp_buffer_get_delay (priv->jbuf);
        ...
        GST_INFO_OBJECT (wfdrtpbuffer, "buffering, elapsed %" GST_TIME_FORMAT
      ...
      if (wfd_rtp_buffer_num_packets (priv->jbuf) > 0)
        break;
      ...
      if (priv->estimated_eos != -1 && !priv->reached_npt_stop) {
        sync_time = get_sync_time (wfdrtpbuffer, priv->estimated_eos);

        GST_OBJECT_LOCK (wfdrtpbuffer);
        clock = GST_ELEMENT_CLOCK (wfdrtpbuffer);
        if (clock) {
          GST_INFO_OBJECT (wfdrtpbuffer, "scheduling timeout");
          id = gst_clock_new_single_shot_id (clock, sync_time);
          gst_clock_id_wait_async (id, (GstClockCallback) eos_reached,
              wfdrtpbuffer);
        }
        GST_OBJECT_UNLOCK (wfdrtpbuffer);
      }
    }
  do_wait:
    /* now we wait */
    GST_DEBUG_OBJECT (wfdrtpbuffer, "waiting");
    priv->waiting = TRUE;
    JBUF_WAIT (priv);
    priv->waiting = FALSE;
    GST_DEBUG_OBJECT (wfdrtpbuffer, "waiting done");
  ...
  GST_DEBUG_OBJECT (wfdrtpbuffer,
      ...
      wfd_rtp_buffer_num_packets (priv->jbuf));

  /* apply our timestamp offset to the incomming buffer, this will be our output
   * timestamp. */
  out_time = apply_offset (wfdrtpbuffer, timestamp);
  ...
  if (G_LIKELY (next_seqnum != -1)) {
    ...
    if (G_UNLIKELY (gap < 0)) {
      GST_DEBUG_OBJECT (wfdrtpbuffer, "Old packet #%d, next #%d dropping",
          seqnum, next_seqnum);
      outbuf = wfd_rtp_buffer_pop (priv->jbuf, &percent);
      gst_buffer_unref (outbuf);
      goto again;
    }
  } else {
    GST_DEBUG_OBJECT (wfdrtpbuffer, "no next seqnum known, first packet");
    gap = -1;
  }
  ...
  if (G_UNLIKELY (gap != 0 && out_time != -1)) {
    ...
    if (gap > 0) {
      ...
      if (priv->last_out_time != -1) {
        ...
        GST_DEBUG_OBJECT (wfdrtpbuffer, "duration %" GST_TIME_FORMAT,
       ...
       * while we wait */
      GST_DEBUG_OBJECT (wfdrtpbuffer, "First buffer %d, do sync", seqnum);
    }
    ...
    if (!clock) {
      ...
      GST_DEBUG_OBJECT (wfdrtpbuffer, "No clock, push right away");
      goto push_buffer;
    }

    GST_DEBUG_OBJECT (wfdrtpbuffer, "sync to timestamp %" GST_TIME_FORMAT,
        GST_TIME_ARGS (out_time));

    /* prepare for sync against clock */
    sync_time = get_sync_time (wfdrtpbuffer, out_time);
  ...
  lost:
    ...
    if (gap > 0) {
      ...
      GST_DEBUG_OBJECT (wfdrtpbuffer, "Packet #%d lost", next_seqnum);
      priv->num_late++;
      discont = TRUE;
    ...
    GST_DEBUG_OBJECT (wfdrtpbuffer, "First packet #%d synced", seqnum);
...
push_buffer:
  ...
  if (G_UNLIKELY (discont || priv->discont)) {
     ...
     * into the wfdrtpbuffer so we can modify now. */
  ...
  if (priv->npt_stop != -1 && priv->ext_timestamp != -1
    ...
    if (elapsed > priv->last_elapsed || !priv->last_elapsed) {
      ...
      GST_LOG_OBJECT (wfdrtpbuffer, "left %" GST_TIME_FORMAT,
          GST_TIME_ARGS (left));
      ...
      GST_LOG_OBJECT (wfdrtpbuffer, "elapsed %" GST_TIME_FORMAT ", estimated %"
...
#if 0
  if (wfd_rtp_buffer_num_packets(priv->jbuf) > DEFAULT_FLUSH_ON_QUEUE_LEN) {
      GstBuffer *lastbuf;
      guint16 last_elem_seqnum;
      GST_WARNING_OBJECT(wfdrtpbuffer,
                         "Queue threshold > %d exceeded, current packets to process %d",
                         DEFAULT_FLUSH_ON_QUEUE_LEN,
                         wfd_rtp_buffer_num_packets (priv->jbuf));
      /*
      JBUF_LOCK (priv);

      lastbuf = g_queue_peek_head (priv->jbuf->packets);
      last_elem_seqnum = gst_rtp_buffer_get_seq (lastbuf);

      while (wfd_rtp_buffer_num_packets(priv->jbuf) > 0) {
          if ((wfd_rtp_buffer_num_packets(priv->jbuf) % (DEFAULT_FLUSH_ON_QUEUE_LEN / 4)) == 0) {
              GST_WARNING_OBJECT(wfdrtpbuffer,
                                 "Removing packets... left = %d",
                                 wfd_rtp_buffer_num_packets (priv->jbuf));
          }
          gst_buffer_unref(wfd_rtp_buffer_pop(priv->jbuf, &percent));
      }
      GST_WARNING_OBJECT(wfdrtpbuffer,
                         "Queue flushed due to exceed > %d packets threshold",
                         DEFAULT_FLUSH_ON_QUEUE_LEN);
      wfd_rtp_buffer_reset_skew (priv->jbuf);
      priv->last_popped_seqnum = -1;
      priv->next_seqnum = (last_elem_seqnum + 1) & 0xffff;
...
do_eos:
  {
    ...
    GST_DEBUG_OBJECT (wfdrtpbuffer, "We are EOS, pushing EOS downstream");
...
do_npt_stop:
  {
    /* store result, we are flushing now */
    GST_DEBUG_OBJECT (wfdrtpbuffer, "We reached the NPT stop");
    JBUF_UNLOCK (priv);

    g_signal_emit (wfdrtpbuffer,
        gst_wfd_rtp_buffer_signals[SIGNAL_ON_NPT_STOP], 0, NULL);
    return;
  }
flushing:
  {
...
pause:
  {
    GST_DEBUG_OBJECT (wfdrtpbuffer, "pausing task, reason %s",
...
static GstFlowReturn
gst_wfd_rtp_buffer_chain_rtcp (GstPad * pad, GstBuffer * buffer)
{
  GstWfdRTPBuffer *wfdrtpbuffer;
  GstWfdRTPBufferPrivate *priv;
  GstFlowReturn ret = GST_FLOW_OK;
  ...
  if (base_rtptime == -1 || clock_rate == -1 || base_time == -1) {
    GST_DEBUG_OBJECT (wfdrtpbuffer, "dropping, no RTP values");
    drop = TRUE;
  } else {
    /* we can't accept anything that happened before we did the last resync */
    if (base_rtptime > ext_rtptime) {
      GST_DEBUG_OBJECT (wfdrtpbuffer, "dropping, older than base time");
      drop = TRUE;
    } else {
      ...
      if (ext_rtptime > last_rtptime) {
        ...
        if (diff > clock_rate) {
          GST_DEBUG_OBJECT (wfdrtpbuffer, "too far ahead");
        ...
        GST_DEBUG_OBJECT (wfdrtpbuffer, "ext last %" G_GUINT64_FORMAT ", diff %"
  ...
  if (!drop) {
    ...
    GST_DEBUG_OBJECT (wfdrtpbuffer, "signaling sync");
    g_signal_emit (wfdrtpbuffer,
        gst_wfd_rtp_buffer_signals[SIGNAL_HANDLE_SYNC], 0, s);
    gst_structure_free (s);
  } else {
    GST_DEBUG_OBJECT (wfdrtpbuffer, "dropping RTCP packet");
    ret = GST_FLOW_OK;
  }
...
ignore_buffer:
  {
    GST_DEBUG_OBJECT (wfdrtpbuffer, "ignoring RTCP packet");
    ret = GST_FLOW_OK;
    goto done;
  }
}
...
static gboolean
gst_wfd_rtp_buffer_query (GstPad * pad, GstQuery * query)
{
  GstWfdRTPBuffer *wfdrtpbuffer;
  GstWfdRTPBufferPrivate *priv;
  gboolean res = FALSE;
  ...
  switch (GST_QUERY_TYPE (query)) {
    case GST_QUERY_LATENCY:
    {
      ...
      if ((res = gst_pad_peer_query (priv->sinkpad, query))) {
        ...
        GST_DEBUG_OBJECT (wfdrtpbuffer, "Peer latency: min %"
        ...
        GST_DEBUG_OBJECT (wfdrtpbuffer, "Our latency: %" GST_TIME_FORMAT,
        ...
        GST_DEBUG_OBJECT (wfdrtpbuffer, "Calculated total latency : min %"
            GST_TIME_FORMAT " max %" GST_TIME_FORMAT,
            GST_TIME_ARGS (min_latency), GST_TIME_ARGS (max_latency));
    ...
    case GST_QUERY_POSITION:
    {
      ...
      GST_DEBUG_OBJECT (wfdrtpbuffer, "npt start %" GST_TIME_FORMAT
  ...
  switch (prop_id) {
    case PROP_LATENCY:
    {
      ...
      if (new_latency != old_latency) {
        GST_DEBUG_OBJECT (wfdrtpbuffer, "latency changed to: %" GST_TIME_FORMAT,
            GST_TIME_ARGS (new_latency * GST_MSECOND));

        gst_element_post_message (GST_ELEMENT_CAST (wfdrtpbuffer),
            gst_message_new_latency (GST_OBJECT_CAST (wfdrtpbuffer)));
      }
      break;
    }
