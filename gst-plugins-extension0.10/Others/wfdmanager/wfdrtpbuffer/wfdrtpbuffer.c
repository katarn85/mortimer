...
{
  static GType wfd_rtp_buffer_mode_type = 0;
  static const GEnumValue wfd_rtp_buffer_modes[] = {
  ...
  if (!wfd_rtp_buffer_mode_type) {
    wfd_rtp_buffer_mode_type =
        g_enum_register_static ("WfdRTPBufferMode", wfd_rtp_buffer_modes);
  }
  return wfd_rtp_buffer_mode_type;
}
 ...
 *
 * Get the current wfdrtpbuffer mode.
 *
 * Returns: the current wfdrtpbuffer mode.
 */
WfdRTPBufferMode
...
no_skew:
  ...
  if (jbuf->base_time != -1) {
    ...
    if (time != -1 && out_time + jbuf->delay < time) {
       ...
       * timestamp, we need to reset the wfdrtpbuffer. Likely the server paused
       * temporarily */
      GST_DEBUG ("out %" GST_TIME_FORMAT " + %" G_GUINT64_FORMAT " < time %"
          GST_TIME_FORMAT ", reset wfdrtpbuffer", GST_TIME_ARGS (out_time),
 ...
 */
gboolean
wfd_rtp_buffer_insert (WfdRTPBuffer * jbuf, GstBuffer * buf,
 ...
 * with wfd_rtp_buffer_set_tail_changed() to be notified when an older packet
 ...
 * @jbuf: an #WfdRTPBuffer
 *
 * Flush all packets from the wfdrtpbuffer.
 */
void
wfd_rtp_buffer_flush (WfdRTPBuffer * jbuf)
{
  GstBuffer *buffer;
 ...
 * @jbuf: an #WfdRTPBuffer
 *
 * Check if @jbuf is buffering currently. Users of the wfdrtpbuffer should not
 ...
 * @jbuf: an #WfdRTPBuffer
 *
 * Get the buffering percent of the wfdrtpbuffer.
 *
