...
gst_rtsp_client_class_init (GstRTSPClientClass * klass)
{
  ...
  gst_rtsp_client_signals[SIGNAL_TEARDOWN] =
        g_signal_new ("teardown_noti", G_TYPE_FROM_CLASS (klass), G_SIGNAL_RUN_LAST,
        G_STRUCT_OFFSET (GstRTSPClientClass, on_teardown), NULL, NULL,
        g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0, G_TYPE_NONE);
...
{
  client->finalize_call = FALSE;

#ifdef ENABLE_WFD_EXTENDED_FEATURES
  gst_rtsp_client_wfd_init(client);
  __gst_extended_feature_open(client);
  if (client->extended_feature_support) {
    void (*func)(GstRTSPClient *) = __gst_extended_func(client, EXT_INIT);
    func(client);
  }
#endif
}
...
#ifdef ENABLE_WFD_EXTENDED_FEATURES
  gst_rtsp_client_wfd_finalize(client);
  if (client->extended_feature_support) {
    void (*func)(GstRTSPClient *) = __gst_extended_func(client, EXT_FINALIZE);
    func(client);
    __gst_extended_feature_close(client);
  }
#endif
...
find_media (GstRTSPClient * client, GstRTSPClientState * state)
{
  ...
  if (!compare_uri (client->uri, state->uri)) {
    ...
    GST_DEBUG ("Going to find factory for media mapping...");
    ...
    GST_DEBUG ("Going to get media factory auth...");
    ...
    GST_DEBUG ("Going to do media factory construct...");

    if (client->connection_mode == WIFI_DISPLAY) {
      GST_DEBUG ("Setting source bin...");
      gst_rtsp_media_factory_set_srcbin (factory, GST_ELEMENT(client->srcbin->srcbin));
    } else {
      gst_rtsp_media_factory_set_connection_mode(factory, client->connection_mode);
    }
    ...
    GST_DEBUG ("Going to do prepare media...");

    /* prepare the media */
    if (!(gst_rtsp_media_prepare (media)))
      goto no_prepare;

    GST_DEBUG ("prepare media done...");
...
handle_teardown_request (GstRTSPClient * client, GstRTSPClientState * state)
{
  ...
  if (client->waiting_teardown) {
    client->waiting_teardown = FALSE;
    g_source_remove(client->teardown_timerid);
  }
...
handle_get_param_request (GstRTSPClient * client, GstRTSPClientState * state)
{
  ...
  if (size == 0) {
    ...
    if (client->connection_mode == WIFI_DISPLAY) {
      if (!handle_wfd_get_param_request(client, state))
        goto bad_request;
    } else {
...
handle_set_param_request (GstRTSPClient * client, GstRTSPClientState * state)
{
  ...
  if (size == 0) {
    ...
    if (client->connection_mode == WIFI_DISPLAY) {
      if (!handle_wfd_set_param_request(client, state))
        goto bad_request;
    } else {
      /* there is a body, handle the params */
...
#ifdef ENABLE_WFD_EXTENDED_FEATURES
  if (client->extended_feature_support) {
    void (*func)(GstRTSPClient *) = __gst_extended_func (client, EXT_SEND_T2_E_MSG);
    func(client);
  }
#endif
  }
...
handle_play_request (GstRTSPClient * client, GstRTSPClientState * state)
{
  ...
  for (i = 0, infocount = 0; i < n_streams; i++) {
    ...
    if(stream == NULL) {
      GST_ERROR ("Stream doesn't exist");
      continue;
    }
...
#ifdef ENABLE_WFD_EXTENDED_FEATURES
    if (client->extended_feature_support && client->connection_mode == WIFI_DISPLAY) {
      GST_DEBUG ("Reading the data on UDP src pad");
      void (*func)(GstRTSPClient *, GstRTSPMediaStream *) = __gst_extended_func(client, EXT_ADD_UDPSRC_PAD_PROBE);
      func(client, stream);
    }
#endif
  }
  ...
  gst_rtsp_message_init_response (state->response, code,
  gst_rtsp_status_as_text (code), state->request);
  g_printf("\n --Send M7 PLAY response-- \n");
  send_response (client, NULL, state->response);

  /* start playing after sending the request */
  GST_INFO("set media to GST_STATE_PLAYING");
  g_printf("\n --set media to GST_STATE_PLAYING-- \n");
  gst_rtsp_session_media_set_state (media, GST_STATE_PLAYING);

  media->state = GST_RTSP_STATE_PLAYING;

  if(client->connection_mode == WIFI_DISPLAY && client->uibc->neg_gen_capability)
    uibc_enable_request(client);

  client->client_state = CLIENT_STATE_CONNECTED;

#ifdef ENABLE_WFD_EXTENDED_FEATURES
  if (client->B_mode_requested) {
    client->B_mode_requested = FALSE;

    if (client->extended_feature_support && client->connection_mode == WIFI_DISPLAY) {
      GST_INFO("There was Video Only signalling. Get started to switch to B mode");
      void (*func)(void *, GstRTSPClient *) = __gst_extended_func(client, EXT_SWITCH_TO_B_MODE);
      func(NULL, client);
    }
  }
#endif
...
handle_setup_request (GstRTSPClient * client, GstRTSPClientState * state)
{
  ...
  GstRTSPUrl *uri;
  gchar *transport;
  gchar *user_agent; //Changes for testing with dongle
  gchar **transports;
  ...
  if (uri->abspath == NULL || !(pos = strstr (uri->abspath, stream_str))) {
    if (uri->query == NULL || !(pos = strstr (uri->query, stream_str)))
    {
      GST_ERROR ("bad request...");
      goto bad_request;
    }
  }
  ...
  *pos = '\0';
  ...
  pos += strlen (stream_str);
  if (sscanf (pos, "%u", &streamid) != 1)
    goto bad_request;

  res = gst_rtsp_message_get_header (state->request, GST_RTSP_HDR_USER_AGENT,
      &user_agent, 0);  //Changes for testing with dongle
  ...
  if (ct->lower_transport == GST_RTSP_LOWER_TRANS_UDP_MCAST) {
    ...
    ct->port.min = client->crtp_port0; //Changes for testing with dongle
  ...
  if (media == NULL) {
    ...
    // TODO: need to decide on arguments...
    if (client->connection_mode == WIFI_DISPLAY) {
      gst_rtsp_client_create_srcbin (client);
      if (NULL == client->srcbin) {
        GST_ERROR_OBJECT (client, "Failed to create src bin...");
        goto no_stream;
      }
    }
...
create_sdp (GstRTSPClient * client, GstRTSPMedia * media)
{
  ...
  if (media->protocols & GST_RTSP_LOWER_TRANS_UDP_MCAST)
    ...
    info.server_ip =  g_strndup (client->server_ip, INET6_ADDRSTRLEN);
...
handle_request (GstRTSPClient * client, GstRTSPMessage * request)
{
  ...
  if (client->connection_mode == WIFI_DISPLAY) gst_wfd_print_rtsp_msg(method);
  ...
  GST_DEBUG ("state -> URI = %s", uri->abspath);
  ...
  if (res == GST_RTSP_OK) {
    if (client->session_pool == NULL)
      goto no_pool;
    GST_DEBUG ("sessid = %s", sessid);
    token = strtok(sessid, ";");
    GST_DEBUG("token =%s", token);
    ...
    if(client->sessionID) g_free(client->sessionID);
    client->sessionID = g_strdup(sessid);
  ...
  GST_DEBUG ("session = %p", session);
  state.session = session;


  if(session) {
    GstRTSPSessionMedia *temp = session->medias->data;
    temp->url->abspath = g_strdup(uri->abspath);
  }
  ...
  GST_INFO ("Method = %d", method);

  /* now see what is asked and dispatch to a dedicated handler */
  switch (method) {
    case GST_RTSP_OPTIONS:
      if (client->connection_mode == RTSP_NORMAL) handle_options_request (client, &state);
      break;
    ...
    case GST_RTSP_PLAY:
      ...
      if (client->connection_mode == WIFI_DISPLAY) set_keep_alive_condition(client);
      break;
    case GST_RTSP_PAUSE:
...
handle_data (GstRTSPClient * client, GstRTSPMessage * message)
{
  ...
  for (walk = client->streams; walk; walk = g_list_next (walk)) {
    ...
    if (tr->lower_transport == GST_RTSP_LOWER_TRANS_TCP) {
      GST_INFO ("Transport is GST_RTSP_LOWER_TRANS_TCP");
      /* dispatch to the stream based on the port number */
      if (client->crtp_port0) {
        gst_rtsp_media_stream_rtp (mstream, buffer);
        handled = TRUE;
        break;
      } else if (client->crtp_port1) {
...
/**
 * gst_rtsp_client_set_connection_mode:
 * @client: a #GstRTSPClient
 * @auth: a #GstRTSPAuth
 *
 ...
 */
void
gst_rtsp_client_set_connection_mode (GstRTSPClient * client, gint mode)
{
  g_return_if_fail (GST_IS_RTSP_CLIENT (client));

  client->connection_mode = mode;
}
    ...
    gpointer user_data)
{
  ...
  GST_INFO_OBJECT (client, "revd message method = %s", 
    message->type == GST_RTSP_MESSAGE_REQUEST ? "request" :
    message->type == GST_RTSP_MESSAGE_RESPONSE ? "reponse" : "data/unknown");

  if ((message->type == GST_RTSP_MESSAGE_RESPONSE) && gst_debug_category_get_threshold (rtsp_client_debug) >= GST_LEVEL_LOG) {
  ...
  switch (message->type) {
    ...
    case GST_RTSP_MESSAGE_RESPONSE:
      {
        if (client->connection_mode == WIFI_DISPLAY) {
           return gst_wfd_handle_response(client, message);
        }
      }
      break;
...
{
  g_signal_emit (client, gst_rtsp_client_signals[signo], 0, NULL);
}
...
gst_rtsp_client_accept (GstRTSPClient * client, GIOChannel * channel, GSource *source)
{
  ...
  client->client_ip = url->host;

///////////////////////////////////////////if (client->connection_mode == WIFI_DISPLAY)
if (client->connection_mode == WIFI_DISPLAY) 
{
  if (url) url->abspath = strdup("/wfd1.0");
  if (url) client->wfdsink_ip = url->host;

  if (url) {
    GST_INFO ("added new client %p ip %s:%d %s", client, url->host, url->port, url->abspath);
    GST_INFO ("WFD Sink IP : %s", client->wfdsink_ip);
  }
}
//////////////////////////////////////////
  client->connection = conn;
#ifdef ENABLE_WFD_EXTENDED_FEATURES
  client->uristr = g_strdup(gst_rtsp_connection_get_ip(conn));
#endif
