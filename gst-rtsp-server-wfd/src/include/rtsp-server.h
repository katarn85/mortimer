...
typedef void (*server_cb_t)(gboolean is_error, void *userdata);
...
struct _GstRTSPServer {
  ...
  /* RTSP server mode : 0 - Normal, 1 - Wifi Display */
...
struct _GstRTSPServerClass {
  ...
  void * (*create_client) (GstRTSPServer *server);
  gboolean        (*accept_client) (GstRTSPServer *server, void *_client, GIOChannel * channel);
};

GType                 gst_rtsp_server_get_type             (void);

GstRTSPServer *       gst_rtsp_server_new                  (server_cb_t user_cb, void * user_param);
...
void                  gst_rtsp_server_set_session_pool     (GstRTSPServer *server, void *_pool);
void *  gst_rtsp_server_get_session_pool     (GstRTSPServer *server);

void                  gst_rtsp_server_set_media_mapping    (GstRTSPServer *server, void *_mapping);
void * gst_rtsp_server_get_media_mapping    (GstRTSPServer *server);

void                  gst_rtsp_server_set_auth             (GstRTSPServer *server, void *_auth);
void *         gst_rtsp_server_get_auth             (GstRTSPServer *server);

void   gst_rtsp_server_set_connection_mode (GstRTSPServer * server, gint mode);
gint gst_rtsp_server_get_connection_mode (GstRTSPServer * server);
...
int gst_rtsp_server_create_socket (GstRTSPServer * server);
void * gst_rtsp_server_accept_client (GstRTSPServer * server);
