...
gst_rtsp_session_pool_find (GstRTSPSessionPool * pool, const gchar * sessionid)
{
  ...
  if(sessionid != NULL) {
    ...
    GList *sessionlist;
    sessionlist = g_hash_table_get_values (pool->sessions);
    result = (GstRTSPSession *)sessionlist->data;
...
create_session_id (GstRTSPSessionPool * pool)
{
  ...
  for (i = 0; i < 8; i++) {
    id[i] = g_random_int_range ('0', '9');
  }

  return g_strndup (id, 8);
#endif
}
 ...
 */
guint
gst_rtsp_session_pool_cleanup (void *_pool)
{
  guint result;
  GTimeVal now;
...
{
  gint timeout;
  gint64 cur_src_time = 0;
  GTimeVal now;

  /*g_source_get_time() will give time in microseconds.*/
  cur_src_time = g_source_get_time ((GSource *) psrc);
  now.tv_sec = cur_src_time / 1000000;
  now.tv_usec = cur_src_time % 1000000;
