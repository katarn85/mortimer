...
#define DEFAULT_LAUNCH         "filesrc location=/opt/usr/media/Videos/Color.mp4 ! qtdemux ! omx_mpeg4dec ! omx_h264enc ! rtph264pay name=pay0"
...
gst_rtsp_media_factory_init (GstRTSPMediaFactory * factory)
{
  ...
  factory->connection_mode = WIFI_DISPLAY;
}
...
gst_rtsp_media_factory_new (void)
{
  ...
  result = (void *)g_object_new (GST_TYPE_RTSP_MEDIA_FACTORY, NULL);
...
void
gst_rtsp_media_factory_set_srcbin (GstRTSPMediaFactory * factory,
    GstElement *srcbin)
{
  g_return_if_fail (GST_IS_RTSP_MEDIA_FACTORY (factory));
  g_return_if_fail (srcbin != NULL);

 GST_DEBUG ("Setting factory %p with srcbin = %p", factory, srcbin);

  GST_RTSP_MEDIA_FACTORY_LOCK (factory);
  if (factory->srcbin)
	gst_object_unref (factory->srcbin);
  factory->srcbin = srcbin;
  GST_RTSP_MEDIA_FACTORY_UNLOCK (factory);
}

void
gst_rtsp_media_factory_set_connection_mode(GstRTSPMediaFactory *factory, gint mode)
{
  g_return_if_fail (GST_IS_RTSP_MEDIA_FACTORY (factory));

  GST_DEBUG ("Setting connection mode to %d", mode);

  GST_RTSP_MEDIA_FACTORY_LOCK (factory);
  factory->connection_mode = mode;
  GST_RTSP_MEDIA_FACTORY_UNLOCK (factory);
}
...
default_get_element (GstRTSPMediaFactory * factory, const GstRTSPUrl * url)
{
  ...
  if (factory->connection_mode == WIFI_DISPLAY) {
    element = factory->srcbin;
  } else {
    GError *error = NULL;
...
no_srcbin:
  {
    GST_RTSP_MEDIA_FACTORY_UNLOCK (factory);
    g_critical ("no source bin specified");
    return NULL;
  }
no_launch:
  {
