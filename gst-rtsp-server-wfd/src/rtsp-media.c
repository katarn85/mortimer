...
alloc_udp_ports (GstRTSPMedia * media, GstRTSPMediaStream * stream)
{
...
#ifdef ENABLE_WFD_EXTENDED_FEATURES
  GstElement *udpsrc0, *udpsrc1, *udpsrc2;
  GstElement *udpsink0, *udpsink1, *udpsink2;
  gint tmp_rtp, tmp_rtcp, tmp_feedback_rtcp;
  gint rtpport, rtcpport, feedback_rtcpport, sockfd;
  gint feedback_rtp_socket;
#else
  ...
  tmp_rtp = 19000;
#ifdef ENABLE_WFD_EXTENDED_FEATURES
  tmp_feedback_rtcp =RTP_RETRANS_PORT+1;
#endif
#endif
  if (media->is_ipv6)
    host = "udp://[::0]";
...
again:
...
#ifdef ENABLE_WFD_EXTENDED_FEATURES
  /* allocate port # for RTCP now */
  udpsrc2 = gst_element_make_from_uri (GST_URI_SRC, host, NULL);
  if (udpsrc2 == NULL)
    goto no_udp_rtcp_protocol;

  /* set port */
  g_object_set (G_OBJECT (udpsrc2), "port", tmp_feedback_rtcp, NULL);
...
#ifdef ENABLE_WFD_EXTENDED_FEATURES
  g_object_get (G_OBJECT (udpsrc2), "port", &feedback_rtcpport, NULL);

  /* this should not happen... */
  if (rtpport != tmp_rtp || rtcpport != tmp_rtcp ||feedback_rtcpport != tmp_feedback_rtcp )
#else
...
#ifdef ENABLE_WFD_EXTENDED_FEATURES
  //udpsink2 = gst_element_factory_make ("multiudpsink", NULL);
  udpsink2 = gst_element_make_from_uri (GST_URI_SINK, "udp://192.168.49.20:19120", NULL);
  if (!udpsink2)
    goto no_udp_protocol;

  feedback_rtp_socket = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
  if(feedback_rtp_socket == -1) {
    goto no_udp_protocol;
  }
  g_object_set (G_OBJECT (udpsink2), "qos-dscp", 63, NULL);
  g_object_set (G_OBJECT (udpsink2), "sockfd", feedback_rtp_socket, NULL);
  g_object_set (G_OBJECT (udpsink2), "closefd", FALSE, NULL);
...
#ifdef ENABLE_WFD_EXTENDED_FEATURES
    g_object_set (G_OBJECT (udpsink2), "send-duplicates", FALSE, NULL);
#endif
  ...
  if (g_object_class_find_property (G_OBJECT_GET_CLASS (udpsink0),
    ...
    g_object_set (G_OBJECT (udpsink0), "buffer-size", 200, NULL);
#ifdef ENABLE_WFD_EXTENDED_FEATURES
    g_object_set (G_OBJECT (udpsink2), "buffer-size", 202, NULL);
#endif
  } else {
...
#ifdef ENABLE_WFD_EXTENDED_FEATURES
  g_object_set (G_OBJECT (udpsink2), "auto-multicast", FALSE, NULL);
  g_object_set (G_OBJECT (udpsink2), "loop", FALSE, NULL);
#endif
...
#ifdef ENABLE_WFD_EXTENDED_FEATURES
  stream->udpsrc[2] = udpsrc2;
  stream->udpsink[2] = udpsink2;
#endif

  stream->server_port.min = rtpport;
#if 1 //Changes for testing with dongle
  stream->server_port.max = rtcpport;
#else
  stream->server_port.max = -1;
#endif
  return TRUE;
...
#ifdef ENABLE_WFD_EXTENDED_FEATURS
    if (udpsrc2) {
      gst_element_set_state (udpsrc2, GST_STATE_NULL);
      gst_object_unref (udpsrc2);
    }
    if (udpsink2) {
      gst_element_set_state (udpsink2, GST_STATE_NULL);
      gst_object_unref (udpsink2);
    }
#endif
    return FALSE;
  }
...
handle_new_buffer (GstAppSink * sink, gpointer user_data)
{
  ...
  for (walk = stream->transports; walk; walk = g_list_next (walk)) {
    ...
    if (GST_ELEMENT_CAST (sink) == stream->appsink[0]) { 
      if (tr->send_rtp) {
        tr->send_rtp (buffer, tr->transport->port.min, tr->user_data);
      }
    } else {
      if (tr->send_rtcp)
      tr->send_rtcp (buffer, tr->transport->port.max, tr->user_data);
    }
  }
...
setup_stream (GstRTSPMediaStream * stream, guint idx, GstRTSPMedia * media)
{
  ...
  media->resender = gst_element_factory_make ("rtpresender", NULL);
  gst_bin_add (GST_BIN_CAST (media->pipeline), media->resender);

  pad = gst_element_get_static_pad (media->resender, "rtp_sink");
...
#ifdef ENABLE_WFD_EXTENDED_FEATURES
  resenderpad = gst_element_get_static_pad (media->resender, "send_src");
  pad = gst_element_get_static_pad (stream->tee[0], "sink");
  gst_pad_link (resenderpad, pad);
...
#ifdef ENABLE_WFD_EXTENDED_FEATURES
  /* make tee for feedback RTP */
  stream->tee[2] = gst_element_factory_make ("tee", NULL);
  gst_bin_add (GST_BIN_CAST (media->pipeline), stream->tee[2]);

  pad = gst_element_get_static_pad (stream->tee[2], "sink");
  resenderpad = gst_element_get_static_pad (media->resender, "resend_src");
  gst_pad_link (resenderpad, pad);
  gst_object_unref (resenderpad);
  gst_object_unref (pad);

  /* link feedback RTP elements */
  teepad = gst_element_get_request_pad (stream->tee[2], "src%d");
  pad = gst_element_get_static_pad (stream->udpsink[2], "sink");
  gst_pad_link (teepad, pad);
  gst_object_unref (pad);
  gst_object_unref (teepad);

  teepad = gst_element_get_request_pad (stream->tee[2], "src%d");
  pad = gst_element_get_static_pad (stream->appsink[2], "sink");
  gst_pad_link (teepad, pad);
  gst_object_unref (pad);
  gst_object_unref (teepad);
#endif
...
#ifdef ENABLE_WFD_EXTENDED_FEATURES
  /* make selector for the feedback RTCP receivers */
  stream->selector[2] = gst_element_factory_make ("rtspfunnel", NULL);
  gst_bin_add (GST_BIN_CAST (media->pipeline), stream->selector[2]);

  resenderpad = gst_element_get_static_pad (media->resender, "rtcp_sink");
  pad = gst_element_get_static_pad (stream->selector[2], "src");
  gst_pad_link (pad, resenderpad);
  gst_object_unref (resenderpad);
  gst_object_unref (pad);

  /* make resender for RTCP and link to stream */
  selpad = gst_element_get_request_pad (stream->selector[2], "sink%d");
  ...
  selpad = gst_element_get_request_pad (stream->selector[2], "sink%d");
  pad = gst_element_get_static_pad (stream->appsrc[2], "src");
  gst_pad_link (pad, selpad);
  gst_object_unref (pad);
  gst_object_unref (selpad);
#endif
...
#ifdef ENABLE_WFD_EXTENDED_FEATURES
  gst_element_set_locked_state (stream->udpsrc[2], TRUE);
#endif
...
#ifdef ENABLE_WFD_EXTENDED_FEATURES
    gst_element_set_locked_state (stream->udpsrc[2], FALSE);
#endif
  }
}
...
default_handle_message (GstRTSPMedia * media, GstMessage * message)
{
  ...
  switch (type) {
    ...
    case GST_MESSAGE_ERROR:
    {
      ...
      GST_DEBUG_BIN_TO_DOT_FILE(GST_BIN(media->pipeline), GST_DEBUG_GRAPH_SHOW_ALL, "wfd-error");
    ...
    case GST_MESSAGE_ASYNC_DONE:
      if (!media->adding) {
        ...
        collect_media_stats (media);
        GST_INFO ("Setting again to PAUSED state....");
        if (media->target_state != GST_STATE_PLAYING){
          gst_element_set_state (media->pipeline, GST_STATE_PAUSED);
...
gst_rtsp_media_prepare (GstRTSPMedia * media)
{
  ...
  g_object_set (media->rtpbin, "latency", 0, NULL);
  ...
  /*To make udpsrc non blocking*/
  GstRTSPMediaStream *stream;
  stream = gst_rtsp_media_get_stream (media, 0);
  if(stream != NULL) {
    GST_ERROR ("udpsrc0 stream is not NULL");
    gint sockfd = 0;
    g_object_get(G_OBJECT (stream->udpsrc[0]), "sock", &sockfd, NULL);
    GST_ERROR ("udpsrc0 sockfd is %d", sockfd);
    fcntl (sockfd, F_SETFL, O_NONBLOCK);
  } else {
    GST_ERROR ("stream is NULL");
  }
  ...
  //GST_DEBUG_BIN_TO_DOT_FILE(media->pipeline, GST_DEBUG_GRAPH_SHOW_ALL, "test-server-arch");
...
#ifdef ENABLE_WFD_EXTENDED_FEATURES
    g_signal_emit_by_name (stream->udpsink[2], "add", dest, RTP_RETRANS_PORT, NULL);
#endif
...
#ifdef ENABLE_WFD_EXTENDED_FEATURES
    g_signal_emit_by_name (stream->udpsink[2], "remove", dest, RTP_RETRANS_PORT, NULL);
#endif
  ...
  if (media->target_state != state) {
    if (do_state) {
      if (state == GST_STATE_NULL) {
        ...
        if(ret == GST_STATE_CHANGE_FAILURE) /* No need to panic for GST_STATE_CHANGE_ASYNC or GST_STATE_CHANGE_NO_PREROLL those are valid */
          GST_ERROR_OBJECT (media, "Pipeline state change failed.");
      }
    }
