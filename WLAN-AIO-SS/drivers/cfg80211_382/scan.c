...
{
    if (NULL == request) {
        printk (KERN_ERR "[ath6kl_err] cfg80211_scan_request NULL argument!\n") ;
        return ;
    }
	trace_cfg80211_scan_done(request, aborted);
    if (NULL == request->wiphy) {
        printk (KERN_ERR "%s %d request->wiphy be NULL \n",__func__,__LINE__);
        return;
    }
    if (NULL == wiphy_to_dev(request->wiphy)) {
        printk (KERN_ERR "%s %d wiphy_to_dev be NULL \n",__func__,__LINE__);
        return;
    }
	WARN_ON(request != wiphy_to_dev(request->wiphy)->scan_req);
	request->aborted = aborted;
    if (NULL == cfg80211_wq) {
        printk (KERN_ERR "%s %d cfg80211_wq be NULL \n",__func__,__LINE__);
        return;
    }
    if (NULL == &wiphy_to_dev(request->wiphy)->scan_done_wk) {
        printk (KERN_ERR "%s %d scan_done_wk be NULL \n",__func__,__LINE__);
        return;
    }
	      ...
	      char *end_buf)
{
	...
	while (ies && rem >= 2) {
		...
		case WLAN_EID_MESH_CONFIG:
			...
			memset(&iwe, 0, sizeof(iwe));
			iwe.cmd = IWEVCUSTOM;
			snprintf(buf, 50, "Mesh Network Path Selection Protocol ID: "
				"0x%02X", cfg[0]);
			iwe.u.data.length = strlen(buf);
			current_ev = iwe_stream_add_point(info, current_ev,
							  end_buf,
							  &iwe, buf);
			snprintf(buf, 50, "Path Selection Metric ID: 0x%02X",
				cfg[1]);
			iwe.u.data.length = strlen(buf);
			current_ev = iwe_stream_add_point(info, current_ev,
							  end_buf,
							  &iwe, buf);
			snprintf(buf, 50, "Congestion Control Mode ID: 0x%02X",
				cfg[2]);
			iwe.u.data.length = strlen(buf);
			current_ev = iwe_stream_add_point(info, current_ev,
							  end_buf,
							  &iwe, buf);
			snprintf(buf, 50, "Synchronization ID: 0x%02X", cfg[3]);
			iwe.u.data.length = strlen(buf);
			current_ev = iwe_stream_add_point(info, current_ev,
							  end_buf,
							  &iwe, buf);
			snprintf(buf, 50, "Authentication ID: 0x%02X", cfg[4]);
			iwe.u.data.length = strlen(buf);
			current_ev = iwe_stream_add_point(info, current_ev,
							  end_buf,
							  &iwe, buf);
			snprintf(buf, 50, "Formation Info: 0x%02X", cfg[5]);
			iwe.u.data.length = strlen(buf);
			current_ev = iwe_stream_add_point(info, current_ev,
							  end_buf,
							  &iwe, buf);
			snprintf(buf, 50, "Capabilities: 0x%02X", cfg[6]);
			iwe.u.data.length = strlen(buf);
			current_ev = iwe_stream_add_point(info, current_ev,
							  end_buf,
							  &iwe, buf);
