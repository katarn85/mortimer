...
#define ATH6KL_PRINT_BUF_MAX_COUNT    256
#define ATH6KL_PRINT_BUF_SIZE    256

char ath6kl_print_buf[ATH6KL_PRINT_BUF_MAX_COUNT][ATH6KL_PRINT_BUF_SIZE] = {{0}};
u8 ath6kl_print_buf_count = 0;

int ath6kl_printk(const char *level, const char *fmt, ...)
{
#if defined(CONFIG_KERNEL_3_8_2) || defined(CONFIG_KERNEL_3_5_7) ||  \
    defined(CONFIG_KERNEL_3_10_9) || defined(CONFIG_KERNEL_3_4_39)
...
#define KERNEL_VERSION(_a,_b,_c) _a*1000+_b*100+_c
	...
	memset(ath6kl_print_buf[ath6kl_print_buf_count], 0, ATH6KL_PRINT_BUF_SIZE);

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,36))
	vaf.fmt = fmt;
	vaf.va = &args;

	if(!(debug_mask_ext & (unsigned int)((unsigned long long)(ATH6KL_DBG_EXT_PRINT_BUF)>>32)))
		rtn = printk(KERN_INFO "%sath6kl: %pV", level, &vaf);
	else{
		do_gettimeofday(&tv);
		sprintf(ath6kl_print_buf[ath6kl_print_buf_count], "%02d:%02d:%02d.%06d ath6kl: %pV", 
			(u_int32_t)((tv.tv_sec%3600*24)/3600),
			(u_int32_t)((tv.tv_sec%3600)/60),
			(u_int32_t)(tv.tv_sec%60),
	...
	if(!(debug_mask_ext & (unsigned int)((unsigned long long)(ATH6KL_DBG_EXT_PRINT_BUF)>>32))){
		...
		sprintf(ath6kl_print_buf[ath6kl_print_buf_count], fmt, args);
#endif
	if(debug_mask_ext & (unsigned int)((unsigned long long)(ATH6KL_DBG_EXT_PRINT_BUF)>>32))
		ath6kl_print_buf_count = (ath6kl_print_buf_count + 1)%ATH6KL_PRINT_BUF_MAX_COUNT;
...
void ath6kl_buf_print(void)
{
	u32 i, count;

	printk("ath6kl_buf_print Start\n");

	for(i=0; i< ATH6KL_PRINT_BUF_MAX_COUNT; i++){
		count = (ath6kl_print_buf_count + i)%ATH6KL_PRINT_BUF_MAX_COUNT;
		printk("%s", ath6kl_print_buf[count]);
	}

	printk("ath6kl_buf_print End\n");
}
    ...
    for (i = if_num; i < ar->vif_max; i++) {
        ...
        if (vif) {
            ath6kl_wmi_allow_aggr_cmd(vif->ar->wmi,
                    ...
                    (ampdu ? 0xff : 0),
                    (ampdu ? 0xff : 0));
            ath6kl_err("ampdu for if[%d] is [%d]%s\n", i, vif->fw_vif_idx, ampdu ? "enabled" : "disabled");
        }else
            ath6kl_err("failed to find if[%d] to set ampdu\n", i);
...
int ath6kl_parse_arp_offload_ip_addrs(const char __user *user_buf,
				size_t count,
				u8 *ip_addrs)
{
	char buf[64];
...
static ssize_t ath6kl_get_nf(struct file *file,
				char __user *user_buf,
				size_t count, loff_t *ppos)
{
	struct ath6kl *ar = file->private_data;
...
#define REG_CH1_NF_Ext 0x02a830
	...
	len = scnprintf(buf, sizeof(buf), "Ch0: %d %d(ext)\nCh1: %d %d(ext)\n",
                                    Reg_to_dB(ch0_nf),Reg_to_dB_Ext(ch0_nf_e),
                                    Reg_to_dB(ch1_nf),Reg_to_dB_Ext(ch1_nf_e));

	return simple_read_from_buffer(user_buf, count, ppos, buf, len);
}

static const struct file_operations fops_get_nf = {
	.read = ath6kl_get_nf,
	.open = ath6kl_debugfs_open,
	.owner = THIS_MODULE,
	.llseek = default_llseek,
				...
				size_t count, loff_t *ppos)
{
	...
	sscanf(p, "%d", &value);
	if (value > 10) {
		ath6kl_err("min_rx_bundle_frame: < 10\n\r");
		value = 10;
	}
	if (value < 0) {
		ath6kl_err("min_rx_bundle_frame: >= 0\n\r");
		value = 0;
	}
	...
	sscanf(p, "%d", &value);
	if (value > 255) {
		ath6kl_err("rx_bundle_timeout: <= 255\n\r");
		value = 255;
	}
	if (value < 0) {
		ath6kl_err("rx_bundle_timeout: >= 0\n\r");
		value = 0;
	}
...
static const struct file_operations fops_get_tx_rate = {
	.read = ath6kl_tx_rate_read,
	.open = ath6kl_debugfs_open,
	.owner = THIS_MODULE,
	.llseek = default_llseek,
};
...
int ath6kl_debug_init(struct ath6kl *ar)
{
	...
	debugfs_create_file("tgt_ap_stats", S_IRUSR | S_IWUSR | S_IROTH | S_IWOTH,
	...
	debugfs_create_file("get_tx_rate", S_IRUSR, 
				ar->debugfs_phy, ar, &fops_get_tx_rate);

    debugfs_create_file("get_nf", S_IRUSR | S_IRGRP | S_IROTH,
			    ar->debugfs_phy, ar, &fops_get_nf);
