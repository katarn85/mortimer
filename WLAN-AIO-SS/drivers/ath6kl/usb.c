...
#define RX_BUNDLE_URB_COUNT        4

#ifdef PRE_ALLOC
struct sk_buff *rx_skb_buf[RX_BUNDLE_URB_COUNT];
struct ath6kl_cookie *cookie_mem_data;
struct ath6kl_cookie *cookie_mem_ctrl;
struct htc_packet *data_htc_packet[MAX_COOKIE_DATA_NUM];
struct htc_packet *ctrl_htc_packet[MAX_COOKIE_CTRL_NUM];
#endif
...
static int ath6kl_usb_bus_state=0;
				       ...
				       u8 ep_address, int *urb_count)
{
	...
	case ATH6KL_USB_EP_ADDR_APP_DATA_IN:
		...
		*urb_count = (htc_bundle_recv)?RX_BUNDLE_URB_COUNT:RX_URB_COUNT;
		break;
				...
				int buffer_length)
{
	...
	while (1) {
		...
		if (buffer_length) {
#ifdef PRE_ALLOC
			printk("### Static Buf assign %d \n",buf_index);
			if (rx_skb_buf[buf_index] == NULL) {
                	//might never happen
				printk("###Try to recover it ~~~(%d)\n", buf_index);
				rx_skb_buf[buf_index] = __dev_alloc_skb(ATH6KL_USB_RX_BUNDLE_BUFFER_SIZE, GFP_KERNEL);
			}
			urb_context->buf = rx_skb_buf[buf_index];
			buf_index++;
#else
			urb_context->buf = dev_alloc_skb(buffer_length);
#endif
		...
		} else if (urb_context->buf == NULL) {
			printk(KERN_ERR "[ath6kl_debug] ath6kl: Should not happen ------\n"); 
			urb_context->buf = dev_alloc_skb(ATH6KL_USB_RX_BUNDLE_BUFFER_SIZE);
			if (urb_context->buf == NULL) {
			    ath6kl_usb_cleanup_recv_urb(urb_context);
			    break;
			}
...
struct sk_buff *g_overflow_buf = NULL;
...
static void ath6kl_usb_recv_complete(struct urb *urb)
{
...
#if defined(CE_OLD_KERNEL_SUPPORT_2_6_23) || defined(CE_INDIV_WORK_ITEM)
	...
	if ( urb->status == -EOVERFLOW ) {
		if(ATH6KL_USB_RX_BUFFER_SIZE != 2048) {
			g_overflow_buf = urb_context->buf;
			status = 0;
		} else {
	...
	} else if (g_overflow_buf != NULL && ATH6KL_USB_RX_BUFFER_SIZE != 2048) {
		new_buf = __dev_alloc_skb(g_overflow_buf->len + urb->actual_length, GFP_ATOMIC);

		if ( new_buf == NULL ) {
			dev_kfree_skb(g_overflow_buf);
			g_overflow_buf = NULL;
		} else {
			memcpy(new_buf->data, g_overflow_buf->data, g_overflow_buf->len);
			memcpy(new_buf->data+g_overflow_buf->len, urb_context->buf->data, urb->actual_length);
			dev_kfree_skb(g_overflow_buf);
			dev_kfree_skb(urb_context->buf);
			urb->actual_length += g_overflow_buf->len;
			urb_context->buf = new_buf;
			g_overflow_buf = NULL;
		}                     
	}
	...
	if (g_overflow_buf != NULL && ATH6KL_USB_RX_BUFFER_SIZE != 2048)
		goto cleanup_recv_urb;
...
static void ath6kl_usb_io_comp_work(struct work_struct *work)
{
	...
	struct ath6kl_usb_pipe_stat *pipe_st = &pipe->usb_pipe_stat;
	u32 tx = 0, rx = 0;
#if defined(CE_OLD_KERNEL_SUPPORT_2_6_23) || defined(USB_AUTO_SUSPEND) || defined(CE_INDIV_WORK_ITEM)
	struct ath6kl *ar = pipe->ar_usb->ar;
#endif

	if(ath6kl_usb_bus_state == 2){
		printk (KERN_ERR "[ath6kl_debug] %s:%d ath6kl_usb_bus_state is removed.\n", __FUNCTION__, __LINE__) ;
			       ...
			       void *v)
{
	struct ath6kl_usb *ar_usb;
	struct ath6kl *ar ;
        printk (KERN_ERR "[%s:%d] val=%ld\n", __FUNCTION__, __LINE__, val); 

	if (nb == NULL)
		return NOTIFY_DONE;

        if(val == SYS_RESTART){ 
                printk (KERN_ERR "[%s:%d] SYS_RESTART CALLED!!!\n", __FUNCTION__, __LINE__); 
...
static int ath6kl_usb_bmi_read(struct ath6kl *ar, u8 *buf, u32 len)
{
	...
	ret = ath6kl_usb_submit_ctrl_in(ar_usb,
					...
					0, 0, buf, len);
	if (ret != 0) {
		printk (KERN_ERR "Unable to read the bmi data from the device: %d\n",
			   ret);
		return ret;
	}
...
static int ath6kl_usb_bmi_write(struct ath6kl *ar, u8 *buf, u32 len)
{
	...
	ret = ath6kl_usb_submit_ctrl_out(ar_usb,
					 ...
					 0, 0, buf, len);
	if (ret != 0) {
		printk (KERN_ERR "unable to send the bmi data to the device: %d\n",
			   ret);
		return ret;
	}
...
{
	printk (KERN_ERR "[%s:%d]\n", __FUNCTION__, __LINE__);
#ifdef CONFIG_HW_CLOCK
    trace_early_message("[QCA] Wi-Fi ath6kl_usb_power_off() ");
#endif
	hif_detach_htc(ar);
	return 0;
}
			    ...
			    const struct usb_device_id *id)
{
    ...
    suspend_idx = 1;
	ath6kl_usb_bus_state = 1;
        // by ssu for representing version and date
        printk(KERN_ERR "QCA Wi-Fi Driver version : %s, 2014(PostRC2-Update) dongle_type=%d\n", DRV_VERSION, dongle_type);
#ifdef CONFIG_HW_CLOCK
        trace_early_message("[QCA] Wi-Fi Probe CB ");
#endif
        
        // make a version string
        snprintf(version_string, VERSION_STRING_LENGTH, "%s\n", DRV_VERSION);
	...
	if (dev->speed == USB_SPEED_HIGH){
		...
		printk (KERN_ERR "Mck doesn't support USB 1.1 Host, do USB reset\n");
		usb_1_1_host = 1;
	}
	...
	if (ar_usb == NULL) {
		printk (KERN_ERR "Failed to create USB interface\n");
		ret = -ENOMEM;
		goto err_usb_put;
	}
	...
	if (ar == NULL) {
		printk (KERN_ERR "Failed to alloc ath6kl core\n");
		ret = -ENOMEM;
	...
	if ((ret) || (usb_1_1_host)) {
		if (ret)
			printk (KERN_ERR "Failed to init ath6kl core: %d\n", ret);
		if (reset_cnt < 3) {
			reset_cnt++;
			printk (KERN_ERR "start USB reset: reset_cnt=%d\n", reset_cnt);
			ath6kl_reset_device(ar, ar->target_type, true, true);
		}
		if (ret)	 
		goto err_core_free;
...
extern u8 reset_skip;
static void ath6kl_usb_remove(struct usb_interface *interface)
{
	struct ath6kl_usb *ar_usb;
	printk (KERN_ERR "[%s:%d]\n", __FUNCTION__, __LINE__);
    
	ath6kl_usb_bus_state = 2;
			      ...
			      pm_message_t message)
{
	...
	printk (KERN_ERR "[%s:%d]\n", __FUNCTION__, __LINE__);
#ifdef CONFIG_HW_CLOCK
    trace_early_message("[QCA] Wi-Fi ath6kl_usb_pm_suspend() ");
#endif
#ifdef USB_AUTO_SUSPEND
	int ret;
#endif
...
static int ath6kl_usb_pm_resume(struct usb_interface *interface)
{
	...
	printk (KERN_ERR "[%s:%d]\n", __FUNCTION__, __LINE__);
#ifdef CONFIG_HW_CLOCK
    trace_early_message("[QCA] Wi-Fi ath6kl_usb_pm_resume() ");
#endif
...
#ifdef USB_CUSTOMIZE_SUSPEND
    if (ath6kl_usb_bus_state != 1) {
	printk (KERN_ERR "[%s:%d] ath6kl_usb_bus_state != 1\n", __FUNCTION__, __LINE__);
        ath6kl_usb_probe(interface, NULL);
    } else {
	printk (KERN_ERR "[%s:%d] ath6kl_usb_bus_state == 1\n", __FUNCTION__, __LINE__);
    }
#else
...
{ 
	...
	printk (KERN_ERR "[%s:%d]\n", __FUNCTION__, __LINE__);
#ifdef CONFIG_HW_CLOCK
    trace_early_message("[QCA] Wi-Fi ath6kl_usb_pm_reset_resume() ");
#endif
    if (ath6kl_usb_bus_state != 1)
        ath6kl_usb_probe(intf, NULL);
#else
	struct ath6kl_usb *device;
	struct ath6kl *ar;
...
static struct usb_device_id ath6kl_usb_ids_external[] = {
	{USB_DEVICE(0x04E8, 0x2085)},
	{ /* Terminating entry */ },
};  // for ext-dongle by max.park

MODULE_DEVICE_TABLE(usb, ath6kl_usb_ids_external);  // for ext-dongle by max.park
...
static int ath6kl_usb_init(void)
{
	printk (KERN_ERR "[%s:%d]\n", __FUNCTION__, __LINE__);
#ifdef CONFIG_HW_CLOCK
    trace_early_message("[QCA] Wi-Fi Driver Module Init ");
#endif
...
static int ath6kl_usb_init(void)
{
    ...
    printk (KERN_ERR "[%s:%d] QCA Wi-Fi Driver Init started from here.\n", __FUNCTION__, __LINE__) ;

    for (i = 0; i < RX_BUNDLE_URB_COUNT; i++) {
        rx_skb_buf[i] = __dev_alloc_skb(ATH6KL_USB_RX_BUNDLE_BUFFER_SIZE, GFP_KERNEL);
        if (rx_skb_buf[i] == NULL)
            ath6kl_err("### (%d): Alloc Fail ###\n", i);
    }
    cookie_mem_data = kzalloc((sizeof(struct ath6kl_cookie) * MAX_COOKIE_DATA_NUM), GFP_KERNEL);
    cookie_mem_ctrl = kzalloc((sizeof(struct ath6kl_cookie) * MAX_COOKIE_CTRL_NUM), GFP_KERNEL);

    for (i = 0; i < MAX_COOKIE_DATA_NUM; i++) {
        data_htc_packet[i] = kzalloc(sizeof(struct htc_packet), GFP_KERNEL);
    }
    for (i = 0; i < MAX_COOKIE_CTRL_NUM; i++) {
        ctrl_htc_packet[i] = kzalloc(sizeof(struct htc_packet), GFP_KERNEL);
    }
#endif

    if (dongle_type == 1) {
        ath6kl_usb_driver.id_table = ath6kl_usb_ids_external;
    }  // for ext-dongle by max.park
...
static void ath6kl_usb_exit(void)
{
...
#ifdef PRE_ALLOC
	{
		...
		for (i = 0; i < RX_BUNDLE_URB_COUNT; i++) {
			if(rx_skb_buf[i])
				dev_kfree_skb(rx_skb_buf[i]);
		}
		for (i = 0; i < MAX_COOKIE_DATA_NUM; i++) {
		if(data_htc_packet[i])
                        kfree(data_htc_packet[i]);
                }
                for (i = 0; i < MAX_COOKIE_CTRL_NUM; i++) {
                        if(ctrl_htc_packet[i])
                        kfree(ctrl_htc_packet[i]);
                }
        }
	if (cookie_mem_data)
		kfree(cookie_mem_data);
	if(cookie_mem_ctrl)
