...
unsigned int dongle_type=0;  // for ext-dongle by max.park
char version_string[VERSION_STRING_LENGTH] = "0";  // for Wi-Fi UI display version

unsigned int debug_mask;
unsigned int debug_mask_ext;
unsigned int htc_bundle_recv=1;
...
unsigned int ath6kl_dsrc = ATH6KL_MODULEDSRC_DEF_MODE;
...
unsigned int ath6kl_softap_max_sta=4;    // for limit max number of gc
...
unsigned int ath6kl_txrxmask = 0;
module_param(dongle_type, uint, 0644);  // for ext-dongle by max.park
module_param_string(version_string, version_string, VERSION_STRING_LENGTH, 0644);   // for Wi-Fi Version Display 
...
module_param(ath6kl_dsrc, uint, 0644);
...
module_param(ath6kl_softap_max_sta, uint, 0644);
...
module_param(ath6kl_txrxmask, uint, 0644);
MODULE_PARM_DESC(ath6kl_txrxmask, 
                 "Manual config the tx rx mask, 0x1:Ch0 ; 02:Ch1");
...
static const struct ath6kl_hw hw_list[] = {
	...
	},
	{
		...
		.name				= "ar6004 hw 1.3",
		.dataset_patch_addr		= 0x437404,
		.app_load_addr			= 0x1234,
		.app_load_ext_addr		= 0x9a7000,
		.board_ext_data_addr		= 0x437000,
		.reserved_ram_size		= 7680,
		.board_addr			= 0x436200,
		.testscript_addr		= 0x434c00,
		.flags				= ATH6KL_HW_TGT_ALIGN_PADDING |
						  ATH6KL_HW_SINGLE_PIPE_SCHED |
...
static int ath6kl_target_config_wlan_params(struct ath6kl *ar, int idx)
{
	...
	if (ath6kl_softap_max_sta > 0 && idx ==0) {
		/* set max connected stas */
		ret = ath6kl_wmi_set_ap_num_sta_cmd(ar->wmi, idx, ath6kl_softap_max_sta);
		if (ret) {
			printk(KERN_ERR "ath6l: Failed to set max connected sta(%d) to SoftAP "
					"(%d)\n", ath6kl_softap_max_sta, ret);
		}else
			printk(KERN_ERR "ath6l: set max connected sta(%d) to SoftAP "
					"(%d)\n", ath6kl_softap_max_sta, ret);
	}
...
static int ath6kl_init_upload(struct ath6kl *ar)
{
    ...
    if (ath6kl_txrxmask)
    {
        ath6kl_bmi_read(ar,
				     ...
				     HI_ITEM(hi_option_flag2)),
				     (u8 *)&param, 4);
        if(ath6kl_txrxmask & 0x1)
            param |= HI_OPTION_TXRXMASK_CHAIN_0;
        if(ath6kl_txrxmask & 0x2)
            param |= HI_OPTION_TXRXMASK_CHAIN_1;
...
static int __ath6kl_init_hw_start(struct ath6kl *ar)
{
...
#else
		ret = ath6kl_fw_watchdog_cold_reset_enable(ar);
		if (ret != 0) {
			ath6kl_err("Failed enable fw watchdog cold reset %d\n",
				ret);
			return ret;
		}
	...
	if (!test_bit(TESTMODE_EPPING, &ar->flag)) {
		...
		if (test_and_clear_bit(FIRST_BOOT, &ar->flag)) {
			//printk(KERN_ERR "[ath6kl-firmware] %s %s fw %s%s\n",
			printk(KERN_ERR "[ath6kl-firmware] %s %s fw 5.0.1.25%s\n",
					ar->hw.name,
