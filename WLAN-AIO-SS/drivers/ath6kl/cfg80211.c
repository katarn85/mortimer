	...
	if (mcc) {
		...
		list_for_each_entry(vif_temp, &ar->vif_list, list) {
			if (test_bit(CONNECTED, &vif_temp->flags)) {
				...
				if (vif_temp->nw_type == AP_NETWORK) {
					ath6kl_ap_keepalive_config(vif_temp,
						ATH6KL_AP_KA_INTERVAL_SHORT,        //ATH6KL_AP_KA_INTERVAL_DEFAULT,
						ATH6KL_AP_KA_RECLAIM_CYCLE_SHORT);  //ATH6KL_AP_KA_RECLAIM_CYCLE_SCC);
				}
			}
		}
	}
...
int ath6kl_cfg80211_set_power_mgmt(struct wiphy *wiphy,
...
#define MAX_SCAN_PERIOD (ATH6KL_SCAN_FG_MAX_PERIOD * HZ)
	...
	if (under_scan_flag) {
		ath6kl_info("Skip the ROC when under scan ###\n");
		return -EBUSY;
	}
    
    ret = down_timeout(&ar->sem, msecs_to_jiffies(1000));
    if( ret < 0 ) {
        if ( ret == -ETIME) {
            ath6kl_err("%s, down_timeout timeout\n", __func__);
        }
        return -ERESTARTSYS;
    }
...
#define MAX_RD_WAIT_CNT	(20)	/* 20 * 100 = 2 sec. */
	...
	if (!ath6kl_mod_debug_quirks(ar,
		...
		if (!IS_STA_AP_ONLY(ar)) {
			...
			rtnl_unlock();
			printk (KERN_ERR "[QCA] Wi-Fi Create dedicated p2p interface completed.\n");
#ifdef CONFIG_HW_CLOCK
            trace_early_message("[QCA] Wi-Fi Create dedicated p2p interface completed.");
#endif
		}
		...
		if (ar->p2p_concurrent_ap) {
			...
			rtnl_unlock();
			printk(KERN_ERR "Create dedicated p2p interface completed!\n");
		}
	}
