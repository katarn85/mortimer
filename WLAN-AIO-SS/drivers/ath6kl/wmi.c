...
int ath6kl_wmi_dot3_2_dix(struct sk_buff *skb)
{
	...
	if(skb->len < (sizeof(eth_hdr) + sizeof(struct ath6kl_llc_snap_hdr)))
		return 0;
...
int ath6kl_wmi_control_rx(struct wmi *wmi, struct sk_buff *skb)
{
	...
	if (!test_bit(WLAN_ENABLED, &vif->flags)) {
		ath6kl_err("ath6kl_wmi_control_rx wlan disabled\n");
        up(&ar->wmi_evt_sem);
		dev_kfree_skb(skb);
		return -EINVAL;
	}
...
{
	struct sk_buff *skb;
	struct wmi_set_ocb_channel_op_cmd *cmd;

	skb = ath6kl_wmi_get_new_buf(sizeof(struct wmi_set_ocb_channel_op_cmd));
	if (!skb)
		return -ENOMEM;

	cmd = (struct wmi_set_ocb_channel_op_cmd *) skb->data;

	memcpy(cmd, channel_op_cmd, sizeof(struct wmi_set_ocb_channel_op_cmd));
...
{
    struct sk_buff *skb;
    struct wmi_set_sync_tsf_cmd *cmd;
	
	skb = ath6kl_wmi_get_new_buf(sizeof(*cmd));
	if (!skb)
		return -ENOMEM;
		
	cmd = (struct wmi_set_sync_tsf_cmd *)skb->data;
    cmd->sync_tsf_p = sync_tsf_p;
	
	return ath6kl_wmi_cmd_send(wmi, 0, skb, WMI_SET_SYNC_TSF_CMDID,
				NO_SYNC_WMIFLAG);
}
...
{
    struct sk_buff *skb;
    struct wmi_set_da_retry_limit_cmd *cmd;

	skb = ath6kl_wmi_get_new_buf(sizeof(*cmd));
	if (!skb)
		return -ENOMEM;

	cmd = (struct wmi_set_da_retry_limit_cmd *)skb->data;
    cmd->retry_limit = retry_limit;

	return ath6kl_wmi_cmd_send(wmi, 0, skb, WMI_SET_DA_RETRY_LIMIT_CMDID,
				NO_SYNC_WMIFLAG);
}

int ath6kl_wmi_set_rx_bundle_param(struct wmi *wmi, u8 if_idx,
	u8 min_rx_bundle_frame, u8 rx_bundle_timeout)
{
	struct sk_buff *skb;
	struct wmi_set_bundle_param_cmd *cmd;

    if(!htc_bundle_recv) {
        ath6kl_err("Rx bundle mode not enabled\n");
        return -EINVAL;
    }
	...
	ath6kl_dbg(ATH6KL_DBG_WMI,
		"rx bundle param: min_rx_bundle_frame %d ,rx_bundle_timeout %d\n",
		min_rx_bundle_frame, rx_bundle_timeout);

	cmd = (struct wmi_set_bundle_param_cmd *)skb->data;
	cmd->min_rx_bundle_frame = min_rx_bundle_frame;
	cmd->min_rx_bundle_timeout = rx_bundle_timeout;
	
	return ath6kl_wmi_cmd_send(wmi, if_idx, skb,
				WMI_SET_BUNDLE_PARAM_CMDID, NO_SYNC_WMIFLAG);
}

int ath6kl_wmi_usb_cut_cmd(struct wmi *wmi, u32 timeout)
{
    struct sk_buff *skb;
    struct wmi_usb_cut_cmd *cmd;
    
    skb = ath6kl_wmi_get_new_buf(sizeof(*cmd));
    if (!skb)
        return -ENOMEM;
    cmd = (struct wmi_usb_cut_cmd *)skb->data;
    cmd->timeout = timeout;

    return ath6kl_wmi_cmd_send(wmi, 0, skb,
                WMI_SET_USB_CUT_CMDID, NO_SYNC_WMIFLAG);
}
