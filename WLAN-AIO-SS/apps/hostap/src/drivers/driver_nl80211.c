...
#include "nl80211_copy382.h"
#elif defined(CONFIG_KERNEL_3_5_7)
#include "nl80211_copy357.h"
#elif defined(CONFIG_KERNEL_3_10_9)
#include "nl80211_copy3109.h"
#elif defined(CONFIG_KERNEL_3_4_39)
#include "nl80211_copy3439.h"
#else
...
/* Sub-IOCTL for acs list */
#define ATHR_CMD_ACS_2G_LIST            1	/* Scan 2GHz channel only */
#define ATHR_CMD_ACS_5G_LIST            2	/* Scan 5GHz channel only */

/* Remove ACS for AIO-SS since it is not supported at driver side */
#ifdef CONFIG_CE_2_SUPPORT
#undef ATHR_WLAN_SCAN_BAND
#undef ATHR_WLAN_FIND_BEST_CHANNEL
#undef ATHR_WLAN_ACS_LIST
#endif
...
struct wpa_driver_nl80211_data {
...
#ifdef ATHR_WLAN_FIND_BEST_CHANNEL
	...
	struct nl_msg *pending_scan_param;
	struct nl_handle *nl_mgmt_p2p;
};
...
static void seamless_roaming_stop(struct wpa_driver_nl80211_data *drv)
{
...
static int send_and_recv_dont_free_msg(struct nl80211_global *global,
			 struct nl_handle *nl_handle, struct nl_msg *msg,
			 int (*valid_handler)(struct nl_msg *, void *),
			 void *valid_data)
{
				  ...
				  struct nlattr *by_ap)
{
	...
	if (reason &&(nla_get_u16(reason) == 99)) {
		struct wpa_supplicant *wpa_s = (struct wpa_supplicant *)(drv->ctx);
#if 0 /* Send by supplciant event */	
		wpa_msg_ctrl(wpa_s, MSG_INFO, "WIFI-RECOVER-NEEDED");
#else /* Touch a new file under filesystem */
		if (system("touch /mtd_rwarea/network/wpa_supplicant_failed") < 0)
			wpa_msg_ctrl(wpa_s, MSG_INFO, "Fail to touch /mtd_rwarea/network/wpa_supplicant_failed");
		else
			wpa_msg_ctrl(wpa_s, MSG_INFO, "Success to touch /mtd_rwarea/network/wpa_supplicant_failed");
#endif
		return;
	}
				 ...
				 int cmd, struct nlattr **tb)
{
	...
	case NL80211_CMD_NEW_SCAN_RESULTS:
		...
		drv->scan_complete_events = 1;
		if(1){          
			struct nl_msg *pending_scan_same=NULL;
			struct nl_msg *pending_scan_other=NULL;
			struct wpa_driver_nl80211_data *driver;
                
			//check any pending scan and send
			dl_list_for_each(driver, &drv->global->interfaces,
        		struct wpa_driver_nl80211_data, list) {
				if(driver->pending_scan_param){
					if(driver == drv)
        					pending_scan_same = driver->pending_scan_param;
					else{
        					pending_scan_other = driver->pending_scan_param;
        					break;
					}
				}                     
			}

			if(pending_scan_other){
				wpa_driver_nl80211_scan(&driver->first_bss, NULL);
				//driver->pending_scan_param = NULL;
			}else if(pending_scan_same){
				wpa_driver_nl80211_scan(&drv->first_bss, NULL);
				//drv->pending_scan_param = NULL;
			}
		}
...
static int nl80211_mgmt_subscribe_non_ap(struct i802_bss *bss)
{
	...
	if ((os_memcmp(bss->ifname, "p2p", 3) == 0) &&
	    bss->drv->nl_mgmt_p2p) {
		bss->nl_mgmt = bss->drv->nl_mgmt_p2p;
		return 0;
	}
	...
	if (os_memcmp(bss->ifname, "p2p", 3) == 0) {
		bss->drv->nl_mgmt_p2p = bss->nl_mgmt;
	}
...
static void nl80211_mgmt_unsubscribe(struct i802_bss *bss, const char *reason)
{
	...
	if ((os_memcmp(bss->ifname, "p2p", 3) == 0) &&
	    (os_memcmp(reason, "deinit", 6) != 0)) {
		wpa_printf(MSG_DEBUG, "nl80211: SKIP Unsubscribe mgmt frames handle %p "
		   "(%s)\n", bss->nl_mgmt, reason);
		return;
	}
	...
	if ((os_memcmp(bss->ifname, "p2p", 3) == 0) &&
	    (os_memcmp(reason, "deinit", 6) == 0)) {
		wpa_printf(MSG_DEBUG, "nl80211: Unsubscribe mgmt frames handle %s (%d)\n",
		   __func__, __LINE__);
		bss->drv->nl_mgmt_p2p = NULL;
	}
...
static void wpa_driver_nl80211_deinit(void *priv)
{
	...
	if(drv->pending_scan_param){
		nlmsg_free(drv->pending_scan_param);
		drv->pending_scan_param = NULL;
	}
...
#ifdef ATHR_WLAN_SCAN_BAND
static int athr_driver_scanmode(struct i802_bss *bss, const char *cmd)
{
	int scanband = ATHR_CMD_SCANBAND_ALL;
	int ioctl_sock;
	struct ifreq ifr;
	athr_cmd_t athr_cmd_data;
	int retv = 0;

	if (os_strstr(cmd, "skip5g") != NULL)
		scanband = ATHR_CMD_SCANBAND_2G;
	else if (os_strstr(cmd, "skip2g") != NULL)
		scanband = ATHR_CMD_SCANBAND_5G;
	else if (os_strstr(cmd, "all") != NULL)
		scanband = ATHR_CMD_SCANBAND_ALL;
	else
		scanband = atoi(cmd);

	wpa_printf(MSG_DEBUG, "athr: Set scanmode %d for %s", scanband, bss->ifname);

	ioctl_sock = socket(PF_INET, SOCK_DGRAM, 0);
	if (ioctl_sock < 0) {
		perror("socket(PF_INET,SOCK_DGRAM)");
		printf("#### Set scanmode ioctrl error\n");
		return -1;
	}

	os_memset(&athr_cmd_data, 0, sizeof(athr_cmd_t));
	athr_cmd_data.cmd = ATHR_WLAN_SCAN_BAND;
	athr_cmd_data.data[0] = scanband;
	os_memset(&ifr, 0, sizeof(struct ifreq));
	os_strncpy(ifr.ifr_name, bss->ifname, IFNAMSIZ);
	ifr.ifr_data = (void *)(&athr_cmd_data);

	wpa_printf(MSG_DEBUG, "%s: ifname=%s subioctl=%d arg=%d",
	           __func__, ifr.ifr_name, athr_cmd_data.cmd, athr_cmd_data.data[0]);
	if (ioctl(ioctl_sock, SIOCIOCTLAPP, &ifr) < 0) {
		wpa_printf(MSG_ERROR, "athr: "
		           "ioctl[SIOCIOCTLAPP] failed: "
		           "%s", strerror(errno));
		retv = -1;
	}
	close(ioctl_sock);
				   ...
				   struct wpa_driver_scan_params *params)
{
	struct i802_bss *bss = priv;
	struct wpa_driver_nl80211_data *drv = bss->drv;
	int ret = 0, timeout, num_valid_SSID = 0;
	...
	if(params==NULL){
		msg = drv->pending_scan_param;
		ssids = freqs = rates = NULL;
		if(msg == NULL)
			return -1;
		goto nla_send_msgs;
	}else if(drv->pending_scan_param){
		nlmsg_free(drv->pending_scan_param);
		drv->pending_scan_param = NULL;
	}
...
#endif /* ATHR_WLAN_FIND_BEST_CHANNEL */
	}

#ifdef ATHR_WLAN_SCAN_BAND
	if (params->scan_band == 1)
		athr_driver_scanmode(bss, "skip5g");
	else if (params->scan_band == 2)
		athr_driver_scanmode(bss, "skip2g");
	else
		athr_driver_scanmode(bss, "all");
#endif
...
nla_send_msgs:
	...
	msg = NULL;
#else
	ret = send_and_recv_dont_free_msg(drv->global, drv->global->nl, msg, NULL, NULL);
#endif
	if (ret) {
#ifdef SEAMLESS_ROAMING
...
#else /* HOSTAPD */
		if(ret == -EBUSY){
			drv->pending_scan_param = msg;
		}else{
			nlmsg_free(msg);
			drv->pending_scan_param = msg = NULL;
			goto nla_put_failure;
		}
#endif /* HOSTAPD */
	}else {
 ...
 */
static int athr_driver_get_best_channel(struct i802_bss *bss)
{
	struct wpa_driver_nl80211_data *drv = bss->drv;
	...
	os_memset(&athr_cmd_data, 0, sizeof(athr_cmd_t));
	athr_cmd_data.cmd = ATHR_WLAN_FIND_BEST_CHANNEL;
	os_memset(&ifr, 0, sizeof(struct ifreq));
	os_strncpy(ifr.ifr_name, bss->ifname, IFNAMSIZ);
	ifr.ifr_data = (void *)(&athr_cmd_data);

	wpa_printf(MSG_DEBUG, "%s: ifname=%s subioctl=%d",
	           __func__, ifr.ifr_name, athr_cmd_data.cmd);
	if (ioctl(ioctl_sock, SIOCIOCTLAPP, &ifr) < 0) {
		wpa_printf(MSG_ERROR, "athr: "
		           "ioctl[SIOCIOCTLAPP] failed: "
		           "%s", strerror(errno));
	...
	best_5 = athr_cmd_data.data[0];
	best_24 = athr_cmd_data.data[1];
	best_overall = athr_cmd_data.data[2];
...
wpa_driver_nl80211_get_scan_results(void *priv)
{
	...
	if (drv->prev_scan_type == ATHR_FULL_SCAN)
		athr_driver_get_best_channel(bss);
#endif /* ATHR_WLAN_FIND_BEST_CHANNEL */
	...
	void *priv, struct wpa_driver_associate_params *params)
{
	...
	/* 
	 * IOT : Some P2P implementation (Intel) need more time from WPS to 
	 *       non-WPS mode. Here add some delay to avoid P2P connection fail 
	 *       after get WPS credential.
	 */
	if ((params->p2p) &&
		(params->wps == WPS_MODE_NONE))
		os_sleep(1, 0);
...
static int wpa_driver_nl80211_shared_freq(void *priv)
{
	...
	dl_list_for_each(driver, &drv->global->interfaces,
		...
		if (drv == driver ||
		    ...
		    (is_sta_interface(driver->nlmode) && !driver->associated))
			continue;
...
static int nl80211_set_param(void *priv, const char *param)
{
	...
	if (os_strstr(param, "p2p_capable=0")) {
		struct i802_bss *bss = priv;
		struct wpa_driver_nl80211_data *drv = bss->drv;

		wpa_printf(MSG_DEBUG, "nl80211: P2P capability removed");
					 ...
					 size_t buf_len)
{
	...
	if ((ret = ioctl(drv->global->ioctl_sock, SIOCDEVPRIVATE + 1, &ifr)) < 0) {
		...
		if (os_strncasecmp(cmd, "SETBAND", 7) == 0) {
 		...
 		else if (os_strncasecmp(cmd, "SETRTS", 6) == 0) {
			wpa_printf(MSG_DEBUG, "%s: %s ", __func__, cmd);
		}
	}
#endif
...
#if defined(CONFIG_WLAN_HB) || defined(CONFIG_WIFI_DISC)
...
enum nl80211_testmode_cmd {
	...
	NL80211_TEST_MODE_WLAN_SWOW_CMD = 5,
	NL80211_TEST_MODE_WLAN_DEBUGFS_CMD = 6,
};

#define NL80211_TEST_MODE_DATA_MAX_LEN		5000

#if 0 //remove for warning free
...
#ifdef CONFIG_WLAN_HB
#define WLAN_HB_MAX_TCP_FILTER_SIZE	32
#define WLAN_HB_MAX_UDP_FILTER_SIZE	32
...
#ifdef CONFIG_SWOW_SUPPORT
#define WLAN_SWOW_MAX_PWD_SIZE 16
...
static int nl80211_wlan_swow_cmd(void *priv, char *cmd, char *buf,
                size_t buf_len)
{
	struct i802_bss *bss = priv;
	struct wpa_driver_nl80211_data *drv = bss->drv;
	...
	swow_params = os_malloc(sizeof(struct wlan_swow_params));

	if (swow_params == NULL) {
		return -ENOMEM;
	}
	os_memset(swow_params, 0, sizeof(struct wlan_swow_params));
	...
	if (os_strcasecmp(cmd, "SWOW_MODE") == 0) {
		swow_params->cmd = NL80211_WLAN_SWOW_SETMODE;

		if (buf_len <= 4) {
			swow_params->mode = atoi(buf);
			if (swow_params->mode > 1) {
			  wpa_printf(MSG_INFO, "cmd: %s should be 0/1 current %x", cmd, swow_params->mode);
			  goto out_free_msg;
			} else if (wpa_s->swow_pwd_len) {
			  /*Fill the Password info */
			  os_memcpy(swow_params->params.pwd_info.password, wpa_s->swow_password, wpa_s->swow_pwd_len);
			  swow_params->params.pwd_info.length = wpa_s->swow_pwd_len;
			}
		} else
			goto out_free_msg;
	} else if (os_strcasecmp(cmd, "STOP_SWOW") == 0) {
		/*-1 : Fail (WoWLAN mode is not running)*/
		swow_params->cmd = NL80211_WLAN_SWOW_STOP;
	} else if (os_strcasecmp(cmd, "SWOW_PWD") == 0) {
		/*Password (0~16 B)*/
		swow_params->cmd = NL80211_WLAN_SWOW_SET_PWD;
		
		if (os_strlen(buf) > WLAN_SWOW_MAX_PWD_SIZE) {
		  wpa_printf(MSG_INFO, "cmd: %s over the MAX length (16)", cmd);
			ret = -E2BIG;
			goto out_free_msg;
		}
		os_memcpy(swow_params->params.pwd_info.password, buf, os_strlen(buf));
		swow_params->params.pwd_info.length = os_strlen(buf);
		  
	}
	else if (os_strcasecmp(cmd, "SWOW_SET_OUT_TCP") == 0) {
		char *str = buf;
		char *p;
		swow_params->cmd = NL80211_WLAN_SWOW_SET_TCP;
		
		p = strsep(&str, " ");
		if (p != NULL) {
			swow_params->params.outdoor_tcp.src_port = atoi(p);
		}
		else
			goto out_free_msg;
		...
		p = strsep(&str, " ");
		if (p != NULL)
			swow_params->params.outdoor_tcp.dst_port = atoi(p);
		else
			goto out_free_msg;

		p = strsep(&str, " ");
		if (p != NULL)
			swow_params->params.outdoor_tcp.tcp_seq = atoi(p);
		else
			goto out_free_msg;

		p = strsep(&str, " ");
		if (p != NULL)
			swow_params->params.outdoor_tcp.ack_seq = atoi(p);
		else
			goto out_free_msg;

		p = strsep(&str, " ");
		if (p != NULL)
			swow_params->params.outdoor_tcp.ip_id = atoi(p);
		else
			goto out_free_msg;

		p = strsep(&str, " ");
		if (p != NULL) {
			swow_params->params.outdoor_tcp.asyncId = atoi(p);
		}
		else
			goto out_free_msg;
		...
		p = strsep(&str, " ");
		if (p != NULL) {
			sscanf(p, "%08x", &(swow_params->params.outdoor_tcp.device_ip));
		}
		else
			goto out_free_msg;
		...
		p = strsep(&str, " ");
		if (p != NULL) {
			sscanf(p, "%08x", &(swow_params->params.outdoor_tcp.server_ip));
		}
		else
			goto out_free_msg;
		...
		p = strsep(&str, " ");
		if (p != NULL) {
			sscanf(p, "%08x", &(swow_params->params.outdoor_tcp.gateway_ip));
		}
		else
			goto out_free_msg;

		printf("src_port=%d, dst_port=%d, tcp_seq=%d, ack_seq=%d, ip_id=%d, asyncId=%d device_ip=%08x, server_ip=%08x, gateway_ip=%08x\n", 
			swow_params->params.outdoor_tcp.src_port,
			swow_params->params.outdoor_tcp.dst_port,
			swow_params->params.outdoor_tcp.tcp_seq,
			swow_params->params.outdoor_tcp.ack_seq,
			swow_params->params.outdoor_tcp.ip_id,
			swow_params->params.outdoor_tcp.asyncId,
			swow_params->params.outdoor_tcp.device_ip,
			swow_params->params.outdoor_tcp.server_ip,
			swow_params->params.outdoor_tcp.gateway_ip);

	}else if (os_strcasecmp(cmd, "SWOW_SET_OUT_MAC_KEY") == 0) {
		char *str = buf;
		char *p;
		swow_params->cmd = NL80211_WLAN_SWOW_SET_MAC;

		p = strsep(&str, " ");
		if (p != NULL) {
			sscanf(p, "%02x:%02x:%02x:%02x:%02x:%02x",
				&swow_params->params.outdoor_gwmac_key.gateway_mac[0], 
				&swow_params->params.outdoor_gwmac_key.gateway_mac[1], 
				&swow_params->params.outdoor_gwmac_key.gateway_mac[2],
				&swow_params->params.outdoor_gwmac_key.gateway_mac[3], 
				&swow_params->params.outdoor_gwmac_key.gateway_mac[4], 
				&swow_params->params.outdoor_gwmac_key.gateway_mac[5]);
		}
		else
			goto out_free_msg;
		...
		if (os_strlen(p) > WLAN_SWOW_MAX_PWD_SIZE) {
			wpa_printf(MSG_INFO, "cmd: %s over the MAX length (16)", cmd);
			ret = -E2BIG;
			goto out_free_msg;
		}
		os_memcpy(swow_params->params.outdoor_gwmac_key.swol_rc4_key, p, os_strlen(p));
		swow_params->params.outdoor_gwmac_key.swol_rc4_length = os_strlen(p);

	      printf("gateway_mac= %02x %02x %02x %02x %02x %02x key=%s\n", 
			swow_params->params.outdoor_gwmac_key.gateway_mac[0],
			swow_params->params.outdoor_gwmac_key.gateway_mac[1],
			swow_params->params.outdoor_gwmac_key.gateway_mac[2],
			swow_params->params.outdoor_gwmac_key.gateway_mac[3],
			swow_params->params.outdoor_gwmac_key.gateway_mac[4],
			swow_params->params.outdoor_gwmac_key.gateway_mac[5],
			swow_params->params.outdoor_gwmac_key.swol_rc4_key);
	...
	NLA_PUT_U32(msg, NL80211_TEST_MODE_ATTR_CMD, NL80211_TEST_MODE_WLAN_SWOW_CMD);
	NLA_PUT(msg, NL80211_TEST_MODE_ATTR_DATA, sizeof(struct wlan_swow_params), swow_params);
...
static int nl80211_wlan_hb_cmd(void *priv, char *cmd, char *buf,
                size_t buf_len)
{
	struct i802_bss *bss = priv;
	struct wpa_driver_nl80211_data *drv = bss->drv;
	...
	hb_params = os_malloc(sizeof(struct wlan_hb_params));
	...
	if (os_strcasecmp(cmd, "HB_ENABLE") == 0) {
		hb_params->cmd = NL80211_WLAN_HB_ENABLE;
		...
		if (buf_len <= 4) {
			...
			switch(mode) {
				case 0://disable
					hb_params->params.hb_params.enable = 0;
					hb_params->params.hb_params.item = 0;
					hb_params->params.hb_params.session = 0;
					break;
				case 1://udp
					hb_params->params.hb_params.enable = 1;
					hb_params->params.hb_params.item = 1;
					hb_params->params.hb_params.session = 0;				
					break;
				case 2://tcp
					hb_params->params.hb_params.enable = 1;
					hb_params->params.hb_params.item = 2;
					hb_params->params.hb_params.session = 0;					
					break;
			}
		}
	...
	else if (os_strcasecmp(cmd, "TCP_PARAMS") == 0) {
		u16 src_port;
		u16 dst_port;
		u16 timeout;
		char *str = buf;
		...
		hb_params->cmd = NL80211_WLAN_TCP_PARAMS;
		hb_params->params.tcp_params.src_port = src_port;
		hb_params->params.tcp_params.dst_port = dst_port;
		hb_params->params.tcp_params.timeout = timeout;		
		hb_params->params.tcp_params.dev_ip = device_ip;
		hb_params->params.tcp_params.srv_ip = server_ip;
		hb_params->params.tcp_params.gateway_mac[0] = gateway_mac[0];
		hb_params->params.tcp_params.gateway_mac[1] = gateway_mac[1];
		hb_params->params.tcp_params.gateway_mac[2] = gateway_mac[2];
		hb_params->params.tcp_params.gateway_mac[3] = gateway_mac[3];
		hb_params->params.tcp_params.gateway_mac[4] = gateway_mac[4];
		hb_params->params.tcp_params.gateway_mac[5] = gateway_mac[5];		
	}
	else if (os_strcasecmp(cmd, "TCP_FILTER") == 0) {
		char *str = buf;
		char *p;
		char *filter_name;
		u8 filter_offset;
		p = strsep(&str, " ");
		if (p != NULL) {
			filter_offset = atoi(p);
		}
		else
			goto out_free_msg;
		p = strsep(&str, " ");
		filter_name = p;
		if (os_strlen(filter_name) > WLAN_HB_MAX_TCP_FILTER_SIZE) {
			ret = -E2BIG;
			goto out_free_msg;
		}
		
		hb_params->cmd = NL80211_WLAN_TCP_FILTER;
		os_memcpy(hb_params->params.tcp_filter.filter, filter_name, os_strlen(filter_name));
		hb_params->params.tcp_filter.length = os_strlen(filter_name);
		hb_params->params.tcp_filter.offset = filter_offset;
	}
	else if (os_strcasecmp(cmd, "UDP_PARAMS") == 0) {
		u16 src_port;
		u16 dst_port;
		u16 timeout;
		u16 interval;
		...
		hb_params->cmd = NL80211_WLAN_UDP_PARAMS;
		hb_params->params.udp_params.src_port = src_port;
		hb_params->params.udp_params.dst_port = dst_port;
		hb_params->params.udp_params.interval = interval;
		hb_params->params.udp_params.timeout = timeout;	
		hb_params->params.udp_params.dev_ip = device_ip;
		hb_params->params.udp_params.srv_ip = server_ip;
		hb_params->params.udp_params.gateway_mac[0] = gateway_mac[0];
		hb_params->params.udp_params.gateway_mac[1] = gateway_mac[1];
		hb_params->params.udp_params.gateway_mac[2] = gateway_mac[2];
		hb_params->params.udp_params.gateway_mac[3] = gateway_mac[3];
		hb_params->params.udp_params.gateway_mac[4] = gateway_mac[4];
		hb_params->params.udp_params.gateway_mac[5] = gateway_mac[5];					
	}
	else if (os_strcasecmp(cmd, "UDP_FILTER") == 0) {
		char *str = buf;
		char *p;
		char *filter_name;
		u8 filter_offset;
		...
		p = strsep(&str, " ");
		filter_name = p;
		if (os_strlen(filter_name) > WLAN_HB_MAX_UDP_FILTER_SIZE) {
			ret = -E2BIG;
			goto out_free_msg;
		}

		hb_params->cmd = NL80211_WLAN_UDP_FILTER;
		os_memcpy(hb_params->params.udp_filter.filter, filter_name, os_strlen(filter_name));
		hb_params->params.udp_filter.length = os_strlen(filter_name);
		hb_params->params.udp_filter.offset = filter_offset;
	}
	else if (os_strcasecmp(cmd, "NET_INFO") == 0) {
		u32 device_ip;
		u32 server_ip;
		u32 gateway_ip;
		int gateway_mac[ETH_ALEN];
		...
		hb_params->cmd = NL80211_WLAN_NET_INFO;
		hb_params->params.net_info.device_ip = device_ip;
		hb_params->params.net_info.server_ip = server_ip;
		hb_params->params.net_info.gateway_ip = gateway_ip;
		hb_params->params.net_info.gateway_mac[0] = gateway_mac[0];
		hb_params->params.net_info.gateway_mac[1] = gateway_mac[1];
		hb_params->params.net_info.gateway_mac[2] = gateway_mac[2];
		hb_params->params.net_info.gateway_mac[3] = gateway_mac[3];
		hb_params->params.net_info.gateway_mac[4] = gateway_mac[4];
		hb_params->params.net_info.gateway_mac[5] = gateway_mac[5];
	}
	else {
		wpa_printf(MSG_INFO, "cmd: %s not suport", cmd);
		goto out_free_msg;
	...
	NLA_PUT_U32(msg, NL80211_TEST_MODE_ATTR_CMD, NL80211_TEST_MODE_WLAN_HB_CMD);
	NLA_PUT(msg, NL80211_TEST_MODE_ATTR_DATA, sizeof(struct wlan_hb_params), hb_params);
...
static int wpa_driver_nl80211_scan_cancel(void *priv)
{
	struct i802_bss *bss = priv;
	struct wpa_driver_nl80211_data *drv = bss->drv;
	struct nl_msg *msg;
	int ret = 0;
	wpa_printf(MSG_DEBUG, "athr: scan cancel request");
	msg = nlmsg_alloc();
	if (!msg ) {
		return -ENOMEM;
	}

	nl80211_cmd(drv, msg, 0, NL80211_CMD_SCAN_ABORTED);	
...
static int debugfs_return_handler(struct nl_msg *msg, void *arg)
{
	struct nlattr *tb[NL80211_ATTR_MAX + 1];
	struct genlmsghdr *gnlh = nlmsg_data(nlmsg_hdr(msg));
	struct wlan_debugfs_read *fill_back_data = arg;
    struct wlan_debugfs_read *back_data;
    void *buf;

    nla_parse(tb, NL80211_ATTR_MAX, genlmsg_attrdata(gnlh, 0),
		  genlmsg_attrlen(gnlh, 0), NULL);
    if (!tb[NL80211_TEST_MODE_ATTR_CMD])
		return NL_SKIP;
        
    if (fill_back_data) {
        if (!tb[NL80211_ATTR_TESTDATA])
			return NL_SKIP;
        buf = nla_data(tb[NL80211_ATTR_TESTDATA]);
        /*Shift to data */
        back_data =  (struct wlan_debugfs_read *)(buf + 4);
        os_memcpy(fill_back_data, back_data, sizeof(struct wlan_debugfs_read));    
    }

	return NL_SKIP;
}
static int nl80211_wlan_debugfs_cmd(void *priv, char *cmd, char *buf,
                size_t buf_len)
{
	struct i802_bss *bss = priv;
	struct wpa_driver_nl80211_data *drv = bss->drv;
    ...
    os_memset(&back_data, 0, sizeof(struct wlan_debugfs_read));

    debugfs_params = os_malloc(sizeof(struct wlan_debugfs_params)); 
	if (debugfs_params == NULL) {
		return -ENOMEM;
	}

	msg = nlmsg_alloc();
	if (!msg) {
		os_free(debugfs_params);
		return -ENOMEM;
	}
	...
	if (os_strcasecmp(cmd, "txpower") == 0) {
        u8 value = atoi(buf);
		debugfs_params->cmd = DEBUGFS_TX_POWER;
        debugfs_params->params.tx_power_params.ratio = value;
    } else if (os_strcasecmp(cmd, "passive_channel_skip") == 0) {
        u8 value = atoi(buf);
	    debugfs_params->cmd = DEBUGFS_PASSIVE_CHANNEL_SKIP;
        debugfs_params->params.passive_ch_skip_parames.skip = value;
	} else if (os_strcasecmp(cmd, "gettsf") == 0) {
        read_cmd = 1;
		debugfs_params->cmd = DEBUGFS_GET_TSF;    
    } else if (os_strcasecmp(cmd, "synctsf") == 0) {
		debugfs_params->cmd = DEBUGFS_SYNC_TSF;    
    } else if (os_strcasecmp(cmd, "getscan_p") == 0) {
		debugfs_params->cmd = DEBUGFS_GET_SCAN_PARAMS;    
    } else if (os_strcasecmp(cmd, "setscan_p") == 0) {
        u16 act_dwell_t, pas_dwell_t, per_ssid;
        char *str = buf;
        char *p;
		...
		debugfs_params->cmd = DEBUGFS_SET_SCAN_PARAMS;
        debugfs_params->params.scan_set_params.act_dwell_t = act_dwell_t;
        debugfs_params->params.scan_set_params.pas_dwell_t = pas_dwell_t;
        debugfs_params->params.scan_set_params.per_ssid = per_ssid;
    } else if (os_strcasecmp(cmd, "getdebug_m") == 0) {
        read_cmd = 1;
		debugfs_params->cmd = DEBUGFS_GET_DEBUG_MASK;
    } else if (os_strcasecmp(cmd, "setdebug_m") == 0) {
        u32 mask, mask_ext;
        char *str = buf;
        char *p;
        ...
        p = strsep(&str, " ");
		if (p != NULL) {
			sscanf(p, "0x%08x", &mask_ext);
		}
		else
			goto out_free_msg;

        debugfs_params->cmd = DEBUGFS_SET_DEBUG_MASK;
        debugfs_params->params.debug_mask_params.mask = mask;
        debugfs_params->params.debug_mask_params.mask_ext = mask_ext;
    } else if (os_strcasecmp(cmd, "getapalive_p") == 0) {
		debugfs_params->cmd = DEBUGFS_GET_AP_ALIVE_PARAMS;
    } else if (os_strcasecmp(cmd, "setapalive_p") == 0) {
        u32 internal, cycle;
        char *str = buf;
        char *p;
		...
		debugfs_params->cmd = DEBUGFS_SET_AP_ALIVE_PARAMS;
        debugfs_params->params.ap_alive_params.internal = internal;
        debugfs_params->params.ap_alive_params.cycle = cycle;
    } else if (os_strcasecmp(cmd, "gethtcap_p") == 0) {
		debugfs_params->cmd = DEBUGFS_GET_HT_CAP_PARAMS;
    } else if (os_strcasecmp(cmd, "sethtcap_p") == 0) {
        u8 band, ht40_supported, short_GI, intolerance_ht40;
        char *str = buf;
        char *p;
        ...
        if (p == NULL)
			goto out_free_msg;
        intolerance_ht40 = atoi(p);

		debugfs_params->cmd = DEBUGFS_SET_HT_CAP_PARAMS;
        debugfs_params->params.ht_cap_params.band = band;
        debugfs_params->params.ht_cap_params.ht40_supported = ht40_supported;
        debugfs_params->params.ht_cap_params.short_GI = short_GI;
        debugfs_params->params.ht_cap_params.intolerance_ht40 = intolerance_ht40;
    } else if (os_strcasecmp(cmd, "chan_list") == 0) {
		debugfs_params->cmd = DEBUGFS_CHAN_LIST;    
    } else if (os_strcasecmp(cmd, "ampdu_enable") == 0) {
        u32 value = atoi(buf);

		debugfs_params->cmd = DEBUGFS_AMPDU;   
        debugfs_params->params.ampdu_parames.enable_ampdu = value;
    } else if (os_strcasecmp(cmd, "reg_read") == 0) {
        u32 reg_addr;
        char *str = buf;
        char *p;
        ...
        p = strsep(&str, " ");
		if (p != NULL) {
			sscanf(p, "0x%08x", &reg_addr);
		}
		else
			goto out_free_msg;

        read_cmd = 1;
		debugfs_params->cmd = DEBUGFS_REG_READ;
        debugfs_params->params.reg_params.reg_addr = reg_addr;
    } else if (os_strcasecmp(cmd, "reg_write") == 0) {
        ...
        p = strsep(&str, " ");
		if (p != NULL) {
			sscanf(p, "0x%08x", &reg_value);
		}
		else
			goto out_free_msg;

		debugfs_params->cmd = DEBUGFS_REG_WRITE;
        debugfs_params->params.reg_params.reg_addr = reg_addr;
        debugfs_params->params.reg_params.reg_value = reg_value;
    } else if (os_strcasecmp(cmd, "set_da_retry") == 0) {
        u32 value = atoi(buf);

		debugfs_params->cmd = DEBUGFS_DA_RETRY_LIMIT;
        debugfs_params->params.direct_audio_parames.retry_limit = value;
    } else {
		wpa_printf(MSG_INFO, "cmd: %s not suport", cmd);
		goto out_free_msg;
	}

	NLA_PUT_U32(msg, NL80211_TEST_MODE_ATTR_CMD, NL80211_TEST_MODE_WLAN_DEBUGFS_CMD);
	NLA_PUT(msg, NL80211_TEST_MODE_ATTR_DATA, sizeof(struct wlan_debugfs_params), debugfs_params);

	nla_nest_end(msg, nest);
    if (read_cmd) {
        ret = send_and_recv_msgs(drv, msg, debugfs_return_handler, &back_data);
        if (debugfs_params->cmd == DEBUGFS_GET_TSF) {
            ret = os_snprintf(buf, buf_len, "%llu\n",
		        back_data.params.tsf_read_params.tsf_value);
        } else if (debugfs_params->cmd == DEBUGFS_GET_DEBUG_MASK) {
            ret = os_snprintf(buf, buf_len, "debug_mask    :0x%08x\ndebug_mask_ext:0x%08x\n",
		        back_data.params.debug_mask_params.mask,
                back_data.params.debug_mask_params.mask_ext);
        } else if (debugfs_params->cmd == DEBUGFS_REG_READ) {
#define READ_FAIL_VALUE 0xbe000bad
            if (back_data.params.reg_params.reg_value == READ_FAIL_VALUE) {
               ret = os_snprintf(buf, buf_len, "Reg Read Fail... \n");
            } else {
                ret = os_snprintf(buf, buf_len, "reg :0x%08x = 0x%08x\n",
		            back_data.params.reg_params.reg_addr,
                    back_data.params.reg_params.reg_value);
            }                    
        } else {
            wpa_printf(MSG_INFO, "read cmd: %s not suport", cmd);
        }
    } else {
        ret = send_and_recv_msgs(drv, msg, NULL, NULL);
    }
...
#endif /* ANDROID */
#ifdef CONFIG_WLAN_HB
	.wlan_hb_cmd = nl80211_wlan_hb_cmd,
#endif
#ifdef CONFIG_WIFI_DISC
	.wifi_disc_cmd = nl80211_wifi_disc_cmd,
#endif
	.scan_cancel = wpa_driver_nl80211_scan_cancel,
#ifdef CONFIG_SWOW_SUPPORT
  .wlan_swow_cmd = nl80211_wlan_swow_cmd,
#endif//swow
    .wlan_debugfs_cmd = nl80211_wlan_debugfs_cmd,
};
