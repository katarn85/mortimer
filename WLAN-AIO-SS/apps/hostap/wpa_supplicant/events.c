					 ...
					 struct wpa_scan_res *bss)
{
	...
	int ret;
	int wep_ok, rsn_mismatch = 0, wpa_mismatch = 0;
	...
	if(wep_ok && !wpa_key_mgmt_wpa(ssid->key_mgmt) && (rsn_mismatch || wpa_mismatch))
	{
		wpa_dbg(wpa_s, MSG_DEBUG, " reject due to WEP but AP have WPA/WPA2");
		return 0;
	}
...
#if defined(ANDROID) && defined(SEAMLESS_ROAMING)
...
#endif
{
	...
	for (ssid = group; ssid; ssid = ssid->pnext) {
		...
		if (!(wpa_s->drv_flags & WPA_DRIVER_FLAGS_MULTI_CHANNEL_CONCURRENT) &&
			wpa_drv_shared_freq(wpa_s) && bss->freq != wpa_drv_shared_freq(wpa_s)) {
			wpa_dbg(wpa_s, MSG_DEBUG, "   skip - bss freq %d, p2p freq %d,"
				" MCC is not allowed", bss->freq, wpa_drv_shared_freq(wpa_s));
...
#ifdef CONFIG_CUSTOM_WFD	
	wpa_supplicant_ctrl_iface_scan_results_widi(wpa_s);
#endif /* CONFIG_CUSTOM_WFD */
			   ...
			   struct wpa_ssid *ssid)
{
	...
	if (wpa_s->reassociate ||
		...
		os_memcpy(wpa_s->wps_onging_ssid, selected->ssid, selected->ssid_len);
		wpa_s->wps_onging_ssid_len = selected->ssid_len;
				       ...
				       union wpa_event_data *data)
{
	...
	if(wpa_s->current_ssid){
			wpa_s->current_ssid->scan_freq_count = 0;
			if(wpa_s->current_ssid->scan_freq == NULL){
				wpa_s->current_ssid->scan_freq = os_zalloc(2* sizeof(int));
				wpa_s->current_ssid->scan_freq[0] = data->assoc_info.freq;
				wpa_s->current_ssid->scan_freq[1] = 0;
			}else if(wpa_s->current_ssid->scan_freq[0] != data->assoc_info.freq)
				wpa_s->current_ssid->scan_freq[0] = data->assoc_info.freq;
	}
...
case EVENT_RX_MGMT:
#ifdef CONFIG_P2P
	{
		...
		if(stype == WLAN_FC_STYPE_PROBE_REQ && 
                wpa_blacklist_get_p2p(wpa_s, mgmt->sa))
			break;
#endif /* CONFIG_P2P */
		if (wpa_s->ap_iface == NULL) {
#ifdef CONFIG_P2P                       
	...
	case EVENT_INTERFACE_ENABLED:
		...
		/* Clear the swow mode flag 
		 * when that might be back from target sleep
		 */
		wpa_s->swow_mode = 0;
#endif//swow
		if (wpa_s->wpa_state == WPA_INTERFACE_DISABLED) {
