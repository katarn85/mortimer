...
extern char *p2p_state_txt(int state);
...
#ifdef CONFIG_CUSTOM_WFD
int wps_build_wfa_widi(struct wpa_supplicant *wpa_s, int enable_WIDI, char *dev_name, u8 *dev_type)
{
	wpa_s->global->widi_enable_wps = enable_WIDI;
	
	if (enable_WIDI == 1) {
		os_memset(wpa_s->global->widi_dev_name, '\0' , sizeof(wpa_s->global->widi_dev_name));
		os_memset(wpa_s->global->widi_dev_type, 0 , sizeof(wpa_s->global->widi_dev_type));
		strncpy(wpa_s->global->widi_dev_name, dev_name,
		        sizeof(wpa_s->global->widi_dev_name));
		wpa_s->global->widi_dev_name[sizeof(wpa_s->global->widi_dev_name) - 1] = '\0';
		os_memcpy(wpa_s->global->widi_dev_type, dev_type, 8);
		return 0;
	} else {
		wpa_printf(MSG_ERROR, "%s: WIDI is disabled!!", __func__);
		return 1;
	}
}
#endif /* CONFIG_CUSTOM_WFD */
...
void
wpa_supplicant_phy_bitrate_unit_convert(char *buffer, int buflen, int bitrate)
{
	double	rate = bitrate;
	char		scale;
	int		divisor;
...
void
wpa_supplicant_phy_freq_unit_convert(char *buffer, int buflen, double freq)
{
	char scale;
	int divisor;
...
static int
wpa_supplicant_phy_range_ioctl(int skfd, char *ifname, struct wpa_supplicant_phy_info *info)
{
	struct iwreq wrq;
	...
	{
		//wpa_printf(MSG_INFO, "%s: %d we_version_compiled=%d", __func__, __LINE__, info->range.we_version_compiled);
		info->has_range = 1;
	}	
...
static int
wpa_supplicant_phy_80211mode_ioctl(int skfd, char *ifname, struct wpa_supplicant_phy_info *info)
{
	struct iwreq wrq;
	...
	else
	{
		...
		//wpa_printf(MSG_INFO, "%s: %d 802.11 mode=%s", __func__, __LINE__, wrq.u.name);
	}
...
static int
wpa_supplicant_phy_rate_ioctl(int skfd, char *ifname, struct wpa_supplicant_phy_info *info)
{
	struct iwreq wrq;
	...
	else
	{
		...
		//wpa_printf(MSG_INFO, "%s: %d bitrate=%d", __func__, __LINE__, (int)info->bitrate.value);		
...
static int
wpa_supplicant_phy_stats_ioctl(int skfd, char *ifname, struct wpa_supplicant_phy_info *info)
{
	struct iwreq wrq;
	...
	{
		info->has_stats = 1;
		//wpa_printf(MSG_INFO, "%s: %d noise=%d level=%d", 
		//	__func__, __LINE__, info->stats.qual.noise, info->stats.qual.level);		
...
static int
wpa_supplicant_phy_freq_ioctl(int skfd, char *ifname, struct wpa_supplicant_phy_info *info)
{
	struct iwreq wrq;
	...
	{	
		...
		//wpa_printf(MSG_INFO, "%s: %d freq=%f", __func__, __LINE__, info->freq);				
	}	
	return 0;
}

static int wpa_supplicant_phy_status(struct wpa_supplicant *wpa_s, char *buf, size_t buflen,
			   int verbose)
{
	int ret, skfd;
					    ...
					    char *buf, size_t buflen)
{
	...
	if (wpa_s->global->p2p &&
	    ...
	    strncmp(wpa_s->ifname, "p2p", 3) == 0) {
	    struct p2p_data *p2p = wpa_s->global->p2p;
		ret = os_snprintf(pos, end - pos, "p2p_state=%s\n",
...
#ifdef CONFIG_CUSTOM_WFD
static int hex2num_WIDI(char c)
{
	if (c >= '0' && c <= '9')
		return c - '0';
	if (c >= 'a' && c <= 'f')
...
static int hex2byte_WIDI(const char *hex)
{
	int a, b;
	a = hex2num_WIDI(*hex++);
	if (a < 0)
...
static int wpa_supplicant_ctrl_iface_widi_get_persistent_profile(
	struct wpa_supplicant *wpa_s, char *buf, size_t buflen)
{
	...
	ssid = wpa_s->conf->ssid;
	while (ssid) {
		if (ssid->disabled == 2 /* only record persistent networks */
			&& ssid->bssid_set /* peer MAC address must exist */ )
		{
			ret = os_snprintf(pos, end - pos, MACSTR,
					  MAC2STR(ssid->bssid));
			if (ret < 0 || ret >= end - pos)
...
/* Remove all the nonpersistent profile, while WiDi shutting down. \ 
*  To prevent any zombie profiles impacting the persistent reconnection \
*/
static int wpa_supplicant_ctrl_iface_widi_remove_nonpersist_network(
	struct wpa_supplicant *wpa_s, char *buf, size_t buflen)
{
	struct wpa_ssid *ssid = NULL;

	wpa_printf(MSG_DEBUG, "CTRL_IFACE: WIDI_REMOVE_NONPERSIST_NETWORK");
	ssid = wpa_s->conf->ssid;
	
	while (ssid) {		
		if (ssid->disabled != 2 /*[P2P-PERSISTENT]*/) {			
			if (ssid == wpa_s->current_ssid){
				wpa_printf(MSG_DEBUG, "Profile %d is CURRENT SSID, do not remove", ssid->id);
				ssid = ssid->next;
				continue;
			}
			...
			/* we break here to prevent ssid link-list messed up */
			break;
		}
		ssid = ssid->next;
	}
	...
	struct wpa_supplicant *wpa_s, char *buf)
{
	...
	hexstr2bin_WIDI(buf, vendor_id, 4);
	wpa_s->global->widi_vendor_id = WPA_GET_BE24(vendor_id);
...
#ifdef CONFIG_CUSTOM_WFD	
	if(wpa_s->global->widi_enable == 1){		
		if (wps_is_vendor_id_match(wpa_s->global->widi_vendor_id, wps_ie, buf_VE)== 4)
		{
			wpa_printf(MSG_DEBUG, "vendor ID is matched");
			wpa_s->global->widi_dev_found = 1;
			txt = "CUSTOM-WFD";
			...
			ret = os_snprintf(pos, end - pos, "%s-", buf_VE);
		}
	}
	else{
#endif /* CONFIG_CUSTOM_WFD */		
...
#ifdef CONFIG_CUSTOM_WFD
static int wpa_supplicant_ctrl_iface_filter_widi_bss(
	struct wpa_supplicant *wpa_s,
	const struct wpa_bss *bss, char *buf, size_t buflen)
{
	...
	if(pos > buf && wpa_s->global->widi_dev_found == 1) {
		if (bss->freq >= 2412 && bss->freq <= 2484) {
			channel = bss->freq == 2484 ? 14 : (bss->freq - 2407) / 5;
		} else if ( bss->freq >= 5000 &&  bss->freq <= 6000) {
			channel = (bss->freq - 5000) / 5;
		}

		ret = os_snprintf(pos, end - pos, "%02x%02x%02x%02x%02x%02x-%02x",           
							MAC2STR(bss->bssid), channel); 
		if (ret < 0 || ret >= end - pos)
			return pos - buf;
		pos += ret;
		ret = os_snprintf(pos, end - pos, "-%02x-%s",     
							bss->ssid_len, wpa_ssid_txt(bss->ssid, bss->ssid_len));
		if (ret < 0 || ret >= end - pos)
			return pos - buf;
		pos += ret;
...
static int wpa_supplicant_ctrl_iface_filter_widi(
	struct wpa_supplicant *wpa_s, char *buf, size_t buflen)
{
	char *pos, *end;
	struct wpa_bss *bss;
	...
	dl_list_for_each(bss, &wpa_s->bss_id, struct wpa_bss, list_id) {
		ret = wpa_supplicant_ctrl_iface_filter_widi_bss(wpa_s, bss, pos,
							    end - pos);
		if (ret < 0 || ret >= end - pos)
			return pos - buf;
		pos += ret;
		if (wpa_s->global->widi_dev_found == 1) {
			wpa_s->global->widi_dev_found = 0;
			wpa_printf(MSG_DEBUG, "reset widi_dev_found");
			return pos - buf;
		}
	}
...
static int wpa_supplicant_ctrl_iface_scan_cancel(
	struct wpa_supplicant *wpa_s, char *cmd)
{
	wpa_printf(MSG_DEBUG, "WPA: cancel scan");

	/* cancel pending scan request */
	wpa_supplicant_cancel_scan(wpa_s);

	/* cancel scan in progress */
	return wpa_drv_scan_cancel(wpa_s);
}

#ifdef CONFIG_CUSTOM_WFD
static void WIDI_scan_init_from_l2sd(struct wpa_supplicant *wpa_s, char *cmd)
{
	wpa_s->global->widi_enable = atoi(cmd);

	if (wpa_s->global->widi_enable != 0) {
		wpa_s->global->widi_enable = 1;
		
		if (wpa_s->global->widi_sock_l2sd == 0) {
			wpa_s->global->widi_sock_l2sd = socket(AF_INET, SOCK_DGRAM, 0);
			if ( wpa_s->global->widi_sock_l2sd < 0 ) {
				wpa_s->global->widi_sock_l2sd = 0;
				wpa_printf(MSG_ERROR, "%s: Create widi_sock_l2sd Fail", __func__);
			}			
		}
	} else {
		if ( wpa_s->global->widi_sock_l2sd == 0 ) {
		...
		close(wpa_s->global->widi_sock_l2sd);
		wpa_s->global->widi_sock_l2sd = 0;

		/* Clear enable_widi_ve and fill_sink_info[]*/
		if(wps_build_wfa_ext_widi(0, NULL) != 0)
			wpa_printf(MSG_ERROR, "%s: Fail", __func__);		
	}
...
static void wpa_supplicant_ctrl_iface_widi_sink_info(
	struct wpa_supplicant *wpa_s,
	char *cmd)
{
	u8 sink_info[11] = {0};
	u8 dev_name_tye_len = 0;
	u8 dev_type[8] = {0};
	int enable_WIDI_Ext_VE = 0;
	char dev_name[32] = {0};

	hexstr2bin_WIDI(cmd, sink_info, 11);
	hexstr2bin_WIDI(cmd + 22, &dev_name_tye_len, 1);
	hexstr2bin_WIDI(cmd + 24, dev_type, 8);
	
	os_strncpy(dev_name, cmd+40, (dev_name_tye_len - 14));
	enable_WIDI_Ext_VE = 1;
	
	if(wps_build_wfa_ext_widi(enable_WIDI_Ext_VE, sink_info) != 0)
		wpa_printf(MSG_ERROR, "%s: Fail", __func__);

	if(wps_build_wfa_widi(wpa_s, enable_WIDI_Ext_VE, dev_name, dev_type) != 0)
		wpa_printf(MSG_ERROR, "%s: Fail", __func__);
...
void wpa_supplicant_ctrl_iface_scan_results_widi(struct wpa_supplicant *wpa_s)
{
	...
	char buf[8192] = {0};
	l2_msg_t l2_msg;
	uint8_t WIDI_src_mac[ETH_ALEN] = {0};
	uint8_t channel = 0, ssid_len = 0, WIDI_L2SD_VE_len = 0;
	uint8_t WIDI_L2SD_VE_len_to_str_len = 0;
	uint8_t WIDI_L2SD_QUERY[L2SD_TYPE] = {0};
	uint8_t WIDI_Soruce_Vendor_Extension[32] = {0x10,0x49,0x00};

	if (wpa_s->global->widi_enable == 0) {
		wpa_printf(MSG_DEBUG, "exit on widi disabled\n");
		return;
	}
    
	memset( &local, 0, sizeof(local));

	wpa_supplicant_ctrl_iface_filter_widi(wpa_s, buf, sizeof(buf));
	
	if(buf[0] != '\0')
		wpa_printf(MSG_DEBUG, "BUF=%s", buf);
	
	if ( strncmp(buf, "CUSTOM-WFD-01", 13) == 0 ) {
		local.sin_family = AF_INET;
		local.sin_addr.s_addr = htonl((127 << 24) | 1);
		local.sin_port = htons(WIDI_CTRL_IFACE_PORT);

		memset(&l2_msg, 0, sizeof(l2_msg_t) );

		l2_msg.msg_type = L2SDTA_MSG_TYPE_QUERY_OR_TRIGGER;

		hexstr2bin_WIDI(&buf[14], &WIDI_L2SD_VE_len, 1);
		WIDI_Soruce_Vendor_Extension[3] = WIDI_L2SD_VE_len;
		hexstr2bin_WIDI(&buf[25], WIDI_L2SD_QUERY, 2);
		WIDI_L2SD_VE_len_to_str_len = (WIDI_L2SD_VE_len << 1);
		hexstr2bin_WIDI(&buf[17], &WIDI_Soruce_Vendor_Extension[4], WIDI_L2SD_VE_len_to_str_len - 4);
		hexstr2bin_WIDI(&buf[50], WIDI_src_mac, 6);
		hexstr2bin_WIDI(&buf[63], &channel, 1);
		hexstr2bin_WIDI(&buf[66], &ssid_len, 1);

		memcpy(l2_msg.u.l2sd_query_or_trig_msg.src_mac, WIDI_src_mac, ETH_ALEN );
		l2_msg.u.l2sd_query_or_trig_msg.channel = channel;
		l2_msg.u.l2sd_query_or_trig_msg.ssid_len = ssid_len;
		memcpy(l2_msg.u.l2sd_query_or_trig_msg.ssid, &buf[69],ssid_len);		
		memcpy(l2_msg.u.l2sd_query_or_trig_msg.qa_ta_ext, 
			WIDI_Soruce_Vendor_Extension, WIDI_L2SD_VE_len+4);

		wpa_printf(MSG_INFO, "	Received L2SD QUERY\n");

		if(sendto(wpa_s->global->widi_sock_l2sd, &l2_msg, sizeof(l2_msg), 
			0,(struct sockaddr*)&local, sizeof(local)) < 0) {
			wpa_printf(MSG_ERROR, "send to L2SD socket fail");
		}
	} else if ( strncmp(buf, "CUSTOM-WFD-02", 13) == 0 ) {
		local.sin_family = AF_INET;
		local.sin_addr.s_addr = htonl((127 << 24) | 1);
		local.sin_port = htons(WIDI_CTRL_IFACE_PORT);
		...
		WIDI_L2SD_VE_len_to_str_len = (WIDI_L2SD_VE_len << 1);
		hexstr2bin_WIDI(&buf[17], &WIDI_Soruce_Vendor_Extension[4], WIDI_L2SD_VE_len_to_str_len-4);
		hexstr2bin_WIDI(&buf[58], WIDI_src_mac, 6);
		hexstr2bin_WIDI(&buf[71], &channel, 1);
		hexstr2bin_WIDI(&buf[74], &ssid_len, 1);
		memcpy(l2_msg.u.l2sd_query_or_trig_msg.src_mac, WIDI_src_mac, ETH_ALEN );
		l2_msg.u.l2sd_query_or_trig_msg.channel = channel;
		l2_msg.u.l2sd_query_or_trig_msg.ssid_len = ssid_len;
		memcpy(l2_msg.u.l2sd_query_or_trig_msg.ssid, &buf[77], ssid_len);
		memcpy(l2_msg.u.l2sd_query_or_trig_msg.qa_ta_ext, WIDI_Soruce_Vendor_Extension, WIDI_L2SD_VE_len+4);

		wpa_printf(MSG_INFO, "	Received L2TA TRIGGER\n");
...
#ifdef CONFIG_CUSTOM_WFD
static int wpa_supplicant_ctrl_iface_widi_get_network(
	struct wpa_supplicant *wpa_s, char *cmd, char *buf, size_t buflen)
{
	int id;
	size_t res;
	...
	*name++ = '\0';
	...
	id = atoi(cmd);
	wpa_printf(MSG_DEBUG, "CTRL_IFACE: WIDI_GET_NETWORK id=%d name='%s'",
		   id, name);
...
static int wpa_supplicant_ctrl_iface_change_device_name(struct wpa_supplicant *wpa_s,
														char *cmd)
{
	if (os_strlen(cmd) > 32) {
		wpa_printf(MSG_DEBUG, "change_device_name: Too long ");
		return -1;
	}

	if (!wpa_s->conf) {
		wpa_printf(MSG_DEBUG, "change_device_name: No wpa_s->conf ");
		return -1;
	}
	os_free(wpa_s->conf->device_name);
	wpa_s->conf->device_name = os_strdup(cmd);
	return 0;
...
#ifdef CONFIG_CUSTOM_WFD
static int widi_p2p_ctrl_listen_ms(struct wpa_supplicant *wpa_s, char *cmd)
{
	unsigned int timeout = atoi(cmd);
	return widi_wpas_p2p_listen_ms(wpa_s, timeout);
}
#endif /* CONFIG_CUSTOM_WFD */
...
static int p2p_ctrl_black_list(struct wpa_supplicant *wpa_s, char *cmd)
{
	u8 addr[ETH_ALEN];
	char *pos;
	enum wpas_p2p_black_list_use use;

	/* <add|remove|flush> <config method> [join|auto] */
	pos = cmd;

	if (os_strstr(pos, "add") != NULL){
		use = WPAS_P2P_BL_ADD;
             pos = cmd+4;
	}else if (os_strstr(pos, "remove") != NULL){
		use = WPAS_P2P_BL_REMOVE;
             pos = cmd+7;
      }else if (os_strstr(pos, "flush") != NULL){
             use = WPAS_P2P_BL_FLUSH;
      }else
             return -1;

      if(use == WPAS_P2P_BL_ADD || use == WPAS_P2P_BL_REMOVE){
             if (hwaddr_aton(pos, addr))
			return -1;
      }

	return wpas_p2p_black_list(wpa_s, addr, use);
}
...
static int p2p_ctrl_group_add(struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	if (os_strcmp(cmd, "persistent") == 0 ||
		...
		return wpas_p2p_group_add(wpa_s, 1, freq, ssid, passphrase);
	if (os_strncmp(cmd, "freq=", 5) == 0 ||
	    os_strncmp(cmd, "ssid=", 5) == 0 ||
	    os_strncmp(cmd, "passphrase=", 11) == 0)
		return wpas_p2p_group_add(wpa_s, 0, freq, ssid, passphrase);
					 ...
					 char *buf, size_t *resp_len)
{
	...
	int reply_len;
#ifdef CONFIG_CUSTOM_WFD
    u8 WIDI_dev_type[8]={0x00,0x07,0x00,0x17,0x35,0x20,0x00,0x20};
#endif /* CONFIG_CUSTOM_WFD */	
	...
	if (os_strcmp(buf, "PING") == 0) {
...
#ifdef CONFIG_CUSTOM_WFD
	} else if (os_strcmp(buf, "WIDI_RESET_SUPPLICANT") == 0) {
		...
		if (wpa_s->wpa_state == WPA_INTERFACE_DISABLED)
			...
			wpa_s->reassociate = 1;
			wps_build_wfa_widi(wpa_s, 0, "WiDi-TSB", WIDI_dev_type);
		}
	} else if (os_strncmp(buf, "WIDI_SINK_INFO ", 15) == 0) {
                wpa_supplicant_ctrl_iface_widi_sink_info(wpa_s, buf + 15);	
	} else if (os_strncmp(buf, "WIDI_P2P_LISTEN_MS ", 19) == 0) {
		if (widi_p2p_ctrl_listen_ms(wpa_s, buf + 19))
			reply_len = -1;
	} else if (os_strcmp(buf, "WIDI_P2P_LISTEN_MS") == 0) {
		if (widi_p2p_ctrl_listen_ms(wpa_s, ""))
			reply_len = -1;
	} else if (os_strcmp(buf, "WIDI_GET_PERSISTENT_PROFILE") == 0) {
		reply_len = wpa_supplicant_ctrl_iface_widi_get_persistent_profile(
			wpa_s, reply, reply_size);
	} else if (os_strncmp(buf, "WIDI_SCAN_RESULTS ", 18) == 0) {
                WIDI_scan_init_from_l2sd(wpa_s, buf + 18); 
	} else if (os_strncmp(buf, "WIDI_GET_NETWORK ", 17) == 0) {
		reply_len = wpa_supplicant_ctrl_iface_widi_get_network(
		...
		if (wpas_p2p_group_add(wpa_s, 0, 0, NULL, NULL))
			reply_len = -1;
		...
		if (p2p_ctrl_prov_disc(wpa_s, buf + 14))
			reply_len = -1;
	} else if (os_strncmp(buf, "P2P_BLACK_LIST ", 15) == 0) {
		if (p2p_ctrl_black_list(wpa_s, buf + 15))
			reply_len = -1;
	...
	} else if (os_strncmp(buf, WPA_CTRL_RSP, os_strlen(WPA_CTRL_RSP)) == 0)
	{
		...
		if (wpa_s->wpa_state == WPA_INTERFACE_DISABLED)
			...
			if (!wpa_s->scanning &&
				...
				wpa_s->normal_scans = 0;
				wpa_s->scan_req = 2;
				if(wpa_s->single_channel_scan_setting)
					wpa_s->single_channel_scan = wpa_s->single_channel_scan_setting;
				wpa_supplicant_req_scan(wpa_s, 0, 0);
	...
	} else if ((os_strncmp(buf, "SCAN FREQ=", 10) == 0) ||
				os_strncmp(buf, "SCAN freq=", 10) == 0) {
		int channel = 0;
		char *cmd = buf + 10;
		...
		if (wpa_s->wpa_state == WPA_INTERFACE_DISABLED)
			...
			if (!wpa_s->scanning &&
				...
				wpa_s->normal_scans = 0;
				wpa_s->scan_req = 2;
				wpa_s->single_channel_scan = channel;
				wpa_supplicant_req_scan(wpa_s, 0, 0);
			} else if (wpa_s->sched_scanning) {
				wpa_printf(MSG_DEBUG, "Stop ongoing "
					   "sched_scan to allow requested "
...
#else
			wpa_s->single_channel_scan_setting = channel;
#endif
		}
	} else if (os_strcmp(buf, "SCAN_RESULTS") == 0) {
		reply_len = wpa_supplicant_ctrl_iface_scan_results(
			wpa_s, reply, reply_size);
        } else if (os_strcmp(buf, "BSS_FLUSH") == 0) {
                wpa_bss_flush(wpa_s);
	} else if (os_strncmp(buf, "SCAN_CANCEL", 11) == 0) {
		if (wpa_supplicant_ctrl_iface_scan_cancel(wpa_s, NULL))
			reply_len = -1;			
		...
		if (wpa_supplicant_ctrl_iface_save_config(wpa_s))
			reply_len = -1;
#endif /* CONFIG_NO_CONFIG_WRITE */
	} else if (os_strncmp(buf, "CHANGE_DEVICE_NAME ", 19) == 0) {
		if (wpa_supplicant_ctrl_iface_change_device_name(wpa_s, buf + 19)) {
			reply_len = -1;
		} else {
#ifndef CONFIG_NO_CONFIG_WRITE
			if (wpa_supplicant_ctrl_iface_save_config(wpa_s)) {
				wpa_printf(MSG_DEBUG, "TRY to Save Config FAIL");
			} else {
				wpa_printf(MSG_DEBUG, "TRY to Save Config DONE");
			}
#endif /* CONFIG_NO_CONFIG_WRITE */
	...
	else if (os_strncmp(buf, "WLAN_SWOW ", 10) == 0) {
		int en;
		...
		if (wpa_s->wpa_state >= WPA_ASSOCIATED) {
		    en = atoi(buf+10);
	      if (en) {
			    os_memcpy(reply, "Outdoor\n", 8);
			    reply_len = 8;
		    } else {
			    os_memcpy(reply, "Indoor\n", 7);
			    reply_len = 7;
		    }
		    if(wpa_drv_wlan_swow_cmd(wpa_s, "SWOW_MODE", buf+10, buf_len-10))
			    reply_len = -1;
			  else if ((en == 0) && (wpa_s->swow_pwd_len == 0 )) {
			    wpa_printf(MSG_INFO, "No swow password to be set ??? \n");
			  }
			  /*Keep the setting to local */
			  wpa_s->swow_mode = en + 1;
		} else {
		     wpa_printf(MSG_INFO, "No AP connection");
		     reply_len = -1;
		}
	} else if (os_strncmp(buf, "STOP_SWOW", 9) == 0) {
		int buf_len = os_strlen(buf);
		if (wpa_s->swow_mode == 0) {
		    wpa_printf(MSG_INFO, "WoWLAN mode is not running");  
		    reply_len = -1;
		} else if (wpa_drv_wlan_swow_cmd(wpa_s, "STOP_SWOW", buf+10, buf_len-10))      
		    reply_len = -1;
		    /*Keep the setting to local */
		    wpa_s->swow_mode = 0;
	} else if (os_strncmp(buf, "SWOW_PWD ", 9) == 0) {
		int buf_len = os_strlen(buf);
#if 0 
		/*Only one command send to driver when do start*/   
		if (wpa_drv_wlan_swow_cmd(wpa_s, "SWOW_PWD", buf+9, buf_len-9))
			  reply_len = -1;
#endif
#define WLAN_SWOW_MAX_PWD_SIZE 16
		if (buf_len-9 > WLAN_SWOW_MAX_PWD_SIZE) {
		    wpa_printf(MSG_INFO, "over the MAX length (16)");
		    reply_len = -1;
		} else {
		    /*Keep the setting to local */
		    wpa_s->swow_pwd_len = buf_len-9;
		    os_memcpy(wpa_s->swow_password, buf+9, wpa_s->swow_pwd_len);
	  }
	} else if (os_strncmp(buf, "SWOW_PWD", 8) == 0) {
		wpa_s->swow_pwd_len = 0;
	} else if (os_strncmp(buf, "SWOW_GET_MODE", 13) == 0) {
		u16 swow_mode = wpa_s->swow_mode - 1;

		if (swow_mode == 0) {
		    os_memcpy(reply, "0: Indoor mode\n", 15);
		    reply_len = 15;
		} else if (swow_mode == 1) {
		    os_memcpy(reply, "1: Outdoor mode\n", 16);
		    reply_len = 16;
		} else {
		    os_memcpy(reply, "Disabled\n", 9);
		    reply_len = 9;	    
		}
	} else if (os_strncmp(buf, "SWOW_GET_PWD", 12) == 0) {
		if (wpa_s->swow_pwd_len) {
		    reply_len = os_snprintf(reply, reply_size, "Password: %s\n",
		    wpa_ssid_txt(wpa_s->swow_password, wpa_s->swow_pwd_len));
		} else {
		    os_memcpy(reply, "No Password Setting\n", 20);
		    reply_len = 20;
		}
	}else if (os_strncmp(buf, "SWOW_SET_OUT_TCP", 16) == 0) {
		int buf_len = os_strlen(buf);
		if (wpa_drv_wlan_swow_cmd(wpa_s, "SWOW_SET_OUT_TCP", buf+17, buf_len-17)){	   
			reply_len = -1;			
		}
	} else if (os_strncmp(buf, "SWOW_SET_OUT_MAC_KEY", 20) == 0) {
		int buf_len = os_strlen(buf);
		if (wpa_drv_wlan_swow_cmd(wpa_s, "SWOW_SET_OUT_MAC_KEY", buf+21, buf_len-21)){	   
			reply_len = -1;			
		}
	}
#endif//swow
#ifdef CONFIG_WLAN_HB
	else if (os_strncmp(buf, "WLAN_HB ", 8) == 0) {
		int en;
                int buf_len = os_strlen(buf);
		...
		wpa_drv_wlan_hb_cmd(wpa_s, "HB_ENABLE", buf+8, buf_len-8);
	} else if (os_strncmp(buf, "WLAN_HB_TCP_PARAMS ", 19) == 0) {
                int buf_len = os_strlen(buf);

		if (wpa_drv_wlan_hb_cmd(wpa_s, "TCP_PARAMS", buf+19, buf_len-19))
			reply_len = -1;

	} else if (os_strncmp(buf, "WLAN_HB_TCP_FILTER ", 19) == 0) {
                int buf_len = os_strlen(buf);

		if (wpa_drv_wlan_hb_cmd(wpa_s, "TCP_FILTER", buf+19, buf_len-19))
			reply_len = -1;

	} else if (os_strncmp(buf, "WLAN_HB_UDP_PARAMS ", 19) == 0) {
                int buf_len = os_strlen(buf);

		if (wpa_drv_wlan_hb_cmd(wpa_s, "UDP_PARAMS", buf+19, buf_len-19))
			reply_len = -1;

	} else if (os_strncmp(buf, "WLAN_HB_UDP_FILTER ", 19) == 0) {
                int buf_len = os_strlen(buf);

		if (wpa_drv_wlan_hb_cmd(wpa_s, "UDP_FILTER", buf+19, buf_len-19))
			reply_len = -1;

	} else if (os_strncmp(buf, "WLAN_HB_NET_INFO ", 17) == 0) {
                int buf_len = os_strlen(buf);

		if (wpa_drv_wlan_hb_cmd(wpa_s, "NET_INFO", buf+17, buf_len-17))
			reply_len = -1;
	}
#endif
...
#endif
	else if (os_strncmp(buf, "debugfs_skip_p_ch ", 18) == 0) {
        int buf_len = os_strlen(buf);
        
		if(wpa_drv_wlan_debugfs_cmd(wpa_s, "passive_channel_skip", buf+18, buf_len-18))
		    reply_len = -1;

	} else if (os_strncmp(buf, "debugfs_txpower ", 16) == 0) {
        int en;
        int buf_len = os_strlen(buf);
        ...
        if (en > 100) {
            wpa_printf(MSG_INFO, "The Value should be within 1~100 \n");
            reply_len = -1;
        } else {
            if(wpa_drv_wlan_debugfs_cmd(wpa_s, "txpower", buf+16, buf_len-16))
		        reply_len = -1;
#ifndef CONFIG_NO_CONFIG_WRITE
            else {
                wpa_s->conf->tx_power = en;
          		if (wpa_supplicant_ctrl_iface_save_config(wpa_s)) {
          			wpa_printf(MSG_DEBUG, "TRY to Save Config FAIL");
          		} else {
          			wpa_printf(MSG_DEBUG, "TRY to Save Config DONE");
    ...
    } else if (os_strncmp(buf, "debugfs_gettsf", 14) == 0) {
        reply_len = wpa_drv_wlan_debugfs_cmd(wpa_s, "gettsf", reply, reply_size);
    } else if (os_strncmp(buf, "debugfs_synctsf", 15) == 0) {
        if(wpa_drv_wlan_debugfs_cmd(wpa_s, "synctsf", NULL, 0))
		    reply_len = -1;
    } else if (os_strncmp(buf, "debugfs_getscan_p", 17) == 0) {
        if(wpa_drv_wlan_debugfs_cmd(wpa_s, "getscan_p", NULL, 0))
		    reply_len = -1;
    } else if (os_strncmp(buf, "debugfs_setscan_p", 17) == 0) {
        int buf_len = os_strlen(buf);
        if(wpa_drv_wlan_debugfs_cmd(wpa_s, "setscan_p", buf+18, buf_len-18))
		    reply_len = -1;
    } else if (os_strncmp(buf, "debugfs_getdebug_m", 18) == 0) {
        reply_len = wpa_drv_wlan_debugfs_cmd(wpa_s, "getdebug_m", reply, reply_size);
    } else if (os_strncmp(buf, "debugfs_setdebug_m", 18) == 0) {
        int buf_len = os_strlen(buf);
        if(wpa_drv_wlan_debugfs_cmd(wpa_s, "setdebug_m", buf+19, buf_len-19))
		    reply_len = -1;
    } else if (os_strncmp(buf, "debugfs_getapalive_p", 20) == 0) {
        if(wpa_drv_wlan_debugfs_cmd(wpa_s, "getapalive_p", NULL, 0))
		    reply_len = -1;
    } else if (os_strncmp(buf, "debugfs_setapalive_p", 20) == 0) {
        int buf_len = os_strlen(buf);
        if(wpa_drv_wlan_debugfs_cmd(wpa_s, "setapalive_p", buf+21, buf_len-21))
		    reply_len = -1;
    } else if (os_strncmp(buf, "debugfs_gethtcap_p", 18) == 0) {
        if(wpa_drv_wlan_debugfs_cmd(wpa_s, "gethtcap_p", NULL, 0))
		    reply_len = -1;
    } else if (os_strncmp(buf, "debugfs_sethtcap_p", 18) == 0) {
        int buf_len = os_strlen(buf);
        if(wpa_drv_wlan_debugfs_cmd(wpa_s, "sethtcap_p", buf+19, buf_len-19))
		    reply_len = -1;
    } else if (os_strncmp(buf, "debugfs_chan_list", 17) == 0) {
        if(wpa_drv_wlan_debugfs_cmd(wpa_s, "chan_list", NULL, 0))
		    reply_len = -1;
    } else if (os_strncmp(buf, "debugfs_ampdu", 13) == 0) {
        int buf_len = os_strlen(buf);
        if(wpa_drv_wlan_debugfs_cmd(wpa_s, "ampdu_enable", buf+14, buf_len-14))
		    reply_len = -1;
    } else if (os_strncmp(buf, "debugfs_reg_read", 16) == 0) {
        int buf_len = os_strlen(buf);
        os_memcpy(reply, (buf+17),buf_len-17);
        reply_len = wpa_drv_wlan_debugfs_cmd(wpa_s, "reg_read", reply, reply_size);
    } else if (os_strncmp(buf, "debugfs_reg_write", 17) == 0) {
        int buf_len = os_strlen(buf);
        if(wpa_drv_wlan_debugfs_cmd(wpa_s, "reg_write", buf+18, buf_len-18))
		    reply_len = -1;
    } else if (os_strncmp(buf, "da_retry_limit", 14) == 0) {
        int en;
        int buf_len = os_strlen(buf);

        en = atoi(buf+15);
        if ((en >= 1) && (en <= 20) ) {
            if(wpa_drv_wlan_debugfs_cmd(wpa_s, "set_da_retry", buf+15, buf_len-15))
		        reply_len = -1;
        } else {
		    reply_len = -1;
            wpa_printf(MSG_INFO, "The Value should be within 1 ~ 20 \n");
        }
    } else {
