...
#define P2P_AUTO_SCAN_ATTEMPTS 5
 ...
 */
#if 1 /* ASSOC-REJECT issue */
#define P2P_MAX_INITIAL_CONN_WAIT 25
#else
			 ...
			 const u8 *req_dev_types, const u8 *dev_id, u16 pw_id)
{
	...
	wps_ie = wps_build_probe_req_ie(wpa_s->global->widi_enable_wps, 
					wpa_s->global->widi_dev_name, 
					pw_id, &wpa_s->wps->dev,
				   ...
				   struct p2p_channels *chan)
{
	...
	struct p2p_oper_class_map op_class[] = {
		...
		{ HOSTAPD_MODE_IEEE80211G, 83, 1, 9, 1, BW20 },
		{ HOSTAPD_MODE_IEEE80211G, 84, 5, 13, 1, BW20 },
...
int wpas_p2p_init(struct wpa_global *global, struct wpa_supplicant *wpa_s)
{
	...
	if (wpa_s->drv_flags & WPA_DRIVER_FLAGS_P2P_MGMT) {
		...
		os_memcpy(params.pri_dev_type, wpa_s->conf->device_type,
			  WPS_DEV_TYPE_LEN);
#ifdef CONFIG_CUSTOM_WFD
		params.num_sec_dev_types = wpa_s->conf->p2p_num_sec_device_types;
		os_memcpy(params.sec_dev_type,
			  wpa_s->conf->p2p_sec_device_type,
	...
	if (wpa_s->conf->p2p_listen_reg_class &&
		...
		wpa_s->conf->p2p_listen_reg_class_backup = wpa_s->conf->p2p_listen_reg_class;
		wpa_s->conf->p2p_listen_channel_backup = wpa_s->conf->p2p_listen_channel;
	} else {
		p2p.reg_class = 81;
		/*
	...
	if (wpa_s->conf->p2p_join_timeout)
		p2p.p2p_join_timeout = wpa_s->conf->p2p_join_timeout;
	else
		p2p.p2p_join_timeout = 60;			// Default set to 60 seconds
				   ...
				   struct wpa_scan_results *scan_res)
{
	...
	if (wpa_s->p2p_auto_pd) {
		...
		if (join == 0 &&
		    wpa_s->auto_scan_retry < P2P_AUTO_SCAN_ATTEMPTS) {
			wpa_s->auto_scan_retry++;
		...
		if (p2p_prov_disc_req(wpa_s->global->p2p,
				      wpa_s->pending_join_dev_addr,
				      wpa_s->pending_pd_config_methods, 0,
				      0) < 0) {
			wpa_s->p2p_auto_pd = 0;
			wpa_msg(wpa_s, MSG_INFO, P2P_EVENT_PROV_DISC_FAILURE
				" p2p_dev_addr=" MACSTR " status=N/A",
...
static int wpas_p2p_join_start(struct wpa_supplicant *wpa_s)
{
	 ...
	 */
	eloop_cancel_timeout(wpas_p2p_group_formation_timeout, wpa_s, NULL);
	eloop_register_timeout(wpa_s->global->p2p->cfg->p2p_join_timeout, 0, wpas_p2p_group_formation_timeout,
			       wpa_s, NULL);
				 ...
				 unsigned int timeout)
{
	...
	os_get_time(&wpa_s->p2p_roc_started);
					  ...
					  unsigned int freq)
{
	...
	if (wpa_s->p2p_long_listen > 0) {
	    os_get_time(&now);
	    os_time_sub(&now, &wpa_s->p2p_roc_started, &diff);
	    if ((diff.sec < 0 )|| ((diff.sec * 1000) >= wpa_s->max_remain_on_chan))
			wpa_s->p2p_long_listen -= wpa_s->max_remain_on_chan; /* No more than max */
	    else
	    	wpa_s->p2p_long_listen -= ((diff.sec * 1000) + (diff.usec / 1000));
	}
	if (wpa_s->p2p_long_listen > 0) {
...
int wpas_p2p_group_add(struct wpa_supplicant *wpa_s, int persistent_group,
		       int freq, char *ssid, char *passphrase)
{
	struct p2p_go_neg_results params;
	unsigned int r;
...
int wpas_p2p_black_list(struct wpa_supplicant *wpa_s, const u8 *peer_addr,
		       enum wpas_p2p_black_list_use use)
{
	if(use == WPAS_P2P_BL_ADD)
		wpa_blacklist_add_p2p(wpa_s, peer_addr);
      else if(use == WPAS_P2P_BL_REMOVE)
             wpa_blacklist_del_p2p(wpa_s, peer_addr);
      else if(use == WPAS_P2P_BL_FLUSH)
            wpa_blacklist_clear_p2p(wpa_s);
...
#ifdef CONFIG_CUSTOM_WFD
/* Note:
 * WiDi 3.5 spec. defined p2p listen stay at social channel for only 500 milliseconds
 * However, original p2p_listen didn't support milliseconds resolution timeout.
 * This patch is just for supporting milliseconds resolution timeout p2p_listen command.
 */
int widi_wpas_p2p_listen_ms(struct wpa_supplicant *wpa_s, unsigned int timeout)
{
	int res;
	...
	if (timeout == 0) {
		...
		timeout = 2147483000; /* max value for 24 days */
#endif
	}
	eloop_cancel_timeout(wpas_p2p_long_listen_timeout, wpa_s, NULL);
	wpa_s->p2p_long_listen = 0;
	...
	if (res == 0 && timeout > wpa_s->max_remain_on_chan) {
		wpa_s->p2p_long_listen = timeout;
		eloop_register_timeout(timeout / 1000, (timeout % 1000) * 1000,
			  ...
			  u8 *buf, size_t len, int p2p_group)
{
	...
	if (strncmp(wpa_s->ifname, "wlan", 4) == 0)
		return -1;
...
void wpas_p2p_update_config(struct wpa_supplicant *wpa_s)
{
...
#ifdef CONFIG_CUSTOM_WFD
	if (wpa_s->conf->changed_parameters & CFG_CHANGED_P2P_SEC_DEVICE_TYPE)
		p2p_set_p2p_sec_dev_types(p2p,
				      (void *) wpa_s->conf->p2p_sec_device_type,
				      wpa_s->conf->p2p_num_sec_device_types);
#endif
	...
	if (wpa_s->conf->changed_parameters & CFG_CHANGED_P2P_LISTEN_CHANNEL) {
		...
		if(wpa_s->conf->p2p_listen_channel == 99){
			...
			if(freq>0){
				p2p_freq_to_channel(wpa_s->global->p2p->cfg->country, freq,
				  (u8 *)(&wpa_s->conf->p2p_listen_reg_class), (u8 *)(&wpa_s->conf->p2p_listen_channel));
			}else{
				wpa_s->conf->p2p_listen_reg_class = wpa_s->conf->p2p_listen_reg_class_backup;
				wpa_s->conf->p2p_listen_channel = wpa_s->conf->p2p_listen_channel_backup;
			}
		}else {
			if(wpa_s->conf->p2p_listen_channel != 0){
				wpa_s->conf->p2p_listen_channel_backup = wpa_s->conf->p2p_listen_channel;
				wpa_s->conf->p2p_listen_reg_class = wpa_s->conf->p2p_listen_reg_class_backup;
			}
		}
