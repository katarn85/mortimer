...
DEVICE_2082=`lsusb | grep "04e8:2082"`
DEVICE_2085=`lsusb | grep "04e8:2085"`
if [ "$DEVICE" = "" -a "$DEVICE_9375" = "" -a "$DEVICE_9372" = "" -a "$DEVICE_1022" = "" -a "$DEVICE_2082" = "" -a "$DEVICE_2085" = "" ]; then
...
insmod $MODULE_PATH/ath6kl_usb.ko ath6kl_p2p=0x9
sleep 3
...
if [ "$WLANDEV" = "" -a -e /sys/class/net ]; then
	for dev in `ls /sys/class/net/`; do
		if [ -e /sys/class/net/$dev/device/idProduct ]; then
			...
			if [ "$PID" = "9374" -o "$PID" = "9375" -o "$PID" = "9372" -o "$PID" = "1022" -o "$PID" = "2082" -o "$PID" = "2085" ]; then
				WLANDEV=$dev
			fi
		fi
