...
/* #define ADAPTER_LOGIC */ /* disable for resolve hls issue */
//#define USE_LEAST_PCR   /* Use the least PCR in each first stream of Video and Audio as a last_PCR */

/* How many ticks we treat are contig in PCR
 * Some content providers provides mpeg-ts streams with PCR sent
 * rare (in standard it should occur at most every 100ms)
*/
#define DISCONT_PCR_DIFF 900000
...
#ifdef USE_LEAST_PCR
static gboolean decide_lastPCR = FALSE;  /* Use the least PCR in each first stream of Video and Audio as a last_PCR */
#endif
    ...
    const guint8 * data, guint data_len, guint * consumed)
{
  ...
  if (length > 0) {
    ...
    if (flags & 0x10) {
	   ...
	   * (previously we have 1 second here but some providers sends
	   * content with PCR rare - every 2-3 seconds.
	   * See DISCONT_PCR_DIFF define.
	  */

/* Use the least PCR in each first stream of Video and Audio as a last_PCR */
#ifdef USE_LEAST_PCR
	if((stream->PID != stream->demux->last_PID)
	&& (stream->last_PCR != -1) && (decide_lastPCR == FALSE) ){
      ...
      if (stream->last_PCR != -1 &&
		  (pcr - stream->last_PCR > DISCONT_PCR_DIFF || pcr < stream->last_PCR)) {
        ...
        if (PMT_stream && PMT_stream->PMT.PCR_PID == stream->PID) {
          if (demux->pcr[0] == -1) {
            ...
            demux->num_packets = 0;
				...
				Some content providers sents PCR rare than 1 sec.
				so we use more time before we treat PCR as discontigues.
				(see DISCONT_PCR_DIFF definition)
			*/
          else if (G_UNLIKELY (demux->pcr[1] == -1
				  && ((pcr - demux->pcr[0]) >= DISCONT_PCR_DIFF))) {
    ...
    const guint8 * data)
{
  ...
  GST_DEBUG_OBJECT (demux, "gst_mpegts_demux_parse_stream submitcount %d %d for pid 0x%04x",demux->submit_count,demux->stream_count, PID);
#ifdef ADAPTER_LOGIC
  ...
  {
      /* parse the stream */
      ret = gst_mpegts_demux_parse_stream (demux, stream, outbuf+consumed_len+1,
          MPEGTS_NORMAL_TS_PACKETSIZE - 1);
      if(ret == GST_FLOW_RESEND)
      {
        GST_WARNING_OBJECT (demux, "gst_mpegts_demux_parse_stream pat PACKET NOT YET RECEIVED need to parse this packet again");
        ret = GST_FLOW_OK;
        break;
      }
      demux->num_packets++;
      consumed_len += demux->packetsize;
      GST_DEBUG_OBJECT (demux, "gst_mpegts_demux_parse_stream consumed length %d actual length %d",consumed_len, src_len);
...
#ifdef ADAPTER_LOGIC
      if((demux->last_PID == stream->PID) && (demux->submit_count > MAX_CONTINUOUS_SUBMIT_COUNT)) {
        GST_WARNING_OBJECT (demux, "gst_mpegts_demux_parse_stream packets of same stream max count reached");
        goto beach;
      }
#endif
  }
