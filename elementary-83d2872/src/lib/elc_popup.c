   ...
   if (sd->items)
     {
        ...
        EINA_LIST_FOREACH(sd->items, elist, item)
          {
#ifdef DA_FAMILYHUB_FORCED_ROT_90_DEGREE
...
//       2015063 modify for popup size change in portrait mode
...
static Eina_Bool
_check_set(Evas_Object *obj,
                Evas_Object *check)
{
   ELM_POPUP_DATA_GET(obj, sd);

   if (sd->check_box == check) return EINA_TRUE;
   if (sd->check) evas_object_del(sd->check);

   if (!sd->check_box)
     {
        sd->check_box = elm_box_add(obj);
        evas_object_show(sd->check_box);
        edje_object_part_swallow
        (ELM_WIDGET_DATA(sd)->resize_obj, "elm.swallow.check",
        sd->check_box);
     }

   sd->check = check;
   elm_box_pack_end(sd->check_box, check);
...
static Evas_Object *
_check_get(const Evas_Object *obj)
{
   ELM_POPUP_DATA_GET(obj, sd);
...
static Evas_Object *
_check_unset(Evas_Object *obj)
{
   Evas_Object *check;

   ELM_POPUP_DATA_GET(obj, sd);

   if (!sd->check) return NULL;

   check = sd->check;
   edje_object_part_unswallow(ELM_WIDGET_DATA(sd)->resize_obj, sd->check);
   sd->check = NULL;
