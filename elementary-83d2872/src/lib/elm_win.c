...
_elm_win_xwin_update(Elm_Win_Smart_Data *sd)
{
   ...
   if (sd->indmode == ELM_WIN_INDICATOR_SHOW)
	...
	// add for alpha window and rotation bug fix
     if ((sd->wm_rot.count) &&
       (sd->wm_rot.rots))
...
#undef SD_CPY
   ...
   //
#ifdef DA_FAMILYHUB_FORCED_ROT_90_DEGREE
	elm_win_wm_rotation_preferred_rotation_set(obj, 0);
#endif
                                           ...
                                           const int rotation)
{
   ...
   if (rotation == -1)
	...
	else if(rotation == 360)
	{
		...
		ecore_evas_wm_rotation_preferred_rotation_set(sd->ee, 90);
		return;
	}
	else
     rot = _win_rotation_degree_check(rotation + 90);
#else
   else
...
#ifdef DA_FAMILYHUB_FORCED_ROT_90_DEGREE
   if(sd->wm_rot.preferred_rot == 360) return 360;
   return _win_rotation_degree_check(sd->wm_rot.preferred_rot - 90);
#else
