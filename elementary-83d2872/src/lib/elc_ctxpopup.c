   ...
   EINA_LIST_FOREACH(sd->items, elist, item)
     {
        ...
        {
          edje_object_signal_emit(VIEW(item), "elm,state,default", "elm");
          edje_object_signal_emit(VIEW(item), "item,top", "elm");
        }
        else
        {
          if(idx == cnt-1)
             edje_object_signal_emit(VIEW(item), "item,bottom", "elm");
          else
             edje_object_signal_emit(VIEW(item), "item,middle", "elm");
