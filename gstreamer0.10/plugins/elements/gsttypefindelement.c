...
gst_type_find_element_chain_do_typefinding (GstTypeFindElement * typefind, GstPad *pad);
static void
...
#ifdef GST_EXT_TYPEFIND
  g_object_class_install_property (gobject_class, PROP_FORCE_PUSH_MODE,
      g_param_spec_boolean ("force-push-mode", _("force-push-mode"),
          "can be activated push mode for typefinding",
...
#ifdef GST_EXT_TYPEFIND
  typefind->force_push_mode = DEFAULT_FORCE_PUSH_MODE;
#endif
  typefind->store = NULL;
  typefind->change_track = FALSE;
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_FORCE_PUSH_MODE:
      typefind->force_push_mode = g_value_get_boolean (value);
      break;
#endif
	case PROP_CHANGE_TRACK:
	  GST_DEBUG_OBJECT (typefind, "set change-track to %d", g_value_get_boolean(value));
	  typefind->change_track = g_value_get_boolean(value);
	  break;
    default:
...
#ifdef GST_EXT_TYPEFIND
    case PROP_FORCE_PUSH_MODE:
      g_value_set_boolean (value, typefind->force_push_mode);
      break;
#endif
	case PROP_CHANGE_TRACK:
      g_value_set_boolean(value, typefind->change_track);
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
      break;
...
gst_type_find_element_chain (GstPad * pad, GstBuffer * buffer)
{
  ...
  switch (typefind->mode) {
    ...
    case MODE_NORMAL:
      ...
      res =  gst_pad_push (typefind->src, buffer);
	  if (typefind->change_track) {
  		res = GST_FLOW_OK;
		GST_LOG_OBJECT(typefind, "not return wrong-state when change track");
  	  }
	  return res;
    case MODE_TYPEFIND:{
      GST_OBJECT_LOCK (typefind);
      ...
      res = gst_type_find_element_chain_do_typefinding (typefind, pad);
...
gst_type_find_element_chain_do_typefinding (GstTypeFindElement * typefind, GstPad* pad)
{
  ...
  /*if probability is higher than GST_TYPE_FIND_MAXMUM  or probe buffer size is more than 128KB, just emit have-type signal*/
  if (probability < GST_TYPE_FIND_MAXIMUM && GST_BUFFER_SIZE(typefind->store) < TYPE_FIND_MAX_SIZE)
      {
        gst_caps_unref(caps);
        GST_OBJECT_UNLOCK(typefind);
        return GST_FLOW_OK;
...
#ifdef GST_EXT_TYPEFIND
  if ((typefind->force_push_mode == TRUE) && (gst_pad_check_pull_range (pad)))
  {
    GST_DEBUG_OBJECT (typefind, "need to deactivate pad for reset: push -> pull");

    gst_pad_activate_push (pad, FALSE);
    ...
    GstActivateMode activate_mode = GST_PAD_ACTIVATE_MODE (pad);
    if (activate_mode == GST_ACTIVATE_PUSH) {
...
gst_type_find_element_activate (GstPad * pad)
{
   ...
   */
#ifdef GST_EXT_TYPEFIND
  if(typefind->force_push_mode == TRUE) {
