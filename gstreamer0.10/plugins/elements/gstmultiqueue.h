...
#define GST_MULTI_QUEUE_GET_CLASS(obj) \
(G_TYPE_INSTANCE_GET_CLASS ((obj), GST_TYPE_MULTI_QUEUE, GstMultiQueueClass))
...
struct _GstMultiQueue {
  ...
  gboolean disable_subtitle_buffering;
  gboolean mmplayer_mode;
};
...
struct _GstMultiQueueClass {
  ...
  /* for tz-queue*/
  int (*tz_flush) (GstMultiQueue *queue ,unsigned int handle);
  int (*tz_queue_flush) (GstMultiQueue *queue);
  int (*tz_check_max_size_bytes) (GstMultiQueue *queue);
};
