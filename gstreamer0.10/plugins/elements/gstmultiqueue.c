 ...
 * Copyright (C) 2011 Sebastian Droge <sebastian.droege@collabora.co.uk>
 *
 * gstmultiqueue.c:
 *
...
#define DEFAULT_LAST_BUFMSG_SEQNUM 0
...
enum
{
  ...
  PROP_DISABLE_SUBTITLE_BUFFERING,
  PROP_PLAYER_MODE,
  PROP_LAST_BUFMSG_SEQNUM,
  PROP_LAST
};
...
gst_multi_queue_class_init (GstMultiQueueClass * klass)
{
  ...
  g_object_class_install_property (gobject_class, PROP_SYNC_BY_RUNNING_TIME,
   ...
   g_object_class_install_property (gobject_class, PROP_DISABLE_SUBTITLE_BUFFERING,
      g_param_spec_boolean ("disable-subtitle-buffering", "Disable subtitle stream buffering message",
          "multiqueue not emit subtitle stream buffering message",
          FALSE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property (gobject_class, PROP_PLAYER_MODE,
          g_param_spec_boolean ("mmplayer-mode", "player caller is mmplayer or not",
              "only effective by mmplayer",
              FALSE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

  g_object_class_install_property (gobject_class, PROP_LAST_BUFMSG_SEQNUM,
      g_param_spec_uint ("last-bufmsg-seqnum", "Last posted buffering message sequence number",
          "For filtering delayed buffering messages",
          0, G_MAXUINT, DEFAULT_LAST_BUFMSG_SEQNUM,
          G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));
...
gst_multi_queue_init (GstMultiQueue * mqueue, GstMultiQueueClass * klass)
{
  ...
  mqueue->mmplayer_mode = FALSE;

  mqueue->disable_subtitle_buffering = FALSE;
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_SYNC_BY_RUNNING_TIME:
      ...
      break;
    case PROP_DISABLE_SUBTITLE_BUFFERING:
      mq->disable_subtitle_buffering = g_value_get_boolean (value);
      break;
	case PROP_PLAYER_MODE:
	  mq->mmplayer_mode = g_value_get_boolean (value);
      GST_DEBUG_OBJECT(mq,"set mmplayer_mode %d\n",mq->mmplayer_mode);
      break;
    default:
    ...
    GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_SYNC_BY_RUNNING_TIME:
      ...
      break;
    case PROP_DISABLE_SUBTITLE_BUFFERING:
      g_value_set_boolean (value, mq->disable_subtitle_buffering);
      break;
	case PROP_PLAYER_MODE:
	  g_value_set_boolean( value, mq->mmplayer_mode);
      break;
    case PROP_LAST_BUFMSG_SEQNUM:
      g_value_set_uint (value, mq->last_bufmsg_seqnum);
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
      break;
...
gst_single_queue_flush (GstMultiQueue * mq, GstSingleQueue * sq, gboolean flush)
{
  ...
  GstMultiQueueClass *klass = GST_MULTI_QUEUE_GET_CLASS(mq);
  if (flush) {
    ...
    if(klass->tz_queue_flush)
    {
        GST_LOG_OBJECT (mq, "SingleQueue %d : tz_queue_flush",sq->id);
        klass->tz_queue_flush(mq);
    }
...
update_buffering (GstMultiQueue * mq, GstSingleQueue * sq)
{
  ...
  gint percent, tmp;
  gboolean post = FALSE;
#ifdef DISABLE_BUFFERING_FOR_SUBTITLE
  GstCaps *caps = NULL;
...
#ifdef DISABLE_BUFFERING_FOR_SUBTITLE
  if(sq && mq->disable_subtitle_buffering){ 
    caps = gst_pad_peer_get_caps(sq->sinkpad);   
    if(NULL == caps){
      GST_DEBUG_OBJECT(mq,"cannot get caps from sq->sinkpad."); 
      goto DONE;
    }

    str = gst_caps_get_structure (caps, 0);
    if (NULL == str){
      GST_DEBUG_OBJECT (mq,"cannot get structure from capse.");
      goto DONE;
    }
   
    mime = gst_structure_get_name (str);
    if(NULL == mime){
      GST_DEBUG_OBJECT(mq,"cannot get mimetype from structure.");
      goto DONE;
    }

    GST_DEBUG_OBJECT(mq,"mime is [%s]",mime);
    if(g_strrstr(mime,"x-subtitle")||g_strrstr(mime,"x-srt")||
       g_strrstr(mime,"x-ssa")||g_strrstr(mime,"x-ass")||
       g_strrstr(mime,"x-xsub")||g_strrstr(mime,"x-smpte-text")||
       g_strrstr(mime,"x-smpte-png")||g_strrstr(mime,"x-mov-ttml")||
       g_strrstr(mime,"x-dvd-subpicture")||g_strrstr(mime,"x-mov-text")){
        GST_DEBUG_OBJECT(mq,"subtitle caps, just jump it!");
       goto DONE;
    }
  }
#endif
  ...
  GST_DEBUG_OBJECT (mq,"queue %d: mq high_percent %d, low_percent %d\n",sq->id,mq->high_percent,mq->low_percent);
  ...
  if (mq->buffering) {
    GST_DEBUG_OBJECT (mq,"queue %d: mq->buffering in,percent %d\n",sq->id,percent);
    post = TRUE;
  ...
  if (post) {
    GST_DEBUG_OBJECT (mq,"queue %d: post in,percent %d\n",sq->id,percent);
    GstMessage *message;

    GST_DEBUG_OBJECT (mq,"mq->mmplayer_mode %d,percent %d,mq->low_percent %d,!mq->buffering %d\n",mq->mmplayer_mode,percent,mq->low_percent,!mq->buffering);
    if(mq->mmplayer_mode == TRUE && percent <= mq->low_percent)
	{
        GST_DEBUG_OBJECT (mq,"check low_percent - 100 case\n");
	    GList *tmp = NULL;
	    int tmp_percent = 0;
	    int sq_percent = 0;                        
	    for (tmp = mq->queues; tmp != NULL; tmp = g_list_next (tmp)) 
		{
	      sq_percent = 0;
	      GstSingleQueue *sq = (GstSingleQueue *) tmp->data;
	      if (sq->max_size.time > 0) 
		  {
             tmp_percent = (sq->cur_time * 100) / sq->max_size.time;
             sq_percent = MAX (sq_percent, tmp_percent);
	      }
	      if (sq->max_size.bytes > 0)
		  {
	         tmp_percent = (size.bytes * 100) / sq->max_size.bytes;
	         sq_percent = MAX (sq_percent, tmp_percent);
	      }
	      GST_DEBUG_OBJECT (mq, "sq_percent %d percent", sq_percent);
	      sq_percent = sq_percent * 100 / mq->high_percent;                     
	      GST_DEBUG_OBJECT (mq, "compare to mq->high_percent,sq_percent %d scaled percent", sq_percent);
	      if(sq_percent >= 100)
		  {
              GST_DEBUG_OBJECT (mq, "one is less than %d,one is more than %d, do not post buffer message",mq->low_percent,mq->high_percent);
              goto DONE;
	      }                             
		}
        GST_DEBUG_OBJECT (mq,"low_percent - 100 case end\n");
	} 
    ...
    GST_DEBUG_OBJECT (mq, "queue %d: buffering %d scaled percent",sq->id, percent);
    message = gst_message_new_buffering (GST_OBJECT_CAST (mq), percent);

    mq->last_bufmsg_seqnum = gst_message_get_seqnum(message);
    ...
    if (percent > 100)
        percent = 100;
    GST_DEBUG_OBJECT (mq, "queue %d: filled %d scaled percent",sq->id, percent);
  }
#ifdef DISABLE_BUFFERING_FOR_SUBTITLE
DONE:
   if(caps){
...
{
  GST_DEBUG("gst_multi_queue_item_destroy");
  GstMultiQueueClass *klass;
  if (item->object)
  {
    if(item->visible)
    {
      GST_DEBUG("GST_IS_BUFFER(item->object) %d\n",GST_IS_BUFFER(item->object));
      if(item->sq && item->sq->mqueue && GST_IS_BUFFER(item->object))
      {
        klass = GST_MULTI_QUEUE_GET_CLASS(item->sq->mqueue);
        GstBuffer* buffer = GST_BUFFER_CAST (item->object);
        GstStructure *s = gst_buffer_get_qdata(buffer, g_quark_from_static_string("GstTzHandleData"));
	if(s)
        {
          unsigned int handle = 0;
	  gst_structure_get_uint (s, "packet_handle",&handle);
          if(handle >0 ){
            if(klass->tz_flush)
	    {
		GST_DEBUG("gst_multi_queue_item_destroy handle %u",handle);
		klass->tz_flush(item->sq->mqueue,handle);
	    }
          }
        }
      }
...
gst_multi_queue_buffer_item_new (GstMiniObject * object, guint32 curid, GstSingleQueue* sq)
{
...
static GstMultiQueueItem *
gst_multi_queue_event_item_new (GstMiniObject * object, guint32 curid, GstSingleQueue* sq)
{
  GstMultiQueueItem *item;
...
gst_multi_queue_chain (GstPad * pad, GstBuffer * buffer)
{
  ...
  item = gst_multi_queue_buffer_item_new (GST_MINI_OBJECT_CAST (buffer), curid,sq);
...
gst_multi_queue_sink_event (GstPad * pad, GstEvent * event)
{
  ...
  item = gst_multi_queue_event_item_new ((GstMiniObject *) event, curid,sq);
