...
static GstClockTime get_active_running_time (GstInputSelector * sel)
{
  gboolean in_paused = FALSE;
  if (GST_STATE (sel) != GST_STATE_PLAYING ||
      GST_STATE_PENDING (sel) != GST_STATE_VOID_PENDING) {
    return GST_ELEMENT_START_TIME(sel);
  }

  GstClockTime now = gst_clock_get_time(GST_ELEMENT_CLOCK(sel));
  GstClockTime base = GST_ELEMENT_CAST(sel)->base_time;
    ...
    GstSelectorPad * pad, GstBuffer * buf)
{
  ...
  if (active_seg->format == GST_FORMAT_TIME) {    
    if (sel->sync_streams_to_position)
      active_running_time = get_active_running_time(sel);
    else
      active_running_time = gst_segment_to_running_time (active_seg, GST_FORMAT_TIME, active_seg->last_stop);
  }
  ...
  while (pad != active_selpad && !sel->flushing && !pad->flushing &&
    ...
    if (sel->sync_streams_to_position) {
      ...
      GST_INPUT_SELECTOR_WAIT (sel); // If the duration of active stream is shorter than inactive stream, nobody can awake this thread.
    }
    ...
    if (active_seg->format == GST_FORMAT_TIME) {
      if (sel->sync_streams_to_position) {
        active_running_time = get_active_running_time(sel);
        if (GST_CLOCK_TIME_IS_VALID(active_running_time) == FALSE)
        {
          GST_ERROR_OBJECT(pad, "active_running_time : %"GST_TIME_FORMAT, GST_TIME_ARGS(active_running_time));
          break;
        }
      }else
        active_running_time = gst_segment_to_running_time (active_seg, GST_FORMAT_TIME, active_seg->last_stop);
...
gst_input_selector_class_init (GstInputSelectorClass * klass)
{
  ...
  g_object_class_install_property (gobject_class, PROP_SYNC_STREAMS_TO_POSITION,
      g_param_spec_boolean ("sync-streams-to-position", "Sync Streams Based On Playback Position",
          "Synchronize inactive streams to the playback position",
    ...
    GstPad * pad, gint64 stop_time, gint64 start_time)
{
  ...
  if (!GST_CLOCK_TIME_IS_VALID (stop_time) && old) {
    ...
    if (!GST_CLOCK_TIME_IS_VALID (stop_time) && !GST_CLOCK_TIME_IS_VALID (start_time) && self->sync_streams_to_position) {
      stop_time = start_time = get_active_running_time(self);
      GST_ERROR_OBJECT (self, "sync_streams_to_position,  using start/stop of %" GST_TIME_FORMAT, GST_TIME_ARGS (start_time));
    }
  }
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_SYNC_STREAMS_TO_POSITION:
    {
      GST_INPUT_SELECTOR_LOCK (sel);
      sel->sync_streams_to_position = g_value_get_boolean (value);
      GST_INPUT_SELECTOR_UNLOCK (sel);
      break;
    }
    default:
    ...
    GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_SYNC_STREAMS_TO_POSITION:
      GST_INPUT_SELECTOR_LOCK (object);
      g_value_set_boolean (value, sel->sync_streams_to_position);
      GST_INPUT_SELECTOR_UNLOCK (object);
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
