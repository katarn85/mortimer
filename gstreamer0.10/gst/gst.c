...
//#define USE_GSTLOG_SWITCH
#ifdef  USE_GSTLOG_SWITCH
#define INPIPE_PREFIX "/tmp/.gstIn"
#define OUTPIPE_PREFIX "/tmp/.gstOut"

static GstTask* log_switch_task = NULL;
static gboolean bTaskInitialized = FALSE;
static gchar inPipeName[32] = {0,};
static gchar outPipeName[32] = {0,};
static FILE* fdInPipe = NULL;
static FILE* fdOutPipe = NULL;
GStaticRecMutex *task_lock = NULL;
...
printout (const char * format, ...)
{
  ...
  vsnprintf(buf, 1024, format, var_args);
  if (fdOutPipe)
  {
//printf("%s, -> %d", format, strlen(format));
//printf("%s, -> %d", buf, strlen(buf));
    if (write(fdOutPipe, buf, strlen(buf)) <= 0)
      GST_ERROR("Fail to output[ %s ]\n", buf);
  }
  else
    GST_ERROR("fdOutPipe is NULL, buf[ %s ]\n", buf);
  va_end (var_args);
}
...
void displayAll()
{
  GSList* list = gst_debug_get_all_categories ();
  GSList* walk = list = g_slist_sort(list, sort_by_category_name);
  int i=0;
  while (walk) {
    GstDebugCategory *cat = (GstDebugCategory *) walk->data;
  printf("[%d][%s : %d]\n", i++, cat->name, cat->threshold);
...
static char catname[][32] = {" ",
					"VDEC", "ADEC", "VREN", "AREN",
					"basesrc", "fflib", "ffdemux", "ffdec", "ffprotocol", // src, demux
					"omx", "omx_util", // decoder
					"basesink", "videosink", "xvimagesink", //  videosink
					"pulse", "ringbuffer", "ximagesink", // audiosink
					"framecollector", "appsrc", "audioclock", "input-selector", // etc
					"sssrc", "souphttpsrc", "mmssrc",  // streaming src
					"queue", "queue2", "query",
					"GST_BUS", "GST_CAPS", "GST_PADS", "GST_EVENT", "GST_MESSAGE", "GST_SIGNAL"
					"GST_BUFFER", "GST_PIPELINE", "GST_CLOCK", "GST_PERFORMANCE", }; //

void displayCategoryLevel(void)
{
  GstDebugCategory * cat = NULL;
  int i=0;
//  printf("dis[ %d %d ]\n", sizeof(catname), sizeof(catname[0]));
  while(i< sizeof(catname)/sizeof(catname[0]))
  {
    cat = _gst_debug_get_category(catname[i]);
//    printf("[ %d ]  cat[ %p, %s ]\n",i, cat, catname[i]);
    if (cat)
      printout("[ %2d ] - [ %s ] : %d\n", i, catname[i], cat->threshold);
    else
      printout("[ %2d ] - [ %s ] : NOT INITIALIZED YET\n", i, catname[i]);
    i++;
  }
}
...
static void gst_log_loop(void* data)
{
  char error_buf[1024];
  if (bTaskInitialized == FALSE)
  {
    snprintf(outPipeName, 32, OUTPIPE_PREFIX"%d", getpid());
    snprintf(inPipeName, 32, INPIPE_PREFIX"%d", getpid());

    if (mkfifo(outPipeName, 0777) != 0) {
      if (!strerror_r(errno, error_buf, 1024))
        GST_WARNING("mkfifo[ %s, %d  ] errno[ %s ]", outPipeName, strlen(outPipeName), error_buf);
    }


    if (mkfifo(inPipeName, 0777) != 0) {
      if (!strerror_r(errno, error_buf, 1024))
        GST_WARNING("mkfifo[ %s, %d  ] errno[ %s ]", outPipeName, strlen(outPipeName), error_buf);
    }

    if (access(outPipeName, F_OK|W_OK)==0)
    {
      fdOutPipe = open(outPipeName, O_RDWR);
      if (!fdOutPipe) {
        if (!strerror_r(errno, error_buf, 1024))
          GST_ERROR("Fail to open [ %s ], errno[ %s ]", outPipeName, error_buf);
      }
    }
    else {
      if (!strerror_r(errno, error_buf, 1024))
       GST_ERROR("NO outPipeName[ %s ], errno[ %s ]", outPipeName, error_buf);
    }

    if (access(inPipeName, F_OK|R_OK)==0)
    {
      fdInPipe = open(inPipeName, O_RDWR); // should be RDWR to prevent error-return caused by no writer.
      if (!fdInPipe) {
        if (!strerror_r(errno, error_buf, 1024))
          GST_ERROR("Fail to open [ %s ], errno[ %s ]", inPipeName, error_buf);
      }

      if (fdInPipe && fdOutPipe)
      {
        bTaskInitialized = TRUE;
        printout("IN/OUT PIPE is prepared : %s, %s\n", inPipeName, outPipeName);
      }
    }
    else {
      if (!strerror_r(errno, error_buf, 1024))
        GST_ERROR("NO inPipeName[ %s ], errno[ %s ]", inPipeName, error_buf);
    }
  }
  ...
  if (bTaskInitialized)
  {
    ...
    cat = NULL;
    int ret = read(fdInPipe, &index, 1);
//    GST_ERROR("ret[ %d ][ %s ]", ret, strerror(errno));
    if (ret > 0)
    {
      switch(index)
      {
        case 0: // select all  (set debug level)
          break;
        case 96: // Multi Selection Start   ( set number of category)
          break;
        case 97: // Multi Selection Stop  ( set debug level )
          break;
        case 98: // Set LEVEL_NONE to all categories..
          break;
        case 99:
          // just wait for next input from read().
          break;
        default:
          if (index > 0 && index < sizeof(catname)/sizeof(catname[0]))
          {
            cat = _gst_debug_get_category(catname[index]);
            if (cat)
            {
              printout("0:NONE,  1:ERROR,   2:WARN,  3:INFO,  4:DEBUG, 5:LOG\n");
              if ((level = inputNum()) != 0xff)
                gst_debug_category_set_threshold(cat, level);
            }
            else
              printout("This category[ %s ] is not created yet.\n", catname[index]);
            // Set LEVEL_NONE
          }
          break;
      }
      ...
      if (!strerror_r(errno, error_buf, 1024))
        GST_ERROR("ret[ %d ][ %s ]", ret, error_buf);
    }
  }
}

static void gst_start_runtime_log_switch(void)
{
  if (!log_switch_task)
  {
    log_switch_task = gst_task_create(gst_log_loop, NULL);
    if (log_switch_task)
    {
      task_lock = g_new (GStaticRecMutex, 1);
      g_static_rec_mutex_init (task_lock);
      gst_task_set_lock (log_switch_task, task_lock);
      bTaskInitialized = FALSE;
      gst_task_start(log_switch_task);
    }
  }
}

static void gst_stop_runtime_log_switch(void)
{
  if (log_switch_task)
  {
    bTaskInitialized = FALSE;
    ...
    gst_task_stop(log_switch_task);
    gst_task_join(log_switch_task);
    log_switch_task = NULL;
    g_static_rec_mutex_free (task_lock);
    g_free (task_lock);
    task_lock = NULL;
  }
}
