...
GstDebugCategory *GST_CAT_VDEC = NULL;
GstDebugCategory *GST_CAT_ADEC = NULL;
GstDebugCategory *GST_CAT_VREN = NULL;
GstDebugCategory *GST_CAT_AREN = NULL;
...
static guint log_target; // ex)export GST_LOG_TARGET=2   // 1:STANDART OUT, 2:DLOG(default), 3:BOTH(STANDARD OUT & DLOG)
static const gchar *log_debug_object; // ex)   export GST_DEBUG_OBJ=imagesink0    // only the object which has a name "imagesink0" can output its log. (THIS IS NOT A CATEGORY NAME)
...
_gst_debug_init (void)
{
  ...
  GST_CAT_VDEC = _gst_debug_category_new ("VDEC",
      GST_DEBUG_BOLD | GST_DEBUG_FG_WHITE | GST_DEBUG_BG_GREEN, NULL);
  GST_CAT_ADEC = _gst_debug_category_new ("ADEC",
      GST_DEBUG_BOLD | GST_DEBUG_FG_WHITE | GST_DEBUG_BG_BLUE, NULL);
  GST_CAT_VREN = _gst_debug_category_new ("VREN",
      GST_DEBUG_BOLD | GST_DEBUG_FG_WHITE | GST_DEBUG_BG_MAGENTA, NULL);
  GST_CAT_AREN = _gst_debug_category_new ("AREN",
      GST_DEBUG_BOLD | GST_DEBUG_FG_WHITE | GST_DEBUG_BG_CYAN, NULL);
  ...
  log_target = 2;
  env = g_getenv ("GST_LOG_TARGET");
  if (env != NULL)
  {
  	log_target = atoi(env);
  }

  log_debug_object = g_getenv ("GST_DEBUG_OBJ");
    ...
    GObject * object, GstDebugMessage * message, gpointer unused)
{
  ...
  if ((log_debug_object) && (g_strrstr(obj, log_debug_object) == NULL))
    return;
...
#define PRINT_FMT_FOR_DLOG " "PTR_FMT" %s%s%s %s"CAT_FMT"%s %s\n"
...
#define __LINE__ 0
      if (level == GST_LEVEL_ERROR) {
        SLOG(LOG_ERROR, "GST_LOG",
            "%" GST_TIME_FORMAT PRINT_FMT_FOR_DLOG, GST_TIME_ARGS (elapsed),
            g_thread_self (), levelcolor,
            gst_debug_level_get_name (level), clear, color,
            gst_debug_category_get_name (category), file, line, function, obj,
            clear, gst_debug_message_get (message));
      }
...
#if defined(USE_DLOG) && defined(GST_EXT_BASIC_MODIFICATION) && !defined(G_OS_WIN32)
    if (log_target & 0x01) {
    fprintf (log_file, "%" GST_TIME_FORMAT PRINT_FMT, GST_TIME_ARGS (elapsed),
