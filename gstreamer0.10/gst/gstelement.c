    ...
    GstState * state, GstState * pending, GstClockTime timeout)
{
  ...
  GstStateChangeReturn result = GST_STATE_CHANGE_FAILURE;
	unsigned char	szMsg[256];

  g_return_val_if_fail (GST_IS_ELEMENT (element), GST_STATE_CHANGE_FAILURE);

  oclass = GST_ELEMENT_GET_CLASS (element);

	snprintf( szMsg, sizeof(szMsg), "gst_element_get_state (%s)", GST_ELEMENT_NAME(element) );
...
gst_element_set_state (GstElement * element, GstState state)
{
  ...
  unsigned char szDebugMsg[256];

  g_return_val_if_fail (GST_IS_ELEMENT (element), GST_STATE_CHANGE_FAILURE);

  snprintf(szDebugMsg, sizeof(szDebugMsg), "set %s state %s to %s", GST_ELEMENT_NAME (element),  gst_element_state_get_name(GST_STATE (element)), gst_element_state_get_name(state) );
...
invalid_return:
  {
    ...
    GST_CAT_ERROR_OBJECT (GST_CAT_STATES, element, "%s: unknown return value %d from a state change function",
