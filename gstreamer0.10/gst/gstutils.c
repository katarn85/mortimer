 ...
 */
gboolean
gst_element_query_resource(GstElement * element,
	GList **resource_list)
{
	GstQuery *query;
	gboolean ret;
	...
	{
		count = gst_child_proxy_get_children_count (element);
		for (i = 0; i < count; i++)
		{
			if (!(object = gst_child_proxy_get_child_by_index (element, i)))
				continue;

			if(GST_IS_BIN(object))
			{
				ret = gst_element_query_resource(object,resource_list);
			...
			if (ret)
			{
				...
				if(NULL==(g_list_find(*resource_list,resource_num)))
				{
					*resource_list = g_list_append(*resource_list,GINT_TO_POINTER(resource_num));
				}
			}
			gst_object_unref (object);
		}
