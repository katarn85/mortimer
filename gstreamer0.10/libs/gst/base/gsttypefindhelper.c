    ...
    const gchar * extension, GstTypeFindProbability * prob)
{
  ...
  for (l = type_list; l; l = l->next) {
    ...
    factory_name = GST_PLUGIN_FEATURE_NAME(helper.factory);
    /*if typfindfunctions probe it, ingore fftype_function*/
    if (helper.best_probability >= 90 && factory_name && g_strrstr(factory_name,"fftype"))
      continue;
