...
struct _GstBaseSinkPrivate
{
  ...
  gboolean query_position_support;
#ifdef GST_EXT_NO_DROP_PROPERTY
  gint no_drop_enabled;
#endif
};
...
gst_base_sink_class_init (GstBaseSinkClass * klass)
{
  ...
  g_object_class_install_property (gobject_class, PROP_ENABLE_CC,
      g_param_spec_boolean ("enable-cc", "Enable Clock Calibration",
          ...
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
   g_object_class_install_property (gobject_class, PROP_QUERY_POSITION,
      g_param_spec_boolean ("query-position-support", "Query Position Support",
          "support query position or not",
          TRUE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
#ifdef GST_EXT_NO_DROP_PROPERTY
  g_object_class_install_property (gobject_class, PROP_NO_DROP,
      g_param_spec_boolean ("no-drop", "no drop buffer (custom property for MSE)",
          "didn't check segment time (no drop for MSE)", DEFAULT_NO_DROP,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
#endif
...
#ifdef GST_EXT_NO_DROP_PROPERTY
  g_atomic_int_set (&priv->no_drop_enabled, DEFAULT_NO_DROP);
#endif
...
/**
 * gst_base_sink_set_query_position_support:
 * @sink: the sink
 * @query_position_support: support query position or not.
 *
 */
void
gst_base_sink_set_query_position_support(GstBaseSink * sink, gboolean query_position_support)
{
  g_return_if_fail (GST_IS_BASE_SINK (sink));
  g_atomic_int_set (&sink->priv->query_position_support, query_position_support);  
  GST_LOG_OBJECT (sink, "set query_position_support to %d", query_position_support);
}
/**
 * gst_base_sink_get_query_position_support:
 * @sink: the sink
 *
 * Checks if @sink is support query position or not
 *
 * Returns: TRUE if the sink is support query position.
 *
 */
gboolean
gst_base_sink_get_query_position_support (GstBaseSink * sink)
{
  gboolean res;

  g_return_val_if_fail (GST_IS_BASE_SINK (sink), FALSE);
  res = g_atomic_int_get (&sink->priv->query_position_support);
...
#ifdef GST_EXT_NO_DROP_PROPERTY
void
gst_base_sink_set_no_drop_enabled (GstBaseSink * sink, gboolean enabled)
{
  g_return_if_fail (GST_IS_BASE_SINK (sink));

  g_atomic_int_set (&sink->priv->no_drop_enabled, enabled);
}

gboolean
gst_base_sink_is_no_drop_enabled (GstBaseSink * sink)
{
  gboolean res;

  g_return_val_if_fail (GST_IS_BASE_SINK (sink), FALSE);

  res = g_atomic_int_get (&sink->priv->no_drop_enabled);
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_ENABLE_CC:
      ...
      break;
    case PROP_QUERY_POSITION:
      gst_base_sink_set_query_position_support (sink, g_value_get_boolean (value));
      break;
#ifdef GST_EXT_NO_DROP_PROPERTY
    case PROP_NO_DROP:
      gst_base_sink_set_no_drop_enabled (sink, g_value_get_boolean (value));
      break;
#endif
    default:
    ...
    GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_ENABLE_CC:
      ...
      break;
    case PROP_QUERY_POSITION:
      g_value_set_boolean (value, gst_base_sink_get_query_position_support(sink));
      break;
#ifdef GST_EXT_NO_DROP_PROPERTY
    case PROP_NO_DROP:
      g_value_set_boolean (value, gst_base_sink_is_no_drop_enabled(sink));
      break;
#endif
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
...
again:
   ...
   * immediately. */
  if (basesink->debugCategory) {
    debugEntryTime = gst_clock_get_time(GST_ELEMENT_CLOCK (basesink));
  }
  ret =
  ...
  if (G_LIKELY (OBJ_IS_BUFFERFULL (obj_type))) {
    ...
    if (G_LIKELY ((OBJ_IS_BUFFERLIST (obj_type) && bclass->render_list) ||
      ...
      if (basesink->debugCategory) {
        GST_CAT_DEBUG_OBJECT(basesink->debugCategory, basesink, "\tElasped Time[ %"GST_TIME_FORMAT " ]\ttimestamp[ %" GST_TIME_FORMAT " ]",
            GST_TIME_ARGS(debugEntryTime),
            GST_TIME_ARGS(GST_BUFFER_TIMESTAMP (GST_BUFFER_CAST (sync_obj)))); // elapsedTime = %"GST_TIME_FORMAT ",
      }
    ...
    guint8 obj_type, gpointer obj, gboolean prerollable)
{
  ...
  if (G_UNLIKELY (basesink->need_preroll)) {
    ...
    GST_DEBUG_OBJECT (basesink, "now %d prerolled items, obj[ %p ], type[ %d ] prerollable[ %d ]", length, obj, get_object_type(obj), prerollable);
...
#ifdef GST_EXT_NO_DROP_PROPERTY
  if (!basesink->priv->no_drop_enabled) {
#endif
    if (GST_CLOCK_TIME_IS_VALID (start) &&
...
default_element_query (GstElement * element, GstQuery * query)
{
  ...
  switch (GST_QUERY_TYPE (query)) {
    case GST_QUERY_POSITION:
    {
      ...
      gboolean query_support = TRUE;

      g_object_get (element, "query-position-support", &query_support, NULL);
      if(FALSE == query_support){
          return FALSE;
      }
